 ...
 * leaf-level diff.
 * @module @relay-harness/rlh-settings-file
 */
...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@relay-harness/rlh-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveRlhHome(config.rlhHome), 'settings.yaml'))
