...
- `register(ns, schema, { base?, applies? })` — 返回 owner 的 `SettingsScope`（`get`/`watch`/`update`）。注册是调用方插件 fiber 上的 effect：dispose（资源释放）该 fiber 会移除 namespace、阻止排队中的观察者调用启动，并等待已经开始的观察者调用全部结算。schema 拒绝的存量分节会使注册本身失败；重复 namespace 立即报错。
- `describe(options?)` — 每个 namespace 一条描述（`schema.toJSON()` 封装、解析值、分离出的 `base`/`user` 层、`applies`），供配置界面使用；字段出现在 `user` 中即标记其被用户覆盖。`describe({ redactSecrets: true })` 从每一层剥离 `role('secret')` 字段，并附加 `secrets` slot 列表（`{ path, set }`）；每个协议接口都必须传入它，纯遍历器 `redactSecrets(schema, value)` 已导出，供其他 wire 使用。 遍历器仅读取输入和 schema 的自有属性，并重建普通数据属性：`__proto__`、`constructor` 等对象继承名称不会作为 setter 或隐式配置生效。
- `get(ns)` — 解析值；未注册时为 `undefined`。
- `update(ns, patch)` — 把普通对象 patch 深合并进用户分节（绝不合并进 `base`），校验解析候选值，经提供方持久化后提交。patch 只能包含与 JSON 兼容的数据：Date、Map、BigInt、非有限数或循环引用会在任何内容持久化前被拒绝，并给出以 `$` 为根的路径（YAML/JSON 存储在重载时会静默改变这类值）。`__proto__` 等合法 JSON 属性名在克隆与合并全程保持为普通 own data，绝不会调用继承来的 setter。校验失败在持久化前拒绝；只读提供方（`writable: false`）拒绝一切写入。同一 namespace 的写入按调用顺序串行。
...
- 服务卸载先拒绝新写入与观察者调用的启动，再排干全部排队写入与已启动的观察者调用后才完成。registrant 自身卸载也会独立排干该注册已经启动的观察者。旧 registrant 在持久化途中被 dispose 时，写入仍到达存储；如果同一 namespace 已有新 owner 注册，新 owner 会重新解析并发布该提交值，而不是永久停留在旧快照。
...
- **`redactSecrets` 并非一条已证明安全的协议边界**：walker 跟随 `object`/`dict`/`array`；只能经由 union、intersection 或 transform 抵达的 `role('secret')` 会使调用抛错，拒绝输出未脱敏值。但 `schema.toJSON()` 仍会把 secret 字段的 `.default(...)` 带给客户端。真正的答案是一个 fail-closed 的 `describeForWire()`，它会净化序列化封装与错误文本；此项暂缓。
