...
- Service teardown refuses new writes and watcher starts, then drains every queued write and every started watcher invocation before disposal completes. Registrant teardown independently drains that registration's started watchers. A write whose old registrant disposed mid-persist still reaches storage; if a new owner registered the same namespace meanwhile, the new owner re-resolves and publishes that committed value instead of remaining stale.
...
- **`redactSecrets` is not a proven wire boundary** — the walker follows `object`/`dict`/`array`; a `role('secret')` reached only through a union, intersection, or transform makes the call throw (refusing to emit an unredacted value), and `schema.toJSON()` still carries a secret field's `.default(...)` to every client. A fail-closed `describeForWire()` — one that sanitizes the serialized envelope and error text — is the real answer and is deferred.
