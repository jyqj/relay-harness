 ...
 * @module @relay-harness/rlh-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@relay-harness/rlh-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `RLH_*` entries are deployment-owned facts for the child harness
   * (e.g. `RLH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and RLH_* names
