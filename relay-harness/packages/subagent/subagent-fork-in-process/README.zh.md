...
运行生命周期、模型继承与深度跟踪均为共享行为，见 [`rlh-subagent-spawn-in-process`](../subagent-spawn-in-process/README.md)。
...
父 agent 只通过 `rlh-tool-subagent` 接收子 agent 自身的最终输出，不接收继承的前缀或中间工作。
