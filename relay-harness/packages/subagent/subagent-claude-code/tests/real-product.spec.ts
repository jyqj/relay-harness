...
const settingsModel = 'rlh-settings-inheritance-marker'
const fakeKey = 'rlh-fake-anthropic-key'
...
async function realInstanceFixture(
  ...
  const root = mkdtempSync(join(tmpdir(), 'rlh-claude-code-real-'))
  roots.push(root)
...
describe('real Claude Agent SDK 0.3.240 and its distributed Claude Code 2.1.240 fixture', {
  timeout: 60_000,
}, () => {
  it('inherits host settings and sends the exact task and fake key to local Messages', async () => {
    const sentinel = 'REAL_CLAUDE_CODE_SENTINEL_2_1_240'
    const task = 'Return the fixture sentinel exactly.'
    const { harness, fixture } = await realHarness({
      kind: 'complete',
      text: sentinel,
    })
    expect(sdkPackage.version).toBe('0.3.240')
    expect(sdkPackage.claudeCodeVersion).toBe('2.1.240')
    expect(sdkPackage.optionalDependencies[platformPackage]).toBe('0.3.240')
    const version = await execFileAsync(claudeBin, ['--version'], {
      env: { ...process.env, ...harness.env },
    })
    expect(version.stdout.trim()).toBe('2.1.240 (Claude Code)')
    ...
    expect(initMessage?.claude_code_version).toBe('2.1.240')
  ...
  it('maps a real SDK max-turns result to safe query-run facts', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-claude-code-max-turns-'))
    roots.push(root)
  ...
  it('overrides interactive settings, denies a write, and returns a safe diagnostic', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-claude-code-denied-target-'))
    roots.push(root)
  ...
  it('runs an explicitly selected bypass write in the isolated workspace', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-claude-code-bypass-target-'))
    roots.push(root)
  ...
  it('returns the completed plan without approving execution', async () => {
    ...
    expect(JSON.stringify(fixture.requests[1]?.body.messages))
      .toContain('ExitPlanMode is disabled for this session, in subagents as well as here')
    await run.dispose()
    await expectQuiescent(harness.handles)
  })
