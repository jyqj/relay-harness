...
The generated [`report` schema](../../../docs/tool-catalog.md#relay-harnessrlh-tool-subagent-report): one required `output` string. Its description states that the child must report once before finishing, that reporting reaches only the Agent that started the child, and that it does not end the turn. It carries no recipient or delivery-mode parameter. The separate `tool:report` prompt section repeats the obligation outside the schema, where a child that ignores tool descriptions still reads it.
...
- **A parent whose host-owned disposal already started can still accept** — `AgentHandle.dispose()` cancels, awaits quiescence, and only then unwinds the scope and leaves the registry; it exposes no signal for "disposal started." Acceptance requires the parent to remain registered through the child-report durability barrier. If it leaves during that await, inbox admission fails while the child retains its durable pending report for recovery. A successful receipt still does not prove that the closing parent consumed the report or appended it to its transcript. A continuation-manager-owned parent rejects forest teardown through the manager's admission boundary.
- **Recovery waits for child materialization** — accepted reports are durable and idempotent, but no background pump activates a cold child solely to retry parent delivery.
