 ...
 * @module @relay-harness/rlh-subagent-rlh-sdk
 */

import type { Context } from '@relay-harness/cordis'
import z from '@relay-harness/schemastery'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@relay-harness/rlh-subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@relay-harness/rlh-subagent'
import {
...
export const name = 'subagent-rlh-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('rlh-sdk'),
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    const spec: SdkRunSpec = {
      ...
      cwd: resolveChildCwd('subagent-rlh-sdk', this.config.cwd, request.parent.session.header.cwd),
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-rlh-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-rlh-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-rlh-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-rlh-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-rlh-sdk maxTokens must be a positive safe integer')
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-rlh-sdk', resolved.cwd)
