/**
 * Fresh-process SDK subagent client. Drives one child Relay Harness
 * runtime over stdio JSON-RPC through `@relay-harness/rlh-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@relay-harness/rlh-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @relay-harness/rlh-subagent-rlh-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { RelayHarness, type HarnessNotification } from '@relay-harness/rlh-sdk-client'
...
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@relay-harness/rlh-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@relay-harness/rlh-subagent'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `rlh-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `RLH_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `RLH_*` fact survives while ambient namesakes never leak.
   */
