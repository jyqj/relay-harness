...
import * as SubagentFork from '@relay-harness/rlh-subagent-fork-in-process'
import type { GenerateOptions, MessageId, StreamChunk } from '@relay-harness/rlh-llm'
import { CallId, createUserMessage, LlmAdapter } from '@relay-harness/rlh-llm'
...
async function setupWith(
  ...
  if (options.persistence !== false) {
    root = mkdtempSync(join(tmpdir(), 'rlh-subagent-continuation-'))
    roots.push(root)
...
describe('continuable settlement delivery', () => {
  ...
  it('reports a turn that failed before reaching its first step', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter)
    // The shipped durability checkpoint (`rlh-session-checkpoint-policy`) is
...
describe('continuable errors', () => {
  ...
  it('unloading the manager drains its live activations', async () => {
    ...
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const root = mkdtempSync(join(tmpdir(), 'rlh-subagent-continuation-'))
    roots.push(root)
