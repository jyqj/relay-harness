...
import type { SubagentListEntry } from '@relay-harness/rlh-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'rlh-tool-list-agents-'))
  roots.push(root)
...
describe('rlh-tool-subagent-control/list-agents', () => {
