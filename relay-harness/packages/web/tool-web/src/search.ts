...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@relay-harness/rlh-tools'
import type { WebSearchResult, WebSearchSource } from '@relay-harness/rlh-web'
import type {} from '@relay-harness/rlh-system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `rlh-tool-fs`'s
 ...
 *   `ToolDefinition.timeoutMs` for `@relay-harness/rlh-tool-call-timeout-policy` to enforce.
