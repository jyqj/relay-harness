...
export class WebRuntime extends Service {
   ...
   * `$RLH_WEB_SEARCH_PROVIDER` / `$RLH_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.RLH_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.RLH_WEB_FETCH_PROVIDER
  }
