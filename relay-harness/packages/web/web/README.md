...
| Package | Role |
|---|---|
| `@relay-harness/rlh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@relay-harness/rlh-web-search-exa` | Search provider: Exa |
| `@relay-harness/rlh-web-search-perplexity` | Search provider: Perplexity |
| `@relay-harness/rlh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@relay-harness/rlh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
...
Providers register **capabilities**, not tools. `rlh-tool-web` is the only owner of model-facing names, descriptions, prompt guidance, JSON schemas, and presentation.

## Selection

Selection never depends on registration, config, or HMR order. A capability has an explicit provider id (config `searchProvider`/`fetchProvider`, or env `$RLH_WEB_SEARCH_PROVIDER`/`$RLH_WEB_FETCH_PROVIDER` feeding the same fields), or auto-selects when exactly one usable provider is registered. `search()`/`fetch()` resolve the provider at execution time:
...
Indirectly, through `rlh-tool-web`, which retains bounded normalized provider data or the exact configured-provider, unavailable-provider, no-provider, multiple-provider, and `Error: <message>` failures while this registry contributes no prompt or schema itself.
