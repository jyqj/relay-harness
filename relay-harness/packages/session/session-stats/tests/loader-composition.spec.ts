...
import * as SessionStatsPlugin from '@relay-harness/rlh-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'rlh-session-stats-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@relay-harness/rlh-session-projection', SessionProjectionRegistry],
    ['@relay-harness/rlh-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@relay-harness/rlh-session-projection'",
