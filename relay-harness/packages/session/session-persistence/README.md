...
| `prepare(id, signal?, fence?): Promise<SessionPreparation>` | Reserve the exact unpublished Session used by resume. A coordinator reuses an earlier inspection when available, commits pending recovery, and releases an unpublished reservation back to its bounded cache on disposal. |
...
Each `session/event` copies its event into the session controller. The first pending event starts a fixed batching window; later events join without resetting its deadline. The configured `writeBatchMaxDelayMs` bounds this intentional wait, not event-loop, initialization, serialized-operation, or backend latency. Events admitted during a write form a new bounded batch. `session/flush` cancels the wait and is a shared quiescence barrier that drains events admitted while it runs. A background failure is logged once, retains the ordered batch, and pauses automatic retry; a new event starts a fresh window, while explicit flush or backend teardown retries immediately and surfaces a repeated failure. When the live Session carries a `SessionPersistenceFence`, the coordinator asserts it before enqueue and explicit flush, after initialization reads, before live adoption or repair, and when a buffered append leaves the per-id queue. The common append path also checks the exact live owner immediately before calling the backend, including direct service appends to an owned id. A failed check rejects without dispatching that storage mutation; retained batches remain subject to the same check on retry. The live controller retains the exact captured proof through batching, retry, and retirement even after its Session registration is retired. Retirement closes new source appends; replacing a Session's lifetime proof rejects instead of transferring old writes to a new owner. A proof with `runExclusive` wraps the entire backend append or repair, including asynchronous I/O and durability; the lease provider must use that same exclusion for release and takeover. Assertion-only proofs remain pre-dispatch checks. `prepare(id, signal?, fence?)` carries the exact proof through cold recovery before live setup installs it; an interrupted caller does not release an in-flight mutation early.

An unpublished preparation that acquired a lifetime `SessionPersistenceFence` is discarded on release, even if setup appended no events. A retry reloads a new Session object for its successor proof; ordinary unfenced preparations remain reusable while their durable revision is current.
...
When a live session emits `session/disposed`, the coordinator waits for its controller, serializes a final drain, then releases state owned by that exact `Session` object. A failed drain arms one backoff retry while the retirement record stays pending; if the retry also fails, the stranded lifecycle is released (its buffered events are lost), a same-id recreate names the underlying write failure instead of a collision, and backend teardown does not report the failure a second time. Backend teardown runs the listeners and the final drain as one composite effect whose serial chain removes every listener before the drain starts, flushes every remaining controller, awaits per-id operations, and only then closes the storage handle.
