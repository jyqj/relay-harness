...
import SessionTitleService, { foldSessionTitle } from '@relay-harness/rlh-session-title'
...
describe('session title persistence round trips', () => {
  it('round-trips through a remounted JSONL backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'rlh-title-jsonl-'))
    roots.push(root)
  ...
  it('round-trips through a remounted SQLite backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'rlh-title-sqlite-'))
    roots.push(root)
