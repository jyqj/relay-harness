...
import * as providerPlugin from '@relay-harness/rlh-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'rlh-title-loader-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@relay-harness/rlh-llm'",
    "- name: '@relay-harness/rlh-session'",
    "- name: '@relay-harness/rlh-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@relay-harness/rlh-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@relay-harness/rlh-session-title', SessionTitleService],
    ['@relay-harness/rlh-session-title-first-prompt-llm', providerPlugin],
  ])
