...
async function freshRoot(prefix = 'rlh-jsonl-zstd-'): Promise<string> {
...
runPersistenceContract('jsonl-zstd', async () => {
  const root = await mkdtemp(join(tmpdir(), 'rlh-jsonl-zstd-contract-'))
  const ctx = new Context()
  await ctx.plugin(SessionStore)
...
runCoordinatorContract('jsonl-zstd', async (): Promise<CoordinatorFixture> => {
  const root = await mkdtemp(join(tmpdir(), 'rlh-jsonl-zstd-coordinator-'))
  return {
...
describe('JsonlSessionPersistence: encoding selection', () => {
  it('rejects roots owned by the opposite encoding in both directions', async () => {
    const rawRoot = await freshRoot('rlh-jsonl-raw-mismatch-')
    ...
    const zstdRoot = await freshRoot('rlh-jsonl-zstd-mismatch-')
