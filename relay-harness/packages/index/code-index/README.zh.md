...
**`CodeIndex`**（`ctx.codeIndex`）定义本地代码索引后端的 WHAT——报告健康状态、刷新派生磁盘索引、排序候选、hydrate 已验证当前源文件的 chunk 正文，并探索派生图——而不规定它如何存储、扫描或排序。
...
| `@relay-harness/rlh-code-index-local` | 显式单 Workspace Provider：扫描、增量 diff、切片和 SQLite 索引 |
| `@relay-harness/rlh-code-index-workspace-router` | 多 Workspace Provider：规范路由、独立 runtime/store 和有界生命周期 |
...
| 成员 | 语义 |
|---|---|
| `forWorkspace(workspaceRoot)` | 将后续读取和变更绑定到调用方选择的规范 Workspace；Web Provider 拒绝未限定 Workspace 的操作。 |
...
| `hydrateChunks(request, signal?)` | 批量解析完整索引正文，并以已存内容哈希重新验证每个源文件。过期、缺失和不可读 identity 只返回显式 rejection，绝不返回正文。 |
...
`EpochPair` 是两个单调时钟：`indexEpoch` 在每个承载内容变更的写事务提交时前进一次，`evidenceEpoch` 预留给语义证据摄入；seam 之上的缓存键必须包含两者。`RepoSizeTier` 是 `src/tiers.ts` 中自适应常量的唯一来源。`SearchResult` 有界且自描述：hit 携带文件内容哈希、真实 parser tier/confidence、顺序固定的加法 `scoreTrace`、确定性 `reasons` 与可选图连通度分，但不携带源码正文。`HydrateChunksResult` 仅在当前源文件验证通过后独立返回正文，既维持候选缓存上限，也给 evidence 消费方逐资源 revision。完整契约见 `src/types.ts`。
