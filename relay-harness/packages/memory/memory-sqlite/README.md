...
- **User review signals pending** — the signal store accepts `user_confirmed` and `user_rejected`, but no review surface currently emits them; every recorded signal is `candidate_hit` or `injected` until that Consumer exists.
