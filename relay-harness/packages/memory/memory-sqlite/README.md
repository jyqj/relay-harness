...
Each returned search or recall hit increments `usefulAccessCount` in a transaction, while a committed injection increments `accessCount`. Ordinary accounting failures remain fail-open so retrieval stays available; an ownership-CAS failure is the exception and fails loud after rollback, because a superseded provider may not keep reading-and-writing the canonical store.

One process owns each canonical database path. The provider claims a pid/boot-id heartbeat row at startup and runs the ownership CAS before the first mutation in every write transaction—including prepared-turn settlement, access accounting, and extraction lease settlement. Same-process sibling handles retain that process claim until the last handle closes. Startup fails loudly against a fresh foreign heartbeat (a stale one is reclaimed), and every later write by a superseded owner rolls back and fails loudly instead of silently co-writing.
