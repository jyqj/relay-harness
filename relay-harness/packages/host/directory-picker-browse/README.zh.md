...
- **不枚举网上邻居**——不会把 SMB／WebDAV 共享列为计算机层的一行；完全限定的 UNC 路径仍可通过 `list` 与路径编辑器列举。
