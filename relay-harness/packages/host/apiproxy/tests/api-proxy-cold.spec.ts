...
import { createUserMessage, MessageId } from '@relay-harness/rlh-llm'
import type { Agent } from '@relay-harness/rlh-agent'
import UserQuestionService from '@relay-harness/rlh-user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@relay-harness/rlh-session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@relay-harness/rlh-session-persistence'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'rlh-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'rlh-cold-race-'))
    const path = join(root, 'small.log')
    writeFileSync(path, 'x')
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@relay-harness/rlh-session').Session
