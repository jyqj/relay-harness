...
describe('sessions domain schemas', () => {
  it('validates ids, summaries, and the event passthrough envelope', () => {
    ...
    expect(sessionSummarySchema.parse({
      ...
      parentSessionId: 'p', seedLength: 7, cwd: '/x',
    })).toMatchObject({ parentSessionId: 'p', seedLength: 7, cwd: '/x' })
    expect(() => sessionSummarySchema.parse({
      sessionId: 's1', updatedAt: 1, running: true, blank: false, seedLength: -1,
    })).toThrow()
...
describe('host domain schemas', () => {
  it('validates describe request/value', () => {
    ...
    const value = hostDescribeValueSchema.parse({
      version: '1', cwd: '/x', provider: 'p', model: 'm', attachedSessions: 2, home: '/h', canOpenPath: true, scratchCwd: '/scratch',
    })
    expect(value).toMatchObject({ provider: 'p', model: 'm', attachedSessions: 2, canOpenPath: true, scratchCwd: '/scratch' })
    expect(hostDescribeValueSchema.parse({
      version: '1', cwd: '/x', attachedSessions: 0, home: '/h', canOpenPath: false, scratchCwd: '/scratch',
...
describe('events frame schemas', () => {
  ...
  it('accepts every host frame branch', () => {
    const frames = [
      { type: 'host/session-added', sessionId: 's', blank: true, parentSessionId: 'p', seedLength: 7 },
