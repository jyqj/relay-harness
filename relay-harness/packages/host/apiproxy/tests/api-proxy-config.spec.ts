...
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@relay-harness/rlh-llm'
import { SettingsProvider, settingsNamespace } from '@relay-harness/rlh-settings'
import type { SettingsNamespace } from '@relay-harness/rlh-settings'
import { CredentialProvider } from '@relay-harness/rlh-credentials'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('rlh-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('rlh-credentials-local')
  })
