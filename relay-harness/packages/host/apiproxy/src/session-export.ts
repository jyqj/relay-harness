...
import type { SessionLineageNode, SessionQueryEngine } from '@relay-harness/rlh-session-query'
import type { SessionId, SessionStore } from '@relay-harness/rlh-session'
import type { SessionPersistence, SessionRawArtifact } from '@relay-harness/rlh-session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `rlh-session-${safeSessionIdSegment(sessionId)}.zip`
}
