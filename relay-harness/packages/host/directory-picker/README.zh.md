...
- **不做按部署限定的浏览根**——`workspace.create` 接受任意路径，因此这里的根会限定 UX 范围，而不是安全边界。Win32 盘符根在 `WINDOWS_VOLUME_ROOT` 列举。不枚举网上邻居。
