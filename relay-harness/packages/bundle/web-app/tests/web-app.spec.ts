...
function stageDist(): string {
  dist = mkdtempSync(join(tmpdir(), 'rlh-web-app-'))
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    ctx.provide(RLH_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot([
    ...
    expect(log).toHaveBeenCalledWith('rlh web: http://127.0.0.1:4567 (LAN: http://192.168.1.5:4567)')
    expect(log).toHaveBeenCalledWith('rlh web: opening the default browser; pass --no-open to disable')
    expect(openBrowser).toHaveBeenCalledWith('http://127.0.0.1:4567')
    expect(lifecycle).toEqual([
      'rlh web: http://127.0.0.1:4567 (LAN: http://192.168.1.5:4567)',
      'rlh web: opening the default browser; pass --no-open to disable',
      'open:http://127.0.0.1:4567',
    ])
    const assembly = await ctx.systemPrompt.assemble()
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('Relay Harness implementation checkout')
    ...
    expect(webRuntime?.resolve()).toEqual({ RLH_WEB_URL: 'http://127.0.0.1:4567' })
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('rlh web: http://127.0.0.1:4567')
    await ctx.fiber.dispose()
  })
  ...
  it('scrubs the helper environment and reports helper spawn or exit failures', async () => {
    ...
    vi.stubEnv('RLH_HOME', '/must-not-reach-browser')
    ...
    expect(options?.env).not.toHaveProperty('RLH_HOME')
