...
afterEach(async () => {
  ...
  vi.unstubAllEnvs()
  Reflect.deleteProperty(globalThis, '__rlhWebAppApply')
  Reflect.deleteProperty(globalThis, '__rlhWebServer')
})

describe('web app browser startup', () => {
  it('opens the canonical URL only after the complete page is reachable', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-web-browser-open-'))
    tempRoots.push(root)
    ...
    writeFileSync(webserverModule, 'export default globalThis.__rlhWebServer\n')
    writeFileSync(webAppModule, [
      "export const name = 'fixture-web-app'",
      "export const inject = ['webServer']",
      'export const apply = (ctx, config) => globalThis.__rlhWebAppApply(ctx, config)',
      '',
    ].join('\n'))
    ...
    const globals = globalThis as unknown as {
      __rlhWebAppApply: typeof apply
      __rlhWebServer: typeof WebServer
    }
    globals.__rlhWebAppApply = apply
    globals.__rlhWebServer = WebServer
