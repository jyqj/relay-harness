...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@relay-harness/rlh-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `rlh-${name}-`)))
}
...
describe('rlh-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.rlh/skills'), 'same', 'user rlh skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.rlh/skills'), 'same', 'project rlh skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.rlh/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project rlh skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-rlh')
    ...
    await writeSkill(join(noGit, '.rlh/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.rlh/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.rlh/skills'), 'runtime-name', 'User loses')
  ...
  it('rejects skill symlinks that escape their declared root', async () => {
    ...
    await mkdir(join(home, '.rlh/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.rlh/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.rlh/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.rlh/skills/broken-link'))
    await symlink('/dev/null', join(home, '.rlh/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { rlhHome: join(home, '.rlh'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.rlh/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.rlh/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousRlhHome = process.env.RLH_HOME
    const previousAgentsHome = process.env.RLH_AGENTS_HOME
    const previousBundledSkillDir = process.env.RLH_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.RLH_HOME = join(envHome, '.rlh')
      process.env.RLH_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.RLH_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.rlh/skills'), 'env-skill', 'Env skill')
      ...
      process.env.RLH_HOME = join(envHome, 'empty-rlh')
      delete process.env.RLH_BUNDLED_SKILL_DIR
      process.env.RLH_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { rlhHome: join(envHome, 'empty-rlh') }).name).toBe('filesystem')
    } finally {
      if (previousRlhHome === undefined) {
        delete process.env.RLH_HOME
      } else {
        process.env.RLH_HOME = previousRlhHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.RLH_AGENTS_HOME
      } else {
        process.env.RLH_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.RLH_BUNDLED_SKILL_DIR
      } else {
        process.env.RLH_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
