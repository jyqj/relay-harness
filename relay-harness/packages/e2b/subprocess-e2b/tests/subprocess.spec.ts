...
import type { SubprocessSpawnSpec } from '@relay-harness/rlh-subprocess'
import E2BSubprocessRuntime from '@relay-harness/rlh-subprocess-e2b'
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.rlh-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.rlh-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$rlh_e2b_env_bin" -i'))!
    expect(command).toContain('"$rlh_e2b_setsid" --wait -- "$rlh_e2b_bash" -c')
    expect(command).not.toContain('DEEPSEEK_API_KEY')
    expect(command).not.toContain('RLH_MODE')
    ...
    expect(command).not.toContain('${!rlh_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$rlh_e2b_home" -a -d "$rlh_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('rlh_e2b_node="$(command -v node)"')
    expect(command).toContain('"$rlh_e2b_env_bin" -i "$rlh_e2b_node" -e')
    expect(command).toContain('"$rlh_e2b_env_bin" -i -- "${rlh_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$rlh_e2b_env_bin" -i -- "${rlh_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.rlh-e2b/processes/one/pid',
      '/workspace/.rlh-e2b/processes/one/exit-code',
      '/workspace/.rlh-e2b/processes/one/environment',
    ...
    expect(fake.removed).toContain('/workspace/.rlh-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('rlh_e2b_tee='))!
    expect(command).toContain('"$rlh_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$rlh_e2b_tee" --output-error=warn-nopipe')
