...
import { FsTargetKey, FsVersion } from '@relay-harness/rlh-fs'
import E2BFileSystem from '@relay-harness/rlh-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.rlh-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['rlh-version']).toBeDefined()
