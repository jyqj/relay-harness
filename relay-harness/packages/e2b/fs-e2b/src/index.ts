 ...
 * @module @relay-harness/rlh-fs-e2b
 */

import { createHash, randomUUID } from 'node:crypto'
import { Buffer } from 'node:buffer'
import { posix } from 'node:path'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@relay-harness/rlh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type { EntryInfo, Sandbox } from '@relay-harness/rlh-e2b'

const VERSION_METADATA_KEY = 'rlh-version'
...
export class E2BFileSystem extends FileSystem {
  ...
  private async writeAtomic(
    ...
    const stagingDirectory = posix.join(posix.dirname(targetPath), `.rlh-${randomUUID()}.tmp`)
