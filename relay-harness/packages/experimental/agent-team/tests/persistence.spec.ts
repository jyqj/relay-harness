...
import SubagentService, { seedDescriptorTurn, snapshotSubagentDescriptor } from '@relay-harness/rlh-subagent'
...
for (const backend of backends) {
  describe(`${backend.name} Agent Teams recovery`, () => {
    it('reconciles a persisted child to active and a missing child to durable failed', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `rlh-team-${backend.name.toLowerCase()}-`))
    ...
    it('reconciles a provisioning child whose initial prompt is durably pending', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `rlh-team-pending-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('replays queued-minus-delivered mail in FIFO order without waking for quiet mail', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `rlh-team-mail-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('acknowledges target-recorded mail after restart without delivering it twice', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `rlh-team-dedup-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('acknowledges durably pending target mail without cold-resume duplication', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `rlh-team-inbox-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
