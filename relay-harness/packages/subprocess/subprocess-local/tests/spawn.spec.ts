...
import type { SubprocessHandle, SubprocessOutputReader } from '@relay-harness/rlh-subprocess'
...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${RLH_STALE:-absent}|$RLH_SHELL|$RLH_SESSION_ID]"':
      return node('console.log("[" + [process.env.RLH_STALE ?? "absent", process.env.RLH_SHELL, process.env.RLH_SESSION_ID].join("|") + "]")')
    case 'echo "[${RLH_TEST_API_KEY:-absent}|${RLH_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${RLH_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.RLH_TEST_API_KEY ?? "absent", process.env.RLH_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.RLH_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
const spillDir = mkdtempSync(join(tmpdir(), 'rlh-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-rlh-test' })).done)
...
describe.skipIf(process.platform === 'win32')('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@relay-harness/cordis')
    const { default: LocalSubprocessRuntime } = await import('@relay-harness/rlh-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ RLH_X: '1' }).RLH_X).toBe('1')
    } finally {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-rlh-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-rlh-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient RLH env vars from child processes', async () => {
    process.env.RLH_TEST_API_KEY = 'super-secret'
    process.env.RLH_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.RLH_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${RLH_TEST_API_KEY:-absent}|${RLH_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${RLH_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.RLH_TEST_API_KEY
      delete process.env.RLH_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.RLH_TEST_PLAIN
    }
  })

  it('forwards explicit RLH_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient RLH_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.RLH_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${RLH_STALE:-absent}|$RLH_SHELL|$RLH_SESSION_ID]"', {
        env: { RLH_SHELL: '1', RLH_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.RLH_STALE
    }
  })
  ...
  it.skipIf(process.platform === 'win32')('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/rlh-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it.skipIf(process.platform === 'win32')('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/rlh-subprocess-/)
