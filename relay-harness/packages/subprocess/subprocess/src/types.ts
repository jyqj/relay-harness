 ...
 * @module rlh-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for Relay Harness-managed child environment facts. */
export const RLH_ENV_PREFIX = 'RLH_' as const

/** One environment key inside the managed {@link RLH_ENV_PREFIX} namespace. */
export type RlhEnvironmentKey = `${typeof RLH_ENV_PREFIX}${string}`

/** Trusted Relay Harness variables for one child-process execution. */
export type RlhEnvironment = Readonly<Record<RlhEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@relay-harness/rlh-terminal` without a cross-seam dependency;
