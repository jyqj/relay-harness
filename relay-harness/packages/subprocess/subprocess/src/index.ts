 ...
 * `RLH_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `RLH_*` fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent `rlh_*` entry
 * would otherwise survive and read back as `$env:RLH_*` in the child;
 * deliberate lowercase `rlh_*` names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(RLH_ENV_PREFIX)) env[key] = value
  }
  return env
}
