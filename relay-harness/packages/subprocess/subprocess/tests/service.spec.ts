...
import { scrubbedParentEnv, SubprocessRuntime } from '@relay-harness/rlh-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and RLH_ names (case-insensitively) but keeps PATH', () => {
    process.env.RLH_SCRUB_PROBE = 'stale'
    process.env.rlh_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.RLH_SCRUB_PROBE).toBeUndefined()
      expect(env.rlh_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.RLH_SCRUB_PROBE
      delete process.env.rlh_scrub_probe_lower
