/**
 * Real-process tests for `@relay-harness/rlh-pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@relay-harness/rlh-pwsh-local'
...
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@relay-harness/rlh-subprocess'
import { MAX_TIMER_DELAY_MS } from '@relay-harness/rlh-timeout'
import type { ShellProcess } from '@relay-harness/rlh-shell'

const spillDir = mkdtempSync(join(tmpdir(), 'rlh-pwsh-exec-spec-'))
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'rlh-pwsh-resolve-'))
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'rlh-pwsh-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'rlh-pwsh-resolve-dir-'))
    const store = join(dir, 'store')
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'rlh-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'rlh-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-rlh' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/rlhEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:RLH_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      rlhEnv: { RLH_SEAM_VAR: 'rlh-ok' },
    })
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:RLH_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      rlhEnv: { RLH_BG_VAR: 'bg-rlh-env' },
    }))
    const partialOutput = await readUntil(proc, '[bg-env][bg-rlh-env]')
    await proc.done
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-rlh' }))
