 ...
 * managed `RLH_*` collection, abort translation, canonical result projection,
...
import type { ShellProcessRead } from '@relay-harness/rlh-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, rlhHome?: string) {
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, rlhHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed RLH_* environment', async () => {
    const rlhHome = mkdtempSync(join(tmpdir(), 'rlh-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, rlhHome)
    ...
    expect(request?.rlhEnv).toEqual({
      RLH_HOME: rlhHome,
      RLH_SHELL: '1',
      RLH_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const rlhEnv = bash.requests[0]?.rlhEnv
    expect(rlhEnv).toBeDefined()
    expect(rlhEnv?.['RLH_SHELL']).toBe('1')
    expect(rlhEnv?.['RLH_HOME']).toEqual(expect.any(String))
    expect(rlhEnv).not.toHaveProperty('RLH_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'rlh-tool-pwsh-policy-'))
