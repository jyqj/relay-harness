 ...
 * `@relay-harness/rlh-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `rlh-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `RLH_*` environment through the
 ...
 * exit-status parse from `@relay-harness/rlh-shell`.
 *
 * @module @relay-harness/rlh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@relay-harness/cordis'
import z from '@relay-harness/schemastery'
import { defineTool, TOOL_ABORTED } from '@relay-harness/rlh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@relay-harness/rlh-tools'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@relay-harness/rlh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@relay-harness/rlh-sandbox'
import type { SandboxPolicyService } from '@relay-harness/rlh-sandbox-policy'
import type { ShellRunResult } from '@relay-harness/rlh-shell'
import { parseExitStatus } from '@relay-harness/rlh-shell'
...
/* jscpd:ignore-start -- minimal mirror of rlh-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:RLH_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror rlh-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of rlh-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of rlh-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of rlh-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors rlh-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with rlh-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors rlh-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        rlhEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @relay-harness/rlh-jobs and @relay-harness/rlh-tool-jobs')
        }
