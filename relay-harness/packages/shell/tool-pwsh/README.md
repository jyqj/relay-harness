...
The model-facing `pwsh` tool registered over the `ctx.shell` executor seam. Intended for Windows compositions where a PowerShell executor (e.g. `@relay-harness/rlh-pwsh-local`) backs `ctx.shell`; the tool contract is PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables. Behavior mirrors `rlh-tool-bash` call-for-call — foreground and `run_in_background` execution through the generic job runtime, the managed `RLH_*` environment through the shared `shell-env` registry, the sandbox denial rendering with the same-turn `sandbox_permissions` escalation surface, and the bash marker/truncation rendering story (a clean exit produces no marker).
...
Every foreground and background model pwsh call receives a freshly collected trusted `RLH_*` environment through the shared [`rlh-shell-env`](../shell-env/) registry: `RLH_HOME` (the absolute Harness home), `RLH_SHELL=1`, the agent's `RLH_SESSION_ID`, and `RLH_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `RLH_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.rlhEnv` channel; `process.env` is never modified. The description teaches the generic `$env:RLH_*` convention rather than naming persistence-specific variables.
...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#relay-harnessrlh-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
...
- **No persistent shell** — every call starts a fresh `pwsh -Command`; the persistent-shell counterpart is [`@relay-harness/rlh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md), which keeps one owner-scoped pwsh alive across calls on Windows (ConPTY) and POSIX hosts with pwsh.
