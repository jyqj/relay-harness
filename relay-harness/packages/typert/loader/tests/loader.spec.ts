...
import * as typertLoader from '@relay-harness/rlh-typert-loader'
import { validateTypertManifest } from '@relay-harness/rlh-typert-loader'
import { z } from 'zod'
...
afterEach(async () => {
  ...
  context = undefined
  Reflect.deleteProperty(globalThis, '__rlhTypertLoaderGate')
...
describe('typert loader', () => {
  it('registers an explicit package without a Loader entry and withdraws it with the loader', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'rlh-typert-loader-'))
    await linkZod(root)
  ...
  it('drops an in-flight manifest when the loader is disposed before import settles', LOADER_TEST_TIMEOUT, async () => {
    ...
    Reflect.set(globalThis, '__rlhTypertLoaderGate', {
    ...
    await writePackage(root, '@fixture/pending', {
      typertSource: [
        'import { z } from \'zod\'',
        'globalThis.__rlhTypertLoaderGate.started()',
        'await globalThis.__rlhTypertLoaderGate.wait',
