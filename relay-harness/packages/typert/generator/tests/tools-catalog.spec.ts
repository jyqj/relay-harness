...
import type { TypertContribution } from '@relay-harness/rlh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@relay-harness/rlh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven rlh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@relay-harness/rlh-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('rlh-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@relay-harness/rlh-tools')
    ...
    const record = ctx.typert.getPackage('@relay-harness/rlh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@relay-harness/rlh-tools', 'host')).toBeUndefined()
  })
})
