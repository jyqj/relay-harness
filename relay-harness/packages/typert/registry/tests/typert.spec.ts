...
} from '@relay-harness/rlh-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@relay-harness/rlh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@relay-harness/rlh-tools', 'ToolInput')).toBe('@relay-harness/rlh-tools#ToolInput')
    expect(typertPackageKey('@relay-harness/rlh-tools', 'host')).toBe('@relay-harness/rlh-tools#host')
    expect(ctx.typert.get('@relay-harness/rlh-tools#ToolInput')).toMatchObject({
      package: '@relay-harness/rlh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@relay-harness/rlh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@relay-harness/rlh-tools', 'host')).toMatchObject({
      key: '@relay-harness/rlh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@relay-harness/rlh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@relay-harness/rlh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@relay-harness/rlh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@relay-harness/rlh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@relay-harness/rlh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@relay-harness/rlh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@relay-harness/rlh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@relay-harness/rlh-tools#ToolInput')).not.toBe(projected)
  })
