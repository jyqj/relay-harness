...
import TrackerRegistry, { TrackerIssueId, type TrackerIssue, type TrackerProvider } from '@relay-harness/rlh-tracker'
...
import { IssueRunner, type IssueRun, type IssueRunId, type IssueRunRequest, type IssueRunResult } from '@relay-harness/rlh-issue-runner'
...
/** Advance a fake scheduler until one public orchestration fact becomes observable. */
async function advanceUntil<T>(read: () => T | undefined, maxVirtualMs = 1_000): Promise<T> {
  for (let elapsed = 0; elapsed <= maxVirtualMs; elapsed += 1) {
    const value = read()
    if (value !== undefined) return value
    await vi.advanceTimersByTimeAsync(1)
  }
  throw new Error('condition did not become true under the virtual scheduler')
}

afterEach(() => { vi.useRealTimers() })

/** Retain published retry errors so short durable retry states cannot be missed by polling. */
function observeRetryErrors(ctx: Context): { readonly errors: string[]; readonly dispose: () => void } {
  const errors: string[] = []
  const dispose = ctx.on('issue-orchestration/changed', () => {
    const error = ctx.issueOrchestration.snapshot().retrying[0]?.error
...
describe('DurableIssueOrchestrator', () => {
  ...
  it('uses bounded retry timing and preserves the incremented attempt', async () => {
    vi.useFakeTimers()
    const harness = await boot()
    await advanceUntil(() => harness.runner.pending[0])
    harness.runner.pending[0]?.settle({
      stopReason: 'failed', sessionId: SessionId('session-1'), turns: 1, error: 'transport',
    })
    const retrying = await advanceUntil(() => harness.ctx.issueOrchestration.snapshot().retrying[0])
    expect(retrying).toMatchObject({ status: 'retrying', attempt: 2, error: 'transport' })
    await advanceUntil(() => harness.runner.pending[1])
    expect(harness.runner.pending[1]?.request.attempt).toBe(2)
    await harness.fiber.dispose()
  })
  ...
  it('queues a growing continuation retry while the completed issue remains active', async () => {
    ...
    const first = await advanceUntil(() => harness.ctx.issueOrchestration.snapshot().retrying[0])
    ...
    const second = await advanceUntil(() => harness.ctx.issueOrchestration.snapshot().retrying[0]?.attempt === 3
  ...
  it('uses fallback settlement errors and retries a completed refresh failure', async () => {
    ...
    await eventually(() => completedObserver.errors.includes('completion refresh failed')
      ? true : undefined)
    completedObserver.dispose()
