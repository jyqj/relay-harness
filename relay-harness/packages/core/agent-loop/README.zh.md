...
恢复会在未发布的 setup 前，将可选 `persistenceFence` 转发给 `SessionPersistence.prepare(id, signal, fence)`。持久化提供方在冷修复全程持有其排他权限；owner 在 setup 中安装同一证明，以保护后续活动写入。
...
可选的 `@relay-harness/rlh-agent-loop/invariant` 配套入口会向 `ctx.invariants` 注册请求重建。循环会把每个确切的冻结请求记录在 `rlh-llm` 拥有的进程本地身份集合中；随后，配套入口要求存在实时会话，并根据日志独立重建消息边界和折叠后的请求 header。即使调用方冻结直接的一次性调用，或为其附加会话 id，这类调用仍不属于该约定。
...
通过配置创建的 agent 会自动启动。模型调用同时需要 `provider` 和 `model`；`agent/request` 可以在分发前补齐缺失的这一对值。可选的正数 `maxTokens` 会为每次对话请求提供初始输出上限，并记录在请求 header 中。`maxParallelToolCalls` 限制每个 agent 针对并行安全调用使用的滚动池，默认值为 `10`；它同时也是 `agent-loop` Settings 段的全部内容，因此叠加在该条目之上的用户层无需重启即可限制下一组工具调用，而非正整数的值会在写入时被拒绝，而不是到那一组时才失败。部署方拥有的 inbox 上限默认为 `4096` 条消息，以及跨 `next-turn` 与 `next-step` 合计 64 MiB 的无损 JSON 消息数据；实时插入若超过任一上限，会在发布事件或通知前失败，而已持久化、且高于后来下调上限的 inbox 仍可恢复并逐步排空。`agents` 刻意不在 Settings 段中——它在服务启动时被消费一次，所以存储的改动只会看起来生效。`cwd` 仅应用于全新会话，而 `resumeSessionId` 保留持久化元数据。通过配置创建的 agent 使用部署 persona；编程式 setup 可以按 agent 遮蔽它。该插件为每个 agent 提供 `provider`、`model` 和 `cwd` 提示词变量；harness 身份与部署 persona 属于 `rlh-system-prompt`。
...
可选 ContextEngine 返回带归属的上下文时，获准步骤会让该准备结果穿过 `agent/pre-step`。AgentLoop 先追加每一条经准入的消息，再在 request header 物化与分派之前记录一条仅存在于日志的 `context/prepared` 事件。每个 contribution 只链接到内容未被改写的精确 `user/message` seq；拟议消息被移除或改写时，其证据与覆盖仍保留，但链接列表为空。因此会话 replay 仍从普通消息 surface 重建模型输入，而客户端可以检查持久来源，不会把 trace 数据当成第二份 transcript。
...
提示词组装前，循环会为该模型请求捕获确切的 scope 工具定义、呈现模式、Code Mode 后端与 schema 集合。最终组装结果会绑定模型可调用的捕获名称。在由此产生的步骤内，独占调用形成屏障，可并行调用使用有界滚动池；资源相关调用会在紧邻主体前获取 runtime 读写锁，因此相同规范 key 会串行化，无关 key 可以重叠。实时注册表替换会影响下一步骤，而不会重新分类本响应。只有分发和调用主体的执行会发生重叠。策略、持久结果和结果上下文仍保持模型顺序。中止会阻止启动新的调用，等待已启动调用的结果处理完毕，并保留其完成终结后的结果上下文，不区分取消原因。内部调度器故障会停止新的分发，等待已启动的分发，然后在不虚构工具结果的情况下到达轮次错误边界。拒绝、空步骤、请求失败、取消与普通结算的每条路径都会释放请求快照（[决策](../../../.agents/notes/implemented/architecture/2026-08-21-request-tool-snapshot.md)）。
...
- 模型请求恢复：`rlh-llm-retry` 在 `agent/request-error` 上记录并等待针对确切提供方配置的 normal 或无界退避，发出不进入表层的 `llm/retry` 状态，然后返回重试动作
...
- subagent：在循环外部实现为 `ctx.subagents` 提供方；进程内提供方使用 `ctx.agents.create()` 创建 agent，并通过其拥有的 `AgentHandle` 执行 teardown，而通用的 [`ctx.jobs`](../../jobs/jobs/) 与 [`rlh-tool-subagent`](../../subagent/tool-subagent/) 负责后台收集。
