 ...
 *
 * @module @relay-harness/rlh-agent-loop
 */

import { Context, FiberState, Service } from '@relay-harness/cordis'
import { randomUUID } from 'node:crypto'
import z from '@relay-harness/schemastery'
import { emitAgentEvent } from '@relay-harness/rlh-agent'
import type {
  Agent,
  AgentFactory,
  AgentHandle,
...
import { SessionId, SessionPreparation } from '@relay-harness/rlh-session'
import type { Session, SessionHeader } from '@relay-harness/rlh-session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a rlh-session-persistence backend)')
    }
      ...
      try {
        try {
          preparation = await raceAbortCall(
            () => persistence.prepare(id, fused, options.persistenceFence),
            fused,
            id,
