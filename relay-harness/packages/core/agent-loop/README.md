...
Resume forwards an optional `persistenceFence` to `SessionPersistence.prepare(id, signal, fence)` before unpublished setup. The persistence provider holds its exclusion through cold repair; the owner installs the same proof during setup for later live writes.
...
The optional `@relay-harness/rlh-agent-loop/invariant` companion registers request reconstruction with `ctx.invariants`. The loop records each exact frozen request in the process-local identity set owned by `rlh-llm`; the companion then requires a live session and independently rebuilds the message boundary and folded request header from the log. Direct one-shot calls remain outside this contract even when callers freeze them or attach a session id.
...
When the optional ContextEngine returns attributed context, the accepted step carries that preparation through `agent/pre-step`. AgentLoop appends every admitted message first, then records a log-only `context/prepared` event before request-header materialization and dispatch. Each contribution links only to exact unmodified `user/message` seqs; a removed or rewritten proposal keeps its evidence and coverage with an empty link list. Session replay therefore reconstructs model input from the ordinary message surface while clients can inspect durable provenance without treating trace data as a second transcript.
...
Before prompt assembly, the loop captures the exact scoped tool definitions, presentation mode, Code Mode backend, and schema set for the sampling request. The final assembly binds which captured names the model may call. Within the resulting step, exclusive calls form barriers and parallel-capable calls use a bounded rolling pool; resource-relative calls acquire runtime read/write locks immediately before their bodies, so equal canonical keys serialize while unrelated keys overlap. Live registry replacement affects the next step rather than reclassifying this response. Only dispatch/body overlaps. Policy, durable results, and result context remain model-ordered. Abort stops new calls, drains started results, and retains their finalized result context without distinguishing the cancellation cause. An internal scheduler failure stops new dispatches, waits for already-started dispatches, and reaches the turn error boundary without fabricating tool results. Every reject, empty step, request failure, cancellation, and ordinary settlement releases the request snapshot ([decision](../../../.agents/notes/implemented/architecture/2026-08-21-request-tool-snapshot.md)).
...
- Model-request recovery: `rlh-llm-retry` records and waits exact-provider normal or unbounded backoff on `agent/request-error`, emits non-surface `llm/retry` status, then returns a retry action
...
- Sub-agents: implemented outside the loop as `ctx.subagents` providers; in-process providers use `ctx.agents.create()` and owned `AgentHandle` teardown, while generic [`ctx.jobs`](../../jobs/jobs/) plus [`rlh-tool-subagent`](../../subagent/tool-subagent/) own background collection.
...
- **Relational concurrency requires canonical identity** — unary `isConcurrencySafe` handles globally safe calls; tools with provider-resolved identity use `resourceIntents`, while undeclared relational safety remains exclusive ([rationale](../../../.agents/notes/implemented/architecture/2026-08-21-tool-resource-intent-locks.md)).
...
- **No loop-internal turn budget** — tool calls or steering continue the current turn; the shipped base composes `rlh-token-budget-controller`, which refuses a pre-step after 64 durable step starts. A custom composition that removes that policy must supply its own lifecycle extension.
