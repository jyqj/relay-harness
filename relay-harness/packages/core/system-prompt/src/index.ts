 ...
 *
 * @module @relay-harness/rlh-system-prompt
 */

import { Context, Service } from '@relay-harness/cordis'
import z from '@relay-harness/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@relay-harness/rlh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@relay-harness/rlh-scope'
import type { ContextSnapshotSection, ToolSchema } from '@relay-harness/rlh-llm'
...
declare module '@relay-harness/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@relay-harness/rlh-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Relay Harness.',
      })
    }
    this.section({
