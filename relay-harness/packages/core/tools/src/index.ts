 ...
 * execution pipeline.
 * @module @relay-harness/rlh-tools
 */

import { Context, Service } from '@relay-harness/cordis'
import z from '@relay-harness/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@relay-harness/rlh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@relay-harness/rlh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@relay-harness/rlh-llm'
import { assertNever, deepFreeze, HarnessError } from '@relay-harness/rlh-llm'
import type { Agent } from '@relay-harness/rlh-agent'
import { snapshotJsonValue } from '@relay-harness/rlh-session'
import type { JsonValue, UserMessage } from '@relay-harness/rlh-session'
import type { AssembleContext, ToolProviderResult } from '@relay-harness/rlh-system-prompt'
import type { CodeRuntime } from '@relay-harness/rlh-code-runtime'
 ...
 * instead of deriving them: the seam's `rlh-code-runtime` README pair, its
...
export type { JsonValue } from '@relay-harness/rlh-session'
...
// lives in its own UI-facing module; re-export it so `@relay-harness/rlh-tools`
...
declare module '@relay-harness/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@relay-harness/rlh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@relay-harness/rlh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol.for('@relay-harness/rlh-tools.scheduler')

/** Agent-loop entry point for request-scoped tool capture. */
export const TOOL_RUNTIME_REQUESTS: unique symbol = Symbol('@relay-harness/rlh-tools.requests')

/** Symbol-keyed prompt-assembly context slot carrying one tool request snapshot. */
export const TOOL_REQUEST_SNAPSHOT: unique symbol = Symbol('@relay-harness/rlh-tools.request-snapshot')

declare module '@relay-harness/rlh-system-prompt' {
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `rlh-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: AssembleContext) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`rlh-tools: no SDK renderer for ${runtime.language}`)
  ...
  private captureRequest(scope?: ScopeKey): ToolRequestSnapshot {
    ...
    if (codeRuntime !== undefined) {
      ...
      if (render === undefined) throw new Error(`rlh-tools: no SDK renderer registered for runtime language ${JSON.stringify(codeRuntime.language)}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`rlh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @relay-harness/rlh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`rlh-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `rlh-agent-tool-presentation` exists for, and reading the deployment default would
