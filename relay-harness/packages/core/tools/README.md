...
Each agent step captures one request-scoped tool snapshot before prompt assembly. The snapshot supplies the model-facing schemas, then binds direct execution to the final authoritative assembly and retains the same definitions, concurrency classifiers, Code Mode SDK, and code backend until every resulting call settles. Registry changes affect the next step; live hooks, approval, guards, and execution policy still apply to the captured call. A removed tool therefore cannot be replaced beneath an in-flight request, while a final assembly listener can still remove it from that request entirely. The [request snapshot decision](../../../.agents/notes/implemented/architecture/2026-08-21-request-tool-snapshot.md) owns the lifecycle and HMR trade-off.

A tool whose overlap safety is resource-relative declares `resourceIntents(args, exec)`. The resolver runs after pre-execute policy and returns canonical namespaced read/write claims; the call joins the parallel pool, then the runtime acquires fair writer-preferred locks immediately before its body. Claims are sorted, duplicates collapse with write dominance, and cancellation removes queued waiters. Native, direct, and Code Mode calls share one lock table. The [resource-lock decision](../../../.agents/notes/implemented/architecture/2026-08-21-tool-resource-intent-locks.md) owns identity and fairness.
...
- `ctx.tools.executionMode(exec)` returns `parallel` when the visible definition declares `resourceIntents`, or when `isConcurrencySafe(exec.arguments)` returns exactly `true`; unknown, hidden, undeclared, invalid, or throwing classifications are exclusive.
...
- `ToolExecutionResult` — discriminated execution-local outcome. Success is `{ isError:false, value:JsonValue, content, meta?, producedFiles?, additionalContexts? }`; failure is `{ isError:true, error:{ message, info? }, content, meta?, additionalContexts? }` and has no value. Call identity stays on the immutable `ToolExecution`. The registry snapshots, validates, and freezes the canonical value before rendering, then materializes the durable presentation fields before final observation. `ToolFailure.info` carries an internal `{ name, code }` for a `HarnessError`; `additionalContexts` preserves every deferred or post-execute identified `UserMessage` for the loop's post-result FIFO.
...
- `ToolGuard` — `(execution) => string | undefined`; the returned string is a final monotonic denial reason evaluated after the reorderable pre-execute waterfall and again after resource acquisition, immediately before each body invocation. A guard supports repeated synchronous checks; ownership revoked during around-dispatch or lock waits denies the body without relying on a cancellation timer.
...
Successful root results carry `producedFiles` captured from the execution-bound definition's `diff` or `generic/edit` locations. Empty capture records no declared output; missing capture records unavailable presentation and does not invalidate a successful tool execution. The loop logs this field outside model content; it does not audit shell or nested Code Mode side effects.
...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@relay-harness/rlh-tool-call-timeout-policy` wrapper.
...
- **Code Mode intermediate values are execution-local** — the canonical typed values cannot be reconstructed from session replay. The default worker Adapter bounds each argument and resolution with `maxBindingBytes`; another Code Runtime Adapter must define its own equivalent resource contract. The durable log copy of each sub-call IS bounded: the `tools/code-dispatch-log` waterfall lets the spill policy replace an oversized `tool/code-dispatch` content with a preview + locator ([rationale](../../../.agents/notes/implemented/feature/2026-07-26-code-dispatch-log-spill.md)).
