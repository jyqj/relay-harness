...
- `installSessionPersistenceFence(session, fence)` attaches one exact cross-process owner proof. A stale proof rejects new appends. `captureSessionPersistenceFence(session)` retains that exact proof for deferred persistence work: retiring the registration closes source-log append admission permanently for that Session object, while accepted writes retain their proof through drain. A successor uses a new Session object, never a replacement proof on the retired one. Storage dispatch rechecks the captured proof. Its optional `runExclusive(operation)` keeps a complete storage commit mutually exclusive with lease release/takeover; assertion-only proofs do not provide that exclusion. Neither form rolls back completed external effects.
...
`rlh-agent-loop` uses this split so final loop flush precedes session detach; see the [ownership Agent Note](../../../.agents/notes/implemented/architecture/2026-06-18-agent-lifecycle-and-ownership-contracts.md).
...
- Compaction: `rlh-compaction-basic` appends a `user/message` replacement for summary checkpoints, while `rlh-compaction-tool-result-pruner` appends a content-only `tool/result` replacement. Tool-pairing boundary policy and its cache belong to the [`rlh-compaction` seam](../../compaction/compaction/README.md), while this package owns ordered surface membership, replacement validation, and `replaceGeneration`.
