...
Per-preset agent composition. A **preset** is a directory holding one `agent.cordis.yml`; the roster mounts each file generation once under a standing scope, and each session that names it joins by having its agent scope key parented to the mount's (`rlh-scope`'s parent chain). The mount's tools, prompt sections, and projection units exist exactly once and cover every joined agent — its plugins key their state by Session/Agent, so sessions stay apart inside one shared instance — and a host reader with no agent at all (a cold transcript read) resolves the same standing registrations by preset id.

The mechanism is two seams. Entry contexts chain to the context a subtree was plugged into, and both [`rlh-tools`](../../core/tools/README.md) and [`rlh-system-prompt`](../../core/system-prompt/README.md) file registrations into the calling context's scope layer — so the standing mount's contributions land in the PRESET's layer. What carries them to each session is `rlh-scope`'s parent chain: an agent's views resolve `agent → preset → global` (nearest shadowing farthest), and the mount's listeners are admitted for every agent parented under it while a sibling preset's stay deaf.
...
- `ctx.agentPresets.acquireAgentScope(agentCtx): PresetScopeLease | undefined` Synchronously retains the exact joined generation and captures immutable registry ancestry including the agent’s own layer. Release in `finally`; recompose cannot redirect an in-flight read to a replacement preset. An unjoined agent returns undefined. Registry contents still obey revisions; disposing agent-owned effects does not make the read adopt a successor.
- `ctx.agentPresets.acquireStandingScope(id?): Promise<PresetScopeLease>` A cold reader hold carrying `key` and idempotent asynchronous `release()`. Hold it through every asynchronous registry read and release in `finally`; it starts no agent, session, or turn and refuses broken presets like `mount()`.
...
The child records the joined id on its own durable header ([`rlh-subagent`](../../subagent/subagent/README.md)), so a cold read of the child's history rebuilds the composition it actually ran under rather than the deployment default.
...
| `includeUserRoot` | `true` | Append `<rlhHome>/.agent-presets` as a `user` root, after every configured root |
...
`includeUserRoot: false` mounts a roster over `roots` alone. A deployment that confines presets to its own directories needs it, and so does any test pinning an exact roster — otherwise the machine's real `<rlhHome>` decides what the roster contains.
