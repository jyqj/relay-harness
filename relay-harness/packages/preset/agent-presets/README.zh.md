...
按 preset 组装 agent（智能体）。**preset** 是一个目录，其中放置一份 `agent.cordis.yml`；roster 在整个进程内只把它挂载一次（常驻 scope），命名它的每个会话通过把自己 agent 的 scope key 认父到该挂载（`rlh-scope` 的父链）来加入。挂载的工具、提示词段落与投影单元只存在一份，覆盖所有已加入的 agent——其插件本就按 Session/Agent 分键存状态，会话在共享实例内互不串扰——而完全没有 agent 的宿主读取方（冷读记录）也能按 preset id 解析到同一份常驻注册。

其机制是两条 seam。entry 上下文沿原型链连到子树被挂载时所在的上下文，而 [`rlh-tools`](../../core/tools/README.md) 与 [`rlh-system-prompt`](../../core/system-prompt/README.md) 本就按调用方上下文的 scope 分层归档注册——因此常驻挂载的贡献落在 **preset 的分层**里。把它们送达每个会话的是 `rlh-scope` 的父链：agent 的视图按 `agent → preset → global` 解析（近者遮蔽远者），挂载的监听器对认父到它的每个 agent 放行，而兄弟 preset 的监听器保持失聪。
...
- `ctx.agentPresets.acquireAgentScope(agentCtx): PresetScopeLease | undefined` 同步持有已加入的确切代际，捕获包括 Agent 自有层在内的不可变注册表祖先链。在 `finally` 中释放；重新组装不能把读取中的请求转向替代 preset。尚未加入的 Agent 返回 undefined。注册表内容仍遵循 revision 规则；Agent 自有 effect 被释放也不会使读取转向继任者。
- `ctx.agentPresets.acquireStandingScope(id?): Promise<PresetScopeLease>` 冷读取方持有的句柄，包含 `key` 与幂等异步 `release()`。在所有异步注册表读取期间保留句柄，并在 `finally` 释放；不启动 agent、会话或轮次，与 `mount()` 一样拒绝损坏的 preset。
...
agent 工厂的 `setup(agentCtx)` 钩子是唯一受支持的调用点。只有在那里，认父是在 agent 尚未发布时完成的，因此组装被拒绝会让整次创建回滚，而不会留下一个组装到一半的会话。常驻子树归 roster 服务自己的 fiber 所有——刻意用其未追踪的上下文，因为从被追踪的 `this.ctx` 派生的子树会经调用方的 shadow fiber 解析一切服务、无视各 entry 自己的 inject store——因此生命周期不依赖某一个 agent。每个已加入的 agent 与冷读取方持有其精确代际；被替代代际在最后一个持有方释放后销毁，每个 preset 只保留一个当前缓存代际。每个代际记录其组装文件的 stamp（mtime 与大小）：发现 stamp 过期的会话会开启下一个代际，而所有已加入的会话保持各自正在运行的那个——正在运行的会话所加入的组装在其文件被修改或删除后继续存活；文件是唯一的组装编辑器，stamp 正是把编辑送达后续会话的机制。

Agent 通过自己的 Cordis effect 在退出时释放持有。`composeFrom()` 保留父 agent 的精确代际，即使该代际已被替代；`recompose()` 先获取替代代际，再重新绑定并释放旧持有。文件编辑、删除和复制使缓存失效，但不会卸载仍有持有方的代际。退役子树的释放等待异步清理完成，包括 watcher 关闭；清理失败会被报告，但不会回滚已经提交的重新绑定或编写操作。
...
子 agent 会把所加入的 id 记在自己的持久化 header 上（见 [`rlh-subagent`](../../subagent/subagent/README.md)），因此冷读子 agent 的历史时重建的是它实际运行过的组装，而不是部署默认值。
...
"仅限尚未产出任何内容的 agent"是一条产品规则而非机制约束：在对话进行中调换工具，会留下新组装无法执行的、已被记录的工具调用。该规则由网关在传输层执行（[`rlh-apiproxy`](../../host/apiproxy/README.md) 返回 `agent-preset-locked`），因为会话历史在那里才拿得到。
...
行的**包名**从宿主组装解析，而非从 preset 目录解析。Loader 通常按 entry 所属树的 `baseUrl` 解析，而对 preset 而言那就是组装文件所在之处；本地创作的 preset 位于用户主目录之下，Node 向上查找 `node_modules` 永远够不到 harness，因此每一个 `@relay-harness/rlh-*` 行都会导入失败。挂载在插入子树之前先记录宿主的基址，并把裸标识符送往那里。
...
| `includeUserRoot` | `true` | 在全部已配置根目录之后，追加 `<rlhHome>/.agent-presets` 作为 `user` 根目录 |

根目录不存在时视为不提供任何 preset，而非失败：用户根目录在写出第一个本地 preset 之前并不存在，而指定了没有任何根目录提供的默认值，在解析时本就会明确报错。

### 可写根目录属于本包，随附根目录属于 app

`<rlhHome>/.agent-presets` 是个人自有 preset 的所在，正如 `<rlhHome>/skills` 是其自有 skill 的所在（[`rlh-skill-filesystem`](../../skill/skill-filesystem/README.md)），因此 roster 自行推导它，而不等某个部署记得配置——一个什么都没配的启动器同样能发现并创作 preset。它追加在全部已配置根目录**之后**，从而保持靠前的根目录赢得重复 id：随附的 `standard` 仍然遮蔽一个占用该名字的家目录目录，而 `copy()` 会拒绝该 id，不会落下一个无人解析得到的 preset。

根目录在服务构造时解析一次。若根目录集合在一次 `list()` 与依据其答案执行的 `copy()` 之间发生变化，写入的将是调用方从未见过的目录。

`includeUserRoot: false` 使 roster 只覆盖 `roots`。把 preset 限制在自有目录内的部署需要它，任何钉住确切 roster 的测试同样需要——否则将由这台机器真实的 `<rlhHome>` 决定 roster 的内容。
