...
import SessionStore, { type Session, type SessionEvent } from '@relay-harness/rlh-session'
import InvariantRegistry, { InvariantError } from '@relay-harness/rlh-invariants'
import * as SandboxPolicyInvariant from '@relay-harness/rlh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@relay-harness/rlh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@relay-harness/rlh-sandbox-policy',
    })
  })
})
