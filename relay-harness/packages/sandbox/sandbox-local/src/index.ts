 ...
 * @module @relay-harness/rlh-sandbox-local
 */
...
} from '@relay-harness/node-addon-landlock-run'
import { Context } from '@relay-harness/cordis'
import z from '@relay-harness/schemastery'
import { assertNever } from '@relay-harness/rlh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@relay-harness/rlh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@relay-harness/rlh-sandbox'
import type { SessionId } from '@relay-harness/rlh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@relay-harness/rlh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@relay-harness/rlh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@relay-harness/rlh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@relay-harness/rlh-sandbox-windows-acl/src/runner.ts'))
