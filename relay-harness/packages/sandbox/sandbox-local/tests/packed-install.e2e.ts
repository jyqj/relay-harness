...
const platformPackageName = `@relay-harness/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @relay-harness peer (koffi arrives
  // from the registry).
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'rlh-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'rlh-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'rlh-packed-work-'))
