...
| Package | Role |
|---|---|
| `@relay-harness/rlh-spill` (this) | Service Definition: abstract service + vocabulary types |
| `@relay-harness/rlh-spill-local` | Service Provider: private session-scoped files on the host filesystem |
| `@relay-harness/rlh-spill-policy` | Consumer: the tool-result policy that spills oversized final results |
