...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Re-link one agent to a different preset\'s standing composition.\n\nOnly valid while the agent has produced nothing: swapping tools mid conversation would leave logged tool calls the new composition cannot make. The CALLER owns that check — this method does not read session history.\n\nThe swap is a parent re-link, not an unmount: standing mounts are shared and reference-held, so the old composition stays for its other agents and the new one is ensured BEFORE the link moves. An unknown or unusable preset therefore throws with the agent exactly as it was — there is no torn-down state to restore. The re-link runs through the binding this roster kept from the agent\'s mount — rlh-scope\'s only re-link authority. An agent that never composed one has nothing to re-link: the switch is then the agent\'s first bind, exactly a mount.',
      ...
      {
        signature: 'async acquireStandingScope(id?: string): Promise<PresetScopeLease>',
      ...
      {
        signature: 'acquireAgentScope(agentCtx: Context): PresetScopeLease | undefined',
        description: 'Hold the exact joined generation and snapshot all live-agent registration layers for an asynchronous read.',
        parameters: [{ name: 'agentCtx', description: 'agent whose current composition and ancestry the read observes.' }],
  ...
  },
  {
    ...
    description: 'Agent service (`ctx.agents`): tracks live agents and carries the initiating Agent through one process-local asynchronous driver chain. Agent *creation* is provided by whichever plugin implements the AgentFactory (`@relay-harness/rlh-agent-loop`), registered via setFactory.\n\nInitiator methods provide same-process causal attribution only. Ambient presence is neither liveness proof nor authorization; subjects and owners remain explicit, as does identity at worker, process, persistence, and wire boundaries. Returned Promise boundaries drain during teardown, except a nested lineage that starts an owning-fiber unload is excluded from its own drain.',
    methods: [
      {
  ...
  {
    key: 'clientModules',
    summary: 'The web plugin table service: incremental `rlh.client` scan + wire composition + bundle route + index tap.',
  ...
  },
  {
    ...
    summary: 'Counts borrowed synchronously from the live proxy\'s authoritative request maps.',
    description: 'Counts borrowed synchronously from the live proxy\'s authoritative request maps.',
    methods: [
      {
        signature: 'pendingFor(sessionId: SessionId): { approvals: number; questions: number }',
        description: 'Count pending approval and question requests for one exact Session.',
  ...
  {
    key: 'mcpServersFile',
    summary: 'Owns `$RLH_HOME/mcp-servers.yaml` and the live mcp-client children it describes.',
    description: 'Owns `$RLH_HOME/mcp-servers.yaml` and the live mcp-client children it describes.',
    methods: [
      {
      ...
      {
        signature: 'useAuthorizeHttp(authorizeHttp: (url: string, signal: AbortSignal) => Promise<McpOAuthTokens>): void',
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async prepare(id: SessionId, signal?: AbortSignal, fence?: SessionPersistenceFence): Promise<SessionPreparation>',
        ...
        parameters: [{ name: 'id', description: 'persisted session to prepare.' }, { name: 'signal', description: 'optional cancellation for preparation work.' }, { name: 'fence', description: 'optional owner excluding takeover throughout cold recovery.' }],
  ...
  {
    key: 'shellEnv',
    summary: 'Registry (`ctx.shellEnv`) for trusted, per-execution `RLH_*` variables.',
    ...
    methods: [
      ...
      {
        signature: 'collect(execution: ToolExecution): RlhEnvironment',
        description: 'Build the trusted `RLH_*` snapshot for one shell tool execution.',
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Reclaim every artifact owned by `sessionId`. Called at that session\'s disposal; idempotent, and a session that spilled nothing resolves.',
        parameters: [{ name: 'sessionId', description: 'the session whose spill storage should be reclaimed.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Register a monotonic guard after the extensible `tools/pre-execute` waterfall. A plain-context guard applies globally; one registered through `agent.ctx` applies only to that agent. Any matching guard may deny by returning a reason, while no guard can force-allow a call another guard denied. Guards run again after around-dispatch and resource waits, before each body invocation; they must support repeated synchronous checks. The exact effect disposer is returned for ordered ownership and HMR cleanup.',
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'currentTrustedRequest(): { readonly endpoint: string; readonly signal: AbortSignal } | undefined',
        description: 'Read live carrier provenance without minting or extending its authority.',
        parameters: [],
  ...
  {
    key: 'workResults',
    summary: 'Stateless business adapter; accepted versions and file inventories remain log projections.',
    ...
    methods: [
      ...
      {
        signature: '@Remote(\'accept\') async accept(agent: Agent, request: WorkAcceptRequest, signal: AbortSignal): Promise<WorkAcceptReceipt>',
        description: 'Record explicit user acceptance after final quiescence and revision checks.',
        parameters: [{ name: 'agent', description: 'exact live runtime root being reviewed.' }, { name: 'request', description: 'expected non-acceptance log revision shown to the user.' }, { name: 'signal', description: 'trusted carrier cancellation, never a wire argument.' }],
        returns: 'a receipt only after durability; later work makes its current flag false.',
      },
      {
        signature: '@Remote(\'list\') async list(request: WorkLibraryRequest, signal: AbortSignal): Promise<WorkLibraryPage>',
      ...
      },
      {
        ...
        description: 'Validate a captured output against its source Session and open it on the Host.',
        parameters: [{ name: 'request', description: 'source Session identity and exact execution-recorded path.' }, { name: 'signal', description: 'carrier cancellation through native-open completion.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.agent - the newly registered agent with its live session and completed setup. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/disposed',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.agent - the exact agent removed from the registry. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/error',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.error - the failure, verbatim. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.turn - the owning turn. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the discarded message. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the inserted message. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/pre-step',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s cancellation signal. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/request',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s explicit abort signal. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.signal - the turn abort signal. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.source - why the session started (fresh startup, resume, …). Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/status',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.status - the status just entered (the transition\'s destination). Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Ask composed answerers for one decision. Return an outcome to claim the request or call `next()`; failure yields the fail-closed default. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.',
    parameters: [{ name: 'req', description: 'the pending decision (agent, tool identity, reason, signal).' }],
  },
  {
  ...
  },
  {
    name: 'goal/changed',
    mode: 'emit',
    signature: '\'goal/changed\'(this: import(\'@relay-harness/rlh-scope\').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void',
    summary: 'Goal mutation accepted by one live agent.',
    description: 'Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@relay-harness/rlh-scope`): agent-scoped listeners receive only that agent.',
  ...
  },
  {
    ...
    description: 'Emitted once when an announced session leaves the store, including publication rollback, but never for an entry whose creation announcement did not begin. Listener failures are logged and contained. Scope-filtered dispatch (`@relay-harness/rlh-scope`) reuses the owner scope.',
  ...
  },
  {
    ...
    description: 'Awaited parallel durability checkpoint: every listener runs and the caller awaits all of them, with no waterfall veto. Scope-filtered dispatch (`@relay-harness/rlh-scope`) reuses the session\'s owner scope.',
    parameters: [{ name: 'session', description: 'the session whose buffered events must reach durable storage.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Observe the frozen, lossless-JSON final outcome. Listener failures are contained. Scope-filtered dispatch (`@relay-harness/rlh-scope`): keyed by `exec.agent`.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'Inbox',
    declaration: 'export class Inbox {\n    constructor(private readonly session: Session, private readonly notifications: InboxNotifications, private readonly maxPendingMessages = Number.MAX_SAFE_INTEGER, private readonly maxPendingBytes = Number.MAX_SAFE_INTEGER);\n    get nextTurn(): readonly UserMessage[];\n    get nextStep(): readonly UserMessage[];\n    get hasPending(): boolean;\n    clear(): void;\n    claim(target: InboxTarget, turn: number): UserMessage[];\n    append(target: InboxTarget, message: UserMessage): void;\n    prepend(target: InboxTarget, message: UserMessage): void;\n    replace(messageId: MessageId, newMessage: UserMessage): boolean;\n    remove(messageId: MessageId): boolean;\n    splice(target: InboxTarget, start: number, deleteCount: number, inserted: UserMessage[]): UserMessage[];\n}',
  },
  {
  ...
  {
    name: 'PresetScopeLease',
    declaration: 'export interface PresetScopeLease {\n    readonly key: ScopeKey;\n    release(): Promise<void>;\n}',
  },
  {
    name: 'PresetSpec',
  ...
  {
    name: 'SessionOrigin',
    declaration: 'export type SessionOrigin = \'subagent\' | \'rlhbot\';',
  },
  {
    name: 'SessionPersistenceFence',
    declaration: 'export interface SessionPersistenceFence {\n    readonly token: string;\n    assertCurrent(): void;\n    runExclusive?<T>(operation: () => Promise<T>): Promise<T>;\n}',
  },
  {
  ...
  {
    name: 'SkillSource',
    declaration: 'export type SkillSource = \'project-rlh\' | \'project-agents\' | \'runtime\' | \'user-rlh\' | \'user-agents\' | \'custom\' | \'bundled\' | (string & {});',
  },
  {
    name: 'SkillSummary',
  ...
  {
    name: 'WorkAcceptanceProjection',
    declaration: 'export interface WorkAcceptanceProjection {\n    readonly reviewRevision: number;\n    readonly reviewable: boolean;\n    readonly acceptedRevision: number | null;\n}',
  },
  {
    name: 'WorkAcceptReceipt',
    declaration: 'export interface WorkAcceptReceipt {\n    readonly reviewedThroughSeq: number;\n    readonly recordedSeq: number;\n    readonly current: boolean;\n}',
  },
  {
    name: 'WorkAcceptRequest',
  ...
  },
  {
    ...
    declaration: 'export interface WorkLibraryRequest {\n    readonly query?: string;\n    readonly corpusRevision?: WorkLibraryRevision;\n    readonly sessionOffset?: number;\n    readonly pathOffset?: number;\n    readonly limit?: number;\n}',
  },
  {
    name: 'WorkLibraryRevision',
    declaration: 'export type WorkLibraryRevision = Branded<\'work-library-revision\'>;',
  },
  {
    name: 'WorkOpenRequest',
