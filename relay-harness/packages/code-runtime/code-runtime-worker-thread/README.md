...
- id: code-runtime
  ...
  config:
    ...
    maxBindingBytes: 67108864     # one binding argument or resolution (64 MiB)
    maxOldGenerationSizeMb: 512   # worker heap cap (resourceLimits)
```

Every field is validated and defaulted; `maxOutputBytes` is a safe integer of at least four bytes, `maxBindingBytes` is a positive safe integer, and the remaining fields are positive finite numbers, `maxWallMs` is additionally at most `2147483647` (Node's maximum `setTimeout` delay), and there are no other tunables.
...
- **The binding cap is enforced after worker-to-host structured clone** — the worker heap limit still backstops a hostile program while its oversized argument reaches the host; the host refuses it before invoking a binding, and refuses an oversized resolution before cloning it back.
