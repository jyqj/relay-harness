 ...
 *
 * @module @relay-harness/rlh-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `rlh-llm-replay` via `$RLH_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      RLH_SNAPSHOT: opts.mode,
      RLH_SNAPSHOT_FILE: opts.fixtureFile,
      RLH_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      RLH_SNAPSHOT_SPILL_ROOT: spillRoot,
      RLH_HOME: join(cwd, '.rlh'),
      RLH_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { RLH_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { RLH_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function waitForPersistedTurnStart(
  ...
  await waitForAsyncIo(async () => {
    ...
    try {
      ...
      // Capture the validation failure as a ready result so it bypasses the
      // transient partial-write retry policy, then rethrow immediately below.
      invalidRecord = { error }
      return true
    }
    return openTurn !== undefined && (minimumTurn === undefined || openTurn >= minimumTurn)
  }, timeoutMs, () => new Error(
...
async function waitForPersistedTurnEnd(
  ...
  await waitForAsyncIo(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    return log !== undefined && latestTurnIsClosed(log.content)
  }, timeoutMs, () => new Error(
...
async function waitForPersistedTitleAfterTurnEnd(
  ...
  await waitForAsyncIo(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    return log !== undefined && latestTitleFollowsTurnEnd(log.content)
  }, timeoutMs, () => new Error(
...
async function waitForPersistedEventAfterTurnEnd(
  ...
  await waitForAsyncIo(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    return log !== undefined && latestEventFollowsTurnEnd(log.content, type)
  }, timeoutMs, () => new Error(
...
/**
 * Poll asynchronous persistence without letting Vitest replace a domain error
 * when one filesystem probe is still pending at the deadline. Probe failures
 * are retried because a concurrent JSONL append can expose a partial record;
 * the final probe error remains authoritative if no later probe succeeds.
 */
async function waitForAsyncIo(
  ...
  while (true) {
    ...
    if (remainingMs <= 0) {
      if (lastProbeError instanceof Error) throw lastProbeError
      if (lastProbeError !== undefined) {
        throw new Error('snapshot-harness: asynchronous persistence probe failed', { cause: lastProbeError })
      }
      throw timeoutError()
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in rlh-llm-replay's loadSessionScripts
