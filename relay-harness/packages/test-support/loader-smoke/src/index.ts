 ...
 * zero-build dev path, resolving `@relay-harness/rlh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @relay-harness/rlh-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'RLH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.RLH_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated RLH homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { RLH_HOME: join(cwd, '.rlh'), RLH_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
