...
`resolveRlhHome()` resolves the single-root Relay Harness home. Precedence, highest first: an explicit configured path, `$RLH_HOME`, then `~/.rlh`. The harness keeps all user data under one root.

`rlhHomePath(...segments)` joins child segments onto that resolved home with Node's platform path rules. With no segments it returns the home itself.

`rlhHomeDisplay()` names an active root symbolically for user-facing paths: `~/.rlh` for the default home, `$RLH_HOME` for any configured home. It never leaks an absolute machine path.

`RLH_HOME_DIR_NAME` owns the default user-data directory name: `.rlh`.

`defaultRlhHome()` returns the default Relay Harness home by joining the operating-system home directory with `.rlh`, using Node's platform path rules.
