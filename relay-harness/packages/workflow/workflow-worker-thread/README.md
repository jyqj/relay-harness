...
With absolute `journalRoot` configured, the engine first holds a SQLite exclusive writer transaction beside the journal; a competing live host fails with `JOURNAL_BUSY` before reading or repairing the file. The transaction releases only after worker exit and child quiescence, or when the owning process exits. A fresh run exclusively creates `<journalRoot>/<sha256(runId)>/journal.jsonl` before worker publication. The version-2 header fingerprints script, validated meta, args, resolved provider, and total-child cap; older formats are rejected. Each `agent()` call fsyncs an intent before provider startup and a terminal outcome before publication to the worker. `resumeRunId` restarts the script and replays matching completed calls without provider work. A different request at the same sequence fails with replay divergence. An intent without a terminal outcome, including a cancelled child, worker-death interruption, or a torn terminal append, refuses replay with `JOURNAL_OUTCOME_UNKNOWN`: reconcile its side effects before starting a new run. Recovery validates the requested header, every complete record, consumed child-result fields, finite JSON values, and contiguous call identities before modifying a tail. It then fsyncs a torn-tail truncation or a missing newline within the complete 64 MiB bound. Divergent requests and malformed complete rows leave the file untouched. Symlinks, including dangling writer-database links, non-regular files, invalid intent/outcome transitions, and missing earlier call sequences fail closed.
...
- **The journal requires local filesystem locking** — its SQLite writer claim excludes overlapping hosts, but network filesystems with unreliable SQLite locking are unsupported. A child abandoned by bounded disposal keeps the claim until it actually quiesces or its host process exits.
- **Only completed host calls replay** — arbitrary JavaScript heap state and phase/log narration are not checkpointed. An unresolved call intent blocks that call on resume; reconciliation requires inspecting its side effects and explicitly starting a new run, not editing the journal or assuming rollback.
