 ...
 * harness home resolved by {@link resolveRlhHome} (`$RLH_HOME` > `~/.rlh`),
 ...
 * machine: every process sharing one `$RLH_HOME` reports the same id, and
 ...
 *
 * @module @relay-harness/rlh-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `RLH_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveRlhHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
