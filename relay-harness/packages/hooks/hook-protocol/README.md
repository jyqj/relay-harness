# @relay-harness/rlh-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@relay-harness/rlh-hooks-claude-code`, `@relay-harness/rlh-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`rlh-hook-protocol`) | The bridge (`rlh-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `rlh-hooks-claude-code` and `rlh-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
