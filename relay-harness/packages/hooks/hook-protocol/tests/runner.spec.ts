...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@relay-harness/rlh-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@relay-harness/rlh-hook-protocol'
import type { RunHookOptions } from '@relay-harness/rlh-hook-protocol'
 ...
 * executor (rlh-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what rlh-bash-local's resolve does for the fields runHook sets.
      return {
