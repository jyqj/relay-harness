/**
 * Shared boot glue for the app bins (`rlh`, `rlh-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.rlh`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @relay-harness/rlh-app-boot
 */
...
import { Context, type FiberState } from '@relay-harness/cordis'
import Loader, { type Entry, type EntryOptions } from '@relay-harness/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@relay-harness/cordis-plugin-include'
import Group from '@relay-harness/cordis-plugin-group'
import { rlhHomePath, resolveRlhHome } from '@relay-harness/rlh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@relay-harness/rlh-launch-environment'
import type {} from '@relay-harness/cordis-plugin-hmr'
...
declare module '@relay-harness/cordis' {
  interface Context {
    ...
    rlhHomePath?: typeof rlhHomePath
  }
}
...
export {
  ...
  type RlhBundleManifest,
  type RlhCompatibilityManifest,
  type RlhManifestSection,
  type RlhProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$RLH_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['RLH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@relay-harness/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@relay-harness/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@relay-harness/cordis-plugin-group`
 ...
 * directory. The self-referential `rlh-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Relay Harness implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend RLH itself.`,
  })
}
