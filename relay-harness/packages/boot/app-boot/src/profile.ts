 ...
 * `rlh --profile` launcher family.
 *
 * A profile is a directory under `$RLH_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `rlh.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"rlh": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `rlh.profile.bundles` order over
 ...
 * first from the rlh installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$RLH_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @relay-harness/rlh-app-boot/profile
 */
...
import { applyEntryPatches, type PatchOptions } from '@relay-harness/cordis-plugin-include'
...
/** The bundle half of the `rlh` manifest section: what a bundle package exports. */
export interface RlhBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The compatibility half of the `rlh` manifest section: what host features a package requires. */
export interface RlhCompatibilityManifest {
  /** Host feature ids this package requires; the host refuses to activate it without all of them. */
  features?: string[]
}

/** The profile half of the `rlh` manifest section: what a profile directory composes. */
export interface RlhProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `rlh`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface RlhManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: RlhBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: RlhProfileManifest
  /** Host-feature requirements consumed by the install and boot gates. */
  compatibility?: RlhCompatibilityManifest
}
...
export interface ProfileManifest {
  ...
  rlh?: RlhManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `rlh.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `rlh.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`rlh --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveRlhHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveRlhHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`rlh: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@relay-harness/rlh-base', '@relay-harness/rlh-web-app'],
  headless: ['@relay-harness/rlh-base', '@relay-harness/rlh-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@relay-harness/rlh-base', '@relay-harness/rlh-web-app', '@relay-harness/rlh-headless'],
}

/** The bundle list a `rlh plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@relay-harness/rlh-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this rlh profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `rlh.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `rlh-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      rlh: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`rlh: ${link} exists and is not a symlink; remove it so rlh can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$RLH_HOME/profiles/node_modules`: one
 * symlink per package in the rlh app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`rlh-compaction`, `rlh-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the rlh app's package.json.
 * @param home - the Harness home; defaults to {@link resolveRlhHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveRlhHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (rlh-subprocess,
    // rlh-compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const normalized: ProfileManifest = {
    ...
    rlh: {
      ...manifest.rlh,
      profile: { ...manifest.rlh?.profile, bundles: [...current] },
    },
  }
 ...
 * `@relay-harness/rlh-base` (and every other in-box bundle) always comes from
 * the same installation as the running rlh, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `rlh.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the rlh app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the rlh installation or ${profileDir}; `
    + `run 'rlh plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `rlh.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `rlh.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the rlh app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveRlhHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveRlhHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'rlh plugin --profile ${name} add <package>'`,
      )
    }
  ...
  // A hand-written profile manifest may omit the rlh section entirely.
  const bundles = options.bundles === 'template'
    ? [...(PROFILE_TEMPLATES[name] ?? DEFAULT_PROFILE_BUNDLES)]
    : (manifest.rlh?.profile?.bundles ?? [])
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.rlh?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no rlh.bundle in its package.json`)
    }
