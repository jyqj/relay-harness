 ...
 * plugin may require through its `rlh.compatibility.features` declaration.
 *
 ...
 * `@relay-harness/rlh-app-boot/features`, and a host too old to export that
 ...
 * @module @relay-harness/rlh-app-boot/features
 */

/** Stable feature ids this rlh host supports (additive only; see the module doc). */
...
/**
 * Parse a plugin's `rlh.compatibility` declaration value (the raw JSON value
 * of the `rlh.compatibility` manifest key) into its required feature ids.
 ...
 * @param value - the raw `rlh.compatibility` value.
...
export function parseCompatibilityFeatures(packageName: string, value: unknown): string[] | undefined {
  ...
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error(`${packageName}: rlh.compatibility must be an object`)
  }
  ...
  if (!Array.isArray(features) || features.some(feature => typeof feature !== 'string')) {
    throw new Error(`${packageName}: rlh.compatibility.features must be a string array of feature ids`)
  }
 ...
 * '@relay-harness/rlh-app-boot/features'`), called from a plugin's Node entry
...
export function assertHostFeatures(pluginName: string, required: readonly string[]): void {
  ...
  throw new Error(
    `${pluginName} requires host features this rlh host does not support: ${missing.join(', ')} `
    + '— update rlh to a host that provides them, or remove the plugin',
  )
}
