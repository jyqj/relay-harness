...
显式工具检索从 `query` 中提取文件引用，将其作为不可信模型数据。读取使用精确存活调用者的文件系统、规范工作区包含关系、有界 UTF-8 流及目标/版本复核，不将远端 target key 解释成本地路径。完整渲染遵守分配的字符/token 预算；字节计数和摘要描述实际纳入的文本。自动人类引用保留原有用途和权限语义。
