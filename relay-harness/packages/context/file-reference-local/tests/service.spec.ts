...
import ToolRegistry, { defineContentToolFixture } from '@relay-harness/rlh-tools'
import { FILE_REFERENCE_PROMPT } from '@relay-harness/rlh-file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'rlh-file-reference-service-'))
  roots.push(root)
