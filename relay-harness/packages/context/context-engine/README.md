...
The planner is deterministic and does not call a model. `StepContextContributor.purposes` declares provider eligibility; omission supports every purpose. Each eligible provider receives `StepContextInput.budget` with local character/token ceilings, timeout, and absolute deadline. Providers retain their own retrieval algorithms and should clip within that allowance. Eligible providers run under a configurable concurrency cap and one complete preparation deadline that includes queue time. A request can only tighten these bounds: `ContextPrepareInput.limits` lowers the total allowance, and `ContextPrepareInput.deadlineAt` caps the complete preparation deadline; neither raises the deployment configuration. Packing never depends on completion order. A timed-out child signal does not abort the parent request, so queued providers can run while the total allowance remains; a late result from a timed-out or disposed registration generation is ignored. The parent request signal and engine unload abort the complete preparation atomically; registration disposal immediately aborts that generation. `ContextProviderError` records a classified `declined`, `degraded`, or `error` outcome without raw queries or exception text. Unknown provider exceptions become `error/provider_failed`; `ContextEngineError` and malformed contributions still fail atomically.
...
| `prepareTimeoutMs` | `5000` | Complete preparation deadline including queued reads; unfinished providers record `deadline`. |
| `maxConcurrentContributors` | `4` | Concurrent provider reads per preparation. |
...
- **Cooperative resource cancellation** — deadlines discard late results and signal cancellation; an uncooperative provider may continue its underlying work. Synchronous JavaScript cannot be preempted by a timer.
