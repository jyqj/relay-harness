...
import { createUserMessage, CallId , createMessage, createToolResultMessage } from '@relay-harness/rlh-llm'
import SessionStore, { Session, SessionId } from '@relay-harness/rlh-session'
import SessionQueryEngine from '@relay-harness/rlh-session-query'
...
} from '@relay-harness/rlh-session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a rlh-session: URI?')).toEqual({
      text: 'what is a rlh-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see rlh-session:%%%')).toEqual({
      text: 'see rlh-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see rlh-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](rlh-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `rlh-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('rlh-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
...
describe('session reference discovery and preparation', () => {
  ...
  it('does not prepare a rejected pre-step and rejects malformed direct mentions', async () => {
    ...
    const malformed = createUserMessage({
      content: [{ type: 'text', text: '@[bad](rlh-session:not-canonical)' }],
      source: { kind: 'user' },
    })
