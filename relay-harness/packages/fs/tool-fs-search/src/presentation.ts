 ...
 * model-facing render consumes ({@link module:@relay-harness/rlh-tool-fs-search/search-core}
 ...
 * final output budget (`rlh-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @relay-harness/rlh-tool-fs-search/presentation
 */
 ...
 * ({@link module:@relay-harness/rlh-tool-fs-search/grep} `formatGrepMatches`), so
