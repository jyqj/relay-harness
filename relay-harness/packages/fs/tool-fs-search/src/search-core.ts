 ...
 *
 * @module @relay-harness/rlh-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@relay-harness/rlh-output-retention'
import type { RetainedItems } from '@relay-harness/rlh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@relay-harness/rlh-subprocess'
 ...
 * `@relay-harness/rlh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`rlh-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@relay-harness/rlh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@relay-harness/rlh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@relay-harness/rlh-tool-fs-search/presentation} `grepSearchMeta`)
