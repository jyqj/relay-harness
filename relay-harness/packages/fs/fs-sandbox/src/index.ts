 ...
 * `@relay-harness/rlh-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@relay-harness/rlh-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@relay-harness/rlh-tool-fs`),
 * exactly as bash's does.
 *
 * @module @relay-harness/rlh-fs-sandbox
 */

import { Context } from '@relay-harness/cordis'
import { LocalFileSystem } from '@relay-harness/rlh-fs-local'
import type { Config as LocalConfig } from '@relay-harness/rlh-fs-local'
import { FsError } from '@relay-harness/rlh-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@relay-harness/rlh-fs'
import { writableRoots } from '@relay-harness/rlh-sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@relay-harness/rlh-sandbox'
import type {} from '@relay-harness/rlh-sandbox-policy'
 ...
 * INSTEAD OF `rlh-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `rlh-tool-fs` resolves
