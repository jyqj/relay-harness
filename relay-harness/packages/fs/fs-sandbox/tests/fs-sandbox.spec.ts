...
import { FsError, FsTargetKey } from '@relay-harness/rlh-fs'
import type { FsTarget } from '@relay-harness/rlh-fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.rlh-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'rlh-fssbx-tmp-')), 'temp.txt')
