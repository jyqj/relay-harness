...
// --------------------------------------------------------------------------
describe('default deployment (with rlh-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'rlh-tool-fs-'))
    ctx = new Context()
...
// --------------------------------------------------------------------------
describe('bare provider (no rlh-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'rlh-tool-fs-bare-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: dir })
...
// caller-selected session workspace wins, matching rlh-tool-bash.
describe('per-session cwd', () => {
  let sessionDir: string
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'rlh-tool-fs-cfg-'))
    sessionDir = await mkdtemp(join(tmpdir(), 'rlh-tool-fs-session-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
