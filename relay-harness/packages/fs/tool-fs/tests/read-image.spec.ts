...
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@relay-harness/rlh-llm'
import SystemPrompt from '@relay-harness/rlh-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@relay-harness/rlh-tools'
import type { Config as ToolConfig } from '@relay-harness/rlh-tools'
...
import { AttachmentError, AttachmentId, AttachmentStore } from '@relay-harness/rlh-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'rlh-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'rlh-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { rlhHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { rlhHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { rlhHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
