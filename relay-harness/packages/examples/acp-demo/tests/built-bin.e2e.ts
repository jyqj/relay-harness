...
describe.skipIf(!existsSync(acpBin))('rlh-acp-demo BUILT bin (node lib/bin.js, no tsx)', () => {
  it('boots the published bin, completes a turn, and writes default Zstandard persistence', async () => {
    ...
    child = spawn(process.execPath, [acpBin, '--config', './cordis.yml'], {
      ...
      env: {
        ...process.env,
        RLH_HOME: join(consumer, '.rlh'),
        RLH_AGENTS_HOME: join(consumer, '.agents'),
      },
