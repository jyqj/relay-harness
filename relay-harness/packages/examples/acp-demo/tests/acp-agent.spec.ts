...
import type { Message } from '@relay-harness/rlh-llm'
import { SessionId } from '@relay-harness/rlh-session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @relay-harness/rlh-acp-demo composition:
...
async function isolatedSkillsConfig(catalogDescriptionMaxLength?: number): Promise<NonNullable<acpAgent.Config['skills']>> {
  const home = await mkdtemp(join(tmpdir(), 'rlh-acp-demo-skills-'))
  return {
    filesystem: { rlhHome: join(home, '.rlh'), agentsHome: join(home, '.agents') },
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  const home = await mkdtemp(join(tmpdir(), 'rlh-acp-demo-default-skills-'))
  process.env.RLH_HOME = join(home, '.rlh')
  process.env.RLH_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
...
describe('rlh-acp-demo composition', () => {
  it('brings up the spine + persistence + the ACP bridge', async () => {
    const ctx = await mount({
      provider: 'mock',
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/rlh-acp-demo-test',
  ...
  it('forwards explicit project-instruction controls to the bundled spine', async () => {
    const ctx = await mount({
      ...
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/rlh-acp-demo-workspace-context',
      workspaceContext: false,
    })
    expect(ctx.get('agents')).toBeDefined()
    expect(ctx.get('agentLoop')).toBeDefined()
  ...
  it('forwards skill config and rlhHome into agent-spine-demo', async () => {
    const skills = await isolatedSkillsConfig(6)
    const ctx = await mount({ provider: 'mock', model: 'mock', persona: 'hi', rlhHome: skills.filesystem!.rlhHome!, skills, workspaceContext: false })
  ...
  it('forwards maxParallelToolCalls to the bundled agent loop', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/rlh-acp-demo-test-parallel',
      skills: await isolatedSkillsConfig(),
      workspaceContext: false,
    })
  ...
  it('forwards toolOrder through agent-spine-demo to the system-prompt assembly', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/rlh-acp-demo-test-tool-order',
      workspaceContext: false,
    })
