...
| [`memory/`](memory/README.md) | 长期记忆能力系列：受治理版本、SQLite 提供方、Agent 召回和面向模型工具 | 产品：稳定 API |
...
| [`tracker/`](tracker/README.md) | Issue tracker 能力系列：提供方注册表与会话绑定工具 + Linear 提供方 | 产品：稳定 API |
| [`automation/`](automation/README.md) | opt-in 的 tracker 驱动 Issue 自动化：orchestrator、runner、workspace、workflow、orchestration 各 seam 及其提供方 | 产品：稳定 API |
...
| [`bundle/`](bundle/README.md) | 可安装的 `rlh --profile` 补丁层 | 产品：稳定 API |
