...
/** Bounded consecutive-read budget for rejecting a moving watcher snapshot. */
...
export class LocalCredentialProvider extends CredentialProvider {
  ...
  private async reconcileFromDisk(): Promise<void> {
    ...
    await assertOwnerOnly(this.spec.filename)
    const text = await this.readStableText()
  ...
  /**
   * Read one watcher-visible commit. A non-atomic editor may expose its
   * truncate before its write; publishing that valid-but-transient empty map
   * would emit removals and then additions for one logical save. Two equal
   * consecutive reads identify the latest stable snapshot without relying on
   * watcher event counts or a wall-clock sleep. A continuously moving file is
   * left to the next watcher event rather than publishing an arbitrary frame.
   */
  private async readStableText(): Promise<string | undefined> {
    ...
    for (let attempt = 1; attempt < STABLE_READ_ATTEMPTS; attempt += 1) {
      // Let the writer continuation that produced the watch event finish its
      // next filesystem operation before verification; this is an event-loop
      // ordering boundary, not a wall-clock debounce.
      await new Promise<void>(resolveImmediate => setImmediate(resolveImmediate))
      const verified = await read()
      if (verified === candidate) return candidate
      candidate = verified
    }
    throw new Error(`credentials-local: document kept changing while reading ${this.spec.filename}`)
  }
