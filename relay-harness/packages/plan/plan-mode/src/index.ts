 ...
 *
 * @module @relay-harness/rlh-plan-mode
 */
...
import type { Session, SessionEvent, UserMessage } from '@relay-harness/rlh-session'
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('rlh-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'rlh-plan-mode: close service lifetime')
