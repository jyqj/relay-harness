...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@relay-harness/rlh-session-persistence-jsonl', JsonlSessionPersistence],
    ...
    ['@relay-harness/rlh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'rlh-message-feedback-loader-'))
    ...
    await writeFile(configPath, [
      ...
      '  config:',
      '    backend: json',
      "- name: '@relay-harness/rlh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
