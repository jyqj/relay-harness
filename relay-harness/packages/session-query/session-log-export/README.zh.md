...
Web Session 日志下载控制，使用 `rlh-host-apiproxy` 拥有的 Host 流式 ZIP 端点。Host 半包注册 `/export`；浏览器半包在标题栏尾簇中提供 111×32 的 `Session log` 操作（cozy/compact 密度下只留图标），以及一个供该按钮与斜杠命令共用的下载控制器和弹窗。界面设置 `session-log-export.titlebarAction`（默认 true）隐藏标题栏按钮；`/export` 与共用 Modal 仍保持挂载。ZIP 生成、原始 JSONL/zstd 读取、子 Session、附件、背压和 HTTP 错误语义仍由 [ApiProxy 下载实现](../../host/apiproxy/README.md)负责。
...
Web bundle 将本包与 `rlh-host-apiproxy`、`rlh-commands`、`rlh-client-ui-commands` 和 `rlh-client-ui-conversation` 一起挂载。本包把按钮和弹窗贡献到最右侧的 `conversation.session.header.utilities` 列表，与标题旁 `conversation.session.header.actions` 中的模式、Subagent 和 Task 配置项相互独立；Trajectory 不包含导出入口。
