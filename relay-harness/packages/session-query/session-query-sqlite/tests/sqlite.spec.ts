...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@relay-harness/rlh-session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@relay-harness/rlh-session'
import SessionPersistence, { SessionPersistenceRevision } from '@relay-harness/rlh-session-persistence'
import type { SessionPersistenceSnapshot } from '@relay-harness/rlh-session-persistence'
import SqliteSessionPersistence from '@relay-harness/rlh-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@relay-harness/rlh-session-query-sqlite'
import {
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'rlh-session-search-'))
