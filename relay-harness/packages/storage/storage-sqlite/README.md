...
Open refuses foreign media before any write: a database stamped with another `PRAGMA application_id`, an unstamped database that already holds user tables, or a database stamped as this backend without the `units`/`unit_globals` metadata tables fails with `foreign-medium`, and a stamped database missing the current layout version fails with `version-mismatch` — the file is left untouched (pre-release stance: backends reject old on-disk formats, so a database written by an earlier build must be recreated). Only a zero-version empty database is adopted and stamped.
