...
打开时会先拒绝外来介质，且拒绝发生在任何写入之前：`PRAGMA application_id` 标记为其他应用的数据库、已含用户表的未标记数据库、或标记为本后端但缺少 `units`／`unit_globals` 元数据表的数据库，都会以 `foreign-medium` 失败；标记了非当前布局版本的数据库以 `version-mismatch` 失败。拒绝时文件原样保留（预发布立场：后端拒绝旧磁盘格式，由早期构建写入的数据库需重建）。只有零版本的空数据库会被收养并打上标记。
