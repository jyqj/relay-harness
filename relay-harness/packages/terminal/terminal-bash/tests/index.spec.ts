...
import AgentRegistry, { Inbox, type Agent } from '@relay-harness/rlh-agent'
...
import SandboxPolicyService, { setSandboxMode } from '@relay-harness/rlh-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@relay-harness/rlh-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@relay-harness/rlh-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@relay-harness/rlh-terminal-bash'
import { ENCODING_PREAMBLE } from '@relay-harness/rlh-pwsh-local'
import * as ptyLocal from '@relay-harness/rlh-terminal-bash'
import type { ResolvedConfig } from '@relay-harness/rlh-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@relay-harness/rlh-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'rlh> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'rlh> \'',
        RLH_SHELL: '1', RLH_SESSION_ID: 'agent', RLH_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07rlh> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo rlh> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo rlh> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', RLH_SHELL: '1', RLH_SESSION_ID: 'agent', RLH_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'rlh> ' : 'PowerShell 7.6.4\n',
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'rlh> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
