...
import { LocalPtySession } from '@relay-harness/rlh-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@relay-harness/rlh-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@relay-harness/rlh-terminal'
import type {
  SubprocessOutcome,
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07rlh> ')
  await vi.advanceTimersByTimeAsync(10)
  await pending
}

describe('LocalPtySession readiness and output', () => {
  it('does not count an unobserved scheduler stall as provider silence', async () => {
    vi.useFakeTimers()
    ...
    // Move wall time beyond idleSilenceMs without running any readiness poll.
    // The first later probe must not collapse that unobserved interval into idle.
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07rlh> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07rlh> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
