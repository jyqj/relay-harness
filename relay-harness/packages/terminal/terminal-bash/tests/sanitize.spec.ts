...
import { normalizeTerminalText, TerminalSanitizer } from '@relay-harness/rlh-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07dsh> ')).toEqual({ text: 'rlh> ', prompt: true, promptTail: 'rlh> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('rlh> ')).toEqual({ text: 'rlh> ', prompt: false, promptTail: 'rlh> ' })
  })
