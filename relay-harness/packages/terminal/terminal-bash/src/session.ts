...
import { TerminalError } from '@relay-harness/rlh-terminal'
import type {
  ...
  TerminalWaitReason,
} from '@relay-harness/rlh-terminal'
...
export class LocalPtySession implements TerminalBackendSession {
  ...
  private observedSilenceMs = 0
  private lastReadinessObservationAt = Date.now()
  ...
  /** Output and send boundaries restart the silence interval from an observed point. */
  private resetObservedSilence(): void {
    this.observedSilenceMs = 0
    this.lastReadinessObservationAt = Date.now()
  }
  ...
  private async pollReadiness(operation: LocalSendOperation): Promise<void> {
    ...
    try {
      ...
      const elapsedSinceObservation = Math.max(0, observedAt - this.lastReadinessObservationAt)
      this.lastReadinessObservationAt = observedAt
      // A host scheduler stall is not evidence that the provider stayed quiet:
      // output and the timer may both be queued while this process cannot run.
      // Charge at most one configured poll interval per actual readiness probe.
      this.observedSilenceMs += Math.min(elapsedSinceObservation, this.config.pollIntervalMs)
      if (this.promptSeen && foreground !== undefined && this.shellPgid === undefined) {
        this.shellPgid = foreground.processGroupId
      }
      if (this.promptSeen && this.promptTextSeen && this.observedSilenceMs >= this.config.pollIntervalMs
      ...
      if (startupHasOutput && this.observedSilenceMs >= this.config.idleSilenceMs + handoffGrace) {
