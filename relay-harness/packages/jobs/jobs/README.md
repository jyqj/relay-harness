# @relay-harness/rlh-jobs

English | [中文](README.zh.md)

The background job registry contract (`ctx.jobs`). The abstract `JobRegistry` and its vocabulary types give long-running producers shared ids, owner isolation, reads, cancellation, waiting, notices, and cleanup under one contract; the process-local registry lives in [`rlh-jobs-local`](../jobs-local/README.md). Producer plugins extend `JobKindMap` with their opaque id namespace.
...
- `kill(id, caller?, reason?)` invokes producer cancellation before changing status. A cancellation throw leaves the job running; success changes it to `stopping` and marks terminal delivery reported. If the producer does not settle within the implementation's bounded stop window, the stop protocol escalates to an optional `JobHooks.terminate` hook and then records `control-lost-unknown` — never a producer-confirmed terminal for an unconfirmed stop.
...
Implementations also owe the lifecycle semantics of the contract: registrations outlive producer and controller fibers, owner and service disposal cancel live work and await producers only for a bounded stop window before recording `control-lost-unknown`, and settlement is first-wins — one closed record, one round of contained listener notification, released waiters.
