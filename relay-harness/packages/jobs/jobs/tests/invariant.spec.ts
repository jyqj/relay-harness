...
import type { JobDoneListener, JobSnapshot } from '@relay-harness/rlh-jobs'
import * as JobsInvariant from '@relay-harness/rlh-jobs/invariant'
...
describe('job-registry invariants', () => {
  ...
  it.each([
    ...
    [{ ...BASE, status: 'running' }, undefined, /finishedAt must be present exactly for a closed status/],
    [TERMINAL_WITHOUT_FINISH, undefined, /finishedAt must be present exactly for a closed status/],
