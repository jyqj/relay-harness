...
import { AnonymousEntries, ScopedLayers, scopeOf } from '@relay-harness/rlh-scope'
import type { ScopeLayer } from '@relay-harness/rlh-scope'
import { deadline, timeoutOf } from '@relay-harness/rlh-timeout'
import { JobRegistry, JobId } from '@relay-harness/rlh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@relay-harness/rlh-jobs'
...
export class LocalJobRegistry extends JobRegistry {
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @relay-harness/rlh-agent)')
    }
