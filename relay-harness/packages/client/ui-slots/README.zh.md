...
渲染策略与运行失败是不同状态。`renderCandidates()` 移除被策略隐藏的单元，但保留已退位的注册项，使渲染器仍能显示真实的实现耗尽错误。`entries()` 保持原始检查和卸载台账不变；`entriesOfSlot()` 从同一策略允许的候选中选择存活实现。渲染器 host 使用候选投影而非原始台账判断空状态与崩溃状态。
