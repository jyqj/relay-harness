 ...
 *
 * The section paints in both themes, and a `--rlw-*` name the theme does not
...
describe('ModelsSection theme styles', () => {
  it('names only theme variables the token sheet defines', () => {
    // A `--rlw-*` name the sheet never declares is not a near miss: it silently
    ...
    // Every theme-variable prefix the sheets actually use, not just `--rlw-`:
    // a `--rlh-` name reads as a plausible sibling and would otherwise slip
  ...
  it('separates the row card from the editor it expands into', () => {
    ...
    expect(block('.editor')).toContain('background: var(--rlw-alias-bg-module-platform)')
    expect(block('.rowCard')).toContain('border: 1px solid var(--rlw-alias-border-l2)')
