...
传输 Promise 拒绝在 UI 边界被处理。初始状态读取失败会进入已有的不可用状态，不会永久停留在加载中；后台 fetch/PR 补充信息失败时保留已知本地状态。操作调用异常使用现有失败提示并释放忙碌状态；操作后的状态刷新是尽力而为，不会把已确认成功的修改误报为失败。

工作区切换会在绘制前清理工作区专属弹窗、文件选择和分支菜单状态。异步状态只可发布给仍被选中的原 cwd；旧操作的读取不能使新工作区刷新失效。已经启动的操作继续使用原 cwd，可以保留完成反馈，但旧后续操作会被移除，不会被重新解释成新工作区操作。

初始化、Pull、Publish、文件打开和外链打开都处理适配器同步抛错及 Promise 拒绝，且不推迟回调调用时机。Publish 失败仅在原工作区重开弹窗；来自旧工作区或旧 Git 操作时点的只读打开错误被忽略。外链打开器返回 false 也会被明确报告为打开失败，不再静默无响应。

组合操作保留界面显示的分支，或已接受确认中明确指名的分支。若预检报告不同 ref，UI 会在调用修改操作前停止，并要求用户核对当前分支。这是客户端预检，不是阻止其他进程在该次读取之后改变 Git 状态的原子锁。
