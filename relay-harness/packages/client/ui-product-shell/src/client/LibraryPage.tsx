...
export interface LibraryPageInjected {
  ...
  openSource: (entry: WorkLibraryEntry) => void
...
export function LibraryPage({ active: visible, useSessions, useConnection, openFiles, openSettings, queryLibrary, openLibraryOutput, openSource, t }: LibraryPageProps) {
  ...
  const currentScan = visible && ready && scan?.epoch === connection.epoch
  ...
  return <section className={css.page} data-library-page>
    ...
    <ul className={css.deliverables}>{entries.map((entry) => {
      ...
      return <li key={key}><button type="button" title={entry.path} disabled={!currentScan || opening === key} onClick={() => { open(entry) }}>{entry.path}</button><small>{entry.cwd ?? entry.sessionId}</small><button type="button" onClick={() => { openSource(entry) }}>{t('library.openSource')}</button></li>
    })}</ul>
