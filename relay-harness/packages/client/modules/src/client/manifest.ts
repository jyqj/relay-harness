 ...
 * `__RLH_BOOT__` wire types, the boot-manifest parser, and the boundaries around
 * {@link ClientModuleSystem}. The package root is the host-side service that
 * composes the wire.
 */

import type {} from '@relay-harness/cordis'
...
/**
 * Validate an optional string-array field read from a `rlh.client` declaration
...
export function parseBootManifest(wire: unknown): BootManifest {
  if (typeof wire !== 'object' || wire === null) {
    throw new Error('client-modules: window.__RLH_BOOT__ is missing or not an object')
  }
...
export interface RlhWindow {
   ...
   */
  __RLH_MODULES__?: ClientModuleSystem
  /**
   ...
   */
  __RLH_BOOT_GATE__?: Promise<void>
}
