...
import { TestSessions, TestWorkspaces } from '@relay-harness/rlh-client-test-runtime'
import type { Stabilizer } from '@relay-harness/rlh-client-test-runtime'
import { apply as nodeApply } from '@relay-harness/rlh-client-ui-renderer'
...
describe('UI renderer plugin', () => {
  ...
  it('hydrates the boot page before switching to the assembled application', async () => {
    ...
    slots.register({ name: 'root' }, () => <div data-testid="root-probe" />)
    const el = container()
    el.innerHTML = '<div class="boot" data-rlh-boot=""><div><div class="spinner" data-rlh-boot-spinner="" style="--rlh-boot-arc: 180deg"></div><div>Loading plugins…</div></div></div>'
