...
describe('wallpaper helpers', () => {
  ...
  it('mixes layered chrome fills at the given solidity and keeps an existing hex', () => {
    ...
    expect(light['--rlw-alias-bg-base']).toContain('var(--rlw-static-neutral-bluish-00)')
    // Canvas 45% (capped), sidebar halfway (63%), raised layers at the full 80%.
    expect(light['--rlw-alias-bg-base']).toContain('45%')
    expect(light['--rlw-specific-sidebar-fill']).toContain('63%')
    expect(light['--rlw-alias-bg-layer-1']).toContain('80%')
    const low = mixWallpaperSurfaces({}, 'light', 40)
    expect(low['--rlw-alias-bg-base']).toContain('15%')
    expect(low['--rlw-specific-sidebar-fill']).toContain('28%')
    const dark = mixWallpaperSurfaces({ '--rlw-alias-bg-base': '#120e18' }, 'dark', 70)
    expect(dark['--rlw-alias-bg-base']).toContain('#120e18')
    expect(dark['--rlw-alias-bg-base']).toContain('color-mix')
    expect(dark['--rlw-alias-bg-layer-1']).toContain('70%')
    const already = mixWallpaperSurfaces({
      '--rlw-alias-bg-base': 'color-mix(in srgb, #fff 58%, transparent)',
    }, 'light', 120)
    expect(already['--rlw-alias-bg-base']).toContain('var(--rlw-static-neutral-bluish-00)')
    expect(already['--rlw-alias-bg-base']).toContain(`${MAX_WALLPAPER_CANVAS_SOLIDITY}%`)
    const solid = mixWallpaperSurfaces({}, 'light', 100)
    expect(solid['--rlw-alias-bg-base']).toContain(`${MAX_WALLPAPER_CANVAS_SOLIDITY}%`)
    expect(solid['--rlw-specific-sidebar-fill']).toBe('var(--rlw-static-neutral-bluish-00)')
    expect(solid['--rlw-alias-bg-layer-1']).toBe('var(--rlw-static-neutral-bluish-00)')
    expect(solid['--rlw-alias-terminal-pane']).toBe('var(--rlw-static-neutral-bluish-00)')
    expect(dark['--rlw-alias-terminal-pane']).toBe('#120e18')
  })
  ...
  it('dims the wallpaper bitmap with mask-1 and does not blur the terminal pane', () => {
    ...
    expect(css).toMatch(/#rlh-wallpaper::after[\s\S]{0,220}--rlw-alias-bg-mask-1/)
    expect(css).not.toMatch(/--rlw-terminal-pane-blur/)
  })
})
...
describe('applyWallpaperLayer', () => {
  ...
  it('creates, updates, and removes the fixed layer', () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--rlh-wallpaper-blur')).toBe('10px')
    ...
    expect(document.documentElement.style.getPropertyValue('--rlh-wallpaper-blur')).toBe('0px')
    applyWallpaperLayer({ wallpaperImage: '', wallpaperBlur: 0, wallpaperPixelate: 0 })
    expect(document.documentElement.hasAttribute(WALLPAPER_ATTR)).toBe(false)
    expect(document.getElementById(WALLPAPER_LAYER_ID)).toBeNull()
    expect(document.documentElement.style.getPropertyValue('--rlh-wallpaper-blur')).toBe('')
  })
