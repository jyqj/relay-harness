...
import { apply as themeApply, inject as themeInject, ThemeRuntime } from '@relay-harness/rlh-client-ui-theme/client'
import { apply, inject, LayoutController } from '@relay-harness/rlh-client-ui-layout/client'
import { apply as nodeApply } from '@relay-harness/rlh-client-ui-layout'
import * as invariant from '@relay-harness/rlh-client-ui-layout/invariant'
...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the three child declarations', async () => {
    ...
    expect(slots.spec('surfaces')).toEqual({ kind: 'single', scope: 'session-maybe' })
    expect(slots.spec('shell.overlay')).toEqual({ kind: 'list', scope: 'root' })
    expect(slots.spec('shell.titlebar.trailing')).toEqual({ kind: 'list', scope: 'root' })
    expect(slots.spec('shell.terminalDrawer')).toEqual({ kind: 'single', scope: 'session-maybe' })
  })
...
describe('node half + invariant companion', () => {
  ...
  it('invariant companion registers under the package name', async () => {
    ...
    expect(register).toHaveBeenCalledWith('@relay-harness/rlh-client-ui-layout', expect.any(Function))
