# @relay-harness/rlh-client-ui-brand-official

English | [中文](README.zh.md)

This package fills `sidebar.brand.mark`, `sidebar.brand.name`, and `conversation.hero.brand.mark` only when `RLH_CLIENT_BUILD_PROFILE` is `official`. Other builds load the plugin but register no occupants, leaving the shell fallbacks visible.
...
- **The browser title is independent** — `RLH_CLIENT_TITLE` selects title text at build time rather than through a UI slot.
