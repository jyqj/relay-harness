...
import type { ConnectionSinks } from '@relay-harness/rlh-api-remotes/client'
...
async function mount(): Promise<Bench> {
  ...
  const handle: ConnectionHandle = {
    api,
    isLoopback: true,
    readiness: { getSnapshot: () => ({ phase: 'ready', epoch: 1 }), subscribe: () => () => {} },
    hostDescription: {
...
describe('runtime client apply', () => {
  it('mounts slots, Sessions, and Workspaces and fans host frames into both managers', async () => {
    ...
    void bench.sinks?.onConnected?.({ version: '0', cwd: '/f', attachedSessions: 0, home: '/h', canOpenPath: true, scratchCwd: '/scratch' })
  })
