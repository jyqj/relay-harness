...
import type { SettingsNamespaceView } from '@relay-harness/rlh-api-remotes/client'
...
describe('PermissionRow', () => {
  ...
  it('requires explicit acknowledgement before saving Full access (high risk)', async () => {
    ...
    fireEvent.click(screen.getByRole('menuitem', { name: 'Full access (high risk)' }))
    expect(mutate).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
    expect(screen.queryByRole('dialog', { name: 'Enable Full access (high risk)?' })).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: 'Read Only' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Full access (high risk)' }))
    const enable = screen.getByRole('button', { name: 'Enable Full access (high risk)' })
