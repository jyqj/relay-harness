 ...
 * @returns the explicit Full access (high risk) product label or the conventional display name.
 */
export function displayPermissionPreset(value: string, name: string): string {
  return value === FULL_ACCESS_PRESET ? 'Full access (high risk)' : displayPresetName(name)
}
