 ...
 */
import type { ClientContext, SnapshotStore } from '@relay-harness/rlh-client-runtime/client'
...
export interface InputActions {
  /**
   * Single public draft write path (full next draft; occurrence math via diff
   * scan). No admission-phase guard: busy-period writes are accepted and land
   * on the NEXT send — the in-flight attempt froze its own draft and
...
export interface SubmitAttempt {
  ...
  /**
   * Occurrence table at enter time, paired with `draftSnapshot`. The sink
   * splices serialized model forms against these offsets, so a draft edit
   * landing while the attempt is adjudicating/submitting shifts the live
   * table for the NEXT send without corrupting this one.
   */
  readonly occurrences: readonly Occurrence[]
