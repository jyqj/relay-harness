# @relay-harness/rlh-client-ui-conversation
...
Composer 草稿拥有显式生命周期。活跃草稿保持其逐会话聊天 store 持久化，跨主页面切换、隐藏页面与刷新均不丢失；导航到 Work 或 Library 既不提交也不清空草稿。连接 epoch 变化从不擦除草稿，但在连接恢复 ready 之前，输入栏保持提交惰性（发送按钮与 Enter 均不触发）。当某个 Session scope 在仍有未发送输入时被销毁，InputHub 会把一条 tombstone 记入根作用域的 discarded-draft registry（`input/drafts.ts`，持久化于 `rlh.conversation.drafts.discarded`，仅保留最新若干条）；会话重新打开时，它以 composer 上方的失效草稿提示呈现，并提供显式的恢复与丢弃操作，绝不作为活跃输入静默出现。
...
完成的一轮会物化一个有序的 `turn-tail` Conversation Node。它由引擎维护的 `TurnLocation` 提供收尾 Assistant 和 Turn data；renderer 在该 Node 的 IconActions 之前渲染 `conversation.chat.turnTail` chain，并派发包含 Turn、收尾 seq 和 `openFile` 的 `TurnTailOwnerProps`。本包只拥有空位；`@relay-harness/rlh-client-ui-deliverables` 把改写工具的 `locations` 累积到 Turn data，并拥有产物行、chip 上限和文案，因此把该插件从 cordis.yml 中组合掉即可关闭该交互面，空位以零成本渲染为空。收尾正文经由同一个开关参与其中：chat 视图向可选的 `chatFileMentions` service（ctx.get；由同一插件提供）索取收尾消息的行内代码词表，并把结果接进 MarkdownText 的 `fileMentions` seam——service 缺席时正文保持死文本。

## 历史窗口

较长的已加载历史使用变高视口回收，行键按 Session 保持稳定。可以双向滚动，仍可加载服务端更早分页；DOM 行数不随已加载历史总量增长。展开的工具、键盘聚焦行和选中文本会在显式交互持续期间保留挂载。这些用户持有行是可见的视口预算例外，不是后台静默保留。切换 Session 清空交互与尺寸缓存；重新挂载按语义锚点恢复，嵌套工具调用还记录其所属节点。

完整已加载历史模式会挂载全部已加载行，以支持浏览器原生查找与长文本选择。用户可以通过控件或 Ctrl/Cmd+F 显式启用；界面说明尚未加载的服务端历史不在查找范围内，并继续提供“加载更早”。返回窗口模式会恢复回收，不丢弃已加载历史。诊断用已加载行数／Turn 数表示数据窗口，不是挂载视口。`chat-long-interactions.e2e.ts` 与性能基准断言默认挂载行数上限加显式保留行数；浏览器场景还覆盖选择、展开工具、原生查找、分页和重排恢复。
