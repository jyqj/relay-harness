...
`Switch` 是受控的原生 checkbox，固定使用 `type="checkbox"` 与 `role="switch"`。标准 input 属性、无障碍标签、键盘激活、`focus-visible` 样式与 disabled 语义都会透传；视觉轨道和滑块只使用 token，并在 reduced motion 下移除过渡。
