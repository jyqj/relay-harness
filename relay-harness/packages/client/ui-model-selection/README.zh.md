# @relay-harness/rlh-client-ui-model-selection

[English](README.md) | 中文

模型选择插件（浏览器侧）：**两个入口共用一份会话级目录**，由 `ModelDirectoryResolver`（`ctx.modelDirectories`）持有。对于普通会话，`/model` popupSelect 贡献项（经 `ctx.commandUi` 注册）与 composer 的具名 `conversation.input.model` slot 都通过同一个 `ModelDirectory` 实例，经 `session.models` 加载会话的建议目录，并经 `session.selectModel` 提交。紧凑型 composer 触发器会打开两级 Model/Effort 菜单：模型仍按提供方分组，所选具体模型则提供由其适配器持有的推理强度名称、说明和默认值。该菜单使用 Presence 与主题 `popover` recipe；模型和推理等级触发器文案在选择变化时由 `FlipText` 翻转（[动效系统](../../../.agents/notes/implemented/architecture/2026-08-14-web-motion-presence-and-recipes.md)）。`/model` 应用所选模型的默认推理强度，composer 随后可以选择任一已公布的推理强度。
...
模型席位通过 `hooks.modelDirectory` 接收共享目录，由渲染器绑定成 `useModelDirectory`。组件不自行订阅外部状态，也不接收原始 store。专用的 `selectionError` 回调在选择确认后读取最新错误，保留原有 toast，避免使用过时的渲染状态。
