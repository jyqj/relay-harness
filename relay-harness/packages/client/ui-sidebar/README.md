...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces`, `sidebar.nav.tab`, `sidebar.page`, and `sidebar.settings` child slots, the region-tab store, and injected `startSession` plus sidebar-toggle callbacks. Plugin tabs occupy `sidebar.nav.tab`; zero occupants draw no strip. Selecting a plugin tab hides New Session and fills the region from `sidebar.page`.
