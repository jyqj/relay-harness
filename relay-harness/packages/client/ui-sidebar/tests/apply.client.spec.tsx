...
import { apply, inject } from '@relay-harness/rlh-client-ui-sidebar/client'
import type { SidebarRootInjected } from '@relay-harness/rlh-client-ui-sidebar/client'

async function bench(declare = true) {
  const ctx = new Context()
  await ctx.plugin(SlotRegistry).await()
  const layout = { toggleSidebar: vi.fn(), openMain: vi.fn() }
...
describe('ui-sidebar apply', () => {
  ...
  it('registers the shell and declares its child seats', async () => {
    ...
    expect(b.slots.spec('sidebar.chat.label')).toEqual({ kind: 'single', scope: 'root' })
  ...
  it('removes the entry and child declaration on teardown', async () => {
    ...
    expect(b.slots.spec('sidebar.chat.label')).toBeUndefined()
    expect(b.slots.spec('sidebar.workspaces')).toBeUndefined()
    expect(b.slots.spec('sidebar.footer.action')).toBeUndefined()
    expect(b.slots.spec('sidebar.nav.tab')).toBeUndefined()
    expect(b.slots.spec('sidebar.page')).toBeUndefined()
  })

  it('projects list-tab occupancy into ordered nav rows', async () => {
    const b = await bench()
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    b.slots.register(
      { name: 'sidebar.nav.tab', id: 'bots', order: 0, label: '机器人' } as never,
      () => null,
    )
    const injected = (b.slots.entries('sidebar')[0]!.inject as () => SidebarRootInjected)()
    expect(injected.hooks.navTabs.getSnapshot()).toEqual([
      { id: 'bots', order: 0, label: '机器人' },
    ])
  })
})
