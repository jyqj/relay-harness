# @relay-harness/rlh-client-ui-sidebar
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces`、`sidebar.nav.tab`、`sidebar.page` 与 `sidebar.settings` 子 slot、区域选项卡 store，以及注入的 `startSession` 与侧边栏切换回调。插件选项卡占用 `sidebar.nav.tab`；零占用方不绘制选项卡条。选中插件选项卡时隐藏 New Session，区域由 `sidebar.page` 填充。
