...
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@relay-harness/rlh-llm'
import type { ContentBlock, StreamChunk } from '@relay-harness/rlh-llm'
...
describe('toPiContext', () => {
  ...
  it('degrades the flat pre-envelope replay state a legacy session log carries', () => {
    ...
    expect(context.messages[0]).toMatchObject({ role: 'assistant', api: 'rlh-foreign' })
