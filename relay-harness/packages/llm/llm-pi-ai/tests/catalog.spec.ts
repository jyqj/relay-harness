...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@relay-harness/rlh-llm'
...
/** A throwaway $RLH_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'rlh-pi-catalog-'))
  homes.push(dir)
