...
import { AttachmentId, AttachmentStore } from '@relay-harness/rlh-attachment'
import type {
...
import LlmRuntime, { createUserMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, ReasoningEffortId, userAgent } from '@relay-harness/rlh-llm'
import * as LlmPiAi from '@relay-harness/rlh-llm-pi-ai'
import { PiAiAdapter } from '@relay-harness/rlh-llm-pi-ai'
...
describe('PiAiAdapter provider routing', () => {
  ...
  it('stops the SDK request when the adapter idle watchdog expires', async () => {
    ...
    const server = await mockServer([{
      events: [textEvents[0] as string],
      holdOpenAfterEvents: true,
      // Reproduce a loaded host delivering the server-side close observation
      // after the old one-second assertion race, without delaying socket close.
      closeObservationDelayMs: 1_200,
    }])
    const ctx = await harness(server.url, { streamIdleTimeoutMs: 500 })

    const assembling = assemble(ctx, { model: 'deepseek-v4-flash', messages: [] })
    await Promise.race([
      server.responseOpened,
      assembling.then(() => { throw new Error('adapter settled before the mock response opened') }),
    ])
