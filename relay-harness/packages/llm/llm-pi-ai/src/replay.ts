 ...
 *
 * @module rlh-llm-pi-ai/replay
 */

import { LlmError } from '@relay-harness/rlh-llm'
import type { Message, ModelMessageSource, ReplayEnvelope } from '@relay-harness/rlh-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'rlh-foreign',
    provider: source?.provider ?? 'rlh-foreign',
    model: source?.model ?? 'rlh-foreign',
    usage: emptyPiUsage(),
