...
Every request carries the shared attribution header from rlh-llm's `attributionHeaders()`, merged through pi-ai's `headers` stream option. Provider-specific app-attribution headers are not synthesized. See [rlh-llm § App attribution](../llm/README.md#app-attribution-attributionts).
...
- **An unauthenticated route depends on its protocol** — naming no credential resolves the route as configured-but-keyless, but pi-ai's OpenAI-compatible implementation still requires an API key, so a keyless local server needs a placeholder credential referenced by `apiKeyEnv`.
