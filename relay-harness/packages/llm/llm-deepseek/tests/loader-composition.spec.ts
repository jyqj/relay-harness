...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'rlh-llm-composition-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@relay-harness/rlh-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-deepseek real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-relay-harness-user-id']).toBe(getOrCreateAnonymousUserId())
