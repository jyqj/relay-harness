...
import LlmRuntime, { createUserMessage, CallId, ReasoningEffortId , createMessage } from '@relay-harness/rlh-llm'
...
import type { Config } from '@relay-harness/rlh-llm-deepseek'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'rlh-e2e-user-id-'))
  vi.stubEnv('RLH_HOME', identityHome)
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-deepseek e2e (real API)', () => {
  it('serves a real request with the key held only by a credentials-local document', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'rlh-e2e-credentials-'))
    try {
