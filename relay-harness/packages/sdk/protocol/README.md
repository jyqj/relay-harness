# @relay-harness/rlh-sdk-protocol

English | [中文](README.zh.md)

The shared wire protocol for the Relay Harness SDK runtime: one newline-delimited JSON-RPC 2.0 transport class plus the named request, result, and notification types both wire ends speak. The package root enumerates the protocol consumer interface; source modules are not exported as deep imports. The server side is the [`rlh-sdk-jsonrpc-server`](../server/README.md) plugin; clients are [`rlh-sdk-client`](../client/README.md) (TypeScript) and the [Python SDK](../../../python/README.md) (which mirrors these shapes but does not import them). A pure library — no plugin, no Config, no registration.

## Transport

`JsonRpcLineTransport` frames JSON-RPC 2.0 over caller-owned byte streams, one compact JSON frame per `\n`-terminated line. Frames with `id` and `method` are requests, `id` alone is a response, `method` alone is a notification; malformed JSON lines are ignored. `start()` attaches stream listeners, `close()` detaches them and rejects pending requests without destroying the streams. Missing request handlers answer `-32601`; handler rejections answer `-32603` with the error message. An error response rejects the pending `request()` with `JsonRpcResponseError`, which preserves the wire `code` and optional `data`. `JsonRpcTransportPeer` is the outbound surface (request/notify) the server class is typed against. Each inbound or outbound line is capped at `maxFrameBytes` (default 64 MiB), including partial input that never sends a newline; unsettled output is capped by `maxQueuedWriteBytes` (default one 64 MiB frame plus its delimiter) and accounted until each stream callback settles. Oversize input terminates the transport through `onFailure`; oversize output and queue pressure throw `JsonRpcFrameTooLargeError` or `JsonRpcWriteQueueOverflowError` before retaining more bytes.
