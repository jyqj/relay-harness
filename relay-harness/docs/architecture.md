...
[Cordis](cordis-primer.md) is the framework under rlh: plugins contribute services, typed events, and reversible effects to a shared context. Every part of the product is a plugin, including the model adapter, the tool registry, the session log, and the agent loop itself, so every part is replaceable from configuration.

There is no privileged core to patch: you extend rlh by mounting a plugin beside the others, and registrations are effects that unwind when their plugin unloads.

## Profiles and bundles

A running `rlh` is a plugin tree composed at boot from ordered layers.
...
Each declares itself in its own `package.json` under a `rlh` field: `rlh.profile` lists a profile's bundles, and `rlh.bundle` points at a bundle's patch file.

[`rlh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`rlh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`rlh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
...
`agent/pre-step` decides what the model sees. Listeners may rewrite the claimed messages or reject them outright; a rejected or empty first claim still closes a durable turn that spent no step, so the log records the attempt. Each step captures the scoped tool definitions and Code Mode backend before assembling the prompt, then binds execution to the final advertised names; registration changes during streaming affect the next step, not the current response. Before tool capture and assembly, an optional `ctx.contextEngine` service (`@relay-harness/rlh-context-engine`) resolves purpose eligibility, provider-local deadlines and allowances, then deterministically deduplicates and packs attributed context from the claimed messages under one total budget. Selected messages append as claimed user messages before the runtime-context snapshot. After `agent/pre-step` admits the step, AgentLoop appends their ordinary `user/message` events and then a log-only `context/prepared` trace containing the retrieval plan, selected/rejected decisions, Evidence and Coverage, plus exact seq links for unmodified messages that survived admission; a rejection-only trace adds no model-visible content. The request remains reconstructable only from surface messages, not from duplicate trace text. A request in which every eligible provider declines remains identical to a deployment without the service. Other prompt sections and contexts remain ordinary per-assembly contributions.

Successful root tool results also retain execution-captured declared mutation paths outside model content; the deliverables projection folds this durable inventory independently of browser history paging.
...
| Add persistent terminal execution | register a `ctx.terminals` backend plus `rlh-tool-terminal` |
