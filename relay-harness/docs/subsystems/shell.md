...
The bash execution seam is split across a Service Definition ([rlh-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([rlh-bash-local](../../packages/shell/bash-local) and [rlh-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([rlh-tool-bash](../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Raw process-group mechanics live behind the [subprocess seam](subprocess.md).

Source: [`packages/shell/shell/src/types.ts`](../../packages/shell/shell/src/types.ts)

## Managed shell environment namespace

`RLH_*` variables are Harness-owned child-process facts. The model-facing bash tool collects them through `ctx.shellEnv` and passes them through `ShellExecRequest.rlhEnv`; the subprocess service removes inherited `RLH_*` names before merging the current snapshot. The `RlhEnvironmentKey`/`RlhEnvironment` vocabulary is owned by the [subprocess seam](subprocess.md) and re-exported by `rlh-shell`.
...
interface ShellExecRequest {
   ...
   * scrub. Managed facts belong in {@link rlhEnv}, which merges after this
  ...
  /**
   * Harness-owned `RLH_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `RLH_*` entries before merging this
   ...
   */
  rlhEnv?: RlhEnvironment | undefined
...
interface ShellExecSpec {
   ...
   * {@link ShellExecRequest.env}; {@link rlhEnv} still merges after them.
   * OPTIONAL on the spec for the same reason as `stdin`: absent means no
   * ordinary extra environment.
   */
  env?: Record<string, string> | undefined
  /** Managed `RLH_*` snapshot (typed to managed keys); merges after {@link env}. */
  rlhEnv?: RlhEnvironment | undefined
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `rlh-tool-bash`. The local executor scrubs ambient credentials before merging explicit caller-supplied env. See [the bash-stdin-env Agent Note](../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.md).
...
Each stream is a `CollectedOutput` — the (possibly truncated) text plus recovery info; when truncated, `text` is the **tail** and the complete stream spills to a private file. The fields are owned by the [subprocess seam](subprocess.md) and re-exported by `rlh-shell`.

## File sandbox: `ShellSandboxInfo`

A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@relay-harness/rlh-sandbox-policy`](../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@relay-harness/rlh-sandbox` seam](sandbox.md); modes govern file effects only.
...
`start()` returns a handle with no id or owner. `rlh-tool-bash` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the process closes and never rejects, reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
...
`ShellExecutor` owns `resolve`, foreground `run`, background-process `start`, and the `sandboxMode` capability fact. `rlh-bash-local` owns command defaulting, timeout/abort classification, the terminal environment, and the background read merge; process groups, bounded collectors, spill files, credential scrubbing, and disposal quiescence are the [subprocess service](subprocess.md)'s. `rlh-tool-bash` owns model-facing rendering and adapts background handles into the [generic job runtime](jobs.md). `rlh-shell` owns the shell tools' shared exit-status contract: the exported `parseExitStatus`/`ParsedExitStatus` inverts the `[exit code: N]` / `[killed by signal: X]` markers `rlh-tool-bash`'s `renderResult` and `rlh-tool-pwsh`'s `renderPwshResult` append, and both tools' `presentResult` use it to split the rendered text into the terminal card's output body and its exit-status pill.
...
Registry (`ctx.shellEnv`) for trusted, per-execution `RLH_*` variables. The namespace is rebuilt for every model shell call: ambient `RLH_*` values are discarded by the executor, then the registry's current snapshot is injected. Built-in shell facts remain owned by the registry itself while plugins can register additional, enumerable facts with effect-scoped disposal.
...
Types: [RlhEnvironment](subprocess.md) · [ToolExecution](tools.md)
