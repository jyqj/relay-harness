...
- **An empty completion is a retryable error, not a silent success.** Both adapters map a terminal `stop` finish that carried no content blocks to `finish {kind:'error'}` with the canonical `EMPTY_RESPONSE` code, and `rlh-llm-retry` retries it by default; see [empty model responses are retryable](../../.agents/notes/implemented/bug-fix/2026-07-24-empty-model-response-is-retryable.md).
...
declare class BlockAssembler {
   ...
   */
  message(source: MessageSource = { kind: 'plugin', plugin: 'rlh-llm/assembler' }): Message;
}
```
...
interface GenerateOptions {
   ...
   * the derived history (rlh-agent-loop); a hand-built one-shot passes any list.
   */
  messages: Message[]
...
`GenerateOptions.tools` carries `ToolSchema` — the JSON-schema description of a tool, as sent to the model. It is declared in rlh-llm (not rlh-tools) precisely because it is part of the request the loop assembles every step:

```ts type-equiv
/**
 * JSON-schema description of a tool, as sent to the model.
 *
 * Declared here (not in rlh-tools) because it is part of {@link GenerateOptions};
 * rlh-tools' ToolDefinition and rlh-system-prompt's PromptAssembly both import
