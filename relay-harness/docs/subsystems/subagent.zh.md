...
Service Definition：[rlh-subagent](../../packages/subagent/subagent)（`ctx.subagents` + 下文词汇）。Service Provider 是六个兄弟包：`rlh-subagent-spawn-in-process`、`-fork`、`-acp`、`-codex`、`-claude-code`、`-rlh-sdk`；面向模型的 Consumer 包括 [rlh-tool-subagent](../../packages/subagent/tool-subagent)（按提供方委派）、[rlh-tool-subagent-control](../../packages/subagent/tool-subagent-control)（可选的全局 `send_message`、`interrupt_agent` 与 `list_agents` 控制工具）和 [rlh-tool-subagent-report](../../packages/subagent/tool-subagent-report)（可选的 child 作用域 `report` 返回通道）。同一个 `ctx.subagents` 服务通过内部激活管理器负责可继续子 agent 编排，并直接基于会话存储和可选的会话持久化提供只读的 child 与后代发现。产品提供方设计理由见 [Codex 与 Claude Code Agent Note](../../.agents/notes/implemented/feature/2026-08-04-claude-code-and-codex-subagent-backends.md)；通用 seam 的设计理由见 [subagent Agent Note](../../.agents/notes/implemented/feature/2026-06-21-subagent-capability-seam.md)、[可继续 subagent Agent Note](../../.agents/notes/implemented/feature/2026-07-28-continuable-subagent-conversations.md)、[report 工具 Agent Note](../../.agents/notes/implemented/feature/2026-07-30-continuable-subagent-report-tool.md)、[持久化目录 Agent Note](../../.agents/notes/implemented/feature/2026-07-22-durable-subagent-catalog-and-list-agents.md)、[列表身份投影 Agent Note](../../.agents/notes/implemented/architecture/2026-08-06-subagent-list-identity-projection.md)和[服务合并 Agent Note](../../.agents/notes/implemented/simplification/2026-07-26-merge-subagent-control-service.md)。

源码：[`packages/subagent/subagent/src/types.ts`](../../packages/subagent/subagent/src/types.ts)、[`packages/subagent/subagent/src/index.ts`](../../packages/subagent/subagent/src/index.ts)、[`packages/subagent/subagent/src/admission.ts`](../../packages/subagent/subagent/src/admission.ts)和 [`packages/subagent/subagent/src/continuation.ts`](../../packages/subagent/subagent/src/continuation.ts)

## 共享根会话树容量

`SubagentRuntime` 会在分派任何提供方或物化任何 Activation 前应用一项可选容量策略。`maxActivePerRoot` 把每个后代计入最高 live 持久会话祖先；`maxActivePerParent` 还可限制直属 sibling。一次性委派、可继续 child、workflow 与 team 都经过同一服务，因此面向特定 consumer 的 adapter 无法绕过上限。不同 root 使用不同计数器。基础 bundle 对每个 root 使用四个活跃 child 生命周期并立即以 `CAPACITY_EXCEEDED` 拒绝；直接组合若未配置则保持无界。

一次性 lease 只会在返回的 run 到达 holder 所有的 `dispose()` 时结束。可继续 lease 属于一个 Activation epoch，并在其 `AgentHandle` 达到静默并完成 disposal 后结束，包括回滚路径。`overflow: queue` 是可选项：其 root 局部队列会移除已取消 waiter，并提升当前符合条件的最早 parent；runtime teardown 会拒绝待处理与未来获取，但不会撤销已发布 child。所有权与默认值理由记录在[根会话树准入 Agent Note](../../.agents/notes/implemented/architecture/2026-08-21-root-tree-subagent-admission.md)中。
