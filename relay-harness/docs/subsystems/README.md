...
| [issue-automation.md](issue-automation.md) | tracker providers, repository workflow policy, per-issue directories, durable claims/retries/blocks, Agent runs, and operator controls |
...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `RLH_*` environment vocabulary |
...
| [memory.md](memory.md) | governed cross-session memory: Scope, evidence, append-only revisions, recall settlement, SQLite retrieval, and model-facing Consumers |
...
| [work-results.md](work-results.md) | log-prefix confirmation, verified receipts, and source-attributed cross-session deliverables |
| [client-modules.md](client-modules.md) | the web plugin table: `rlh.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
