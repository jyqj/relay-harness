...
- Use `--rlw-alias-*` semantic tokens in feature components. Do not copy static palette values or write literal colors there.
...
Shared enter/exit motion lives in [`ui-theme` `motion.css`](../packages/client/ui-theme/src/styles/motion.css) and [`usePresence`](../packages/client/ui-primitives/src/usePresence.ts). Overlay, popover, fade, swap, and flip recipes animate only `opacity` and `transform`. A surface sets `data-rlh-motion` and `data-state` from `usePresence`; it does not invent another duration or easing. The composer plus, permission, model, and context-meter popovers all use `popover`. `FlipText` plays the 400ms flip recipe when a permission, model, or effort trigger label changes. The motion vocabulary is the `--rlw-motion-quick/base/slow` ladder in `base.css` plus three curves: `--rlw-ease-out-quart` for arrivals, `--rlw-ease-in-cubic` for exits, and `--rlw-ease-standard` for neutral state changes. The five recipes pin their durations in `motion.css` because `usePresence` and `FlipText` hold matching JS constants. `prefers-reduced-motion: reduce` zeros the `--rlw-motion-*` ladder and disables the recipe transitions and animations. Do not animate `backdrop-filter`, large-panel width/height, or add an animation library. New dialogs, menus, and in-place swaps reuse a primitive or the same hook and recipe. Rationale: [the motion-system Agent Note](../.agents/notes/implemented/architecture/2026-08-14-web-motion-presence-and-recipes.md).
