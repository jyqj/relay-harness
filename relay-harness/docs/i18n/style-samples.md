...
> This document describes the architecture of the Relay Harness — the foundation of **Relay Code**. The governing principle, from the microkernel design discussion: **everything is a plugin**. The core is deliberately tiny — a handful of abstract services plus one concrete loop plugin (`rlh-agent-loop`) — and every product feature is a plugin against the extension API described here, without modifying the loop.

本文介绍 Relay Harness 整体架构，它是 **Relay Code** 的底层基座。微内核设计讨论中确立了核心设计准则：**一切皆插件**。内核刻意做得极精简，仅包含少量抽象服务，外加一个实体循环插件 `rlh-agent-loop`。所有产品功能均基于本文定义的扩展接口开发为独立插件，无需改动主循环逻辑。

> Dependency rule: extension plugins depend on interfaces, never on `rlh-agent-loop` (the loop is swappable); the sanctioned exception is the composition bundle `rlh-agent-spine-demo`, whose job is assembling the concrete spine.

依赖约束规范：各类扩展插件仅依赖抽象接口，严禁直接依赖 `rlh-agent-loop`（该主循环支持替换实现）；唯一允许的特例是组合包 `rlh-agent-spine-demo`，它的职责是组装整套实体主干。
...
- 代码体标识符（事件名 `agent/status`、状态值 `running`、包名 `rlh-bash-local` 等）在译文中保留 code span 原文，不得口语化改写；Pass 2 必须逐句核验。
