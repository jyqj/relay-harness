...
Corpus discovery reads Git-tracked files plus ordinary untracked files through `git ls-files --cached --others --exclude-standard`. A newly created untracked document therefore enters the gate immediately, while files excluded by `.gitignore`, `.git/info/exclude`, or the user's global Git excludes remain outside this checkout's corpus.
