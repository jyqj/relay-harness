...
[Cordis](cordis-primer.md) 是 rlh 底层的框架：插件向共享上下文贡献服务、类型化事件和可逆的副作用。产品的每一部分都是插件，包括模型适配器、工具注册表、会话日志，以及 agent loop（智能体循环）本身，因此每一部分都可以从配置替换。

不存在需要打补丁的特权内核：扩展 rlh 的方式是把插件挂载到其他插件旁边，而各项注册都是副作用，会在其插件卸载时撤销。

## Profile 与组合包

运行中的 `rlh` 是一棵插件树，由启动时按序叠加的各层组合而成。
...
两者都在各自的 `package.json` 中通过 `rlh` 字段声明自己：`rlh.profile` 列出一个 profile 的组合包，`rlh.bundle` 指向一个组合包的 patch 文件。

[`rlh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`rlh-web-app`](../packages/bundle/web-app/README.md) 增加浏览器应用；[`rlh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器。
...
`agent/pre-step` 决定模型看到什么。监听器可以改写已领取的消息，也可以直接拒绝它们；首次领取被拒绝或被改写为空时，仍会关闭一个不含步骤的持久轮次，因此日志会记录这次尝试。每个步骤都会在组装提示词前捕获 scope 工具定义与 Code Mode 后端，再把执行绑定到最终声明的名称；streaming 期间的注册变更影响下一步骤，而不是当前响应。其他提示词片段与上下文仍是普通的逐组装贡献。
...
| 添加持久化终端执行 | 注册 `ctx.terminals` 后端和 `rlh-tool-terminal` |
