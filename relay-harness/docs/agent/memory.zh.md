...
> 当前实现位于 [`packages/memory/`](../../packages/memory/README.md)，由本地存储、治理状态、outcome reconciliation、Context contributor 与 Memory Center 组成。确切实现状态见 [`../feature-status.json`](../feature-status.json)，运行时协议见 [`../subsystems/memory.md`](../subsystems/memory.md)。
