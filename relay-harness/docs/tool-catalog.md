...
| `@deepseek-ai/dsh-tool-memory` | `memory_forget`, `memory_read`, `memory_remember`, `memory_search`, `memory_update` | `ctx.tools`, `ctx.longTermMemory`, `a calling Agent for exact Scope and write evidence` | `tool/call`, `canonical memory revisions or tombstones`, `tool/result` | - | Search and read are Scope-bound. Remember and update activate only exact direct-user or successful-tool-result evidence; unverified proposals remain candidates, and forget requires a direct-user deletion excerpt. |
