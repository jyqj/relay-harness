...
- **覆盖率门禁**（`pnpm run test:coverage`）：被测 `packages/*/*/src` 文件逐文件达到 100%，不代表整个产品的覆盖率。`vitest.config.ts` 显式列出平台、入口与暂缓的 GUI 排除项。覆盖率证明执行发生过，不证明交付行为；应删除死代码，而不是为它制造测试。PowerShell 覆盖率需要真实 `pwsh`：缺少它的主机会跳过执行器测试并豁免该源码；CI runner 提供它并执行完整阈值。
...
无密钥测试证明本地行为与已记录的协议兼容性；只有经过授权的真实调用才能证明当前提供方行为。真实提供方 workflow 需要显式触发并使用已配置的凭据。缺少凭据会使触发后的预检失败，而不是计为已验证。提供方或发行报告区分已执行的真实测试、回放与不可用的证据。通过检查外部世界的 smoke 覆盖文件写入、多轮对话、工具与流中取消（[事故复盘 0001](postmortem/0001-acp-default-export-drops-inject.md)）。每个示例保留无密钥与带密钥 smoke（[examples/AGENTS.md](../examples/AGENTS.md)）。

## 优先使用真实实现而非 mock

只 mock 开销高或不确定的边界（LLM（大语言模型）适配器、网络、时钟）；下游一切保持真实。手写替身只能证明桥接层在搬运字节，不能证明交付的工具行为符合断言。桥接工具调用测试将脚本化 mock 模型与真实工具和执行器配合使用：`makeBridgeHarness({ withBash: true })` 接入 `rlh-bash-local` 与 `rlh-tool-bash`，然后运行 `echo`。
