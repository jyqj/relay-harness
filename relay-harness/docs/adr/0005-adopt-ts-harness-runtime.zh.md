...
仓库实际在 checkout 的 `relay-harness/` runtime root 下包含成熟的 TypeScript Relay Harness monorepo，已经提供 agent loop、会话持久化与恢复、工具与 skill、subagent、权限与审批、计划模式、压缩、LSP、沙箱、CLI/Web/Electron 应用，以及成套覆盖率、快照和文档门禁。从零开发 Rust 实现会长期落后于这份经过验证的运行时。

## Decision

1. Relay 采用 `relay-harness/` runtime root 下的 TypeScript monorepo 作为 agent 运行时，品牌为 Relay Harness（`rlh`），不另行启动 Rust 内核实现。
