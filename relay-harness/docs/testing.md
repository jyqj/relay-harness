...
A keyless test proves local behavior and recorded protocol compatibility; only an authorized live run proves current provider behavior. Live provider workflows are explicitly dispatched and require their configured credentials. Missing credentials fail the dispatched preflight rather than count as validation. Provider or release reports distinguish executed live tests from replay and unavailable evidence. Cover file writes, multi-turn conversations, tools, and mid-stream cancellation through world-verified smoke tests ([postmortem 0001](postmortem/0001-acp-default-export-drops-inject.md)). Each example retains keyless and with-key smokes ([examples/AGENTS.md](../examples/AGENTS.md)).

## Prefer the real implementation over a mock

Mock only the expensive or non-deterministic boundary (LLM adapter, network, clock); keep everything downstream real. A hand-rolled stand-in proves the bridge moves bytes, not that the shipping tool behaves as asserted. Bridge tool-call tests use the scripted mock model with the real tool and executor: `makeBridgeHarness({ withBash: true })` plugs in `rlh-bash-local` and `rlh-tool-bash`, then runs `echo`.
