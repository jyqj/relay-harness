...
The public package family belongs to the `@relay-harness` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@relay-harness/node-addon-landlock-run
@relay-harness/node-addon-landlock-run-<platform>
```
