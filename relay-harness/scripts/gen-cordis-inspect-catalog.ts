...
import type { CordisCatalogModel, ServiceMethodEntry } from '@relay-harness/rlh-typert-generator'
...
const CLIENT_SERVICES: Readonly<Record<string, readonly string[]>> = {
  layout: [
    ...
    'toggleSurfaces', 'openSurfaces', 'closeSurfaces',
    'toggleTerminalDrawer', 'setTerminalDrawer',
  ],
...
function main(): void {
  ...
  const source = projector.renderRuntimeApi(clientModel(model))
    .replaceAll('@relay-harness/rlh-tool-cordis/api-catalog', '@relay-harness/rlh-cordis-client-runner/client/api-catalog')
