 ...
 * consumes the briefing is `.agents/skills/rlh-translate-docs/SKILL.md`.
 */
...
function maskCodeSpans(markdown: string, spans: MarkdownSpan[]): string {
  return replaceSpanTexts(markdown, spans, new Map(spans.map(span => [span.index, `RLH_TRANSLATION_CODE_${span.index}\n`])))
}
