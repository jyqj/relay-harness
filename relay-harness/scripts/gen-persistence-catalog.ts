...
const SESSION_PACKAGE = '@relay-harness/rlh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@relay-harness/rlh-session/types'
 ...
 * top-level declaration (in `@relay-harness/rlh-session`) and any declaration
 * merge inside a `declare module '@relay-harness/rlh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @relay-harness/rlh-session/known-event-types',
    ' */',
    '',
    '/**',
