...
export interface ClientDeclaration {
  ...
  /** Whether the manifest declares a dynamic rlh.client row. */
 ...
 * @returns Declarations and malformed rlh.client fields.
 */
...
export function fixClientPackageManifests(root: string, facts: ClientPackageFacts): string[] {
  ...
  for (const declaration of facts.declarations.filter(entry => entry.dynamic)) {
    ...
    const rlh = isRecord(target.manifest.rlh) ? target.manifest.rlh : undefined
    const client = isRecord(rlh?.client) ? rlh.client : undefined
  ...
  for (const pkg of facts.packages) {
    ...
    if (pkg.dynamic) {
      ...
      for (const name of productionNames) {
        ...
        } else if (section(target.manifest, 'dependencies')[name] !== undefined && isInternalRlh(name)) {
...
function preferredRange(
  ...
  if (isInternalRlh(name)) return 'workspace:^'
...
function collectModeViolations(facts: ClientPackageFacts): string[] {
  ...
  for (const pkg of facts.packages) {
    if (pkg.dynamic && pkg.staticLinked) {
      violations.push(
        pkg.manifest + ': ' + pkg.name + ' declares rlh.client and uses the staticLinked preset;'
      ...
      violations.push(
        ...
        + ' declare rlh.client or use the staticLinked preset',
      )
    }
  }
  ...
  for (const pkg of [...facts.packages].sort((left, right) => left.manifest.localeCompare(right.manifest))) {
    ...
    for (const [name, rule] of [...expected].sort(([left], [right]) => left.localeCompare(right))) {
      ...
      violations.push(
        ...
        + ' is a peer-installed RLH relationship; declare it in peerDependencies and devDependencies'
    ...
    for (const section of ['dependencies', 'peerDependencies'] as const) {
      for (const name of Object.keys(pkg[section]).sort()) {
        ...
        } else if (section === 'dependencies' && isInternalRlh(name)) {
          violations.push(
            pkg.manifest + ': dynamic package declares ' + name + ' in dependencies;'
            + ' dynamic RLH relationships are peer plus dev, and static client inputs are dev-only',
          )
        }
      }
    }
...
function expectedSections(pkg: ClientPackage, staticInputs: ReadonlySet<string>): Map<string, ExpectedRule> {
  ...
  if (!pkg.dynamic) {
    ...
    for (const [name, locations] of Object.entries(pkg.runtimeSourceUses)) {
      if (name === pkg.name || name === CORDIS || isInternalRlh(name)) continue
  ...
  const add = (name: string, origin: string): void => {
    ...
    const kind = staticInputs.has(name) ? 'dev' : isInternalRlh(name) ? 'peer-dev' : undefined
  ...
  for (const name of pkg.inject) add(name, 'rlh.client.inject')
  return expected
}
  ...
  for (const pkg of facts.declarations.filter(entry => entry.dynamic)) {
    for (const field of ['external', 'inject'] as const) {
      ...
      for (const value of pkg[field]) {
        if (value === '') violations.push(pkg.manifest + ': rlh.client.' + field + ' contains an empty value')
        else if (seen.has(value)) {
          violations.push(pkg.manifest + ': rlh.client.' + field + ' lists ' + JSON.stringify(value) + ' twice')
        }
        seen.add(value)
      }
    }
    ...
    for (const specifier of new Set(pkg.external)) {
      ...
      if (baseline.has(specifier)) {
        violations.push(
          pkg.manifest + ': rlh.client.external repeats baseline module ' + JSON.stringify(specifier)
      ...
      if (supplier === pkg.name) {
        violations.push(pkg.manifest + ': rlh.client.external names its own row ' + JSON.stringify(specifier))
      } else if (supplier !== undefined) {
        edges.push({ from: pkg.name, to: supplier, specifier })
      } else {
        const owner = stripClientSuffix(specifier)
        violations.push(
          pkg.manifest + ': rlh.client.external ' + JSON.stringify(specifier) + ' has no supplier;'
          + (byName.has(owner)
            ? ' workspace package ' + owner
              + ' declares no dynamic rlh.client row and the shell does not seed this specifier'
...
function formatCycle(
  ...
  return manifest + ': synchronous rlh.client.external cycle: ' + chain + ' ' + (entry?.from ?? '')
}
...
function readDeclaration(
  ...
  const rlh = isRecord(manifest.rlh) ? manifest.rlh : undefined
  const rawClient = rlh?.client
  if (rawClient === undefined) {
    return { name: manifest.name, manifest: manifestPath, dynamic: false, external: [], inject: [] }
  }
  if (!isRecord(rawClient)) {
    malformed.push(manifestPath + ': ' + manifest.name + ' rlh.client must be an object')
    return { name: manifest.name, manifest: manifestPath, dynamic: false, external: [], inject: [] }
  }
  return {
    name: manifest.name,
...
function stringArray(
  ...
  if (!Array.isArray(value) || value.some(entry => typeof entry !== 'string')) {
    malformed.push(manifestPath + ': ' + packageName + ' rlh.client.' + field + ' must be a string array')
    return []
  }
...
function isInternalRlh(name: string): boolean {
  return name === CORDIS || name.startsWith(RLH_PREFIX)
}
