 ...
 *
 * The rlh family shares one version across its publishable members, private
...
/** Release types the rlh family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the rlh family's version. */
...
/** One private rlh package whose version follows the publishable family. */
interface PrivateRlhVersion {
  /** Repository-relative manifest path. */
  readonly manifestPath: string
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:rlh <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Discover private package manifests that share the rlh version without joining
 * its publish set.
 * @param root - repository root.
 * @returns Private package manifests sorted by path.
 */
function privateRlhVersions(root: string): PrivateRlhVersion[] {
...
/**
 * Plan the rlh family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the rlh family.
 * @param root - repository root.
...
export function planShared(
  ...
  for (const entry of privateRlhVersions(root)) {
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <rlh|vendor> [version]')
  ...
  if (family.id === 'rlh') {
    ...
    if (request === undefined) throw new Error('usage: release:rlh <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:rlh takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
