/**
 * Rescope the vendored Cordis packages into the `@relay-harness` scope, and undo
 ...
 * `@relay-harness/rlh-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@relay-harness/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@relay-harness/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@relay-harness/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@relay-harness/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@relay-harness/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@relay-harness/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@relay-harness/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@relay-harness/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@relay-harness/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@relay-harness/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@relay-harness/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@relay-harness\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@relay-harness/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@relay-harness/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@relay-harness/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@relay-harness/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@relay-harness/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@relay-harness/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@relay-harness` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@relay-harness\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@relay-harness/cordis']
    const dev = manifest.devDependencies?.['@relay-harness/cordis']

    if (!peer) errors.push(\`\${label}: @relay-harness/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @relay-harness/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @relay-harness/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    // The rescoped name is already covered by the `@relay-harness/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@relay-harness/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@relay-harness/')) {`,
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@relay-harness/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@relay-harness/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@relay-harness/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@relay-harness/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @relay-harness, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@relay-harness\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@relay-harness/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@relay-harness\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@relay-harness/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @relay-harness, so the consumer installs this repository's
  ...
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'rlh-packed-consumer', private: true, type: 'module' }))
