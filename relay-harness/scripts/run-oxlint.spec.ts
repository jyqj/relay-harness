...
describe('Oxlint invocation', () => {
  ...
  it('bounds both worker pools from one setting', () => {
    expect(resolveOxlintInvocation(['.', '--fix'], { RLH_OXLINT_THREADS: '4', GOMAXPROCS: '12' })).toEqual({
      args: ['.', '--fix', '--threads=4'],
      env: { RLH_OXLINT_THREADS: '4', GOMAXPROCS: '4' },
    })
  })

  it('uses location-preserving diagnostics in CI', () => {
    expect(resolveOxlintInvocation(['.'], { CI: 'true', RLH_OXLINT_THREADS: '4' })).toEqual({
      args: ['.', '--format=unix', '--threads=4'],
      env: { CI: 'true', RLH_OXLINT_THREADS: '4', GOMAXPROCS: '4' },
    })
  })
  ...
  it.each(['0', '-1', '1.5', 'auto'])('rejects invalid worker bound %s', (value) => {
    expect(() => resolveOxlintInvocation(['.'], { RLH_OXLINT_THREADS: value }))
      .toThrow('RLH_OXLINT_THREADS must be a positive integer')
  })

  it('rejects a competing direct worker bound', () => {
    expect(() => resolveOxlintInvocation(['.', '--threads=2'], { RLH_OXLINT_THREADS: '4' }))
      .toThrow('use RLH_OXLINT_THREADS instead')
  })
})
