...
const vendoredPackages = new Set([
  ...
  '@relay-harness/cosmokit',
  '@relay-harness/schemastery',
  ...
  '@relay-harness/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@relay-harness/node-addon-landlock-run',
  '@relay-harness/node-addon-landlock-run-linux-arm64',
  '@relay-harness/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@relay-harness/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/relay-harness/relay-harness.git'
/**
...
const experimentalPackageNamePrefix = '@relay-harness/rlh-experimental-'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/
/** Product applications that are distributed as installers rather than npm packages. */
const privateApplicationDirectories = new Set(['apps/desktop'])

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@relay-harness/rlh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@relay-harness/rlh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@relay-harness/rlh-client-ui-primitives': ['lib/**/*.css'],
  '@relay-harness/rlh-client-web': ['lib/**/*.css'],
  '@relay-harness/rlh-client-ui-theme': ['lib/styles'],
  // The Ghostty terminal adapter loads wasm, font, and shader assets from the
  // browser bundle's adjacent published directory.
  '@relay-harness/rlh-client-ui-user-terminal': ['lib/assets/**'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@relay-harness/rlh-code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@relay-harness/rlh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
...
function expectedRlhPackageFiles(manifest: PackageManifest): readonly string[] {
  const declaredPatch = manifest.rlh?.bundle?.patch
  ...
  return [
    ...
    ...exportDefault(manifest, './features') === './lib/features.js' ? ['lib/features.js'] : [],
    ...extras,
...
export function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the rlh family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@relay-harness/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@relay-harness/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@relay-harness/rlh-')) {
    const peer = manifest.peerDependencies?.['@relay-harness/cordis']
    ...
    const expectedFiles = expectedRlhPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the rlh release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:rlh` and its publication.
