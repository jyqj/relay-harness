...
const CLIENT_BUILD_ENV_PREFIX = 'RLH_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'RLH_BUILD_CLIENT_PROFILE'

/** Public client environment required by official RLH artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'RLH_CLIENT_COMMIT_HASH'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.rlh-build/client-build-environment.json'
...
): Readonly<Record<`RLH_CLIENT_${string}`, string>> {
  return {
    RLH_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
 ...
 * @returns defined `RLH_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return { RLH_CLIENT_COMMIT_HASH: commitHash, ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`RLH_CLIENT_${string}`, string>>,
): void {
