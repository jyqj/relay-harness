...
const MERGE_HEAD = /declare module ['"](?:@relay-harness\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@relay-harness/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@relay-harness/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@relay-harness/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@relay-harness/cordis'` Context merge
...
/**
 * Every event name a `declare module '@relay-harness/cordis'` Events merge declares in one
