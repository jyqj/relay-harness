...
import { gitVisibleRepoFiles } from './repo-files.ts'
import {
...
describe('translation pairing switchers', () => {
  ...
  it('accepts only the canonical public URL for an absolute switcher', () => {
    ...
    const canonical = parseTranslationMarkdown(
      '[中文](https://github.com/jyqj/relay-harness/blob/master/relay-harness/python/sdk/README.zh.md)',
    )
    const wrongPath = parseTranslationMarkdown(
      '[中文](https://github.com/jyqj/relay-harness/blob/master/relay-harness/other/README.zh.md)',
    )
...
describe('translation scope discovery', () => {
  it('includes tracked and ordinary untracked docs but excludes Git-ignored docs', () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-translation-discovery-'))
    try {
      execFileSync('git', ['init', '--quiet', root])
      for (const directory of ['docs', 'docs/gitignored', 'docs/info-excluded']) {
        mkdirSync(join(root, directory), { recursive: true })
      }
      writeFileSync(join(root, '.gitignore'), '/docs/gitignored/\n')
      writeFileSync(join(root, '.git', 'info', 'exclude'), '/docs/info-excluded/\n')
      writeFileSync(join(root, 'docs', 'tracked.md'), '# Tracked\n')
      writeFileSync(join(root, 'docs', 'untracked.md'), '# Untracked\n')
      writeFileSync(join(root, 'docs', 'gitignored', 'hidden.md'), '# Hidden\n')
      writeFileSync(join(root, 'docs', 'info-excluded', 'local.md'), '# Local\n')
      execFileSync('git', ['-C', root, 'add', '.gitignore', 'docs/tracked.md'])

      const files = gitVisibleRepoFiles(root).filter(isTranslationScopeFile)

      expect(files).toContain('docs/tracked.md')
      expect(files).toContain('docs/untracked.md')
      expect(files).not.toContain('docs/gitignored/hidden.md')
      expect(files).not.toContain('docs/info-excluded/local.md')
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  })
