...
if TYPE_CHECKING:
    from relay_harness import RunResult
...
# (relay-harness#2488), and the file must replay on both. Everything else is compared.
RUNTIME_CONTEXT_PREFIX = "Current runtime context"
CUSTOM_CORDIS = """\
- id: sdk-jsonrpc-server
  name: '@relay-harness/rlh-sdk-jsonrpc-server'
- id: agent-core
  name: '@relay-harness/rlh-agent-spine-demo'
  config:
    ...
    tools:
      mode: both
- id: sessions
  name: '@relay-harness/rlh-session-persistence-jsonl'
  config:
    root: !!js process.env.RLH_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@relay-harness/rlh-code-runtime-worker-thread'
- id: subagents
  name: '@relay-harness/rlh-subagent'
- id: subagent-spawn-in-process
  name: '@relay-harness/rlh-subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
...
- id: cordis-host-runner
  name: '@relay-harness/rlh-cordis-host-runner'
- id: cordis-tool
  name: '@relay-harness/rlh-tool-cordis'
"""
FS_SEARCH_CORDIS = """\
- id: sdk-jsonrpc-server
  name: '@relay-harness/rlh-sdk-jsonrpc-server'
...
- id: fs-search
  name: '@relay-harness/rlh-tool-fs-search'
  config:
...
def smoke_sdk_default(base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rlh-sdk-default-") as temporary:
...
def smoke_sdk_custom(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rlh-sdk-custom-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_minimal(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rlh-sdk-minimal-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_fs_search(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rlh-sdk-fs-search-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_mcp(base_url: str, executable: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rlh-sdk-mcp-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rlh-sdk-snapshot-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="rlh-direct-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
        environment = {
            **os.environ,
            "RLH_CORDIS_CONFIG": str(cordis),
            "RLH_SESSION_ROOT": str(sessions),
            "RLH_CWD": str(root),
