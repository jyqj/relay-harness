 ...
 * itself (`rlh web`), so any process that rewrites `lib/client.js` files
 ...
 * `lib/client.js`, and `vite build` rewrites `apps/web/dist`, which `rlh web`
...
/** Compile-shell workspace whose dist `rlh web` serves. */
const SHELL_PACKAGE = '@relay-harness/rlh-web-frontend'
 ...
 * whose package.json carries `rlh.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      rlh?: { client?: { platform?: unknown } }
    }
    if (manifest.rlh?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
 ...
 * client preset without declaring `rlh.client` has no loader-delivered browser
...
export function discoverLibraryDirs(root = repoRoot): string[] {
  ...
  for (const configPath of globSync('packages/*/*/tsdown.config.ts', { cwd: root }).sort()) {
    ...
    if (manifest.rlh?.client === undefined) dirs.push(dir)
  }
  return dirs
}
...
if (isMain) {
  ...
  if (pluginDirs.length === 0) {
    console.error('dev-web: no rlh.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} rlh.client plugin packages`
