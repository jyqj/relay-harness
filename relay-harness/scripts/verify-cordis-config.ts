 ...
 * silently changes composition. Example configs and the rlh Web composition
...
// These example files are overlays consumed by the built rlh app, so their bare
...
const CHOOSER_PACKAGE = '@relay-harness/rlh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@relay-harness/rlh-host-directory-picker-native',
  '@relay-harness/rlh-host-directory-picker-browse',
  '@relay-harness/rlh-client-ui-directory-picker-browse',
  '@relay-harness/rlh-client-ui-directory-picker-native',
]
const errors: string[] = []
 ...
 * `rlh.client` block, and the node half of a surface plugin is an empty
 ...
 * `rlh.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.rlh?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no rlh.client, so its browser half is never served`
      : `${manifestPath}: declares rlh.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `RLH_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@relay-harness/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // rlh app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.rlh?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).rlh?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `rlh` source
