...
from relay_harness import RelayHarness, HarnessClient, HarnessConfig
from relay_harness.errors import TransportClosedError
from relay_harness_runtime import resolve_bundled_launch_args
...
- id: session-checkpoints
  name: '@relay-harness/rlh-session-checkpoint-policy'
- id: subprocess
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> HarnessClient:
    return HarnessClient(
        HarnessConfig(
            ...
            cwd=str(tmp_path),
            env={
                "RLH_CORDIS_CONFIG": "./cordis.yml",
                "RLH_SESSION_ROOT": str(tmp_path / "sessions"),
                "RLH_CWD": str(tmp_path),
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    assert init.serverInfo.name == "relay-harness-sdk-runtime"
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = RelayHarness(
        ...
        env={
            "RLH_MODEL": model,
            "RLH_CONTEXT_WINDOW": "1000000",
            "RLH_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@relay-harness/rlh-does-not-exist'\n"
    )
    ...
    assert "@relay-harness/rlh-does-not-exist" in str(excinfo.value)
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("RLH_RUNTIME_MODE", mode)
