...
json.dump({
    ...
    "RLH_CWD": os.environ.get("RLH_CWD"),
    "RLH_SESSION_ROOT": os.environ.get("RLH_SESSION_ROOT"),
    "RLH_CORDIS_CONFIG": os.environ.get("RLH_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["RLH_CWD"] == str(tmp_path)
    assert dumped_env["RLH_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["RLH_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
""".strip()
    ...
    with RelayHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
""".strip()
    ...
    with HarnessClient(
        ...
        assert init.serverInfo.name == "fake-rlh"
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    ...
    assert "profile" not in inspect.signature(RelayHarness.run).parameters
...
""".strip()
    ...
    module_dir = tmp_path / "relay_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "relay_harness_runtime", raising=False)
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    if ambient_config is None:
        ...
        monkeypatch.setenv("RLH_CORDIS_CONFIG", ambient_config)
    ...
    assert json.loads(env_dump.read_text())["RLH_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "RLH_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="deepseek-official", cwd="/workspace", model="deepseek-v4-pro")

    assert json.loads(env_dump.read_text())["RLH_CORDIS_CONFIG"] == "./explicit.yml"
