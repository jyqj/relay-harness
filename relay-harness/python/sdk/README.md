...
`RelayHarness` keeps its lazily started runtime subprocess for reuse across calls. Use it as a context manager, as above, or call `close()` explicitly when finished. `close()` is idempotent and terminal: it sends the best-effort `shutdown` request bounded by `shutdown_timeout_seconds`, closes stdin, waits `eof_grace_seconds=6.0` for cooperative teardown, then terminates and waits `terminate_grace_seconds=3.0` before force kill; afterwards the client rejects `start()` and fails subscriptions and pending requests immediately with `TransportClosedError`. Both grace fields exist on `RelayHarnessConfig` and `HarnessConfig`.
