...
class HarnessClient:
    """Synchronous JSON-RPC client for the Relay Harness SDK runtime over stdio."""
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Relay Harness runtime{suffix}"
                        )
    ...
    def _start_stderr_thread(self) -> None:
        self._stderr_thread = threading.Thread(target=self._stderr_loop, name="rlh-runtime-stderr", daemon=True)
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            ...
            raise FileNotFoundError(
                "Unable to locate the bundled Relay Harness SDK runtime. "
                "Install relay-harness-runtime-bin or set HarnessConfig.runtime_bin."
            ) from exc
    ...
    def _inject_bundled_default_config(self, env: dict[str, str]) -> None:
        ...
        if not uses_bundled_runtime or env.get("RLH_CORDIS_CONFIG"):
            return
        # _default_launch_args already imported the package or raised its install error.
        from relay_harness_runtime import bundled_default_config_path
