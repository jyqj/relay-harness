...
Run the archive verifier's focused test, `pnpm run verify-archived-agent-notes`, `pnpm run doc-sync`, `pnpm run lint`, and `git diff --check`; select any additional evidence through [rlh-pre-push-checks](../rlh-pre-push-checks/SKILL.md).
