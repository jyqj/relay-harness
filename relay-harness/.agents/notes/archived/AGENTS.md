...
Artifacts sealed before the [Relay Harness rename](../implemented/process/2026-08-23-relay-harness-rename.md) name the product Relay Harness and use the retired `rlh` token. The seal forbids rewriting them, so translate while reading: `@relay-harness/rlh-<pkg>` is today's `@relay-harness/rlh-<pkg>`, the `rlh` binary is `rlh`, `RLH_*` variables are `RLH_*`, `--rlw-*` CSS custom properties are `--rlw-*`, and `relay-harness` paths are `relay-harness`. `DEEPSEEK_API_KEY`, `DEEPSEEK_BASE_URL`, `api.deepseek.com`, and the `llm-deepseek` adapter name refer to a third-party model vendor and read literally in both eras.
