...
Compaction is split between `@relay-harness/rlh-compaction`, which owns an abstract two-method service and shared types, and `@relay-harness/rlh-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@relay-harness/rlh-compaction` and remove `@relay-harness/rlh-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@relay-harness/rlh-compaction-basic` and its workspace/package metadata are removed.
- `@relay-harness/rlh-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
