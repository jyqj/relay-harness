...
**Preinstall `rlh-better-sidebar`.** Rejected: it portals out of the official column, pins an older harness, refuses localhost (the opposite of our Browser), and has no push. The interaction to copy is the `openPath` funnel and tab chrome, not the host.
