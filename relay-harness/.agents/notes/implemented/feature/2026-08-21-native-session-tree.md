...
**Adopt Pi's entry `parentId` inside one Session file.** Rejected because it would add a second ordering relation beside RLH Turn/Step balance and duplicate the existing cross-Session fork lineage. Historical navigation is represented as a child Session instead of moving a mutable leaf pointer.
