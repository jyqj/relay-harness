...
完整浏览器测试在一次确定性流式 turn 中测得六次 `scrollToEnd` 写入，超过原有五次预算。插桩显示各次高度不同，均从 layout effect 派发：问题是连续结构更新未合并，而非重复写入相同偏移。

首次尾部定位仍同步执行，保证首帧就绪。后续 layout 更新通过一个动画帧标记保留最新的尾部操作，不在每次渲染时取消并重新计时。回调重新检查跟随意图，用户滚动或选择旧记录优先；卸载取消并清空待执行任务。测试覆盖同一绘制帧前的多次更新、最新高度、滚离尾部和卸载后的迟到回调。原有单测尾部断言改为等待计划的绘制帧，仍要求完全相同的偏移值。浏览器滚动调用、几何与虚拟化预算保持不变。
