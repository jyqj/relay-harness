...
- **code-context contributor**（`packages/context/code-context`）。注册严格 opt-in：带 config section 的 Loader 条目经 `ctx.inject(['contextEngine'])` 后期绑定注册一个 step-context contributor（`code-index-recall`）；没有 config section 的条目只构造 `ctx.codeContext`、不注册任何东西，且该包不进任何 bundle。每个被认领的 step 把 direct user 文本拼成 query，把去重的 `@file` 提及作为显式 scope，执行一次 search，再经 `ctx.get('codeIndex')` 批量 hydrate 有界候选。健康答案贡献包含 source-verified fenced snippet 的不可信 `## Code-index recall` 消息，以及以内容哈希为 revision、digest 覆盖实际接纳源码的 evidence；stale/unavailable hydration 会进入 warning 与 coverage，绝不回退成 path-only 内容。[候选 hydration 决策](../architecture/2026-08-29-code-index-candidate-hydration.md)持有该源验证分离。无命中的 step 贡献简短有界否定消息；degraded search 或 hydration 失败不贡献内容。反递归仍是结构性的：query 只读 direct user 消息，`form: 'recall'` 仍不进入 session-query 语料抽取。
