...
A Workspace's `sessionIds` is an ordered candidate index. A membership projection requires both that an id appear in the index and that the corresponding canonicalized `SessionHeader.cwd` equal the Workspace path; SessionHeader does not gain a `workspaceId`. A Session whose cwd matches but whose id is absent from the index remains in Tasks, while an indexed id is filtered out if its header is missing, its cwd is invalid, or its cwd does not match. Two Workspace indexes claiming the same Session is corrupt state and fails loudly.
...
When no Workspace exists, the page stays on the blank New Session view without writing to the Host. The composer is inert until the operator picks a Workspace or No workspace folder. Picking a Workspace connects or reuses that Workspace's blank Host Session. Picking No workspace folder calls `workspaces.connectNoDirectory()`: reuse a blank Session whose cwd is `host.describe.scratchCwd` and whose id is in no Workspace index, else `session.create({ cwd })`. That path never calls `workspace.create` ([one-route Note](../simplification/2026-07-31-one-route-to-add-a-workspace.md); [no-directory sessions](2026-08-15-no-directory-task-sessions.md)).

Top-level New Session and the plus button on a Workspace row invoke the same New Session action. An explicit Workspace id becomes the target directly; when none is specified, the action uses the current Session's Workspace, then the most recent Workspace, and enters the blank New Session page when no real Workspace exists. The Tasks-section plus calls `connectNoDirectory` instead. The Workspace picker's Add workspace action immediately creates a real Workspace when the user confirms a directory, then retargets the Session to it; an explicitly created empty Workspace remains even if the user sends no message.

A new Workspace takes its display name from the directory it was created in. Distinct canonical paths may share the same basename-derived title ([identity decision](../bug-fix/2026-07-31-same-basename-workspace-adoption.md)); the explicit rename operation retains its duplicate-title check. Moving Sessions across Workspaces, manual adoption from Tasks, and separate display-name and directory-name inputs remain outside this flow.
...
Real Sessions that cannot be assigned to any Workspace appear under Tasks. The grouped browser labels directory Workspaces as Projects and renders Tasks as a collapsible section without a folder row; an empty Tasks section is omitted. Host `session-added` and `workspace-changed` events may arrive in either order; list merging does not depend on frame order.

Deleting a Workspace registration removes its group without deleting or closing any Session. Its accounted Sessions immediately join Tasks, including the current Session; a reload reconstructs the same result from the independent Workspace and Session baselines.
...
- The zero state with no Workspace writes nothing to the Host; the composer stays inert until a Workspace or No workspace folder is selected. Explicit Add workspace immediately creates and displays an empty Workspace. No workspace folder never calls `workspace.create`.
...
- The UI and Host admit distinct same-basename directories as separate Workspaces, while the explicit rename operation rejects duplicate titles; cwd-only Sessions, Sessions with invalid historical cwd values, and unattached Sessions remain in Tasks.
...
- Historical Sessions with a missing cwd, an invalid directory, or a failed realpath remain in Tasks; this iteration has no manual-adoption entry point.
