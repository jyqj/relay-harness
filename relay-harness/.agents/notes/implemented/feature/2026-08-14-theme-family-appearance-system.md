...
The Web UI exposed only Light / Dark / System. Those three cubes lived in General, the durable document stored a single `ui-theme.preference`, and any other palette was an in-process `register()` id that vanished on reload. The desktop shell already shipped six boot palettes, but they painted only the Electron chrome and never reached `--rlw-alias-*`. Users could not keep a non-default look across restarts, could not author a custom family, and could not tune overlay solidity or type independently of the token sheets.

## Decision

**Keep ThemeRuntime and the alias-layer contract.** The presenter still paints `body` from `ThemeSnapshot.active`. New work adds a `ThemeFamily` document (one card, light and dark seed halves), a derive step that writes `--rlw-alias-*`, and durable half ids. The DeepSeek family ships empty derived tokens so existing CSS sheets remain the look for profiles that only store `preference`. Derived families keep the canvas at the seed background and map the accent onto the colorful chrome tokens the sheets otherwise pin to DeepSeek blue (`--rlw-alias-state-business-primary`, `--rlw-alias-button-info-fill`, `--rlw-specific-bubble`, sidebar selection) so a custom color is visible in chat, not only on Appearance sliders.
...
**Glass and type are extras, not per-theme documents.** `--rlw-alias-glass-opacity` mixes menu, dialog, settings panel, and composer fills. Session column surfaces stay solid. Font stacks land on `documentElement` (`--rlw-font-family`, `--rl-font-family-code`, composer / terminal extras) rather than inside a family document.
...
**Import a parallel `--background` / `--primary` sheet.** Rejected because every shipped sheet and presenter already speak `--rlw-alias-*`. Derivation writes our names.
