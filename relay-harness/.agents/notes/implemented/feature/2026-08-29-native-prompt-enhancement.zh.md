...
- `rlh-client-ui-prompt-enhancement` 在不修改 InputBar 的情况下占用 `conversation.input.right`。它在卸载时取消，并展示带 assumptions 和 open questions 的原文/增强 diff。只有显式 Accept 才能应用建议，而且草稿值与单调 revision 必须仍等于记录尝试；写入通过普通输入事务进行，因此既有撤销和显式 revision-guarded 撤销都会恢复原文。忙碌点击会穿过 Remote 取消请求，Session 切换／移除会取消陈旧工作。它绝不提交。
...
建议对话框把完整新旧草稿渲染为 removed 与 added block，而不计算 word-level inline diff。结构化结果携带两份草稿、assumptions、open questions 和 trace，因此更丰富的展示不需要另一套 Host 协议。
