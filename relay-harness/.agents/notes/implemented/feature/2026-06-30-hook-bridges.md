# Agent Note: rlh-hooks-claude-code + rlh-hooks-codex — the Claude Code / Codex hook bridges
...
Matcher semantics, exit-code handling, and merge precedence live in `rlh-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. Per-file coverage includes config branches plus end-to-end mappings through a real loop, `rlh-bash-local`, and shell scripts, while a real-Loader smoke guards the package export shape. Each real-process test owns and disposes its complete Context before deleting scripts; detached hook tests wait for an observable marker, and only the outer test watchdog carries process kill/reap headroom under full-suite contention. Native plugins bypass the wire protocol and return typed decisions directly.
