...
- The commands package now depends on `rlh-attachment` and `rlh-llm`, and `commands/execute` carries a required `images` wire parameter — every caller states its envelope explicitly.
