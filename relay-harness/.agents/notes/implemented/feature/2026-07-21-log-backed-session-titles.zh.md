...
首消息提供方在会话的第一条合格人类消息到达、且最新标题不是用户钉住时调度一次。种子中没有合格人类消息的 fork 子会话也走这条路径。自动执行失败后，后续提示词不会重新调度；`refresh()` 是重试路径。全部消息提供方会在每条合格且由人类发出的提示词后调度，并传入截至该修订的所有合格消息，包括预置历史记录。较新的修订会中止并取代更早的待执行或活跃工作。
...
与源日志的其他部分相同，fork 会原样继承作为种子的标题事件——被钉住（user 来源）的标题在子会话中保持钉住，直到显式 refresh。首消息提供方不会自动为已经继承合格人类消息的子会话重新生成标题。种子中没有合格人类消息、且最新标题未被用户钉住的子会话，会从它的第一条新合格消息调度首消息生成。全部消息提供方可以在子会话出现后续提示词后追加一项归子会话所有的修订，并使用继承的合格消息和新增的合格消息。空 fork 避免钉住见[空 fork 首消息重新生成标题](../bug-fix/2026-08-17-empty-fork-session-title.md)。
