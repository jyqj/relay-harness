...
Harness already connects MCP servers through `rlh-mcp-client` and discovers skills through `rlh-skill-filesystem`, but neither catalog had a Settings management page. Users had to edit `cordis.patch.yml` or skill files by hand. Desktop has no separate persistence for these catalogs: they belong under `$RLH_HOME` and must work for CLI and Web alike.
...
**A new skill enablement list beside frontmatter.** Rejected because `rlh-skill-filesystem` already owns invocation flags; a second list would drift from the files the watcher already reloads.

**Desktop `window.shell` APIs for these catalogs.** Rejected because persistence is `$RLH_HOME` and Settings already lives in the Harness Web UI. Desktop only needs the same section jump About and Plugins already use.
