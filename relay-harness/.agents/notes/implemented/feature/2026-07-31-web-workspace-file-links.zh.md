...
> 范围：完成的轮次以其产出文件收尾的那一行、读得出是链接的文件路径链接，以及 Host 打开器对浏览器可渲染文档优先选用默认浏览器。经决定，`rlh web` 不在范围内：从 harness 源以 HTTP 提供工作区文件，以及为不在 Host 机器上的客户端提供预览。

## 问题

一个产出了文件的 web 会话，没有办法看到那个文件。agent（智能体）写出了 `deepseek-homepage.html` 并如实告知，而用户唯一的办法是把 `/private/tmp/rlh-client-hotplug.ygPvsm/workspaces/plugin-hotplug/deepseek-homepage.html` 这样的绝对路径复制进终端。
...
- **桌面端外壳中的内嵌 WebView**——可得到的最强隔离，因为那时预览跑在产品自己拥有的容器里，而不是用户的浏览器里。桌面对话和 `openPath` 打开 html/svg 时，通过带 token 前缀的 `127.0.0.1` 静态服务使用该 guest（[对话链接进入 Files 与 Browser](2026-08-19-conversation-surface-links.md)）。本 web 笔记仍然拒绝从 harness 源提供工作区文件。
