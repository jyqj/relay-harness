...
普通发行 composition 中每个 turn 的模型请求数有限；默认 worker Adapter 的 Code Mode binding 无法保留或 clone 无界规范值；TypeScript SDK transport／subscription 内存拥有显式失败点。移除 controller 的自定义 Agent composition 会重新承担缺失的 turn 策略。恶意 worker 参数会在 worker→host structured clone 后才被计量，因此 worker heap 上限仍是更早的兜底；未来可以在不改变公开契约的情况下通过 wire-token preflight 更早拒绝。Python SDK 是独立实现，目前尚未继承 TypeScript client 的 subscription 上限。
