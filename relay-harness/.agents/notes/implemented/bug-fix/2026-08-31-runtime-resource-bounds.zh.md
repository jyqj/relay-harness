...
独立的 Python SDK 会原生实现同一组默认值，而不是依赖 TypeScript client：byte-mode stdout 分帧会拒绝超大完整行或没有终止换行符的 partial frame；同步 stdio writer 会在等待写锁之前预留编码后的字节，并循环直到每个字节都写完；每个 subscription 使用 4096 项队列。底层 write 返回 `0` 或 `None` 会在 flush 前 fail closed。有类型的 Python 错误保留同样三类失败，`RelayHarnessConfig` 会把每项限制传给底层 client。runtime wheel 的默认 Cordis composition 还会显式固定 server 侧 frame 与写入限制。
...
普通发行 composition 中每个 turn 的模型请求数有限；默认 worker Adapter 的 Code Mode binding 无法保留或 clone 无界规范值；两个 SDK client 都拥有显式 transport／subscription 失败点。移除 controller 的自定义 Agent composition 会重新承担缺失的 turn 策略。恶意 worker 参数会在 worker→host structured clone 后才被计量，因此 worker heap 上限仍是更早的兜底；未来可以在不改变公开约定的情况下通过 wire-token preflight 更早拒绝。Python 遗留的全局 notification 与 incoming-request 队列仍是独立底层 API；有界的高层 run 路径使用 subscription。
