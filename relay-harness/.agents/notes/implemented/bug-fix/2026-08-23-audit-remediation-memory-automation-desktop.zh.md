...
**把 `secretEnvironmentNames` 接入 subprocess 环境构造。** 本次拒绝：会侵入上游布局拥有的 `dsh-subprocess` seam；改为如实记录声明性语义。
