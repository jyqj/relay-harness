...
1. **调度器身份可能断裂。** 工具运行时的内部调度视图通过模块作用域的 `unique symbol` 寻址。同一进程如果意外加载了两份 `@relay-harness/rlh-tools`（重复安装、陈旧的 profile 回退链接，或混合打包的运行时），会得到两个不同的 symbol，于是消费方的 `ctx.tools[TOOL_RUNTIME_SCHEDULER]` 读到 `undefined`，`prepare` 以裸的 `TypeError: Cannot read properties of undefined (reading 'prepare')` 崩溃。
...
- 调度器在每个 group 通过 `requireToolRuntimeScheduler` 解析一次，缺失时给出可操作的诊断（陈旧的 `$RLH_HOME/profiles/<name>/node_modules` 副本、混合打包的运行时），而不是裸的 `TypeError`。

### 稳定的调度器身份（tools）

`TOOL_RUNTIME_SCHEDULER` 改为 `Symbol.for('@relay-harness/rlh-tools.scheduler')`，同一 realm 内的两份模块副本因此共享同一个键。`unique symbol` 类型与生成的 Cordis API 面不变。
...
- rlh-tools 的重复模块副本不再破坏调度器查找；调度器缺失会得到诊断而不是 `TypeError` 崩溃。
...
- 打包：`npm run pack` 成功；交付的运行时归档内含修复（调度器守卫、outcome-unknown 合成、canonicalizer、`Symbol.for` 键）。脚本直接导入归档内的 `rlh-session`/`rlh-llm` bundle，验证损坏形状可派生为 provider 合法 transcript。
