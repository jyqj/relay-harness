...
The independent Python SDK implements the same defaults natively rather than relying on the TypeScript client: byte-mode stdout framing rejects an oversized line or unterminated partial frame, synchronous stdio writers reserve their encoded bytes before waiting on the writer lock and loop until every byte is written, and each subscription uses a 4096-item queue. A raw write returning `0` or `None` fails closed before flush. Its legacy global-notification and incoming-request channels also retain at most 4096 items, preserve the admitted prefix on overflow, and fail only their own consumer. Closing a subscription atomically unregisters it, drops the prefix, and gives later reads a typed closed error. `RelayHarnessConfig` passes all five limits to the low-level client. The runtime wheel's default Cordis composition pins the server-side frame and write limits explicitly.
...
Ordinary shipped compositions have finite model requests per turn, Code Mode bindings cannot retain or clone an unbounded canonical value through the default worker Adapter, and both SDK clients have explicit transport and notification retention failure points. Custom Agent compositions that remove the controller again own the missing turn policy. A hostile worker argument is measured after worker-to-host structured clone, so the worker heap limit remains the earlier backstop; a future wire-token preflight can move that rejection earlier without changing the public contract. Python retains separate low-level global notification and incoming-request APIs, but neither queue is unbounded. `pnpm run test:python-carriers` turns missing exe/node artifacts into failures after the host build; ordinary source-only pytest keeps independent skips so contributors do not rebuild a 200+ MB executable for unrelated SDK edits.
