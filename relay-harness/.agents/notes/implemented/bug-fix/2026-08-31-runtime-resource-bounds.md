...
Ordinary shipped compositions have finite model requests per turn, Code Mode bindings cannot retain or clone an unbounded canonical value through the default worker Adapter, and TypeScript SDK transport/subscription memory has explicit failure points. Custom Agent compositions that remove the controller again own the missing turn policy. A hostile worker argument is measured after worker-to-host structured clone, so the worker heap limit remains the earlier backstop; a future wire-token preflight can move that rejection earlier without changing the public contract. The Python SDK remains a separate implementation and does not yet inherit the TypeScript client's subscription limit.
