...
The Web Appearance, Language, and busy-Enter preferences lived in browser `localStorage`. Browser storage is scoped to an origin, so reopening `rlh web` on another port selected a different partition and lost choices even though both processes used the same RLH home. These are user-level product preferences; session selection, drafts, disclosure state, and other transient browser state remain page-local.
...
Remote browsers cannot call the loopback-only configuration API, so their preferences remain process-local. In-process `ThemeRuntime.register()` ids remain an extension outside the built-in Host schema; removing one resets the live registry without replacing the last durable built-in preference. Product theme families — including user-created documents — persist in the same `ui-theme` section as `customThemes` plus the two half ids, so a custom family survives reload the same way `preference` does.
...
Appearance, Language, and busy-Enter choices follow the RLH user home across reloads, ports, and loopback origins. Direct edits to `settings.yaml` converge through the existing invalidation stream, while legacy `rlh.theme`, `rlh.locale`, and `rlh.conversation.busyEnter` entries are neither read nor written.
