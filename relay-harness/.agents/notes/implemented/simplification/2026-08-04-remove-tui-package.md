...
This decision supersedes the reusable-package retention in [the explicit-config `rlh` entrypoint decision](../../archived/simplification/2026-08-03-explicit-config-rlh-entrypoint.md) and the current applicability of the archived TUI implementation notes. Their historical records remain frozen, but they are not authority for the supported package or application inventory.
