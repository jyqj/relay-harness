...
`rlh` shipped two full config trees that were 43 rows the same. `apps/cli/cordis.yml` composed the web surface as 74 flat rows, while the TUI booted `examples/tui-agent/cordis.yml`, whose single `@relay-harness/rlh-tui-demo` row mounted twelve plugins and re-declared their configuration as its own twenty-key pass-through `Config`.

Neither file was what its location claimed. `examples/tui-agent` was not an example: `apps/cli/src/tui.ts` hardcoded it as the product's default config, and it owned the TUI PTY smoke, the eight terminal snapshot scenarios, and the PTY harness the `cordis-agent` leaf imported. `rlh-tui-demo` was not a demo either — it was the application, mounted by the shipped binary from `packages/examples/`.
...
Precedence is list order, last write winning per row: base, then the surface overlay, then either a `--config` overlay or the personal `~/.rlh/config.yaml`, then the launcher's own flag and profile patches.
...
An overlay or `--config` tree that named `@relay-harness/rlh-tui-demo`, or patched the `tui-agent` row, no longer resolves. Overlays now patch the row that owns each key: the model route on `agent-loop`, the persona on `system-prompt`, presentation on `tui`.
