...
The repository also used `SDK` in two meanings. The supported Python and TypeScript clients use the JSON-RPC SDK protocol. The project as a whole is Relay Harness, not an SDK project. The removed SDK project toolchain made the broad meaning obsolete, but prose and names preserved parts of it.
...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@relay-harness/rlh-sdk-client`, `@relay-harness/rlh-sdk-protocol`, and the wire identity `relay-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. Relay Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
Do not remove an intentional vendor qualifier to avoid repetition. `rlh-subagent-rlh-sdk` names the Relay Harness SDK provider and avoids confusion with another SDK. Its private class becomes `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `@relay-harness/rlh-jsonrpc` | `@relay-harness/rlh-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@relay-harness/rlh-sdk-client`, `@relay-harness/rlh-sdk-protocol`, and `relay-harness-sdk-runtime`. Exclude `@relay-harness/create-sdk`, `@relay-harness/rlh-scripts`, `@relay-harness/rlh-helper`, and `@relay-harness/rlh-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@relay-harness/rlh-bash`, `ctx.bash` | `@relay-harness/rlh-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@relay-harness/rlh-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@relay-harness/rlh-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@relay-harness/rlh-pty`, `ctx.pty`, `PtyService` | `@relay-harness/rlh-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@relay-harness/rlh-pty-local`, `LocalPtyBackend` | `@relay-harness/rlh-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@relay-harness/rlh-tool-pty` | `@relay-harness/rlh-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@relay-harness/rlh-lsp-local` | `@relay-harness/rlh-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@relay-harness/rlh-tasks`, `ctx.tasks`, `TaskService` | `@relay-harness/rlh-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@relay-harness/rlh-tasks-local`, `LocalTaskService` | `@relay-harness/rlh-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@relay-harness/rlh-tool-tasks` | `@relay-harness/rlh-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@relay-harness/rlh-client-ui-task`, `client/ui-task/` | `@relay-harness/rlh-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@relay-harness/rlh-client-ui-slash`, `ui-slash/` | `@relay-harness/rlh-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@relay-harness/rlh-agent-tool-mode`, plugin `tool-mode` | `@relay-harness/rlh-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@relay-harness/rlh-permission`, `ctx.permission`, `PermissionService` | `@relay-harness/rlh-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@relay-harness/rlh-client-ui-permission` | `@relay-harness/rlh-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@relay-harness/rlh-type-meta` | `typert/protocol/`, `@relay-harness/rlh-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@relay-harness/rlh-tools` and `ctx.tools`. Keep `@relay-harness/rlh-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@relay-harness/rlh-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@relay-harness/rlh-workspace-context`, `context/workspace-context/` | `@relay-harness/rlh-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `rlh-session-telemetry` and `rlh-session-telemetry-otel`. |
...
| `session/user-id/`, `@relay-harness/rlh-user-id` | `identity/anonymous-user-id/`, `@relay-harness/rlh-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@relay-harness/rlh-environment` | `util/launch-environment/`, `@relay-harness/rlh-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
| Public `Environment*`, `createEnvironmentSnapshot`, `environmentOf`, `RLH_ENVIRONMENT_KEY` | `LaunchEnvironment*`, `createLaunchEnvironmentSnapshot`, `launchEnvironmentOf`, `RLH_LAUNCH_ENVIRONMENT_KEY` | The names state the snapshot's lifetime and purpose. |
...
| Former | Current | Reason |
|---|---|---|
| `@relay-harness/rlh-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@relay-harness/rlh-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@relay-harness/rlh-workflow` and workflow events and tools. |
| `@relay-harness/rlh-workflow-workerthread`, `WorkerWorkflowEngine` | `@relay-harness/rlh-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@relay-harness/rlh-goal-session`, `goal/goal-session/` | `@relay-harness/rlh-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@relay-harness/rlh-compact`, `ctx.compact`, `CompactService` | `@relay-harness/rlh-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@relay-harness/rlh-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@relay-harness/rlh-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@relay-harness/rlh-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@relay-harness/rlh-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@relay-harness/rlh-settings-local`, `SettingsLocal` | `@relay-harness/rlh-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@relay-harness/rlh-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@relay-harness/rlh-session-title-first-message-llm`, cadence `first-message` | `@relay-harness/rlh-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@relay-harness/rlh-session-title-all-messages-llm`, cadence `all-user-messages` | `@relay-harness/rlh-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@relay-harness/rlh-code-runtime-worker`, `WorkerCodeRuntime` | `@relay-harness/rlh-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@relay-harness/rlh-fs-policy` | `@relay-harness/rlh-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@relay-harness/rlh-skill-local`, `LocalSkillProvider`, provider id `local` | `@relay-harness/rlh-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@relay-harness/rlh-subagent-spawn`, `SpawnProvider` | `@relay-harness/rlh-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@relay-harness/rlh-subagent-fork`, `ForkProvider` | `@relay-harness/rlh-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@relay-harness/rlh-subagent-inprocess`, `subagent-inprocess/` | `@relay-harness/rlh-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
| Private `SdkProvider` in `rlh-subagent-rlh-sdk` | `SdkSubagentProvider` | The repeated package qualifier is intentional, and the class must say that it provides subagents through the SDK. |
...
| `@relay-harness/rlh-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@relay-harness/rlh-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@relay-harness/rlh-subagent-rlh-sdk`, its provider id `rlh-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@relay-harness/rlh-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@relay-harness/rlh-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@relay-harness/rlh-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@relay-harness/rlh-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@relay-harness/rlh-timeout-policy` | `@relay-harness/rlh-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@relay-harness/rlh-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@relay-harness/rlh-paths` | `util/home-paths/`, `@relay-harness/rlh-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@relay-harness/rlh-retention` | `util/output-retention/`, `@relay-harness/rlh-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@relay-harness/rlh-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@relay-harness/rlh-frontend-static` | `@relay-harness/rlh-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@relay-harness/rlh-jsonrpc-demo` | `@relay-harness/rlh-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@relay-harness/rlh-frontend` | `@relay-harness/rlh-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `rlh-base`, `rlh-web-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `@relay-harness/rlh-client-ui-model` | `@relay-harness/rlh-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@relay-harness/rlh-client-ui-models` | `@relay-harness/rlh-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@relay-harness/rlh-client-ui-plugin-config`, `client/ui-plugin-config/` | `@relay-harness/rlh-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@relay-harness/rlh-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@relay-harness/rlh-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@relay-harness/rlh-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@relay-harness/rlh-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@relay-harness/rlh-client-ui-question` | `@relay-harness/rlh-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@relay-harness/rlh-client-ui-command`, `ui-command/` | `@relay-harness/rlh-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@relay-harness/rlh-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@relay-harness/rlh-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@relay-harness/rlh-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@relay-harness/rlh-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `rlh-subagent-rlh-sdk` and provider id `rlh-sdk`; the repeated qualifier prevents ambiguity.
...
- Keep `@relay-harness/rlh-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@relay-harness/rlh-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `rlh-compact-basic` to `rlh-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
...
- The paired package-creation guide contains the role-word contract, `packages/AGENTS.md` links to it, the terminology table records the chosen words and `Typert` spelling, and root project prose calls the product Relay Harness rather than Relay Harness SDK.
