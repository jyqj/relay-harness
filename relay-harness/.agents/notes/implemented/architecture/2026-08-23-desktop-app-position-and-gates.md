...
[`apps/desktop/oxlintrc.json`](../../../../apps/desktop/oxlintrc.json) is a separate configuration rather than an exception carved into the root one, because the root config's `typeAware: true` is exactly what makes plain JavaScript unlintable there. Turning it on found 16 real problems, including a dead `asCwd` in `workspace-fs.js` and a `throw` inside a test's `finally` block that would have replaced any assertion failure in that test with a cleanup error. The filename omits the leading dot on purpose: oxlint treats any nested `.oxlintrc.json` as an extension of the run's root config, and this one sets `options.reportUnusedDisableDirectives`, which only a root config may carry. Under the discovered name the repository-wide `pnpm run lint` refuses to start.
