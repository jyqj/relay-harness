...
| Package | Role |
|---|---|
| `@relay-harness/rlh-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@relay-harness/rlh-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@relay-harness/rlh-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |

There is no dedicated model-facing Consumer package. The Consumer is the existing `ctx.tools` execution pipeline: `rlh-spill-policy` consumes final tool results through the `tools/post-execute` waterfall, and the model follows the backend-supplied retrieval hint for the returned locator.
...
With `rlh-spill-policy` configured, a large formatted fetch result is automatically retained and spilled. A deployment demonstrates the behavior by setting the provider resource cap higher than the policy cap:
...
- id: spill-local
  name: '@relay-harness/rlh-spill-local'

- id: spill-policy
  name: '@relay-harness/rlh-spill-policy'
  config:
...
- `@relay-harness/rlh-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@relay-harness/rlh-spill` owns saving final text and returning a locator plus retrieval hint.
- `@relay-harness/rlh-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
...
- ~~Cleanup and retention policy for old spill files, likely tied to session cleanup~~ — shipped as `SpillStore.disposeSession` plus the `rlh-spill-*` orphan-root startup sweep ([spill failure containment and reclamation](../bug-fix/2026-09-03-spill-failure-containment-and-reclamation.md)).

## Testing

- `rlh-spill` unit tests pin the seam contract: registration as `ctx.spillStore`, one-implementation-per-context, and disposal release.
- `rlh-spill-local` unit tests cover `saveText`, `encodeSegment` sanitization (separators/tilde/whole-segment dots/empty), the session-hash directory, owner-only permissions, distinct paths per save, the configured/private root, and a storage-failure rejection.
- `rlh-spill-policy` unit tests drive real tools through `ctx.tools.execute`: disabled-mode no-op, oversized-text replacement, small/non-text passthrough, `read` skip, best-effort fallback (save failure / no backend / no owner), and downstream-composition (bounding a replaced result, preserving `additionalContexts`).
- `rlh-tool-web` integration drives `web_fetch` through `ctx.tools.execute` with the real `spill-local` backend + policy, proving the model-facing text changes only by the deliberate spill notice while the spill file holds the full formatted result.
