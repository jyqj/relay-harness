...
`usePresence` 钉住第二帧进场、200ms 退场滞留，以及减少动效时的跳过。`FlipText` 钉住 400ms 退场滞留、超时卸掉，以及减少动效时的跳过。`motion.css` 钉住五种 recipe、只过渡 opacity/transform，以及被收成零的时长 token（含 `--rl-motion-duration-flip: 0s`）。壳的 `base.css` 钉住 `motion.css` 排在 `base.css` 之后。弹层消费方通过 role / `aria-hidden` 断言逻辑关闭，而不是立刻卸载。这些套件走 `test:gui` 和逐文件 100% 覆盖门禁，不是浏览器 golden 像素。
