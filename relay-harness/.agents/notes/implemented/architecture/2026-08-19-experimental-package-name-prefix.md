...
Every package directly below `packages/experimental/` uses the `@relay-harness/rlh-experimental-*` npm prefix. The workspace constraints gate discovers those manifests and rejects a missing prefix alongside the existing `private: true` and omitted-`publishConfig` requirements.

Agent Teams uses `@relay-harness/rlh-experimental-agent-team` from `packages/experimental/agent-team` and `@relay-harness/rlh-experimental-tool-agent-team` from `packages/experimental/tool-agent-team`. Package imports, Cordis configuration rows, generated catalogs, and repository metadata use those names without compatibility aliases.
