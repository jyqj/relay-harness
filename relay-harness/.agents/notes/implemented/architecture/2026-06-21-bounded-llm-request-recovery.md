...
The pi-ai OpenAI Completions implementation links that stable signal to OpenAI's Stream controller after response acquisition. OpenAI's request signal can end SDK iteration after headers without aborting the controller that owns the response body; the explicit link closes the socket on timeout or caller cancellation, and its listener is removed when iteration settles. The dependency patch is version-pinned in `patchedDependencies`, so a pi-ai upgrade must either carry the upstream fix or deliberately rebase this ownership rule.
