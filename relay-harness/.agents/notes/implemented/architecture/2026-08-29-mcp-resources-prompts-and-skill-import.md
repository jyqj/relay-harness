...
Every Tool result is admitted as one complete raw JSON value before output-schema validation, rich decoding, or rendering. `maxToolResultBytes` defaults to 4 MiB and counts `content` plus `structuredContent`, so text, base64 payloads, and structured data share one bound; rejection diagnostics report only tool identity and byte counts, never payload content. The MCP SDK has already parsed the JSON-RPC response at this point. A pre-parse wire-body limit remains an upstream transport-seam requirement rather than a claim of this bridge.
