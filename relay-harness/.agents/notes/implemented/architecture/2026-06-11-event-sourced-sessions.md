...
- Persistence stays a plugin concern; the in-memory store ships in rlh-session.
...
- Derivation cost grows with log length — compaction (rlh-compaction) is the intended mitigation, not log mutation.
