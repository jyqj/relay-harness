...
WebSocket carries only the host→browser downlink. All client→host unary calls and `respond` operations for server requests continue to use the existing `POST /api/*`; the WebSocket accepts no client application messages. `WebApiClient` therefore holds HTTP `fetch` for uplink and WebSocket for downlink on every origin, including phone remote through the HTTP relay. `ConnectionController` waits for the shell boot gate, then completes `host.describe` and awaits `onConnected` (including a returned promise) before opening those sockets so plugin script loads and unary list/history cannot starve behind two downlinks; see [remote SSE downlinks](../feature/2026-08-15-remote-sse-downlinks.md). The fixture and `InProcessApiClient(toFetchHandler(api))` continue to implement the same two-stream `IApiClient` abstraction. The in-process fetch carrier retains SSE encoding and decoding to verify the channel-independent protocol's isomorphism. Network GET requests to `/api/events.*` without `Accept: text/event-stream` still answer 426.
...
**Keep a network SSE fallback for loopback.** Two carriers would let the production desktop path silently fork and would leave the HTTP/1.1 connection-limit problem in a supported branch. Production pages ship WebSocket only. The `/api` HTTP bridge still streams SSE when a GET sends `Accept: text/event-stream`; the official page does not open that path. See [remote SSE downlinks](../feature/2026-08-15-remote-sse-downlinks.md).
