...
The executable lint contract drives a deliberately overlapping style violation through the repository runner and requires a successful exit plus exact final bytes. Output orchestration uses the runner's synchronous process seam: a deterministic successful first pass pins stdout/stderr preservation, while deterministic first- and second-pass failure results prove that obsolete captured diagnostics stay hidden and the final inherited diagnostic remains visible. These unit assertions do not couple their clock to real linter startup; separate process-bound executable probes retain real successful fix convergence. The same contract pins the complete Stylistic rule set, project-free TypeGraph fixture coverage, the package scripts, the staged hook command, the deleted formatter configuration, and the absence of direct ESLint parser and runner dependencies. Existing executable probes continue to cover the Stylistic and SonarJS compatibility plugins, project-free staged validation, and type-aware project discovery.
