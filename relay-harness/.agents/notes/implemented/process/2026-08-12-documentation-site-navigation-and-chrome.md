...
The wordmark is a second copy of a mark that also lives in `apps/web/public/favicon.svg` and `packages/client/ui-primitives/src/RelayMark.tsx`, each carrying its own presentation. A change to the wordmark reaches the documentation site only by updating this copy.
