...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `rlh-tool-bash` (`bash`), `rlh-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `rlh-tool-subagent` (`subagent`). Example-only tools are excluded.
...
- The generator imports and executes workspace packages (the first repo script to do so; the others only read text). It runs under `tsx` via the root `tsconfig` `paths` map, the same unbuilt-source path the demos and tests use, so it needs no build step. Every new `packages/<group>` directory must therefore join the generic source mapping; the clean-tree static gate is the proof that no workspace import fell back to `lib/`.
