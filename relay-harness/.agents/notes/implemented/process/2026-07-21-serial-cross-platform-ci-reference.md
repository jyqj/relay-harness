...
The [Sandbox](../../../../../.github/workflows/sandbox.yml) workflow supports both the standalone master reference and reusable invocation from pull-request CI. Its bwrap, Landlock x64/arm64, Seatbelt, and packed-install checks feed `all checks passed` through the caller job; ordinary unit coverage does not substitute for those kernel proofs.

Standalone master reference jobs remain diagnostic. Pull-request invocations of the Sandbox matrix are required. Performance is reported from completed job timestamps rather than encoded as a latency-target timeout.

The active serial references run on the self-hosted `vm-backup` (`serial / linux`) and `rlh-win-ci` (`serial / windows`) pools; the one remaining disabled hosted serial reference (`serial-macos`) uses `macos-latest`, and there is no standard-hosted `serial / linux` label. The required pull-request Windows job runs under Wine on `ubuntu-latest`, while the independent pull-request native job uses the hosted `rlh-windows-2025-16core` runner under normal operation and the self-hosted `[self-hosted, rlh-win-ci, windows]` pool under failover (see the [failover runbook](2026-07-26-ci-failover-runbook.md)), and participates in the required aggregate under the [dual Windows decision](2026-08-08-native-windows-pull-request-ci.md). Required pull-request jobs use portable standard capacity under the [required-CI decision](2026-07-23-portable-required-pull-request-ci.md). Higher-core hosted runners remain manual benchmarks because a correctness path must remain runnable without repository-external runner configuration.
...
- **Keep real-kernel Sandbox checks outside the pull-request verdict** - rejected because post-merge detection permits a supported confinement regression to merge. The reusable matrix participates in the required aggregate; the standalone master trigger retains its reference signal.
...
Real-kernel Sandbox and packed Landlock checks participate in the pull-request verdict through the reusable Sandbox workflow. Source and packaged Desktop smoke run on macOS and Windows, and native Windows joins Wine in the same aggregate. These checks increase runner cost but prevent source-only green results from substituting for supported-platform evidence.
