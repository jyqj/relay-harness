...
*Amendment — wave 2 landed on the rename branch as three commits.* Same reasoning as wave 1: what makes a bump revertable is the group boundary, not the PR boundary, and three commits give three boundaries. The tooling group moved `oxlint` 1.76.0 → 1.79.0, `playwright` 1.61.1 → 1.62.1, `pnpm` 11.8.0 → 11.22.0, `@yarnpkg/cli-dist` 4.17.1 → 4.18.0, and `eslint-plugin-sonarjs` 4.1.0 → 4.2.0. The model group moved `@anthropic-ai/sdk` 0.93.0 → 0.120.0, `@anthropic-ai/claude-agent-sdk` 0.3.220 → 0.3.240, `@openai/codex` 0.147.0 → 0.149.0, `@modelcontextprotocol/sdk` to ^1.30.0 with `@modelcontextprotocol/server-everything` 2026.7.4 → 2026.8.18, and `e2b` 2.29.1 → 2.45.0. The rendering group moved `shiki` and `@shikijs/langs` 4.3.1 → 4.4.3, `katex` 0.16.47 → 0.18.4, `mermaid` 11.16.0 → 11.17.0, `lightningcss` 1.32.0 → 1.33.0, `fast-check` 4.8.0 → 4.9.0, `smol-toml` 1.7.1 → 1.8.0, `tsx` 4.22.4 → 4.23.12, `use-sync-external-store` 1.2.0 → 1.6.0, and the four `@opentelemetry/*` logs packages 0.220.0 → 0.221.0.

The grouping predicted the shape of the work but not always its direction. The linter did change a rule, in the opposite direction from the one budgeted for: `oxlint` 1.79 stops reporting `no-control-regex` for escaped control characters, so the finding was an unused suppression under `reportUnusedDisableDirectives`, removed rather than added. The model group produced both real contract moves — Claude Code 2.1.240 refuses a disabled `ExitPlanMode` with new wording, and Codex 0.149 adds a twelfth `codexErrorInfo` variant, `misalignmentPolicyViolation`, which the wire now names instead of collapsing to `unknown`. The rendering group produced the snapshot churn it was meant to and nothing else: KaTeX 0.18 prefixes the generic class names inside its own HTML arm (`base` → `katex-base` and its kin), which no stylesheet or assertion of ours reads, so two math DOM-parity fixtures moved by exactly that rename.

Two of the named packages were dropped under the wave's own rule and remain outstanding. `knip` 6.16.1 → 6.32.2 has no green baseline to move from — `pnpm run knip` is already red at this branch's head because `apps/desktop` arrived without a knip workspace entry — and 6.32 replaces most of the finding set anyway, so reconciling it is a configuration repair, not a bump. `@earendil-works/pi-ai` 0.82.1 → 0.84.2 breaks two contracts the adapter asserts rather than a version string: a request carrying `maxTokens` no longer reaches the provider as `max_completion_tokens`, and an already-aborted caller signal finishes the stream as `error` instead of `aborted`, which would report a user cancellation as a failure. Each needs its own change and its own note.
