...
import { boot, resolveConfigPath } from '@relay-harness/rlh-app-boot'
...
process.env.RLH_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.RLH_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
