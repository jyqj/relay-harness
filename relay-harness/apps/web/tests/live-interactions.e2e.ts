...
// The model adapter is rlh-llm-replay with override sidecars: `hang` (+ a
...
import { deriveReplayScript, parseSessionLog } from '@relay-harness/rlh-llm-replay'
import type { ReplayOverrideDoc } from '@relay-harness/rlh-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(buildOverride?: (sidecarHome: string) => ReplayOverrideDoc): Promise<void> {
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'rlh-web-e2e-sidecar-'))
