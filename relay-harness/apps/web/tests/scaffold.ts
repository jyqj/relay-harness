...
// Boots the REAL web composition — the rlh-base and rlh-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $RLH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts rlh-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `rlh web`, all deliberate, all via include
...
import Include, { type PatchOptions } from '@relay-harness/cordis-plugin-include'
import Group from '@relay-harness/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@relay-harness/rlh-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
...
import type { ReplayHandle } from '@relay-harness/rlh-llm-replay'
import { installLlmReplay, parseSessionLog } from '@relay-harness/rlh-llm-replay'
import SessionStore, {
  packChunkRuns,
...
// } from '@relay-harness/rlh-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'Relay Harness 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 Relay Harness 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 RLH 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $RLH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.RLH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`RLH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the rlh-base and rlh-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($RLH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted rlh-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'rlh-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $RLH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.rlh document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.rlh-home')
  ...
  // session is created. Without this a developer's real ~/.rlh/skills silently
  // enters replay requests and goldens while CI sees none. `RLH_HOME` follows
  ...
  const skillRootEnvironment = {
    RLH_HOME: harnessHome,
    RLH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    RLH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    for (const skill of options.bundledSkills ?? []) {
      ...
      const skillDir = join(skillRootEnvironment.RLH_BUNDLED_SKILL_DIR, skill.name)
      await mkdir(skillDir, { recursive: true })
      await writeFile(join(skillDir, 'SKILL.md'), skill.markdown)
    }
    persistenceRoot = await mkdtemp(join(tmpdir(), 'rlh-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in rlh.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.rlh/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $RLH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.rlh-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.rlh, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        rlhHome: join(workspaceCwd, '.rlh-home'),
    ...
    // built-in endpoint but honors whatever RLH_TELEMETRY_OTLP_URL names in
    ...
    // (apps/web IS @relay-harness/rlh-web-frontend); native browser opening and the
    ...
    { id: 'settings', config: { rlhHome: harnessHome } },
    { id: 'credentials', config: { rlhHome: harnessHome } },
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@relay-harness/rlh-host-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@relay-harness/rlh-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    ctx.provide('rlhHomePath', rlhHomePath)
    ...
    // `@relay-harness/cordis-plugin-group` by name.
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run RLH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
