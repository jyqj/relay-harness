// Real-host smoke: spawn `rlh web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`rlh web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /rlh web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`rlh web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `rlh web` serves every production manifest plugin; until every UI
...
describe('rlh web keyless CLI smoke', () => {
  it('listens on 127.0.0.1 by default', async () => {
    requireDist()
    const sessionsDir = mkdtempSync(join(tmpdir(), 'rlh-web-keyless-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...
          RLH_HOME: join(sessionsDir, '.rlh'),
          RLH_AGENTS_HOME: join(sessionsDir, '.agents'),
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'rlh-web-workspace-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...
          RLH_HOME: join(workspace, '.rlh'),
          RLH_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'rlh-web-retry-'))
  ...
  it('RLH_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'rlh-web-code-mode-'))
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    requireDist()
    sessionsDir = mkdtempSync(join(tmpdir(), 'rlh-web-w5-'))
    const port = await probeFreePort()
    // tsx boot mirrors the runtime half of the root rlh script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $RLH_HOME with the
