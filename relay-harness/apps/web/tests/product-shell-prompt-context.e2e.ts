...
describe('web e2e: ordinary product shell and Prompt Enhancement', () => {
  ...
  it('runs draft → real Remote enhance → source explanation → diff → accept → undo without auto-submit, and cancels in flight', async () => {
    ...
    const mode = page.getByRole('dialog', { name: 'Settings' }).getByRole('switch', { name: 'Detailed view' })
    await mode.waitFor()
    ...
    expect(await mode.ariaSnapshot()).toMatchInlineSnapshot('"- switch "Detailed view" [checked]"')
    await mode.uncheck()
    await expect.poll(() => mode.isEnabled()).toBe(true)
    expect(scaffold.ctx.productMode.get().mode).toBe('simple')
    expect(await mode.ariaSnapshot()).toMatchInlineSnapshot('"- switch "Detailed view""')
    ...
    await proposal.getByText(ENHANCED, { exact: true }).first().waitFor({ timeout: 30_000 })
