...
import { injectBootManifest, orderByModuleGraph } from '@relay-harness/rlh-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@relay-harness/rlh-client-modules/client'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@relay-harness/rlh-app-boot')).href) as unknown as BootComposition
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares rlh.client without a ./client export`)
  }
  return resolve(dirname(packagePath), relative)
}

/** Derive the assembled browser graph from the same bundle patches and package declarations as `rlh web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.rlh?.client
...
interface FixtureWindow extends Window {
  __RLH_BOOT__?: { rev: string; entries: WebBootEntry[] }
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  win.__RLH_BOOT__ = { rev: 'fx', entries: PLUGINS.map(({ bundlePath: _bundlePath, ...plugin }) => plugin) }
  const html = injectBootManifest('<head></head>', win.__RLH_BOOT__)
  ...
  for (const id of ['@relay-harness/rlh-client-modules', '@relay-harness/rlh-client-runtime']) {
 ...
 * the snapshot gate's `RLH_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.RLH_SNAPSHOT === 'record' || process.env.RLH_SNAPSHOT === 'refresh'
