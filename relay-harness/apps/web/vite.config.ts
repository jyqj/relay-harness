...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__RLH_BOOT__. '
  + 'From a repository checkout, run `pnpm rlh web`; an installed package uses `rlh web`. '
  + 'For client-plugin HMR, run `pnpm rlh web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'RLH Local Build'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.RLH_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'rlh-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>RLH Local Build</title>', `<title>${title}</title>`)
    },
  }
}
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'rlh-reject-standalone-web-serve',
    config(_config, env) {
