...
2. **Respect the active permission preset after the copy.** The shipped default is `danger-full-access`, so the user preset root can be edited directly. If the deployment selects `workspace-write`, that root lies outside the session workspace: retry the denied command once with `sandbox_permissions` escalation and a short justification, and batch related writes. Reading any composition by absolute path and `copy()` itself need no escalation.
...
- id: delegation
  ...
  config:
    - id: workflow-worker-thread
      name: '@relay-harness/rlh-workflow-worker-thread'
      config:
        provider: spawn
    - id: tool-workflow
      name: '@relay-harness/rlh-tool-workflow'
```
