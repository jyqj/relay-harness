...
export default async function open(url) {
  ...
  console.log(`rlh browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__RLH_BOOT__'),
    apiKeyPresent: process.env.DEEPSEEK_API_KEY !== undefined,
    rlhHomePresent: process.env.RLH_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.rlh-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
