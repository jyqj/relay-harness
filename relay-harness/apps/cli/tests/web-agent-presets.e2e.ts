...
import { boot, healProfilesModuleFallback, loadOverlayPatches, loadProfile } from '@relay-harness/rlh-app-boot'
...
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@relay-harness/rlh-agent-presets'
import { applyChildComposition, childSessionMeta } from '@relay-harness/rlh-subagent'
import {
  CallId,
  LlmAdapter,
  createUserMessage,
...
import type {} from '@relay-harness/rlh-compaction-basic'
...
/** The shipped Web surface: the rlh-base and rlh-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    // The settings row defaults to `$RLH_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $RLH_HOME. Unpinned, this
    // file writes the developer's own `~/.rlh/storages/` — and then reads it
    ...
    // developer's own `~/.rlh/.preset` cannot change this test's outcome.
  ...
  // Product Bundles are installed into the Profile, not the rlh app. Model
  ...
  let bundlePatches: PatchOptions[] = [
    ...loadOverlayPatches('rlh-test', BASE_PATCH),
    ...loadOverlayPatches('rlh-test', WEB_PATCH),
  ]
  if (profileBundles !== undefined) {
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      private: true,
      dependencies: Object.fromEntries(profileBundles.map(name => [name, 'workspace:*'])),
      rlh: { profile: { bundles: profileBundles } },
    }, null, 2) + '\n')
    const profile = loadProfile('rlh-test', 'spec', INSTALL_ANCHOR, home, { userLayer: false })
    bundlePatches = profile.layers.flatMap(layer => layer.patches)
  }
  const rootConfig = join(profileDir, 'cordis.yml')
  await writeFile(rootConfig, '[]\n')
  return await boot('rlh-test', rootConfig, [...bundlePatches, ...overrides], (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'rlh-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('logs the exact proactive memory suffix sent by the standard preset', async () => {
    ...
    const workspace = await mkdtemp(join(tmpdir(), 'rlh-memory-snapshot-'))
    const memory = await ctx.longTermMemory.remember({
      scope: { workspaceId: workspace, userId, agentId: 'relay-harness' },
      kind: 'constraint',
  ...
  it('extracts a standard turn durably and recalls it in a fresh composed session', async () => {
    const home = await mkdtemp(join(tmpdir(), 'rlh-memory-extraction-composition-'))
    const settingsFile = join(home, 'settings.yaml')
    const workspace = await mkdtemp(join(tmpdir(), 'rlh-memory-extraction-workspace-'))
    const userId = process.env.USER || process.env.USERNAME || 'local'
    await writeFile(settingsFile, '{}\n')
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'rlh-preset-skill-proj-'))
    await mkdir(join(proj, '.rlh', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.rlh', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient RLH home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['rlh-badge'])
      ...
      expect(scoped).toContain('rlh-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by rlh')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('rlh-badge')
...
describe('product Bundle and user-preset intersection', () => {
  ...
  async function bootProducts(installed: readonly Product[]): Promise<Context> {
    const root = await mkdtemp(join(tmpdir(), 'rlh-product-presets-'))
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<rlhHome>/.agent-presets` itself. `$RLH_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'rlh-preset-derived-'))
    ...
    await writeFile(
      ...
      '- id: tool-todo\n  name: \'@relay-harness/rlh-tool-todo\'\n  config:\n    allowParallelInProgress: true\n',
    )
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'rlh-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'rlh-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'rlh-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
