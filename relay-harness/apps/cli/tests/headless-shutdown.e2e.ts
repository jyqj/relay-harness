...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@relay-harness/rlh-loader-smoke'

const rlhBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"rlh-test: never-dispose ready", b"rlh-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'rlh-headless-shutdown-'))
  try {
    const home = join(cwd, '.rlh')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'rlh-profile-headless',
      private: true,
      dependencies: {},
      rlh: { profile: { bundles: ['@relay-harness/rlh-base', '@relay-harness/rlh-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        RLH_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`rlh headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`rlh headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('rlh: observing at ')
    expect(output).toContain('rlh-test: never-dispose ready')
    expect(output).toContain('rlh-test: never-dispose started')
