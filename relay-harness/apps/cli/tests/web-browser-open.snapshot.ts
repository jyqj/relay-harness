/** Assembled keyless snapshot for the default `rlh web` browser handoff. */
...
const openingMessage = 'rlh web: opening the default browser; pass --no-open to disable'
const tempRoots: string[] = []
const builtArtifactsExist = existsSync(builtBin) && existsSync(frontendIndex)

if (process.env.RLH_EXAMPLE_MODE === 'lib' && !builtArtifactsExist) {
  throw new Error('rlh web browser-open snapshot requires built CLI and Web artifacts in lib mode')
}
...
describe.skipIf(!builtArtifactsExist)('rlh web browser-open assembled snapshot', () => {
  it('hands the reachable page to the default browser after the shipped tree settles', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-web-browser-open-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      ...
      env: {
        ...
        RLH_AGENTS_HOME: join(root, '.agents'),
        RLH_HOME: join(root, '.rlh'),
    ...
    const readyUrl = /rlh web: (http:\/\/[^\s]+)/u.exec(result.stdout)?.[1]
    const openLine = result.stdout.split('\n').find(line => line.startsWith('rlh browser-open: '))
    const opening = result.stdout.includes(openingMessage)
    if (readyUrl === undefined || openLine === undefined || !opening) {
      throw new Error(`rlh web browser-open evidence missing\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    const opened = JSON.parse(openLine.slice('rlh browser-open: '.length)) as BrowserOpenRecord
    ...
    expect({
      ...
      rlhHomePresent: opened.rlhHomePresent,
  ...
  it('prints the launcher reason and manual URL after the Web app is ready', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-web-browser-open-failure-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      '--import', openerHook,
      builtBin,
      ...
      env: {
        ...
        DEEPSEEK_API_KEY: 'keyless-browser-open-no-call',
        RLH_AGENTS_HOME: join(root, '.agents'),
        RLH_BROWSER_OPEN_TEST_EXIT_ON_FAILURE: '1',
        RLH_HOME: join(root, '.rlh'),
        RLH_TELEMETRY_DISABLED: '1',
        NODE_NO_WARNINGS: '1',
        SSH_CONNECTION: '',
    ...
    expect({
      diagnostic,
      exitCode: result.exitCode,
      opened: result.stdout.includes('rlh browser-open: '),
  ...
  it('prints the host URL without launching a browser in a VS Code Remote SSH session', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-web-browser-open-ssh-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      '--import', openerHook,
      builtBin,
      ...
      env: {
        ...
        DEEPSEEK_API_KEY: 'keyless-browser-open-no-call',
        RLH_AGENTS_HOME: join(root, '.agents'),
        RLH_BROWSER_OPEN_TEST_EXIT_ON_READY: '1',
        RLH_HOME: join(root, '.rlh'),
        RLH_TELEMETRY_DISABLED: '1',
        NODE_NO_WARNINGS: '1',
  ...
  it('rejects a project browser command before starting the Web app', async () => {
    const root = mkdtempSync(join(tmpdir(), 'rlh-web-browser-open-env-snapshot-'))
    tempRoots.push(root)
    ...
    const diagnostic = result.stderr.split(/\r?\n/u)
      .find(line => line.startsWith('Error: rlh: '))
      ?.replace(/^Error: rlh: .*[/\\]\.env/u, 'rlh: {{root}}/.env')
    ...
    expect({
      ...
      opening: result.stdout.includes(openingMessage),
      opened: result.stdout.includes('rlh browser-open: '),
      ready: result.stdout.includes('rlh web: '),
    }).toMatchInlineSnapshot(`
      {
        "diagnostic": "rlh: {{root}}/.env sets "BROWSER", which only the launching environment may set (it decides how this process starts, where its code and instructions load from, or how it reaches the network); export BROWSER instead of putting it in a .env file",
        "exitCode": 1,
        "opened": false,
        "opening": false,
        "ready": false,
