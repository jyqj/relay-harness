...
New sessions default to the `danger-full-access` permission preset. Bash and filesystem mutations run without the DSH file sandbox or approval prompts. `DSH_PERMISSION_MODE` can select a narrower process fallback. Stored General-settings permissions affect later Web sessions, not an already-open one.
