/**
 * Commander adapter for the `rlh` command line.
 *
 ...
 * `@relay-harness/rlh-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `rlh --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `rlh --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @relay-harness/rlh/args
 */
...
const HELP_EXAMPLES = `
Examples:
  rlh --profile web                          boot the web profile (same as: rlh web)
  rlh --profile headless "run the tests"     answer one task, print the result, and exit
  rlh --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  rlh --profile tui --resume <session>       arguments after the launcher flags reach the app
  rlh --profile web --help                   the web app's own flags and help
  rlh plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): RlhInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseRlhArgs(argv: readonly string[], version: string): RlhInvocation {
  let resolved: RlhInvocation | undefined
  ...
  program
    ...
    .description('rlh: boot a Relay Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `rlh -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: rlh --profile <name> --help)')
    .option('--profile <name>', 'the profile under $RLH_HOME/profiles to boot')
    ...
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      ...
      // `rlh -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: rlh web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('rlh: no invocation resolved')
  return resolved
}
