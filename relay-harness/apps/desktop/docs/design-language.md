# Relay Harness Web UI 设计语言

中文 · [English](design-language.en.md)

本产品的视觉语言就是官方 `rlh web`。桌面壳、关闭遮罩、标题栏注入、右边栏、手机远程打开的官方页、以及任何新增前端，都必须看起来像同一套界面，不得另起一套皮肤。启动页的仪器风只活在 [`src/renderer/boot.html`](../src/renderer/boot.html)，见 [桌面启动页](#桌面启动页)，不得扩散。

改 UI / 布局 / 前端之前先读本文。工程落地细则（CSS Modules、token 分层、动效 recipe）以官方文档为准，本文不重复那份清单：

- Token 源码：[design-platform.css](../vendor/relay-harness/packages/client/ui-theme/src/styles/design-platform.css)、[base.css](../vendor/relay-harness/packages/client/ui-theme/src/styles/base.css)、[gradient-shadow-text.css](../vendor/relay-harness/packages/client/ui-theme/src/styles/gradient-shadow-text.css)、[motion.css](../vendor/relay-harness/packages/client/ui-theme/src/styles/motion.css)
- 控件原语：`vendor/relay-harness/packages/client/ui-primitives/`（`Button` / `Input` / `Menu` / `Modal` / `Tooltip` / 图标）
- 工程规则：[web-styling.md](../vendor/relay-harness/docs/web-styling.md)
...
| 角色 | Token / 几何 |
| --- | --- |
| 画布 | `--rlw-alias-bg-base` |
| 侧栏 | `--rlw-specific-sidebar-fill` |
| 抬起层 | `--rlw-alias-bg-layer-1`～`3` |
| 主文字 / 次文字 / 说明 | `--rlw-alias-label-primary` / `secondary` / `tertiary` |
| 用户气泡 | `--rlw-specific-bubble` |
| 选中行 | `--rlw-specific-sidebar-nav-item-active`（强调用 `*-accent`） |
| 字体栈 | `--rlw-font-family`（系统 UI + 苹方 / 雅黑）；代码 `--rl-font-family-code` |
...
- **无法 import 主题包的壳层**（远程登录页、手机 Web SPA）：复用同一套语义色和几何。手机 SPA 把 `--rlw-alias-*` 抄进 `mobile/web/tokens.css`，不挂官方 CSS Modules，也不把启动页 `--boot-*` 带过去。禁止再开 `--bg` / `--accent` 平行色板。
...
产品页使用官方 token 与 `ui-primitives`。手机远程 Web（`mobile/web`）是文档化例外：抄 `--rlw-alias-*`，不嵌入官方插件树，不用启动页仪器画布。设置里的插件市场是预置 `rlhmarket` 插件的 `settings.section`（id `market`），沿用该插件自己的界面。不要再开 `--bg` / `--accent` 平行色板。桌面启动页是文档化的仪器画布例外，见 [桌面启动页](#桌面启动页)，不得扩散。
