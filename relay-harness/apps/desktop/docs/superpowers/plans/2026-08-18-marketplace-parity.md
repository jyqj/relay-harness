...
工作目录：本 git worktree 根（`feat/marketplace-parity`）。Windows 上用 `node --test`；vendor 客户端测试在 `vendor/relay-harness` 下跑 `pnpm exec vitest run packages/client/ui-settings-plugin-inventory`。

TDD 强制：每个任务先写失败测试并跑红，再写生产代码。不要先改实现再补测试。

## Global Constraints

- 不要预装或 vendor `rlhmarket`。不要抄 `src/renderer/marketplace/marketplace.css` 或 rlh-market 皮肤。
- 前端只用 `ui-primitives`（Input / Button / Modal / Menu）和 `--rlw-alias-*`。产品文案中文，注释英文。
...
- add 成功后检查 `$RLH_HOME/profiles/web/node_modules/<name>`：有 `package.json`，且含 `rlh.bundle.patch` 或可解析的 `rlh.client` / 包 `main` 入口文件存在。否则 `remove` 并失败。`ok: false` 时 IPC 不得 `startHarness()`。
...
- spec 字段表写「有 npm 用 npm 包名，否则解析 github」过粗，曾裁定 last-token。**现已改回与 rlh-market `installTargetFor` 对齐**：合法 npm 字段，否则 GitHub URL → `github:` / `#path:`；last-token 只作允许规格的回退。这样 Release `.tgz` `install` 命令仍走 github，不会把 tarball 送进 CLI。
...
| `packageName` | `npm` 或 `''` |
| `homepage` | `url` |
| `installSpec` | 与 rlh-market `installTargetFor` 相同：合法 npm 字段，否则 GitHub URL → `github:` / `#path:`；last-token 只在已是允许规格时回退 |
...
**不要**改 IPC/preload/UI。`installPlugin(spec)` 保持 github-only（继续 `isValidGithubSpec`）。不要改 `src/host/install-rlh-plugin-client.js`。
...
8. add 成功后：在 `$RLH_HOME/profiles/web/node_modules/<packageName>`（scoped 用 `@scope/name` 路径）检查可加载入口。包名：npm spec 用 spec；github 用安装后 `package.json` 的 `name`，若读不到则失败并 remove。入口：`package.json` 存在，且 `rlh.bundle.patch` 为真，或 `rlh.client` 可解析到现存文件，或 `main`/`exports` 默认入口文件存在。不满足则 `runPlugin(['remove', name])` 并 `{ ok: false, error }` 说明不是可加载 rlh 插件。失败正文不要把 pnpm ndjson 进度当人话（沿用现有 `error` 字段策略：allowBuilds 提示或「安装失败」）。
...
- preload：删除 `marketplace` 角色——`SHELL_ROLES` 去掉 `marketplace`，删除 `marketplaceApi`，`shellRole` 对 `--rlhd-shell-role=marketplace` 返回 null。更新 `shell-api.test.js`：断言 marketplace 角色不再暴露 API；harness 有 `installMarketplacePlugin` 且无 `seedInstallDraft`。
...
- `openMarketplace()`：`showMain()`；若 Harness 已就绪，打开设置 → plugins → 点击 `[data-rlh-settings-plugin-tab="marketplace"]`（保留现有 executeJavaScript 轮询）。**若未就绪：只显示主窗，记下 pending 标志；在 Harness 第一次就绪（`showHarness` 成功加载或现有 `isHarnessLoaded` 变真的那条路径）后执行一次同样的设置跳转，然后清标志。** 不要打开第二个窗口。
...
改 vendor 包 `vendor/relay-harness/packages/client/ui-settings-plugin-inventory/`：
...
对标 [McpSection.tsx](../../../vendor/relay-harness/packages/client/ui-settings-mcp/src/client/McpSection.tsx) 的 Button/Modal/Input/Menu 用法。
...
测（在 `vendor/relay-harness`）：`pnpm exec vitest run packages/client/ui-settings-plugin-inventory`。不要跑整份 `test:gui`。
...
- `vendor/relay-harness/packages/client/ui-settings-plugin-inventory/README.md` 与 `README.zh.md`：Marketplace 是桌面设置页一键安装，不再预填 composer。
- Agent Note 写在 vendor 树，implemented/feature，日期 2026-08-18，例如 `2026-08-18-desktop-marketplace-curated-catalog.md` + `.zh.md` + sidecar。遵循 vendor Agent Note 格式（Problem / Decision / Alternatives considered / Consequences；中英结构对齐；记录拒绝 rlhmarket 预装、拒绝独立窗口、拒绝用 `isValidGithubSpec` 套 `#path:`）。按仓库现有 sidecar 流程生成（看邻近日记的 `.json` sidecar 或 `pnpm run` 相关 verify）。更新 [2026-08-16-surfaces-terminal-work-loops.md](../../../vendor/relay-harness/.agents/notes/implemented/feature/2026-08-16-surfaces-terminal-work-loops.md) 里「marketplace file: pinned to marketplace/index.html」的过时事实（窗口已删）。
