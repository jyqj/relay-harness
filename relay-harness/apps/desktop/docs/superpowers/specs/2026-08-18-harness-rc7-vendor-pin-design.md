...
Reference for how Relay-Harness-Desktop records the official Relay Harness baseline, merges a new official commit into `vendor/relay-harness/`, and keeps desktop forks, npx, node-pty, and pack assertions on one version.
...
A later successful apply of `rlh-v0.1.0-rc.7` (`99f6f02fecdb7dff40c3fbc9470f5907c29f74ca`) rewrites the same four keys; the next sync uses `pin.sha` as the official merge base.
...
5. Read `HEAD:vendor/relay-harness` and create an unreferenced synthetic ours commit with `git commit-tree <oursTree> -p <pin.sha>`.
6. Add a detached worktree at `<git-dir>/rlh-harness-sync-worktree` on that synthetic commit.
7. In that worktree run `git merge --no-commit --no-ff <targetSha>` so Git three-way-merges against the real official parent.
8. On conflict, write `.git/rlh-harness-sync.json` with `worktree`, `backupRef`, `targetRef`, `targetSha`, `syntheticOurs`, and `pinBefore`, print the worktree path, and leave the main index, worktree, and pin unchanged.
...
10. Build a candidate root tree with a temporary `GIT_INDEX_FILE`: read HEAD, remove the cached `vendor/relay-harness` prefix, then `git read-tree --prefix=vendor/relay-harness/ <mergedTree>`.
11. `git diff-tree --name-only HEAD <candidate>` must contain only paths under `vendor/relay-harness/`.
12. Re-check that the main worktree is still clean, then `git checkout <candidate> -- vendor/relay-harness`.
...
Desktop customization stays inside `vendor/relay-harness/`.
...
- `packages/bundle/web-app/cordis.patch.yml` keeps `directory-picker` pointed at `@relay-harness/rlh-host-directory-picker-browse`.
...
`src/main/rlh.js` `buildLaunch` uses `@relay-harness/rlh@${pin.npm}` on the npx path.
...
Vendor checks are `pnpm run constraints`, `pnpm run test:gui`, `RLH_SNAPSHOT=replay pnpm run test:web`, `pnpm run verify-client-catalog`, `pnpm run gen-third-party-notices --check`, and `pnpm run doc-sync`.

After those pass, delete `refs/backup/harness-pre-sync`, the temp worktree, and `.git/rlh-harness-sync.json`.
...
It does not run user-invoked `rlh-translate-docs`.
