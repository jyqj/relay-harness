...
#${OVERLAY_ID} .rlhd-shell-closing-card {
  display: flex;
...
#${OVERLAY_ID} .rlhd-shell-closing-spinner {
  width: 36px;
  height: 36px;
  border: 2px solid ${theme.line};
  border-top-color: ${theme.accent};
  border-radius: 50%;
  animation: rlhd-shell-closing-spin 0.85s linear infinite;
}
#${OVERLAY_ID} .rlhd-shell-closing-title {
  margin: 8px 0 0;
  font-size: 16px;
  font-weight: 500;
  line-height: 24px;
}
#${OVERLAY_ID} .rlhd-shell-closing-detail {
  margin: 0;
  font-size: 12px;
  line-height: 18px;
  color: ${theme.muted};
}
@keyframes rlhd-shell-closing-spin {
...
function overlayScript(copy) {
  return `(() => {
    ...
    if (!root.id) {
      ...
      card.className = 'rlhd-shell-closing-card';
      const spinner = document.createElement('div');
      spinner.className = 'rlhd-shell-closing-spinner';
      spinner.setAttribute('aria-hidden', 'true');
      const title = document.createElement('p');
      title.id = id + '-title';
      title.className = 'rlhd-shell-closing-title';
      title.textContent = ${JSON.stringify(copy.title)};
      const detail = document.createElement('p');
      detail.className = 'rlhd-shell-closing-detail';
      detail.textContent = ${JSON.stringify(copy.detail)};
      card.append(spinner, title, detail);
      root.append(card);
      (document.body || document.documentElement).append(root);
    }
    const bg = pick('--rlw-alias-bg-base', '--bg');
    const fg = pick('--rlw-alias-label-primary', '--fg');
    const muted = pick('--rlw-alias-label-tertiary', '--muted');
    const accent = pick('--rlw-alias-state-business-primary', '--rlw-alias-button-info-fill', '--accent');
    const field = pick('--rlw-alias-bg-layer-2', '--rlw-alias-bg-module-platform', '--field');
    const line = pick('--rlw-alias-border-l2', '--line');
    ...
    if (muted) root.querySelector('.rlhd-shell-closing-detail').style.color = muted;
    const spinner = root.querySelector('.rlhd-shell-closing-spinner');
