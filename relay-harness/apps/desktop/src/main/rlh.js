...
function resolveRlhBin(config) {
  const source = sourceHarnessStatus();
  if (source.present) {
    return source.bin;
  }
  if (config.rlhBin && fs.existsSync(config.rlhBin)) {
    return config.rlhBin;
  }
  const npmGlobal = process.env.APPDATA
    ? path.join(process.env.APPDATA, 'npm', process.platform === 'win32' ? 'rlh.cmd' : 'rlh')
    : null;
  const fromPath = whichAll(process.platform === 'win32' ? 'rlh.cmd' : 'rlh')[0]
    || whichAll('rlh')[0];
...
function pidFilePath() {
  try {
    return path.join(path.dirname(configPath()), 'rlhd-web.pid');
  } catch {
    return null;
  }
}
...
class RlhManager extends EventEmitter {
  /**
  ...
  buildLaunch(config) {
    ...
    // Electron owns the page in BrowserView. Official rlh web opens the OS
    ...
    if (source.present) {
      if (!source.installed || !source.built) {
        throw new Error(
          `Monorepo Harness 源码还没安装依赖或构建。请在仓库根目录运行 pnpm install && pnpm run build`,
        );
      }
      if (!nodeBin) {
    ...
    const rlhBin = this._deps.resolveRlhBin(config);
    if (rlhBin) {
      return {
        command: rlhBin,
        args,
    ...
    return {
      command: npxBin,
      args: ['--yes', `@relay-harness/rlh@${pin.npm}`, ...args],
      nodeBin,
      kind: 'npx',
      host,
      port,
  ...
  attachOutput(child) {
    const onChunk = (chunk, source) => {
      ...
      for (const line of text.split(/\r?\n/)) {
        ...
        const match = line.match(/rlh web:\s*(https?:\/\/(?:127\.0\.0\.1|localhost):\d+)/i);
        if (match) {
    ...
    child.stdout?.on('data', (chunk) => onChunk(chunk, 'rlh'));
    child.stderr?.on('data', (chunk) => onChunk(chunk, 'rlh'));
  }
  ...
  async _start(options = {}) {
    ...
    if (this.child) {
      ...
      this.log(`清理残留 rlh 进程（pid ${leftover}）`);
      this.child = null;
    ...
    } else if (launch.kind === 'rlh') {
      this.log(`启动 ${launch.command}`);
    } else {
      this.log('通过 npx 启动 @relay-harness/rlh（首次会下载运行时）');
    }
  ...
  _onChildExit(gen, child, code, signal, readiness) {
    ...
    const message = `rlh 进程结束（code ${code ?? 'null'}, signal ${signal || 'none'}）`;
    const phase = this.state === 'ready' ? 'runtime' : 'startup';
  ...
  /**
   * 就绪轮询：每一轮都校验 generation 与 child 身份；被 stop 取消时抛 RLH_CANCELLED。
  ...
  async waitUntilReady(baseUrl, child, gen) {
    ...
    while (true) {
      ...
      if (child.exitCode !== null) {
        throw new Error(this.error || `rlh 已退出（code ${child.exitCode}）`);
      }
      ...
      if (Date.now() - started >= READY_TIMEOUT_MS) {
        throw new Error('启动超时。若本机已安装 rlh，检查端口占用；否则确认 npx 能运行 @relay-harness/rlh。');
      }
