...
function makeHarness(overrides = {}) {
  ...
  let reachable = false;
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 'rlh-test-'));
  const deps = {
    ...
    spawnHarness: () => {
      ...
      if (reachable && overrides.announceReady !== false) {
        queueMicrotask(() => child.stdout.emit('data', Buffer.from(`rlh web: ${EXPECTED_URL}\n`)));
      }
      return child;
    },
  ...
  return {
    ...
    setReachable: (value, announceReady = overrides.announceReady !== false) => {
      ...
      if (value && announceReady) {
        ...
        if (child) child.stdout.emit('data', Buffer.from(`rlh web: ${EXPECTED_URL}\n`));
      }
    },
...
test('HTTP 探活单独不能标记 ready，必须等 rlh web 行', async (t) => {
  ...
  h.lastChild().stdout.emit('data', Buffer.from(`rlh web: ${EXPECTED_URL}\n`));
...
test('stop 取消 in-flight start：最后 idle，绝不被旧 catch 改 error，可重新启动', async (t) => {
  ...
  const result = await outcome;
  assert.equal(result.ok, false);
  assert.equal(result.error && result.error.code, 'RLH_CANCELLED');
...
test('stop during runtime preparation cancels the stale generation before spawn', async (t) => {
  ...
  const result = await outcome;
  assert.equal(result.ok, false);
  assert.equal(result.error?.code, 'RLH_CANCELLED');
...
test('start 与 stop 重叠不死锁：start 等待 stop 完成后新起一代', async (t) => {
  ...
  assert.equal(r1.reason && r1.reason.code, 'RLH_CANCELLED');
...
test('npx fallback pins @relay-harness/rlh to pin.npm', () => {
...
test('every launch kind passes --no-open so rlh web does not open the OS browser', () => {
  const source = new RlhManager({
    sourceHarnessStatus: () => ({
      present: true,
      installed: true,
      built: true,
  ...
  const rlhLaunch = rlh.buildLaunch({ host: '127.0.0.1', port: 3080 });
  assert.equal(rlhLaunch.kind, 'rlh');
  assert.equal(rlhLaunch.args.includes('--no-open'), true);

  const pin = readPin(path.join(__dirname, '..', '..'));
  const npx = new RlhManager({
    sourceHarnessStatus: () => ({ present: false }),
    resolveRlhBin: () => null,
    resolveNpx: () => 'npx',
    resolveNodeBin: () => process.execPath,
