...
const catalogUrl = pathToFileURL(
  path.join(__dirname, '..', '..', 'vendor', 'rlhbot', 'lib', 'catalog.js'),
).href;
...
test('personaText injects only 1:1 bot descriptions for the live session', async () => {
  ...
  assert.equal(personaText([], 'orphan', 'rlhbot-room'), '');
});
...
test('childPersonaText names the character, other members, and forbids the harness identity', async () => {
  ...
  assert.match(text, /Relay Harness/);
...
test('sessionCreatePayload stamps origin and prefers workspace over scratch cwd', async () => {
  ...
  assert.deepEqual(sessionCreatePayload(), { origin: 'rlhbot' });
...
test('client registers the bots tab, overlay, and ask_participant toolview', () => {
  const src = require('fs').readFileSync(
    path.join(__dirname, '..', '..', 'vendor', 'rlhbot', 'client', 'client.js'),
    'utf8',
  );
  ...
  assert.match(src, /rlhbot-modal/);
  assert.match(src, /rlhbot-blob-eye-pupil/);
  assert.match(src, /--rlhbot-blob-ink/);
  assert.equal(/:root\s*\{[^}]*--rlhbot-blob-ink/.test(src), false);
  assert.equal(src.includes('transition: d'), false);
  assert.match(src, /requestAnimationFrame/);
  assert.match(src, /rlhbot-blob-blink/);
  ...
  assert.match(src, /rlhbot-bubble-omit/);
  assert.match(src, /rlhbot-room/);
  assert.match(src, /conversation\.chat\.empty/);
  assert.match(src, /emptyRoster/);
  assert.match(src, /rlhbot-roster/);
...
test('profile host apply short-circuits room llm/stream and registers ask_participant', () => {
  const src = require('fs').readFileSync(
    path.join(__dirname, '..', '..', 'vendor', 'rlhbot', 'lib', 'index.js'),
    'utf8',
  );
  assert.match(src, /export const name = 'rlh-bot'/);
  ...
  assert.match(src, /rlhbot:member/);
...
test('room preset tool registers ask_participant as a one-shot child', () => {
  const src = require('fs').readFileSync(
    path.join(__dirname, '..', '..', 'vendor', 'rlhbot', 'lib', 'ask-participant.js'),
    'utf8',
  );
...
test('room preset mounts ask_participant without a dispatcher persona', () => {
  const yml = require('fs').readFileSync(
    path.join(__dirname, '..', '..', 'vendor', 'rlhbot', 'presets', 'rlhbot-room', 'agent.cordis.yml'),
    'utf8',
  );
  assert.match(yml, /rlhbot\/ask-participant/);
...
test('room preset metadata is not a session-picker group name', () => {
  const yml = require('fs').readFileSync(
    path.join(__dirname, '..', '..', 'vendor', 'rlhbot', 'presets', 'rlhbot-room', 'preset.yml'),
    'utf8',
  );
  assert.equal(yml.includes('群聊'), false);
  assert.match(yml, /rlhbot-room/);
});
