...
const { executeInstallRlhPlugin, renderInstall } = createRequire(import.meta.url)('./install-rlh-plugin-client.js')

export const name = 'rlhd-desktop-plugin-install'
 ...
 * copy. The plugin file lives under $RLH_HOME; a static
 * `import '@relay-harness/rlh-tools'` from that path aborts the whole Host when
...
async function loadDefineTool() {
  ...
  throw lastError || new Error('Cannot resolve @relay-harness/rlh-tools')
}
...
export async function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      return executeInstallRlhPlugin(control, args.spec, args.allowBuilds ?? [])
    },
