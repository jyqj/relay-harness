...
/**
 * HTTP client and result helpers for install_rlh_plugin.
 * Kept free of Cordis and rlh-tools so node:test can load it.
 */
...
function renderInstall(value) {
  if (value.needsAllowBuilds) {
    ...
    return `pnpm blocked prepare scripts for ${value.spec}. Ask the user, then retry install_rlh_plugin with allowBuilds: ${keys}.`;
  }
  if (value.ok) {
 ...
 * @param url - loopback base URL from RLH_DESKTOP_INSTALL_URL.
 * @param token - bearer token from RLH_DESKTOP_INSTALL_TOKEN.
 ...
 */
async function executeInstallRlhPlugin(control, spec, allowBuilds = [], request = requestDesktopInstall) {
