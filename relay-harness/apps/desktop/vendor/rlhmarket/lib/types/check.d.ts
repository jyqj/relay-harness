...
/** One layer of the bundle stack, in `rlh.profile.bundles` order. */
export interface BundleLayer {
    name: string;
    /** Dependency spec from the profile package.json (npm range, git, link:…). */
    source: string;
    /** 'official' = in-box rlh bundle; 'community' = everything else. */
    ...
    /** Why this layer cannot load at boot (missing dir / no rlh.bundle / …). */
    error: string | null;
...
/** A patch row that matched nothing at boot (rlh warns and skips it). */
...
export interface CheckOptions {
    /** RLH host installation dir; auto-detected from process.argv when omitted (tests inject it). */
    rlhInstallDir?: string;
    /** Harness home for the home-level patch layer; defaults to $RLH_HOME or ~/.rlh. */
    homeDir?: string;
}
...
export declare function corePackageNames(rlhInstallDir: string | null): Set<string>;
/**
 ...
 */
export declare function findRlhInstallDir(entry?: string): string | null;
 ...
 */
export declare function buildBundleLayers(profileDirectory: string, bundleNames: string[], specs: Record<string, string>, rlhInstallDir: string | null): {
    bundles: BundleLayer[];
    layers: LayerInput[];
};
/**
