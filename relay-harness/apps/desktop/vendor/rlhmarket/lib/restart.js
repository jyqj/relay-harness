/**
 * Self-restart: relaunch the exact RLH invocation that booted this host so
...
import { rlhArgv } from './rlh-cli.js';
 ...
 * RLH sandbox tool runners) pops a visible node window. Wrapping the launch
...
export function restartHelperSource(spawned, launch, logs, port) {
    return [
        ...
        'const note = (line) => { try { fs.appendFileSync(logErr, `[rlh-market] ${line}\n`) } catch {} }',
...
/**
 * Relaunch this exact RLH entry after a detached handoff, then stop this
...
export function scheduleRestart(port = null) {
    ...
    const logOut = join(tmpdir(), `rlh-market-restart-${stamp}.out.log`);
    const logErr = join(tmpdir(), `rlh-market-restart-${stamp}.err.log`);
