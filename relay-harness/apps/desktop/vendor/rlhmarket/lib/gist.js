 ...
 *   memory only) or the operator sets RLH_GITHUB_TOKEN on the host; nothing
...
/** The single file every rlhmarket backup Gist carries. */
export const GIST_FILENAME = 'rlh-profile-backup.json';
...
export const GIST_TOKEN_ENV = 'RLH_GITHUB_TOKEN';
 ...
 * 2. the host-configured RLH_GITHUB_TOKEN environment variable;
...
export async function resolveGistTokenSource(bodyToken) {
    ...
    throw new GistError('GitHub token is required (enter it in the Backup tab, set RLH_GITHUB_TOKEN, or log in with the gh CLI)', 'auth');
}
...
export async function createGist(token, content, signal) {
    const body = JSON.stringify({
        description: 'rlhmarket profile backup',
        public: false,
