/**
 * Profile filesystem reads — everything the market learns from a rlh
...
/**
 * Resolve a profile name to its directory under RLH_HOME (default ~/.rlh).
...
export function profileDir(profile, explicitDir) {
    if (explicitDir !== undefined)
        return explicitDir;
    const home = process.env.RLH_HOME ?? join(homedir(), '.rlh');
    return join(home, 'profiles', profile);
}
/**
 * The in-box bundles rlh's profile templates install themselves — the ONLY
 ...
 * a filtered view would delete @relay-harness/rlh-base and friends.
 */
...
/** True when the installed package's manifest declares a rlh plugin surface. */
export function hasRlhManifest(dir) {
    try {
        const manifest = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8'));
        return manifest.rlh !== undefined;
    }
    catch {
        return false;
    }
 ...
 * declared `rlh.bundle.patch` file. Line-wise on purpose: the strict
 ...
 * the conventional path INSTEAD of declaring `rlh.bundle.patch`, and the
...
/** Rows of the patch a package DECLARES through `rlh.bundle.patch`. */
function readBundlePatchRows(dir) {
    const empty = { names: [], ids: [], insertedIds: [] };
    try {
        const manifest = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8'));
        const declared = manifest.rlh?.bundle?.patch;
 ...
 * declare `id: storage`) leaves RLH unable to start at all, with an error
 ...
 * `@linxin666/rlh-client-ui-skin-center`, and declares no main/exports/
...
export function hasLoadableEntry(profileDirectory, name) {
    ...
    // Targets resolve hoisted (the rlh profile default) or nested under it.
...
export function pluginSubdirs(root) {
    ...
    for (const sub of level1) {
        if (hasRlhManifest(join(root, sub))) {
            found.push(sub);
            continue;
        }
        try {
            for (const inner of readdirSync(join(root, sub), { withFileTypes: true })) {
                if (!inner.isDirectory() || !/^[A-Za-z0-9_.-]+$/.test(inner.name) || inner.name === 'node_modules')
                    continue;
                if (hasRlhManifest(join(root, sub, inner.name)))
 ...
 * pnpm-workspace.yaml `allowBuilds` block (the key rlh profiles use),
