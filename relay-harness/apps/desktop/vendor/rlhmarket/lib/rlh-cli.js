/**
 * Process layer: re-invoking the rlh CLI that launched this host, spawning
 * `rlh plugin` commands with timeouts and live progress, and provisioning
 ...
 * market's children find their tools regardless of how rlh was started.
 */
/**
 ...
 *
 * #167: a Windows desktop host spawned rlh without the Node install
...
const INSTALL_TIMEOUT_MS = Number(process.env.RLH_MARKET_INSTALL_TIMEOUT_MS) || 15 * 60 * 1000;
/**
 * Windows npm/corepack/pnpm are `.cmd` shims. Node's `spawn` without a shell
 * cannot start them (ENOENT / EINVAL). Same pattern as rlh's `plugin` forwarder.
 */
 ...
 * whether rlh runs from a global bin, a local install, or repo source
 * (`node --import tsx/esm .../bin.ts`). Falls back to a PATH `rlh`.
 */
export function rlhArgv() {
    const entry = process.argv[1];
    if (entry !== undefined && /[\\/](?:bin\.(?:js|ts)|rlh)$/.test(entry)) {
        // Absolute paths are required: source launches (`pnpm rlh`) pass a
    ...
    // Bare `rlh` is a .cmd shim on Windows that only a shell can start (#13).
    return { file: 'rlh', args: [], cwd: undefined, viaShell: winCmdShim };
}
/**
...
/** The child of the operation currently running, for /rlh-market/cancel. */
...
function killTree(child) {
    ...
    // POSIX: the rlh wrapper runs pnpm as a grandchild (spawnSync), which a
...
export function provisionHint(corepackOutput, npmOutput, npmFound = true) {
    ...
    if (!npmFound || (/ENOENT/.test(corepackOutput) && /ENOENT/.test(npmOutput))) {
        return `这台机器的 rlh 进程找不到 npm/corepack（图形界面或桌面端启动时不继承终端 PATH）。已在 Node 自己的目录里找过（${nodeBinDir}）也没有——多半是宿主内置的 Node 运行时不带 npm。请改从终端启动 rlh，或单独装一个 pnpm：Windows 用 iwr https://get.pnpm.io/install.ps1 -useb | iex，macOS/Linux 用 brew install pnpm / This rlh process cannot find npm/corepack (GUI and desktop launches skip your shell PATH). The directory Node itself runs from (${nodeBinDir}) was searched too — a bundled Node runtime without npm is the usual cause. Start rlh from a terminal, or install pnpm on its own: \`iwr https://get.pnpm.io/install.ps1 -useb | iex\` (Windows) or \`brew install pnpm\` (macOS/Linux)`;
    }
...
/** Singleton progress state; the status route reads it, runRlhPlugin writes it. */
 ...
 * it (defense in depth on top of per-route validation — the win32 bare-rlh
...
/** Run one `rlh plugin --profile <p> …` command with timeout and progress tracking. */
export function runRlhPlugin(profile, pluginArgs) {
    const { file, args, cwd, viaShell } = rlhArgv();
    ...
    return new Promise((resolvePromise) => {
        const child = spawnShim(file, [...args, 'plugin', '--profile', profile, ...pluginArgs], {
            ...
            // tree (rlh wrapper + pnpm grandchild) with one group signal.
...
export function createDesktopPluginRuntime(service, activeProfileDir, invokingDir = process.cwd(), timeoutMs = INSTALL_TIMEOUT_MS) {
    if (!isAbsolute(activeProfileDir) || activeProfileDir.includes('\0')) {
        throw new Error('rlh-market: Desktop profile directory must be an absolute path without NUL');
    }
    if (!isAbsolute(invokingDir) || invokingDir.includes('\0')) {
        throw new Error('rlh-market: Desktop invoking directory must be an absolute path without NUL');
    }
    const owner = Symbol('rlh-market desktop runtime');
    let closed = false;
    const runPlugin = async (_profile, pluginArgs) => {
        if (closed) {
            return {
                ...
                timedOut: false,
                stdout: '',
                stderr: 'rlh-market: Desktop package runtime is disposed',
                cancelled: false,
            };
        }
        ...
        timer = setTimeout(() => {
            timedOut = true;
            abort.abort(new Error('rlh-market: Desktop package operation timed out'));
