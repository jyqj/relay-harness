 ...
 *   broken  – would fail to load: listed as a bundle without a rlh surface,
...
import { bundlePatchInsertedIds, hasRlhManifest, hasLoadableEntry, profileDir } from './profile.js';
/** The profile manifest's `rlh.profile.bundles` — what the CLI reconciled. */
 ...
 * package (`rlhmarket`, `@scope/pkg`), but may point at a subpath entry
 * (`@vectorize-io/hindsight-coding-agents/rlh`, `aegis/extensions/rlh/index.js`).
...
function readPkgRlh(profile, name, explicitDir) {
    try {
...
export function verifyActivation(profile, name, live = new Set(listHotMounts()), explicitDir, isDisabled = false) {
    ...
    const rlh = readPkgRlh(profile, name, activeProfileDir);
    if (rlh === null) {
    ...
    // Plain library packages legitimately carry no `rlh` field and are still
    // loaded by name from a bundle patch — @relay-harness/rlh-tools is loaded by
    // the official rlh-base patch and has no `rlh` field at all — so this check
    ...
    if (!hasRlhManifest(dir)) {
        if (loaderLive) {
            return {
                state: 'live',
                reasons: ['已由 Loader 加载(该包未声明 rlh 元数据,由某个 bundle patch 按名加载)/ loaded by the loader (no rlh metadata of its own — a bundle patch loads it by name)'],
                bundle: inBundles,
                hot: true,
            };
        }
        // Not live and no rlh surface: for a package the profile lists as a
        // BUNDLE this is a real defect; for a plain dependency it is normal —
        // most dependencies are libraries, not plugins (#135).
        return inBundles
            ? {
                state: 'broken',
                reasons: ['已列入 profile bundle 层但未声明 rlh 元数据,加载会失败 / listed in the profile bundle layer but declares no rlh metadata — loading it fails'],
                bundle: true,
                hot: false,
            }
            : {
                state: 'inert',
                reasons: ['普通依赖(未声明 rlh 元数据),不是 profile 层插件;若它由某个 bundle patch 按名加载,启动后会显示为已加载 / a plain dependency with no rlh metadata — not a profile-layer plugin; if some bundle patch loads it by name it will read as live once running'],
                bundle: false,
                hot: false,
            };
    }
    ...
    if (loaderLive) {
        const clientOnly = rlh.bundle === undefined && rlh.client !== undefined;
        return {
            state: 'live',
            reasons: [
                clientOnly
    ...
    // (the rlh CLI skips them), so the market shim-mounts them at boot —
    // they still work, but "installed" never means "bundle layer".
    if (rlh.client !== undefined) {
        return {
            state: 'inert',
            reasons: [
                '未声明 rlh.bundle,不会进入 profile bundle 层(纯客户端插件);重启后由市场自动挂载生效 / no rlh.bundle — client-only plugins never enter the bundle layer; the market shim-mounts them at the next boot',
            ],
            bundle: false,
            hot: false,
        };
    ...
    return {
        state: 'inert',
        reasons: [
            '未声明 rlh.bundle,已作为普通依赖安装,不会成为 profile 层 / no rlh.bundle — installed as a plain dependency, never a profile-layer plugin',
        ],
        bundle: false,
        hot: false,
    };
 ...
 * from 1.11.3 to 1.12.2 left `/rlh-market/status` reporting 1.11.3 with an
 ...
 *
 * A `rlh.client`-only package — themes, skins, most pure-UI plugins — runs
...
export function hasHostHalf(profile, name, explicitDir) {
    const rlh = readPkgRlh(profile, name, explicitDir);
    if (rlh === null)
        return false;
    // Only a DEFINITE client-only package is excluded — the same test
    // verifyActivation uses for its own verdict. Testing `rlh.bundle` on its
    // own would read a package that declares neither key (`"rlh": {}`, which
    // the bundle layer still loads) as client-only, and quietly disable the
    // correction for it.
    return !(rlh.bundle === undefined && rlh.client !== undefined);
}
