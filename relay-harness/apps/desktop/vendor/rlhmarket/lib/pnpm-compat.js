 ...
 * different pnpm majors behave inside a rlh profile directory, kept pure and
...
/**
 * Decide the argv for a `rlh plugin <add|remove> …` call in the given profile.
 *
 ...
 * @param pluginArgs - the raw args, e.g. ['add', 'rlhmarket@latest'].
 ...
 * that surface through rlh's wrapper. Permanent shapes (404, auth) are
 ...
 *
 * rlh's own wrapper line ("rlh: pnpm failed in profile directory …") names no
...
export function classifyPnpmFailure(output) {
    ...
    if (output.includes('ERR_PNPM_ADDING_TO_ROOT')) {
        return {
            code: 'adding-to-root',
            recoverable: false,
            message: 'pnpm 拒绝在 workspace 根目录安装（缺少 -w）。这是市场的 bug，请升级 rlhmarket 到最新版 / pnpm refused to add at a workspace root (missing -w); this is a market bug — please update rlhmarket',
        };
    }
    if (/--workspace-root may only be used inside a workspace/i.test(output)) {
        return {
            ...
            recoverable: false,
            message: 'profile 目录不是 pnpm workspace，却传入了 -w。这是市场的 bug，请升级 rlhmarket 到最新版 / -w was passed but the profile is not a pnpm workspace; this is a market bug — please update rlhmarket',
        };
    }
