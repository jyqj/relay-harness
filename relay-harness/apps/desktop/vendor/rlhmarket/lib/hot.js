 ...
 *
 * Durable state stays with the profile's `rlh.profile.bundles` (reconciled by
 * the rlh CLI at install time), so the next boot loads the plugin through the
 * normal bundle layer. The subtree here exists only for the current process:
 * its input files live under `<profile>/.rlh-market/` and are wiped on every
 ...
 * tree changes back to the file it read (see rlh's agent-presets PresetTree
...
const HOT_DIR = '.rlh-market';
/**
 * Ceiling for one hot-mount activation, env-overridable like the install
 * timeout in rlh-cli.ts. An activation that exceeds it is treated as wedged
 * (typically a plugin pending on a service nothing provides) and falls back
 * to restart activation.
 */
const HOT_MOUNT_TIMEOUT_MS = Number(process.env.RLH_MARKET_HOT_MOUNT_TIMEOUT_MS) || 10000;
let hotTreeClass;
/**
 ...
 * (`rlh.client` without `rlh.bundle`) have no importable host half, but
...
async function loadHotTreeClass() {
    ...
    try {
        ...
        const specifier = '@relay-harness/cordis-plugin-include';
...
/** The `rlh` declaration block of an installed package, or null when unreadable. */
function readPkgRlh(profileDir, packageName) {
    try {
...
export async function hotMount(ctx, profileDir, packageName) {
    try {
        ...
        if (patchText !== null) {
            ...
            // No host patch. Client-only packages (rlh.client, no rlh.bundle) never
            ...
            const rlh = readPkgRlh(profileDir, packageName);
            if (rlh === null || rlh.client === undefined || rlh.bundle !== undefined) {
                return {
                    ok: false,
                    reason: '该包无 bundle patch 且未声明 rlh.client,没有可热挂载的内容 / no bundle patch and no rlh.client surface — nothing to hot-mount',
                };
            }
        ...
        ctx.logger?.info?.(`[rlh-market] hot-mounted ${packageName}`);
        logEvent('info', 'hot-mount', `${packageName}: live${shimNames.has(packageName) ? ' (client-only shim)' : ''}`);
        return { ok: true, reason: null };
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        ctx.logger?.warn(`[rlh-market] hot mount of ${packageName} failed, restart required: ${message}`);
...
/**
 * Mount every installed client-only package (`rlh.client` without
 * `rlh.bundle`) at market startup. The bundle reconcile skips these packages
...
export async function mountClientOnlyDeps(ctx, profileDir) {
    ...
    try {
        ...
        const bundles = new Set(manifest.rlh?.profile?.bundles ?? []);
    ...
    for (const name of deps) {
        ...
        // rows in cordis.patch.yml, e.g. via rlh-web-plugin-manager) are theirs
        ...
        if (patchLayerManages(userManaged, name))
            continue;
        const rlh = readPkgRlh(profileDir, name);
        if (rlh === null || rlh.client === undefined || rlh.bundle !== undefined)
            continue;
