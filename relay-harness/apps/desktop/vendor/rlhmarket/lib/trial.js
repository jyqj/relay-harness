 ...
 * (check.ts's buildBundleLayers): the rlh installation anchor first, then
...
import { buildBundleLayers, composeLayers, findRlhInstallDir, parsePatchFile, } from './check.js';
...
export function trialValidate(profileDir, newCommunityOrder, options = {}) {
    ...
    // (rlh install anchor first, then Node module search from the profile).
    let specs = {};
    try {
    ...
    const rlhInstall = options.rlhInstallDir !== undefined ? options.rlhInstallDir : findRlhInstallDir();
    ...
    const compose = (bundleOrder) => {
        const built = buildBundleLayers(profileDir, bundleOrder, specs, rlhInstall);
