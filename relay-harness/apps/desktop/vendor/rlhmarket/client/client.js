window.__ModuleLoader__.load({ id: "rlhmarket", factory: (require) => {
		...
		let _deepseek_ai_rlh_client_ui_primitives = require("@relay-harness/rlh-client-ui-primitives");
		_deepseek_ai_rlh_client_ui_primitives = __toESM(_deepseek_ai_rlh_client_ui_primitives, 1);
		...
		const zh = {
			...
			setSelfUpdate: "更新",
			setSelfUpdatedHint: "已下载完成。重启 Relay Harness 后新版本才会生效——页面本身会立即变新，服务端不会。",
			setChannel: "更新通道",
			...
			setSelfWorking: "正在移除…",
			setSelfRemoved: "已移除",
			setSelfRemovedHint: "重启 Relay Harness 后完全清理。",
			setSelfFailed: "操作失败",
			versionHint: "插件市场版本 — 反馈问题时请把它一起截图",
			subtitle: "发现社区为 Relay Harness 打造的能力",
			...
			restartBanner: "项变更完成，重启 Relay Harness 后生效",
			uninstall: "卸载",
			confirmRemove: "确认卸载？",
			uninstalling: "卸载中…",
			uninstallConfirmDesc: "将从当前 profile 中移除该插件。",
			restartHint: "重启方式：关闭当前 rlh 进程后重新运行（例如 rlh web）",
			confirmTitle: "安装",
			...
			restarting: "正在重启…",
			restartFail: "重启失败",
			restartTimeout: "等待 Relay Harness 启动超时",
			updateNow: "立即更新",
			updateFail: "更新失败",
			upToDate: "已是最新",
			linkedDev: "本地开发链接",
			exportLog: "导出日志",
			exportingLog: "导出中…",
			exportedLog: "日志已导出：rlh-market-log.txt，请把它附到 issue 里",
			exportLogFail: "日志导出失败",
			readme: "使用说明",
			terminalWarn: "这看起来是终端/命令行插件：装进网页版可能无效，甚至导致 Relay Harness 无法启动。建议先看它的使用说明，按说明装进对应的 profile。",
			...
			gist: "GitHub Gist",
			gistToken: "GitHub token（仅本次会话内存，刷新后需重新输入；留空则使用服务端 RLH_GITHUB_TOKEN 或已登录的 gh CLI）",
			...
			gistSrcToken: "手动输入",
			gistSrcEnv: "环境变量 RLH_GITHUB_TOKEN",
			gistSrcGh: "gh CLI",
			...
			gistNote: "与本地导出（本机存档）不同，Gist 用于跨机器同步：导出到私有 Gist 后，可在其他电脑用同一账号导入复用。Token 仅保存在本次会话内存中，绝不写入浏览器存储；已登录 gh CLI 或设置 RLH_GITHUB_TOKEN 则无需输入。Gist 为私有，单文件上限 1 MB。",
			backupWorking: "处理中…",
			backupDone: "备份已上传",
			restoreDone: "恢复完成，请重启 Relay Harness",
			...
			aiFixIntro: "请帮我修复 Relay Harness 的插件问题（profile：{0}）。诊断发现如下：",
			aiFixScope: "你可以修改 profile 的 rlh.profile.bundles 顺序、停用/启用插件、调整 cordis.patch.yml。注意：官方 bundle 不可移动；动手前先说明计划。",
			...
			hostDependencyWarning: "插件在 dependencies 中声明了已知的 RLH 共享宿主包，可能遮蔽宿主版本（仅依据清单，未确认运行时重复）：",
		...
		const en = {
			...
			setSelfUpdatedHint: "Downloaded. Restart Relay Harness for it to take effect — the page itself updates at once, the server half does not.",
			...
			setSelfRemovedHint: "Restart Relay Harness to finish cleaning up.",
			setSelfFailed: "The operation failed",
			versionHint: "Plugin market version — include it when reporting an issue",
			subtitle: "Discover community plugins for Relay Harness",
			...
			restartBanner: "change(s) done — restart Relay Harness to apply",
			uninstall: "Uninstall",
			confirmRemove: "Confirm?",
			uninstalling: "Removing…",
			uninstallConfirmDesc: "This removes the plugin from the current profile.",
			restartHint: "To restart: stop the current rlh process and run it again (e.g. rlh web)",
			...
			restartTimeout: "Timed out waiting for Relay Harness to start",
			...
			exportedLog: "Log exported: rlh-market-log.txt — please attach it to your issue",
			exportLogFail: "Log export failed",
			readme: "README",
			terminalWarn: "This looks like a terminal/CLI plugin: installing it into the web profile may do nothing, or even break Relay Harness startup. Read its README and install it into the profile it targets.",
			...
			gist: "GitHub Gist",
			gistToken: "GitHub token (session memory only — re-enter after refresh; leave empty to use server-side RLH_GITHUB_TOKEN or a logged-in gh CLI)",
			...
			gistSrcEnv: "RLH_GITHUB_TOKEN env var",
			gistSrcGh: "gh CLI",
			...
			aiFixIntro: "Please help me fix the plugin issues of Relay Harness (profile: {0}). Diagnostics found:",
			aiFixScope: "You may reorder rlh.profile.bundles, disable/enable plugins, or adjust cordis.patch.yml. Note: official bundles are fixed; state your plan before changing anything.",
			...
			hostDependencyWarning: "A plugin lists known shared RLH host packages in dependencies. This may shadow the host version; the check is manifest-only and does not confirm a duplicate runtime instance:",
		...
		* name-groups at the time of #66 (both rlh-usage-stats, four rlh-memory…).
		...
		*/
		...
		function themeSwatch(def) {
			...
			return [
				pick(["--rlw-alias-bg-base", "--rlw-alias-bg-layer-1"]) || (dark ? "#0f1115" : "#ffffff"),
				pick(["--rlw-alias-bg-layer-2", "--rlw-alias-bg-overlay"]) || (dark ? "#1a1d23" : "#f3f4f6"),
				pick(["--rlw-alias-brand-primary"]) || "#4f6ef7",
				pick(["--rlw-alias-label-primary"]) || (dark ? "#e5e7eb" : "#1f2328")
			];
		}
		/**
		...
		*/
		function InstallToast(props) {
			...
			const [mode] = (0, react.useState)(() => {
				const value = sessionStorage.getItem("rlhm-toast-mode");
				sessionStorage.removeItem("rlhm-toast-mode");
				return value;
			});
			const [names, setNames] = (0, react.useState)(() => {
				const value = readSession("rlhm-toast");
				sessionStorage.removeItem("rlhm-toast");
				return Array.isArray(value) ? value : [];
			});
			if (names.length === 0) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Toast, {
				text: names.join(", ") + " " + t(mode === "theme" ? "toastTheme" : "toastReady"),
				icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconSparkle16, { size: 14 }),
				onDone: () => setNames([])
			});
		}
		//#endregion
		//#region \0dsh-css:src/client/Market.module.css.mjs
		const css = ".eGUBIq_root{min-width:0;height:100%;color:var(--rlw-alias-label-primary,#1f2328);flex-direction:column;display:flex;position:relative}.eGUBIq_head{flex-direction:column;gap:12px;padding:4px 4px 12px;display:flex}.eGUBIq_title{margin:0;font-size:16px;font-weight:500;line-height:24px}.eGUBIq_sub{color:var(--rlw-alias-label-tertiary,#8b93a1);align-items:center;gap:8px;margin:0;font-size:14px;line-height:22px;display:flex}.eGUBIq_tabs{border-bottom:1px solid var(--rlw-alias-border-l2,#e5e7eb);align-items:flex-end;gap:2px;display:flex}.eGUBIq_tab{font:inherit;color:var(--rlw-alias-label-secondary,#6b7280);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-bottom:2px solid #0000;padding:7px 12px;font-size:13px}.eGUBIq_tab.eGUBIq_on{color:var(--rlw-alias-brand-primary,#4f6ef7);border-bottom-color:var(--rlw-alias-brand-primary,#4f6ef7);font-weight:600}.eGUBIq_banner{background:var(--rlw-alias-bg-layer-2,#fdf3e3);border:1px solid var(--rlw-alias-border-l2,#f3e3c3);border-radius:8px;align-items:center;gap:8px;margin:0;padding:8px 12px;font-size:12px;display:flex}.eGUBIq_bannerIcon{color:var(--rlw-alias-label-secondary,#6b7280);flex-shrink:0}.eGUBIq_bannerHint{color:var(--rlw-alias-label-tertiary,#8b93a1);cursor:help;display:inline-flex}.eGUBIq_body{flex:1;padding:12px 4px 24px;overflow-x:hidden;overflow-y:auto}.eGUBIq_cats{z-index:5;background:var(--rlw-alias-bg-layer-2,#f7f8fa);margin:0 -4px 2px;padding:12px 4px 4px;position:sticky;top:-1px}.eGUBIq_cats:before{content:\"\";background:inherit;pointer-events:none;height:14px;position:absolute;bottom:100%;left:0;right:0}.eGUBIq_catsRow{align-items:flex-start;gap:8px;display:flex;position:relative}.eGUBIq_star{color:var(--rlw-alias-label-secondary,#9ca3af);font-size:11px}.eGUBIq_top{z-index:20;display:inline-flex;position:absolute;bottom:18px;right:18px}.eGUBIq_topBtn{border-radius:99px;width:38px;height:38px;padding:0}.eGUBIq_tag{border:1px solid var(--rlw-alias-border-l3,#d9dde3);color:var(--rlw-alias-label-secondary,#6b7280);border-radius:4px;flex-shrink:0;padding:1px 6px;font-size:11px;line-height:16px}.eGUBIq_okState{color:var(--rlw-alias-state-success-primary,#16a34a);white-space:nowrap;font-size:12px;font-weight:600}.eGUBIq_catsWrap{flex-wrap:wrap;flex:1;align-items:center;gap:6px;min-width:0;display:flex}.eGUBIq_catsCollapsed{max-height:62px;overflow:hidden}.eGUBIq_catsToggle.eGUBIq_catsToggle{height:26px;min-height:26px;color:var(--rlw-alias-label-secondary,#6b7280);padding:0 6px}.eGUBIq_shots{-webkit-overflow-scrolling:touch;scrollbar-width:thin;gap:8px;margin:0 0 8px;padding:2px 0 6px;display:flex;overflow-x:auto}.eGUBIq_shot{object-fit:cover;border:1px solid var(--rlw-alias-border-default,#e5e7eb);background:var(--rlw-alias-bg-layer-2,#f3f4f6);border-radius:8px;flex:none;max-width:260px;height:150px}.eGUBIq_cmd{background:var(--rlw-alias-bg-layer-2,#f3f4f6);word-break:break-all;border-radius:6px;margin:8px 0 0;padding:8px 10px;font-family:ui-monospace,Menlo,monospace;font-size:11px;line-height:18px}.eGUBIq_warnLine{color:var(--rlw-alias-state-warn-primary,#b45309);align-items:center;gap:4px;margin:0;font-size:12px;font-weight:600;line-height:18px;display:flex}.eGUBIq_modalNote{color:var(--rlw-alias-label-tertiary,#8b93a1);align-items:center;gap:4px;margin:12px 0 0;font-size:12px;line-height:18px;display:flex}.eGUBIq_grid{grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;display:grid}.eGUBIq_swatches{border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:8px;gap:0;height:34px;display:flex;overflow:hidden}.eGUBIq_themesGrid{margin-bottom:12px}.eGUBIq_swatches i{flex:1}.eGUBIq_card{background:var(--rlw-alias-bg-layer-1,#fff);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:12px;flex-direction:column;gap:12px;padding:12px 14px;display:flex}.eGUBIq_row1{align-items:center;gap:10px;min-width:0;display:flex}.eGUBIq_srcBtn.eGUBIq_srcBtn{color:var(--rlw-alias-label-secondary,#9ca3af);flex-shrink:0;align-self:flex-start;font-weight:400}.eGUBIq_av{color:#fff;object-fit:cover;background:var(--rlw-alias-bg-layer-2,#f3f4f6);border-radius:50%;flex-shrink:0;place-items:center;width:16px;height:16px;font-size:9px;font-weight:700;display:grid}.eGUBIq_nm{text-overflow:ellipsis;white-space:nowrap;font-size:15px;font-weight:600;line-height:22px;overflow:hidden}.eGUBIq_byline{align-items:center;gap:6px;min-width:0;margin-top:2px;display:flex}.eGUBIq_owner{color:var(--rlw-alias-label-secondary,#9ca3af);font-size:11px}.eGUBIq_desc{color:var(--rlw-alias-label-tertiary,#8b93a1);min-height:36px;margin:0;font-size:12px;line-height:18px}.eGUBIq_foot{align-items:center;gap:8px;margin-top:auto;display:flex}.eGUBIq_grow{flex:1}.eGUBIq_titleRow{align-items:center;gap:10px;display:flex}.eGUBIq_version{color:var(--rlw-alias-label-tertiary,#8b93a1);font-variant-numeric:tabular-nums;flex-shrink:0;font-size:12px;line-height:20px}.eGUBIq_descTight{min-height:0}.eGUBIq_src{color:var(--rlw-alias-label-secondary,#9ca3af);font-size:11px;text-decoration:none}.eGUBIq_src:hover{color:var(--rlw-alias-brand-primary,#4f6ef7)}.eGUBIq_dot{vertical-align:2px;margin-left:5px}.eGUBIq_act{flex-wrap:wrap;align-items:center;gap:6px;margin-top:6px;font-size:11px;display:flex}.eGUBIq_actLive{color:var(--rlw-alias-state-success-primary,#16a34a);align-items:center;gap:4px;font-weight:600;display:inline-flex}.eGUBIq_actWarn{color:var(--rlw-alias-state-warn-primary,#b45309);align-items:center;gap:4px;font-weight:600;display:inline-flex}.eGUBIq_actBroken{color:var(--rlw-alias-state-error-primary,#dc2626);align-items:center;gap:4px;font-weight:600;display:inline-flex}.eGUBIq_actWhy{color:var(--rlw-alias-label-secondary,#6b7280);margin-top:2px}.eGUBIq_loading{color:var(--rlw-alias-label-secondary,#9ca3af);flex-direction:column;align-items:center;gap:12px;padding:48px;font-size:13px;display:flex}.eGUBIq_spin{color:var(--rlw-alias-brand-primary,#4f6ef7);flex-shrink:0;animation:.8s linear infinite eGUBIq_sp;display:inline-flex}.eGUBIq_logoMark{color:var(--rlw-alias-brand-primary,#4f6ef7);flex-shrink:0;display:inline-flex}.eGUBIq_logoPlug{transform-box:fill-box;transform-origin:50%;animation:1.5s cubic-bezier(.4,0,.2,1) infinite eGUBIq_rlhmPlug}@keyframes eGUBIq_rlhmPlug{0%,12%{transform:rotate(9deg)}45%,62%{transform:translate(-1.28px,1.27px)rotate(0)}95%,to{transform:rotate(9deg)}}@media (prefers-reduced-motion:reduce){.eGUBIq_logoPlug,.eGUBIq_spin{animation:1.5s ease-in-out infinite eGUBIq_rlhmPlugFade}}@keyframes eGUBIq_rlhmPlugFade{0%,to{opacity:1}50%{opacity:.35}}@keyframes eGUBIq_sp{to{transform:rotate(360deg)}}.eGUBIq_progress{background:var(--rlw-alias-bg-layer-2,#f3f4f6);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);color:var(--rlw-alias-label-secondary,#6b7280);border-radius:8px;flex-wrap:wrap;align-items:center;gap:9px;margin:0;padding:8px 12px;font-size:12px;display:flex}.eGUBIq_bar{background:var(--rlw-alias-border-l1,#e5e7eb);border-radius:99px;width:100%;height:4px;overflow:hidden}.eGUBIq_barFill{background:var(--rlw-alias-brand-primary,#4f6ef7);border-radius:99px;height:100%;transition:width .6s}.eGUBIq_barWave{width:30%;animation:1.2s ease-in-out infinite eGUBIq_rlhmSlide}@keyframes eGUBIq_rlhmSlide{0%{margin-left:-30%}to{margin-left:100%}}.eGUBIq_irow .eGUBIq_progress{margin-top:8px}.eGUBIq_progress code{text-overflow:ellipsis;white-space:nowrap;font-family:ui-monospace,Menlo,monospace;font-size:11px;overflow:hidden}.eGUBIq_empty{color:var(--rlw-alias-label-secondary,#9ca3af);text-align:center;padding:32px;font-size:13px}.eGUBIq_err{color:var(--rlw-alias-state-error-primary,#dc2626);white-space:pre-wrap;word-break:break-all;margin:8px 0;font-size:12px}.eGUBIq_irow{background:var(--rlw-alias-bg-layer-1,#fff);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:12px;align-items:center;gap:10px;margin-bottom:8px;padding:12px 14px;display:flex}.eGUBIq_irowMissing{filter:grayscale();opacity:.5}.eGUBIq_irow>.eGUBIq_src,.eGUBIq_irow>.eGUBIq_owner,.eGUBIq_irow button{white-space:nowrap;flex-shrink:0}.eGUBIq_tabSearchRow{padding:0 4px 10px;display:flex}.eGUBIq_tabSearch{width:260px}.eGUBIq_spec{color:var(--rlw-alias-label-secondary,#9ca3af);overflow-wrap:anywhere;min-width:0;font-family:ui-monospace,Menlo,monospace;font-size:11px}.eGUBIq_specTag{white-space:nowrap;border-radius:4px;flex-shrink:0;align-items:center;height:16px;padding:0 5px;font-size:10px;font-weight:600;display:inline-flex}.eGUBIq_specTagGit{color:#0b7285;background:#12a3c41f}.eGUBIq_specTagFile{color:#b07d1b;background:#f0b42924}.eGUBIq_backupCheckList .eGUBIq_grow{white-space:nowrap;text-overflow:ellipsis;flex:70%;min-width:0;overflow:hidden}.eGUBIq_backupCheckList .eGUBIq_spec{text-align:right;white-space:nowrap;text-overflow:ellipsis;flex:0 30%;max-width:30%;overflow:hidden}.eGUBIq_staleAction{margin-top:8px}.eGUBIq_pct{color:var(--rlw-alias-label-secondary,#6b7280);flex-shrink:0;font-size:11px;font-weight:600}.eGUBIq_pager{flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px;margin:16px 0 4px;display:flex}.eGUBIq_pagerPages{flex-wrap:wrap;flex:1;justify-content:center;align-items:center;gap:6px;min-width:0;display:flex}.eGUBIq_pageEllipsis{color:var(--rlw-alias-label-secondary,#9ca3af);padding:0 2px;font-size:12px}.eGUBIq_pageInfo{color:var(--rlw-alias-label-secondary,#6b7280);white-space:nowrap;font-size:12px}.eGUBIq_depBadge{border:1px solid var(--rlw-alias-state-warn-primary,#b45309);color:var(--rlw-alias-state-warn-primary,#b45309);white-space:nowrap;border-radius:4px;flex-shrink:0;margin-left:6px;padding:1px 6px;font-size:11px;font-weight:600;line-height:16px}.eGUBIq_deprecate{color:var(--rlw-alias-state-warn-primary,#b45309);background:var(--rlw-alias-bg-layer-2,#fdf3e3);border:1px solid var(--rlw-alias-border-l2,#f3e3c3);border-radius:8px;margin:0;padding:8px 10px;font-size:12px;line-height:18px}.eGUBIq_deprecate a{color:var(--rlw-alias-state-warn-primary,#b45309);text-decoration:underline}.eGUBIq_deprecate .eGUBIq_src{margin-left:8px}.eGUBIq_depLine{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.eGUBIq_switch{border:1px solid var(--rlw-alias-border-l2,#d9dde3);background:var(--rlw-alias-bg-layer-2,#e5e7eb);cursor:pointer;border-radius:99px;flex-shrink:0;width:38px;height:22px;padding:0;transition:background .15s,border-color .15s;position:relative}.eGUBIq_switchOn{background:var(--rlw-alias-state-success-primary,#16a34a);border-color:var(--rlw-alias-state-success-primary,#16a34a)}.eGUBIq_switchMixed{background:var(--rlw-alias-state-warn-primary,#b45309);border-color:var(--rlw-alias-state-warn-primary,#b45309)}.eGUBIq_switchKnob{background:#fff;border-radius:99px;width:16px;height:16px;transition:left .15s;position:absolute;top:2px;left:2px;box-shadow:0 1px 2px #00000040}.eGUBIq_switchOn .eGUBIq_switchKnob,.eGUBIq_switchMixed .eGUBIq_switchKnob{left:18px}.eGUBIq_switch:disabled{opacity:.5;cursor:default}.eGUBIq_viewBar{border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:8px;align-items:center;gap:2px;width:fit-content;margin-bottom:12px;padding:2px;display:flex}.eGUBIq_viewBtn{font:inherit;color:var(--rlw-alias-label-secondary,#6b7280);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-radius:6px;padding:4px 10px;font-size:12px;line-height:18px}.eGUBIq_viewBtn:hover{color:var(--rlw-alias-brand-primary,#4f6ef7)}.eGUBIq_viewOn{background:var(--rlw-alias-bg-layer-2,#eef0f4);color:var(--rlw-alias-label-primary,#1f2328);font-weight:600}.eGUBIq_groupRow{background:var(--rlw-alias-bg-layer-1,#fff);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:12px;margin-bottom:10px;padding:12px 14px}.eGUBIq_groupHead{align-items:center;gap:10px;min-width:0;display:flex}.eGUBIq_groupName{text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.eGUBIq_groupActions{flex-shrink:0;align-items:center;gap:6px;display:flex}.eGUBIq_groupMembers{flex-direction:column;gap:6px;margin-top:10px;display:flex}.eGUBIq_groupMember{background:var(--rlw-alias-bg-layer-2,#f7f8fa);border-radius:8px;align-items:center;gap:8px;padding:6px 8px;font-size:12px;line-height:18px;display:flex}.eGUBIq_groupMember .eGUBIq_nm{flex:1;min-width:0;font-size:12px}.eGUBIq_groupAddPanel{border-top:1px dashed var(--rlw-alias-border-l2,#e5e7eb);flex-direction:column;gap:6px;margin-top:10px;padding-top:10px;display:flex}.eGUBIq_groupCreate{align-items:center;gap:8px;margin-bottom:10px;display:flex}.eGUBIq_inlineInput{flex:1;min-width:120px}.eGUBIq_assignRow{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.eGUBIq_assignSelect{border:1px solid var(--rlw-alias-border-l2,#e5e7eb);background:var(--rlw-alias-bg-layer-1,#fff);color:var(--rlw-alias-label-primary,#1f2328);font:inherit;border-radius:6px;padding:3px 6px;font-size:12px;line-height:18px}.eGUBIq_groupHint{color:var(--rlw-alias-label-tertiary,#8b93a1);font-size:11px}.eGUBIq_sectAction{color:var(--rlw-alias-label-secondary,#6b7280);align-items:center;gap:8px;margin:14px 2px 8px;font-size:12px;font-weight:600;display:flex}.eGUBIq_backupGrid{grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;display:grid}.eGUBIq_backupCard{background:var(--rlw-alias-bg-layer-1,#fff);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:12px;flex-direction:column;gap:10px;padding:16px;display:flex}.eGUBIq_backupCard h3{margin:0;font-size:14px}.eGUBIq_backupCard p{color:var(--rlw-alias-label-secondary,#6b7280);margin:0;font-size:12px;line-height:18px}.eGUBIq_backupActions{flex-wrap:wrap;gap:8px;display:flex;position:relative}.eGUBIq_hiddenFile{opacity:0;pointer-events:none;width:1px;height:1px;position:absolute}.eGUBIq_backupInput{box-sizing:border-box;width:100%}.eGUBIq_backupCheck{cursor:pointer;align-items:center;gap:6px;font-size:12px;display:flex}.eGUBIq_backupWarn{margin:0;font-size:12px;line-height:18px;color:var(--rlw-alias-state-warn-primary,#b45309)!important}.eGUBIq_backupMessage{color:var(--rlw-alias-label-secondary,#6b7280);grid-column:1/-1;font-size:12px}.eGUBIq_backupCheckList{flex-direction:column;gap:6px;max-height:260px;margin:10px 0 4px;padding-right:4px;display:flex;overflow-y:auto}.eGUBIq_backupCheckList .eGUBIq_backupCheck{justify-content:space-between;gap:8px}.eGUBIq_diagPage{flex-direction:column;gap:12px;height:100%;min-height:0;display:flex;overflow-y:auto}.eGUBIq_diagSummary{background:var(--rlw-alias-bg-layer-1,#fff);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:12px;flex-wrap:wrap;align-items:center;gap:12px;padding:10px 14px;font-size:12px;display:flex}.eGUBIq_diagSummaryItem{color:var(--rlw-alias-label-secondary,#6b7280);white-space:nowrap;align-items:center;gap:6px;display:inline-flex}.eGUBIq_diagSummaryMeta{color:var(--rlw-alias-label-tertiary,#9ca3af);text-overflow:ellipsis;white-space:nowrap;max-width:320px;font-family:ui-monospace,Menlo,monospace;font-size:11px;overflow:hidden}.eGUBIq_diagSection{background:var(--rlw-alias-bg-layer-1,#fff);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:12px;flex-direction:column;gap:8px;padding:12px 14px;display:flex}.eGUBIq_diagSection h3{color:var(--rlw-alias-label-primary,#1f2328);margin:0;font-size:13px;font-weight:600}.eGUBIq_diagCount{color:var(--rlw-alias-label-tertiary,#9ca3af);font-size:11px;font-weight:400}.eGUBIq_diagEmpty{color:var(--rlw-alias-label-secondary,#9ca3af);padding:8px 0;font-size:12px}.eGUBIq_diagBundle{border-top:1px solid var(--rlw-alias-border-l2,#f0f1f3);flex-direction:column;gap:6px;padding-top:8px;display:flex}.eGUBIq_diagBundle:first-of-type{border-top:none;padding-top:0}.eGUBIq_diagRow{flex-wrap:wrap;align-items:center;gap:8px;min-width:0;font-size:12px;line-height:18px;display:flex}.eGUBIq_diagMeta{align-items:baseline;gap:8px;min-width:0;font-size:12px;display:flex}.eGUBIq_diagKey{color:var(--rlw-alias-label-tertiary,#9ca3af);flex-shrink:0;min-width:64px;font-size:11px}.eGUBIq_diagVal{color:var(--rlw-alias-label-primary,#1f2328);overflow-wrap:anywhere;min-width:0;font-family:ui-monospace,Menlo,monospace;font-size:12px;font-weight:500}.eGUBIq_diagIndex{background:var(--rlw-alias-bg-layer-2,#f3f4f6);min-width:18px;height:18px;color:var(--rlw-alias-label-secondary,#6b7280);border-radius:9px;flex-shrink:0;justify-content:center;align-items:center;font-size:11px;font-weight:600;display:inline-flex}.eGUBIq_diagArrow{color:var(--rlw-alias-label-tertiary,#9ca3af);flex-shrink:0;font-size:12px}.eGUBIq_diagBadgeOfficial{background:var(--rlw-alias-brand-primary,#4f6ef7);color:#fff;border-radius:9px;flex-shrink:0;align-items:center;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex}.eGUBIq_diagBadgeCommunity{background:var(--rlw-alias-bg-layer-2,#f3f4f6);height:18px;color:var(--rlw-alias-label-secondary,#6b7280);border-radius:9px;flex-shrink:0;align-items:center;padding:0 8px;font-size:11px;display:inline-flex}.eGUBIq_diagBadgeShadow{background:var(--rlw-alias-state-error-primary,#dc2626);color:#fff;border-radius:9px;flex-shrink:0;align-items:center;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex}.eGUBIq_diagList{flex-direction:column;gap:6px;display:flex}.eGUBIq_sectionOverview{color:var(--rlw-alias-label-tertiary,#8b93a1);text-overflow:ellipsis;white-space:nowrap;max-width:100%;padding:2px 0 6px;font-size:12px;line-height:18px;overflow:hidden}.eGUBIq_diagAlert{color:var(--rlw-alias-state-warn-primary,#b45309)}.eGUBIq_ovRow{background:var(--rlw-alias-bg-layer-2,#f7f8fa);border-radius:8px;flex-wrap:wrap;align-items:center;gap:8px;min-width:0;padding:6px 10px;font-size:12px;line-height:18px;display:flex}.eGUBIq_ovArrow{color:var(--rlw-alias-label-tertiary,#9ca3af);flex-shrink:0;font-size:12px}.eGUBIq_ovByTag{background:var(--rlw-alias-brand-primary,#4f6ef7);color:#fff;text-overflow:ellipsis;white-space:nowrap;border-radius:9px;flex-shrink:0;align-items:center;max-width:260px;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex;overflow:hidden}.eGUBIq_ovFrom{color:var(--rlw-alias-label-secondary,#6b7280);text-overflow:ellipsis;white-space:nowrap;min-width:0;font-size:12px;overflow:hidden}.eGUBIq_orphRow{background:var(--rlw-alias-bg-layer-2,#f7f8fa);border-radius:8px;flex-wrap:wrap;align-items:center;gap:8px;min-width:0;padding:6px 10px;font-size:12px;line-height:18px;display:flex}.eGUBIq_orphBadge{background:var(--rlw-alias-state-warn-primary,#b45309);color:#fff;white-space:nowrap;border-radius:9px;flex-shrink:0;align-items:center;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex}.eGUBIq_dragHandle{width:20px;height:20px;color:var(--rlw-alias-label-tertiary,#9ca3af);cursor:grab;-webkit-user-select:none;user-select:none;flex-shrink:0;justify-content:center;align-items:center;font-size:12px;line-height:20px;display:inline-flex}.eGUBIq_dragOver{outline:2px dashed var(--rlw-alias-brand-primary,#4f6ef7);outline-offset:2px;background:var(--rlw-alias-bg-layer-2,#f0f2f8);border-radius:8px}.eGUBIq_dragging{opacity:.45;background:var(--rlw-alias-bg-layer-2,#f3f4f6)}.eGUBIq_collapseHead{font:inherit;color:var(--rlw-alias-label-primary,#1f2328);cursor:pointer;text-align:left;background:0 0;border:none;align-items:center;gap:8px;width:100%;padding:0;font-size:13px;font-weight:600;display:flex}.eGUBIq_collapseIcon{color:var(--rlw-alias-label-secondary,#6b7280);flex-shrink:0;display:inline-flex}.eGUBIq_collapseTitle{flex:1;min-width:0}.eGUBIq_collapseBody{border-top:1px solid var(--rlw-alias-border-l2,#f0f1f3);overflow-wrap:anywhere;flex-direction:column;gap:10px;min-width:0;margin-top:8px;padding-top:10px;display:flex}.eGUBIq_panelNote{color:var(--rlw-alias-label-secondary,#6b7280);margin:0;font-size:12px;line-height:18px}.eGUBIq_fixFallback{flex-direction:column;gap:6px;margin:8px 0;display:flex}.eGUBIq_fixFallbackText{box-sizing:border-box;width:100%;color:var(--rlw-alias-label-primary,#1f2328);background:var(--rlw-alias-bg-layer-2,#f3f4f6);border:1px solid var(--rlw-alias-border-l2,#e5e7eb);resize:vertical;white-space:pre-wrap;word-break:break-all;border-radius:6px;padding:8px 10px;font-family:ui-monospace,Menlo,monospace;font-size:11px;line-height:16px}.eGUBIq_setCard{border:1px solid var(--rlw-alias-border-l2,#e5e7eb);background:var(--rlw-alias-bg-layer-3,#fff);border-radius:12px;list-style:none;transition:border-color .16s,background .16s}.eGUBIq_setCard:hover{border-color:var(--rlw-alias-label-dimmed,#c8ccd4)}.eGUBIq_setCardOpen{background:var(--rlw-alias-bg-layer-2,#f7f8fa);border-color:var(--rlw-alias-label-dimmed,#c8ccd4)}.eGUBIq_setHeader{-webkit-appearance:none;appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}.eGUBIq_setHeader:focus-visible{outline:2px solid var(--rlw-alias-brand-primary,#4f6ef7);outline-offset:-2px}.eGUBIq_setHeadText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}.eGUBIq_setName{color:var(--rlw-alias-label-primary,#1f2328);font-size:15px;font-weight:600;line-height:1.4}.eGUBIq_setDesc{color:var(--rlw-alias-label-tertiary,#8b93a1);font-size:13px;line-height:1.5}.eGUBIq_setChevron{color:var(--rlw-alias-label-tertiary,#8b93a1);flex:none;transition:transform .16s;display:inline-flex}.eGUBIq_setChevronOpen{transform:rotate(180deg)}.eGUBIq_setBody{border-top:1px solid var(--rlw-alias-border-l2,#e5e7eb);margin:0 16px;padding-bottom:8px}.eGUBIq_setRow{align-items:center;gap:12px;padding:12px 0;display:flex}.eGUBIq_setRow+.eGUBIq_setRow{border-top:1px solid var(--rlw-alias-border-l2,#e5e7eb)}.eGUBIq_setLabelBox{flex-direction:column;flex:1;gap:3px;min-width:0;display:flex}.eGUBIq_setLabel{font-size:13px;line-height:20px}.eGUBIq_setHint{color:var(--rlw-alias-label-tertiary,#8b93a1);font-size:12px;line-height:18px}.eGUBIq_setConfirm{border-top:1px solid var(--rlw-alias-border-l2,#e5e7eb);flex-direction:column;gap:8px;padding:12px 0 4px;display:flex}.eGUBIq_setCheck{cursor:pointer;align-items:center;gap:8px;font-size:12px;line-height:18px;display:flex}.eGUBIq_setActions{border-top:1px solid var(--rlw-alias-border-l2,#e5e7eb);justify-content:flex-end;align-items:center;gap:8px;padding:12px 0 4px;display:flex}.eGUBIq_setDanger{color:var(--rlw-alias-state-error-primary,#dc2626)}.eGUBIq_setSeg{border:1px solid var(--rlw-alias-border-l2,#e5e7eb);border-radius:8px;flex-shrink:0;gap:2px;padding:2px;display:inline-flex}.eGUBIq_setSegBtn{font:inherit;color:var(--rlw-alias-label-secondary,#6b7280);cursor:pointer;background:0 0;border:none;border-radius:6px;padding:3px 10px;font-size:12px;line-height:18px}.eGUBIq_setSegBtn:disabled{cursor:default;opacity:.5}.eGUBIq_setSegOn{background:var(--rlw-alias-bg-layer-2,#eef0f4);color:var(--rlw-alias-label-primary,#1f2328);font-weight:600}.eGUBIq_setBetaTag{background:var(--rlw-alias-bg-module-platform,#eef0f4);color:var(--rlw-alias-label-secondary,#6b7280);border-radius:9px;margin-left:6px;padding:0 6px;font-size:11px;font-weight:600;line-height:17px}";
		const tagId = "rlhmarket/Market.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "rlhmarket";
		...
		var Market_module_css_default = {
			...
			"rlhmPlug": "eGUBIq_rlhmPlug",
			"rlhmPlugFade": "eGUBIq_rlhmPlugFade",
			"rlhmSlide": "eGUBIq_rlhmSlide",
			"empty": "eGUBIq_empty",
			"err": "eGUBIq_err",
		...
		* served by the host route /rlh-market/check (see src/check.ts). Below the
		* report sits the phase 2 action panel: community-bundle ordering (reorder
		* locally with ↑/↓ or drag, POST to /rlh-market/bundle-order) plus the AI-fix
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: open ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronDownOutline14, { size: 14 }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronRightOutline14, { size: 14 })
							}),
		...
		*/
		function Diagnostics(props) {
			...
			* selectable text block so the user can still copy it manually. */
			...
			/** Bump to re-run the /rlh-market/check fetch after an order apply. */
			...
			* candidate composition (dry-run replay) before writing. */
			...
			(0, react.useEffect)(() => {
				let live = true;
				setError(null);
				fetch("/rlh-market/check", { cache: "no-store" }).then(async (res) => {
			...
			if (report === null) return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconLoadingOutline16, { size: 22 })
				}), t("checkLoading")]
			});
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.StateDot, {
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.StateDot, {
										state: "error",
										size: 8
									}),
									t("catConflict"),
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: Market_module_css_default.grow }),
							hasHardIssues && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
								variant: "outline",
								size: "sm",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
								variant: "ghost",
								size: "sm",
								"aria-label": t("checkRefresh"),
								onClick: refresh,
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconRefreshOutline14, { size: 14 })
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Section, {
						...
						}), peerInfo.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.DisclosureRow, {
							icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronDownOutline14, { size: 14 }),
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									suggested !== null && suggested.ok === true && suggested.order.join("\0") !== communityNames.join("\0") && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "outline",
										size: "sm",
										disabled: orderBusy,
									...
									order.join("\0") !== communityNames.join("\0") && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "ghost",
										size: "sm",
										disabled: orderBusy,
		...
		* /rlh-market/* host routes, with install/update/uninstall flows and the
		...
		*/
		...
		function HostDependencyDiagnostics({ findings, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconWarningOutline16, {
					size: 14,
		...
		*/
		function OwnerAvatar({ name, owner }) {
			...
			if (failed || owner === "") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: name.replace(/^rlh[-_]/i, "").charAt(0).toUpperCase() || "P"
			});
		...
		*/
		...
		const WEBDAV_STORAGE_KEY = "rlhm-webdav";
		...
		function backupDependencies(value) {
			...
			const backup = value;
			if (backup.format !== "rlh-profile-backup" || backup.version !== .2) throw new Error("unsupported backup format");
		...
		function MarketSection(props) {
			...
			const [tab, setTab] = (0, react.useState)(() => {
				const saved = sessionStorage.getItem("rlhm-tab");
				if (saved !== null) sessionStorage.removeItem("rlhm-tab");
			...
			*/
			const doExportLog = (0, react.useCallback)(() => {
				setExportState("busy");
				fetch("/rlh-market/logs").then(async (res) => {
					if (!res.ok) throw new Error(`HTTP ${String(res.status)}`);
					const blob = await res.blob();
					const url = URL.createObjectURL(blob);
					const anchor = document.createElement("a");
					anchor.href = url;
					anchor.download = "rlh-market-log.txt";
			...
			*/
			...
			/** Per-package activation states from /rlh-market/installed + operations. */
			...
			/**
			* Patch-layer flags (port of rlh-plugin-hub): packages whose bundle rows
			...
			* it duplicated the Discover tab). */
			...
			/** Server-side operation lock from /rlh-market/status (#91). */
			...
			*/
			...
			const [gistId, setGistId] = (0, react.useState)(() => {
				try {
					return localStorage.getItem("rlhm-gist-id") ?? "";
				} catch {
					return "";
				}
			});
			...
			const [gistMode, setGistMode] = (0, react.useState)(() => {
				try {
					return localStorage.getItem("rlhm-gist-id") ? "update" : "create";
				} catch {
					return "create";
				}
			});
			...
			/** Bundle-only plugin names from /rlh-market/installed (picker list). */
			...
			const refreshInstalled = (0, react.useCallback)((force) => {
				fetch("/rlh-market/installed", { cache: "no-store" }).then((res) => res.json()).then((body) => {
					...
					const findings = body.diagnostics?.schema === "rlh-market/diagnostics/v1" && Array.isArray(body.diagnostics.findings) ? body.diagnostics.findings.filter(isHostDependencyFinding) : [];
					setHostDependencyFindings(findings);
				}).catch(() => {});
				fetch("/rlh-market/updates" + (force === true ? "?force=1" : ""), { cache: "no-store" }).then((res) => res.json()).then((body) => setUpdates(body.updates || {})).catch(() => {});
			}, []);
			...
			(0, react.useEffect)(() => {
				fetch("/rlh-market/registry", { cache: "no-store" }).then(async (res) => {
				...
				fetch("/rlh-market/status", { cache: "no-store" }).then((res) => res.json()).then((status) => {
					setEnvReady(status.pnpm !== false);
					if (typeof status.boot === "string") {
						setBootId(status.boot);
						try {
							setRestartNoticeDismissed(sessionStorage.getItem("rlhm-restart-dismissed") === status.boot);
						} catch {}
					}
			...
			(0, react.useEffect)(() => {
				...
				const saved = readSession("rlhm-restart");
				if (saved === null) return;
				if (saved.boot !== bootId) {
					sessionStorage.removeItem("rlhm-restart");
					return;
				}
			...
			(0, react.useEffect)(() => {
				...
				sessionStorage.setItem("rlhm-restart", JSON.stringify({
					boot: bootId,
					doneUrls,
			...
			(0, react.useEffect)(() => {
				const pending = readSession("rlhm-pending");
				...
				const timer = setInterval(() => {
					fetch("/rlh-market/status", { cache: "no-store" }).then((res) => res.json()).then((status) => {
						...
						if (status.active) {
							...
							if (readSession("rlhm-pending") !== null && busyUrl !== null && status.busy !== true) {
								if (data !== null && data.plugins.some((p) => p.url === busyUrl && isInstalled(p, status.installed || {}))) {
									idleStrikes.current = 0;
									sessionStorage.removeItem("rlhm-pending");
			...
			* backup export) and falls back to the caller's name. */
			...
			const doInstall = (0, react.useCallback)((plugin) => {
				...
				sessionStorage.setItem("rlhm-pending", JSON.stringify({ url: plugin.url }));
				fetch("/rlh-market/install", {
					method: "POST",
					headers: { "content-type": "application/json" },
					...
					if (status === 200 && body.ok && body.hot && plugin.category === "theme") {
						sessionStorage.setItem("rlhm-toast", JSON.stringify([plugin.name]));
						sessionStorage.setItem("rlhm-tab", "themes");
						location.reload();
						return;
					}
					...
					if (status === 200 && body.ok) {
						sessionStorage.setItem("rlhm-tab", "installed");
			...
			*/
			const doRestart = (0, react.useCallback)(() => {
				...
				const awaitNewBoot = () => {
					...
					const poll = () => {
						fetch("/rlh-market/status", { cache: "no-store" }).then((res) => res.json()).then((next) => {
			...
			const doUpdate = (0, react.useCallback)((name, force = false) => {
				...
				return fetch("/rlh-market/update", {
					method: "POST",
					headers: { "content-type": "application/json" },
			...
			const doUseSkin = (0, react.useCallback)((name) => {
				...
				fetch("/rlh-market/use-skin", {
					...
					if (status === 200 && body.ok) {
						sessionStorage.setItem("rlhm-toast", JSON.stringify([name]));
						sessionStorage.setItem("rlhm-toast-mode", "theme");
						sessionStorage.setItem("rlhm-tab", "themes");
						location.reload();
			...
			const doUninstall = (0, react.useCallback)((name) => {
				...
				return fetch("/rlh-market/uninstall", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({ name })
				}).then((res) => res.json().then((body) => ({
			...
			* immediately (mirrors the use-skin reload on activate). */
			const doToggle = (0, react.useCallback)((name, enabled, reload = false) => {
				...
				setInstallError(null);
				return fetch("/rlh-market/toggle", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({
						name,
			...
			/** Adopt the groups payload returned by POST /rlh-market/groups. */
			const setGroupPayload = (0, react.useCallback)((body) => {
				if (body.groups && typeof body.groups === "object") setGroups(body.groups);
				if (Array.isArray(body.groupOrder)) setGroupOrder(body.groupOrder);
				if (Array.isArray(body.disabled)) setDisabledNames(body.disabled);
			}, []);
			/** One POST /rlh-market/groups round trip (create/rename/delete/members/toggle). */
			const doGroupAction = (0, react.useCallback)((payload) => {
				setInstallError(null);
				return fetch("/rlh-market/groups", {
					method: "POST",
					headers: { "content-type": "application/json" },
			...
			const selfName = installed["rlhmarket"] !== void 0 ? "rlhmarket" : "rlh-market";
			...
			const doRestore = (0, react.useCallback)(() => {
				...
				setRestoreErrors([]);
				return fetch("/rlh-market/restore", {
					method: "POST",
					headers: { "content-type": "application/json" },
			...
			const runWebdav = (0, react.useCallback)((action) => {
				...
				fetch("/rlh-market/webdav", {
					...
					if (action === "backup") {
						try {
							localStorage.setItem("rlhm-webdav-last", String(Date.now()));
						} catch {}
				...
				fetch("/rlh-market/gist", {
					...
					if (action === "export") {
						...
						if (typeof ref.gistId === "string" && ref.gistId !== "") {
							...
							setGistMode("update");
							try {
								localStorage.setItem("rlhm-gist-id", ref.gistId);
							} catch {}
						}
			...
			(0, react.useEffect)(() => {
				...
				let last = 0;
				try {
					last = Number(localStorage.getItem("rlhm-webdav-last")) || 0;
				} catch {}
			...
			*/
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
									...
									icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconCodeOutline16, { size: 14 }),
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								}) : busy ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
									variant: "primary",
									size: "sm",
									disabled: true,
						...
						busy && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: Market_module_css_default.spin,
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconLoadingOutline16, { size: 14 })
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
									className: Market_module_css_default.grow,
			...
			const bootEntries = typeof window !== "undefined" && window.__RLH_BOOT__ && Array.isArray(window.__RLH_BOOT__.entries) ? window.__RLH_BOOT__.entries : [];
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: Market_module_css_default.grow }),
								removingName === instName ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
									variant: "outline",
									size: "sm",
									disabled: true,
			...
			const groupableNames = Object.keys(installed).filter((name) => name !== "rlh-market" && name !== "rlhmarket");
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									(() => {
										const self = installed["rlhmarket"] !== void 0 ? "rlhmarket" : "rlh-market";
										return updates[self] && updates[self].updateAvailable && !updatedNames.includes(self) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "primary",
											size: "sm",
									...
									updatableNames.length >= 2 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "primary",
										size: "sm",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "outline",
										size: "sm",
										icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconDownloadOutline16, { size: 14 }),
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										...
										children: [t("tabInstalled") + (Object.keys(installed).length > 0 ? " (" + Object.keys(installed).length + ")" : ""), hasUpdates && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.StateDot, {
											state: "error",
											size: 7,
							...
							!envReady && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: Market_module_css_default.banner,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconCordisPluginOutline14, {
										size: 14,
										className: Market_module_css_default.bannerIcon
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									!envFailed && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "primary",
										size: "sm",
										disabled: envFixing,
										onClick: fixEnv,
							...
							tab === "installed" && pendingBackup !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: Market_module_css_default.banner,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconRefreshOutline14, {
										size: 14,
										className: Market_module_css_default.bannerIcon
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							hotUrls.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: Market_module_css_default.banner,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconSparkle16, {
										size: 14,
										className: Market_module_css_default.bannerIcon
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										...
										size: "sm",
										onClick: () => {
											sessionStorage.setItem("rlhm-toast", JSON.stringify(hotNames));
											sessionStorage.setItem("rlhm-tab", "installed");
											location.reload();
										},
										children: t("refresh")
							...
							pendingRestart > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Tooltip, {
										label: t("restartHint"),
										side: "bottom",
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: Market_module_css_default.bannerHint,
											children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconQuestionOutline14, { size: 14 })
										})
									}),
									restartEnabled && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "primary",
										size: "sm",
									...
									showHostPending && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
										variant: "ghost",
										size: "sm",
										...
										onClick: () => {
											...
											try {
												sessionStorage.setItem("rlhm-restart-dismissed", String(bootId ?? ""));
											} catch {}
										},
										children: t("dismiss")
									})
					...
					buildsSkipped !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: Market_module_css_default.banner,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconWarningOutline16, {
								size: 14,
								className: Market_module_css_default.bannerIcon
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
								...
								onClick: () => {
									...
									setBuildsSkipped(null);
									fetch("/rlh-market/approve-builds", {
										method: "POST",
										headers: { "content-type": "application/json" },
					...
					installError !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [installError, /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [staleName !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
								size: "sm",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: tab === "backup" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
													...
													icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconDownloadOutline16, { size: 14 }),
													disabled: backupBusy,
													onClick: () => downloadFile("/rlh-market/backup", "rlh-profile-backup.json"),
													children: backupBusy ? t("backupWorking") : t("backupDownload")
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
													variant: "outline",
													size: "sm",
													icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconFolderOpen16, { size: 14 }),
													disabled: backupBusy,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Menu, {
											open: presetOpen,
											onClose: () => setPresetOpen(false),
											onSelect: (id) => {
												const urls = {
													jianguoyun: "https://dav.jianguoyun.com/dav/rlh-profile-backup.json",
													koofr: "https://app.koofr.net/dav/Koofr/rlh-profile-backup.json",
													nextcloud: "https://nextcloud.example/remote.php/dav/files/USERNAME/rlh-profile-backup.json"
												};
												if (urls[id] !== void 0) setWebdavUrl(urls[id]);
											},
											align: "start",
											anchor: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
												variant: "outline",
												size: "sm",
												icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronDownOutline14, { size: 14 }),
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Input, {
											className: Market_module_css_default.backupInput,
											icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconLinkOutline14, { size: 14 }),
											type: "url",
											value: webdavUrl,
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: Market_module_css_default.backupActions,
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
												variant: "primary",
												size: "sm",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Input, {
									className: Market_module_css_default.tabSearch,
									icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconSearchOutline16, { size: 14 }),
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										...
										children: (() => {
											...
											return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Pill, {
													"data-chip": "1",
													active: cat === "all",
													onClick: () => setCat("all"),
													children: t("all")
												}),
												shown.map((id) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Pill, {
													"data-chip": "1",
													active: cat === id,
													onClick: () => setCat(id),
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
													...
													icon: catsOpen ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronUpOutline14, { size: 14 }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronDownOutline14, { size: 14 }),
										...
										anchor: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "outline",
											size: "sm",
											icon: filterOpen ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronUpOutline14, { size: 14 }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronDownOutline14, { size: 14 }),
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: totalPages > 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "outline",
											size: "sm",
											icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronLeftOutline14, { size: 14 }),
											disabled: currentPage === 1,
										...
										pageItems(currentPage, totalPages).map((item, i) => item === "…" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											...
											children: "…"
										}, "e" + i) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: item === currentPage ? "primary" : "outline",
											size: "sm",
											onClick: () => goToPage(item),
											children: item
										}, item)),
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "outline",
											size: "sm",
											disabled: currentPage === totalPages,
											onClick: () => goToPage(currentPage + 1),
											children: [t("nextPage"), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronRightOutline14, { size: 14 })]
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "outline",
											size: "sm",
							...
							installedView === "groups" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
												...
												children: [
													...
													/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
														...
														children: [
															...
															deletingGroup === gid ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
																variant: "primary",
																size: "sm",
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: [
												...
												off ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
													...
													}), act.state !== "live" && act.reasons.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.DisclosureRow, {
														icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconQuestionOutline14, { size: 14 }),
														title: t("actWhy"),
														open: whyOpen === name,
														expandable: true,
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: Market_module_css_default.grow }),
										toggleable && (name === "rlh-market" || name === "rlhmarket" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Tooltip, {
										...
										entry !== void 0 && entry.deprecated === true && entry.replacement !== void 0 && (() => {
											...
											return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
												variant: "outline",
												size: "sm",
												onClick: () => {
													setCat("all");
											...
											}), !isInstalled(replacement, installed) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
												variant: "outline",
												size: "sm",
										...
										}) : updatingName === name ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "primary",
											size: "sm",
											className: Market_module_css_default.warnBtn,
											disabled: true,
											children: t("updating")
										}) : status && status.updateAvailable ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "primary",
											size: "sm",
											className: Market_module_css_default.warnBtn,
										...
										!missing && name !== "rlh-market" && name !== "rlhmarket" && (removingName === name ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
											variant: "outline",
											size: "sm",
											className: Market_module_css_default.dangerBtn,
											disabled: true,
					...
					showTop && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Tooltip, {
						label: t("backTop"),
						side: "top",
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconChevronUpOutline14, { size: 16 })
							})
						})
					}),
					confirming !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(_deepseek_ai_rlh_client_ui_primitives.Modal, {
						open: true,
						onClose: () => {
							setConfirming(null);
							setCmdOpen(false);
						...
						footer: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Button, {
							variant: "ghost",
							onClick: () => {
								setConfirming(null);
								setCmdOpen(false);
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.DisclosureRow, {
								icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconCodeOutline16, { size: 16 }),
								title: t("cmdDetails"),
								open: cmdOpen,
								expandable: true,
							...
							looksTerminal(confirming, lang) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
								className: Market_module_css_default.warnLine,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconCodeOutline16, {
										size: 14,
										className: Market_module_css_default.bannerIcon
									}),
									" " + t("terminalWarn") + " ",
					...
					removeConfirm !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Modal, {
						open: true,
					...
					restoreConfirmOpen && pendingBackup !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Modal, {
						open: true,
					...
					exportOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(_deepseek_ai_rlh_client_ui_primitives.Modal, {
						open: true,
					...
					exportState === "done" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Toast, {
						text: t("exportedLog"),
						icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconCheckOutline16, { size: 14 }),
						onDone: exportToastDone
					}),
					exportState === "fail" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.Toast, {
						text: t("exportLogFail"),
						icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_rlh_client_ui_primitives.IconWarningOutline16, { size: 14 }),
						onDone: exportToastDone
					})
				]
			});
		...
		/**
		* The market's card on the plugin configuration page (rlh >= 0.1.0-rc.7).
		*
		...
		* that anybody can act on without knowing how RLH is put together.
		*
		...
		* import from `rlh-client-ui-settings-plugins` would fail the client
		...
		*/
		...
		const BROWSER_KEYS = ["rlhm-webdav", "rlhm-gist-id"];
		const CHANNELS = [
			"stable",
			"beta",
			"dev"
		...
		*/
		...
		function SettingsCard({ t, onRemoved }) {
			...
			(0, react.useEffect)(() => {
				...
				(async () => {
					try {
						const body = await (await fetch("/rlh-market/status", { cache: "no-store" })).json();
					...
					try {
						const body = await (await fetch("/rlh-market/updates", { cache: "no-store" })).json();
						const own = body.updates?.["rlhmarket"] ?? body.updates?.["rlh-market"];
			...
			const onUpdate = (0, react.useCallback)(() => {
				...
				(async () => {
					try {
						const body = await post("/rlh-market/update", { name: "rlhmarket" });
			...
			const onRemove = (0, react.useCallback)(() => {
				...
				(async () => {
					try {
						const body = await post("/rlh-market/self-uninstall", {
							confirm: true,
							purge
						});
						if (body.ok === true) {
			...
			const refreshUpdate = (0, react.useCallback)(async () => {
				const body = await (await fetch("/rlh-market/updates?force=1", { cache: "no-store" })).json();
				const own = body.updates?.["rlhmarket"] ?? body.updates?.["rlh-market"];
			...
			*/
			const onChannel = (0, react.useCallback)((next) => {
				...
				(async () => {
					try {
						const body = await post("/rlh-market/channel", { channel: next });
						if (body.ok !== true) {
							setError(body.error ?? t("setSelfFailed"));
							return;
						}
			...
			}, t(CHANNEL_LABEL[id]))))), row(t("setSelfRemove"), t("setSelfRemoveHint"), phase === "confirming" || busy ? null : (0, react.createElement)(_deepseek_ai_rlh_client_ui_primitives.Button, {
				variant: "outline",
				size: "sm",
			...
			}, t("setSelfCancel")), (0, react.createElement)(_deepseek_ai_rlh_client_ui_primitives.Button, {
				variant: "primary",
				size: "sm",
				className: Market_module_css_default.setDanger,
				disabled: busy,
				icon: busy ? (0, react.createElement)("span", { className: Market_module_css_default.spin }, (0, react.createElement)(_deepseek_ai_rlh_client_ui_primitives.IconLoadingOutline16, { size: 16 })) : void 0,
				onClick: onRemove
		...
		*/
		...
		function apply(ctx) {
			const gaps = missingPrimitives(_deepseek_ai_rlh_client_ui_primitives);
			if (gaps.length > 0) {
				console.warn("[rlh-market] host ui-primitives missing " + gaps.join(", ") + " — market section disabled (rlh web >= 0.1.0-rc.6 required)");
				return;
			}
			ctx.effect(() => ctx.locale.register(NS, {
				zh,
				en
			}), "rlh-market: dictionaries");
