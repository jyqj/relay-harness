...
One model-visible message is excluded: the agent loop's dynamic runtime-context snapshot. The same composition emits it on macOS and not on Linux, which the required lane runs, so no single expected output can carry it. That difference is a defect in its own right ([#2488](https://github.com/relay-harness/relay-harness/issues/2488)) — this expected output covers every other model-visible message rather than waiting for it.
...
Assistant and tool message text is no longer compared, and the runtime-context snapshot is not compared at all. The scenario's own assertions continue to own persistent-shell state, editor output, and the final response; [#2488](https://github.com/relay-harness/relay-harness/issues/2488) owns the excluded message until its platform difference is resolved.
