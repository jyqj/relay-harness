...
One surface escaped the codemod entirely: the DOM attribute `data-ds-dark-theme` had no rule (the codemod mapped the CSS custom-property prefixes `--ds-`/`--dsw-`/`--dsh-`/`--dsl-` but not `data-ds-` attributes), so 86 occurrences across 50 files kept the old token until 2026-08-26, when they were renamed to `data-rl-dark-theme` in one pass anchored on the `DARK_ATTRIBUTE` constant in `packages/client/ui-layout/src/client/theme-presenter.ts`.
