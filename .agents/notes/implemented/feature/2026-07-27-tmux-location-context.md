...
`@relay-harness/rlh-tmux-context` is an opt-in function plugin in `packages/context/tmux-context/`, alongside the other bounded request-context enrichments that define neither a tool nor a service. The shipped TUI mounts it because terminal-multiplexer context is specific to that surface; `rlh-agent-spine-demo` and the Web/headless surfaces stay silent.
