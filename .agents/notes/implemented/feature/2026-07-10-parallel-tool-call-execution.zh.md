...
这个一元分类器仍然可以感知输入。工具可以将只读操作分类为并行，将变更操作分类为独占。关系安全使用独立的 `resourceIntents` 约定：调用进入并行池，随后公平读写锁会串行化相同的规范 key，同时让无关 key 重叠（[决策](../architecture/2026-08-21-tool-resource-intent-locks.md)）。两种机制都未声明的调用仍按独占方式执行。

`defineTool()` 先验证参数，再调用类型化分类器。无效参数会被归为独占，且只有该调用真正执行时才会产生常规参数错误。`ctx.tools.executionMode(exec)` 会为直接检查调用解析当前有效的工具定义；agent loop 则通过向模型提供 schema 的请求级快照进行分类。未知工具将按安全侧原则归为独占。[请求工具快照](../architecture/2026-08-21-request-tool-snapshot.md)负责捕获与 HMR 身份。

使用带标签的模式，而不是公开布尔型调度器 API，使得以后可以表达感知资源的变体，无需改变分类器约定。

## 调度与顺序

循环会等待完整的 assistant 消息，对每个调用只解析一次，为每个调用创建独立的 `ToolExecution`，再按模型顺序扫描。连续的并行调用组成一组；每个独占调用单独组成一组，并构成顺序屏障。各组按顺序执行。每次启动前，分类都会通过声明该定义的同一请求快照计算。注册表变更不能替换或重新分类该响应中仍待处理的调用；它从下一次提示词组装起生效。
...
Code Mode 会呈现一个原生 `run_code` 调用，随后通过绑定请求快照的 staged scheduler 调度其嵌套 binding 调用，其有序 pre／post lane、独占屏障与有界并行分派都和原生调用一致。`mode: 'both'` 中的原生并列调用使用循环调度器。
...
当前实现中的声明保持保守。Web 搜索、Web 获取、文件系统读取、会话查询的 trace/read 工具和 subagent 委派选择并行；委派之所以并行，是因为子 agent 在自己的会话中工作，其运行绝不变更父会话，并列委派间的工作区协调由模型负责（[并行 subagent Agent Note](2026-08-09-parallel-subagent-delegations.md)）。文件系统 read／write／edit 与字符串替换编辑器使用规范提供方 key 资源锁，因此无关目标可以重叠，冲突会被串行化。Bash 工具、会话查询 search 工具、工作流、用户交互、todo 变更与 Cordis 变更工具仍按独占方式执行。Code Mode 自身是一个外层调用，其嵌套 scheduler 会应用捕获的分类器与资源锁。Bash 没有已证明的输入敏感分类器，因此仍按独占方式执行。

文件系统读取依赖一个范围很窄的记录器例外：其同步观察更新可以不按顺序结算，但写入和编辑在变更前会重新检查已观察的版本，因此陈旧状态只会导致 `FS_STALE_VERSION`。

## 验证

单元测试固定了按安全侧原则进行的分类、类型化参数验证、分组、屏障、注册表替换期间的请求快照稳定性、滚动上限、独立执行对象、中间件顺序、有序结果与上下文、中止排空，以及调度器故障后的完全停稳。第一方测试固定了每项并行声明。

快照测试固定了可见的多调用 transcript（文本记录）：待处理调用可以重叠执行，已完成结果仍按模型顺序排列。Code Mode 测试固定了其捕获的嵌套调度器与有序结果边界。此调度属于确定性循环行为，因此无需依赖提供方的 e2e 测试。
...
**让 scheduler 推断并列冲突。** 不予采用，因为参数名与路径拼写无法建立共享资源身份。改为采用显式、由工具负责的 `resourceIntents`：提供方解析规范 key，runtime 持有通用冲突语义。
...
一元路径仍遵循安全侧原则，对工具作者也很简单。拥有提供方支持的规范身份时，工具可以选择资源锁；两者都未声明的工具保持独占。工具过于宽泛地返回 `true`，或声明错误资源 key，仍可能暴露潜在共享状态竞态。
...
工具注册是模型请求边界。注册表变更从下一份请求快照起生效；已经采样的响应中，每个调用都会保留模型看到的定义与分类。
