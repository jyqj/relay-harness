...
主进程关闭处理通过 `hideOnClose` 读取当前配置。托盘隐藏只收起窗口。退出——`closeToTray: false` 时点标题栏关闭、托盘「退出」，或应用菜单退出——会设立退出标志、先铺一层全屏「关闭中」遮罩，再执行 `rlh.stop()`，然后 `app.quit()`。`before-quit` 仍是唯一的服务拆除点。遮罩在拆除开始前画上，因此偏慢的 `rlh.stop()` 不会看起来像卡住。

遮罩 CSS 使用 `currentTheme()` 给出的当前浅色或深色具体颜色，不会回退到深色画布。插入后，若页面上的 `--rlw-alias-*` token 能解析，遮罩脚本会用它们覆盖。
...
**退出后让残留 rlh 继续跑。** 否决：下次启动就要抢端口或换端口，而且用户要求退出时停掉服务。
...
没有 Playwright 或 Electron e2e 覆盖画出的遮罩，也没有覆盖退出过程中的 `rlh.stop()`。
