...
Relay Harness uses the same primitive so project-specific review, plugin-authoring, and tool-usage guidance lives next to the workspace or the user's agent configuration instead of being hard-coded into the loop.

## Decision

`@relay-harness/rlh-skill` is the pure provider registry (`ctx.skills`), `@relay-harness/rlh-skill-filesystem` is the shipped local filesystem provider, and `@relay-harness/rlh-tool-skill` owns the durable session catalog and model-facing loader tool. `rlh-agent-spine-demo` loads the registry, local provider, and consumer by default so TUI, headless, and ACP apps get the same behavior while embedded or remote providers contribute skills without changing the registry or consumer. Its `skills` config forwards `registry`, `local`, and `tool` branches to those owners.

Dedicated packaged providers can contribute immutable skills without filesystem discovery. The shipped CLI declares `@relay-harness/rlh-skill-badge` disabled by default; enabling its composition row contributes the official badge instructions through the same registry and consumer ([decision](2026-08-06-bundled-rlh-badge-skill.md)).
...
The local provider scans cwd-sensitive project roots, custom roots, and user roots in first-wins rank order: project `.rlh`, project `.agents`, `customSkillDirs`, user `.rlh`, then user `.agents`. The user `.rlh/skills` scan skips `.system` so a system-owned directory is not treated as normal user content. The local provider does not synthesize built-in system skills; configured bundled roots and dedicated providers supply additional skills.
...
**Materialize built-in RLH authoring skills under `~/.rlh/skills/.system`.** Rejected because bundled skills do not write user home on startup, and embedded or remote providers supply configured skills.
