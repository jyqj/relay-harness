...
- `rlh-subagent` gains optional peer types on `rlh-sandbox-policy` and `rlh-user-approval` (the `ctx.get` pattern the one-shot driver used); `rlh-subagent-in-process-driver` drops its policy-service peers and type imports entirely and delegates to the shared helpers.
...
- Out-of-process providers (`acp`, `rlh-sdk`, `claude-code`, `codex`) support no continuable children (`prepareContinuable` absent), and their one-shot children keep their own deployment policy (`inheritsParentContext = false`); cross-process policy propagation remains out of scope.
