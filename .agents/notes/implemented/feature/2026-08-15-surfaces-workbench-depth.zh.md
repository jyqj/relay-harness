...
**预装 `rlh-better-sidebar`。** 否决：它 portal 出官方列、钉在更旧的 harness、拒绝 localhost（与我们的 Browser 相反），也没有 push。要借鉴的是 `openPath` 漏斗和 Tab 壳，不是那套宿主。
