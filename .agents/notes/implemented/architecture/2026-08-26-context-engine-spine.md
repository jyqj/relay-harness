...
AgentLoop has one optional seam in `preStep()`: after `this.inbox.claim(...)` and before the tool snapshot capture and `systemPrompt.assemble`, the loop reads `this.loopCtx.get('contextEngine')` and, when present, awaits `prepareStep({ purpose: 'agent_step', messages: claimed, signal, cwd })`. Prompt Enhancement uses the same required-purpose entry with `prompt_enhancement`, so contributors receive caller intent without parsing prose. Contributed messages append to the step's user messages ahead of the runtime-context snapshot, flow through the ordinary `agent/pre-step` waterfall, and are recorded as durable `user/message` events, so model-visible ⟺ logged holds. An accepted preparation also becomes the log-only `context/prepared` fact described by the [durable context-preparation trace decision](2026-08-29-durable-context-preparation-trace.md). A step with no contributions, or a deployment without the service, is byte-identical to one without the seam.

The loop discovers the engine through the same optional-service pattern as `visionFallback`; its structural service face uses a type-only dependency on the ContextEngine package so the returned attribution, evidence, coverage, and durable-event vocabulary cannot drift.

The seam itself is `@relay-harness/rlh-context-engine` (`ctx.contextEngine`, Service Definition): a contributor registry (`registerContributor`, unique ids, disposers, all-or-nothing validation) and `prepareStep`, which runs contributors sequentially in registration order so packed message order is deterministic across restarts. Each result retains contributor identity around its message, evidence, and optional coverage, while direct aggregate arrays remain available to consumers. Its `src/types.ts` owns the protocol vocabulary migrated from the audited reference designs: `ResourceRef`/`Evidence` (revision-bound observations, `unverified` as an explicit state), `CoverageRecord`/`NegativeFinding` (zero hits read as "not found here", never "does not exist"), and `ProviderHealthState`/`ProviderGeneration`/`ProviderExplain` (two-clock generation, stable-token truncation reasons) for the knowledge-provider adapters later phases will add.
