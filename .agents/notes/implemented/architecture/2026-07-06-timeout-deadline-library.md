...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `rlh-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `rlh-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `rlh-timeout` (`idleWatchdog`) |
| Clear the timer | `rlh-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `rlh-timeout` (`timeoutOf`) |
