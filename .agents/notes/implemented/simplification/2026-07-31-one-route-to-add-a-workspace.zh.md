...
**菜单的存在是为了在多个目标之间消歧。** 当只剩添加这一个入口时——从不提供「无工作目录」的仅添加侧边栏表层——锚点手势*就是*该动作：直接拉起流程，不渲染浮层。只有一行的浮层多花一次点击，却没有任何可选项。主视觉区空列表保留菜单，因为「无工作目录」和「添加工作区」是两个选项（[无目录会话](../feature/2026-08-15-no-directory-task-sessions.md)）。这条规则是一个谓词（`addIsTheOnlyEntry`，存在无目录入口时为假）覆盖两处表层，而不是逐表层特判。
...
`workspace.create` 只接受 `{ path }`；wire schema 与 `WorkspaceApi` 没有 `name` 成员。网关没有 `workspaceRoot` 配置；客户端约定只通过 `WorkspaceCreateInput`、`WorkspaceRuntime.create` 与 `intentName` 提供按路径接纳，`rlh web` 没有 `--workspace-root` flag。`workspace-name-conflict` 仍保留在 wire 上，作为 `workspace.rename` 的标题重名错误。
