...
- **Six merge-extensible maps** (~370 LOC of core types): `ContentBlockMap`, `MessageSourceMap`, `FinishReasonMap` (in `rlh-llm`); `TurnTriggerMap`, `TurnEndReasonMap`, `SessionEventMap` (in `rlh-session`).
- **~10 `declare module` augmentation sites** across `rlh-agent`, `rlh-agent-loop`, `rlh-shell`, `rlh-llm`, `rlh-session`, `rlh-session-persistence`, `rlh-system-prompt`, `rlh-tools` — each would move from declaration merging to a runtime `register()` call.
- **The event producers** — 16 `session.append(...)` call sites in the loop — unchanged in shape but now validated at the boundary.
- **~7 switch-consumers** that branch on these unions: `deriveMessages` and the package-owned invariant companion (`rlh-session`), `BlockAssembler` (`rlh-llm`), both LLM adapters (`rlh-llm-deepseek`, `rlh-llm-pi-ai`), and the tool schema layer (`rlh-tools`). The `assertNever`-on-closed-unions vs fall-through-on-extensible-unions convention (a documented lint rule) would need rethinking — runtime variants are not statically exhaustive.
- **The `defineTool` `InferArgs` DSL** (`rlh-tools`), which derives zero-cast `execute` arg types from a compile-time schema spec — the showcase of the current approach.
