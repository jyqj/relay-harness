...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`rlh-llm-mock-server` `pause()`, `rlh-lsp-stdio`, `rlh-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
