 ...
 *
 * Official in-box bundles (@relay-harness/rlh-base, @relay-harness/rlh-web-app,
 * @relay-harness/rlh-headless) are fixed: they keep their exact positions in
...
/** Profile bundles that ship with the rlh host and must stay put (#98). */
...
export function readBundleStack(profileDir) {
    try {
        ...
        const bundles = Array.isArray(manifest.rlh?.profile?.bundles)
...
/**
 * Locate the rlh host installation from the process entry (same source as
 * rlh-cli.ts / check.ts): walk up from dirname(argv[1]) until a package.json
 * named @relay-harness/rlh is found.
 */
function findRlhInstallDir(entry = process.argv[1]) {
    if (entry === undefined)
        return null;
...
/**
 * The bundle package.json, resolved the way the rlh boot resolves bundles
 * (rlh-app-boot's resolveBundleDir, mirrored by check.ts): the rlh
...
function resolveBundlePackageJson(profileDir, name) {
    const rlhInstall = findRlhInstallDir();
    const anchors = [
        rlhInstall !== null ? join(rlhInstall, 'package.json') : null,
 ...
 * (`rlh.bundle.order.{before,after}` — a list of bundle package names).
...
export function readBundleRules(profileDir) {
    ...
    for (const name of bundles) {
        ...
        try {
            ...
            const order = manifest.rlh?.bundle?.order;
...
export function applyBundleOrder(profileDir, newOrder) {
    ...
    try {
        ...
        manifest.rlh.profile.bundles = merged.bundles;
