 ...
 *
 * Pure filesystem analysis of one rlh profile directory; no processes, no
 * network, no writes. It answers, for the profile the market is serving:
 *
 *  1. What is the actual bundle stack (rlh.profile.bundles order) and where
 ...
 *  3. Does any installed plugin pull a RLH host core package
 *     (@relay-harness/rlh, @relay-harness/rlh-tools, @relay-harness/cordis, …) in
 *     as an ordinary dependency — the rlh-excel-chat failure mode where the
 ...
 *
 * The composition step mirrors @relay-harness/rlh-app-boot's applyEntryPatches
...
/** js-yaml dialect for `!!js` scalars — identical to rlh-app-boot's entryListSchema. */
...
/** Parse one entry-list patch file with the rlh dialect; null when unreadable. */
...
/** RLH host core packages: what the rlh installation ships under @relay-harness. */
export function corePackageNames(rlhInstallDir) {
    const names = new Set([
        ...
        '@relay-harness/rlh-app-boot',
        '@relay-harness/rlh-home-paths',
        '@relay-harness/rlh-launch-environment',
        '@relay-harness/rlh-cmdline',
        '@relay-harness/rlh-tools',
        '@relay-harness/rlh-llm',
        '@relay-harness/rlh-system-prompt',
        '@relay-harness/rlh-attachment',
        '@relay-harness/rlh-agent',
        '@relay-harness/rlh-agent-loop',
        '@relay-harness/rlh-session',
        '@relay-harness/rlh-subagent',
        '@relay-harness/cordis',
        '@relay-harness/cordis-plugin-loader',
        '@relay-harness/cordis-plugin-include',
        '@relay-harness/cordis-plugin-hmr',
        '@relay-harness/cordis-plugin-timer',
        '@relay-harness/cordis-plugin-group',
    ]);
    if (rlhInstallDir === null)
        return names;
    try {
        // The install's own node_modules is the authoritative host-core inventory:
        // everything under @relay-harness whose bare name starts with rlh or cordis,
        // plus the app package itself.
        for (const entry of readdirSync(join(rlhInstallDir, 'node_modules', '@relay-harness'), { withFileTypes: true })) {
            if (!entry.isDirectory() && !entry.isSymbolicLink())
                continue;
            if (/^(?:rlh|cordis)/.test(entry.name))
                names.add(`@relay-harness/${entry.name}`);
        }
    }
    catch { /* install node_modules unreadable — the curated seed stands */ }
    try {
        const manifest = JSON.parse(readFileSync(join(rlhInstallDir, 'package.json'), 'utf8'));
...
/**
 * Locate the rlh host installation from the process entry (the same source
 * rlh-cli.ts uses to re-invoke the CLI): walk up from dirname(argv[1]) until
 * a package.json named @relay-harness/rlh is found.
 */
export function findRlhInstallDir(entry = process.argv[1]) {
    if (entry === undefined)
        return null;
    ...
    for (let depth = 0; depth < 10; depth += 1) {
        try {
            ...
            if (manifest.name === '@relay-harness/rlh')
                return dir;
        }
...
/**
 * Resolve one bundle package's directory the way the rlh boot does
 * (rlh-app-boot's resolveBundleDir): probe Node's own node_modules search
 ...
 * there when the profile is a workspace member — the rlh layout keeps
...
function installedPackageNames(profileDir) {
    ...
    try {
        root = readdirSync(join(profileDir, 'node_modules'), { withFileTypes: true })
            .filter(entry => isPkgDir(entry) && entry.name !== '.bin' && entry.name !== '.pnpm' && entry.name !== '.rlh-plugin-backups')
...
/**
 * Apply the layer stack over an empty root exactly like the rlh boot include.
...
export function composeLayers(layers) {
     ...
     * inserts overwrite the map entry — exactly rlh-app-boot's applyEntryPatches
...
/** Distinct versions of `@relay-harness/{rlh,cordis}*` packages in the lockfile. */
function lockfileCoreVersions(profileDir) {
    ...
    for (const m of text.matchAll(/(@relay-harness\/(?:rlh|cordis)[^@\s'"]*?)@([0-9][^\s:'"()]*)/g)) {
        const name = m[1] ?? '';
        // pnpm v9 peer-resolution keys carry a suffix: `name@1.0.0(peer@x)`. The
        // version capture stops at the `(` — and at `)` too, so the peer
        // reference INSIDE the suffix (`name@1.0.0(@relay-harness/rlh-base@4.0.1)`)
 ...
 * the rlh installation anchor first (in-box bundles always come from the
 * running rlh, never a profile-local copy), then Node's module search from
 ...
 */
export function buildBundleLayers(profileDirectory, bundleNames, specs, rlhInstallDir) {
    const bundles = bundleNames.map((name) => {
        const anchors = [
            rlhInstallDir !== null ? join(rlhInstallDir, 'package.json') : null,
        ...
        if (typeof declared !== 'string') {
            layer.error = 'bundle declares no rlh.bundle.patch — the profile will fail to boot';
            return layer;
        }
        ...
        const order = bundleManifest.rlh?.bundle?.order;
...
export function analyzeProfile(profileDirectory, options = {}) {
    const rlhInstall = options.rlhInstallDir ?? findRlhInstallDir();
    const home = options.homeDir ?? process.env.RLH_HOME ?? join(homedir(), '.rlh');
    const core = corePackageNames(rlhInstall);
    // --- 1. bundle stack ---
    const manifest = (() => {
        try {
    ...
    const bundleNames = Array.isArray(manifest?.rlh?.profile?.bundles)
        ? manifest.rlh.profile.bundles.filter((name) => typeof name === 'string')
        : [];
    const specs = manifest?.dependencies ?? {};
    const built = buildBundleLayers(profileDirectory, bundleNames, specs, rlhInstall);
    ...
    for (const plugin of installedPackageNames(profileDirectory)) {
        ...
        for (const [name, spec] of Object.entries(map)) {
            ...
            const host = rlhInstall !== null ? readNodeModulesVersion(rlhInstall, name) : null;
