 ...
 * process spawning lives in rlh-cli.ts, filesystem reads in profile.ts,
...
import { BOOT_ID, cancelActive, probePnpm, progress, provisionPnpm, runRlhPlugin, } from './rlh-cli.js';
...
const SELF_NAMES = new Set(['rlhmarket', 'rlh-market']);
/**
 * Whether an installed package declares a client part (`rlh.client`). Its UI
...
function packageHasClientPart(profileDirectory, name) {
    try {
        ...
        return manifest.rlh?.client !== undefined;
    }
    catch {
        return false;
    }
...
export function mountMarketRoutes(host, config, commandRuntime) {
    // Ordinary RLH profile names cross the CLI boundary and keep the legacy
    ...
    if (config.profileDirectory === undefined && !PROFILE_RE.test(config.profile)) {
        throw new Error(`rlh-market: invalid profile name: ${config.profile}`);
    }
    ...
    // here so RLH's own HMR re-composes the tree (no restart) and the loader
    ...
    const commands = commandRuntime ?? { runPlugin: runRlhPlugin, probePnpm, provisionPnpm, cancelActive };
    ...
    // Client-only packages (rlh.client without rlh.bundle) are invisible to the
    ...
    // Self-healing guard: rlh's own patch overlay can re-update entries during
    ...
    function liveNames() {
        ...
        for (const entry of host.loader.entries()) {
            ...
            // itself (#156: @tt-a1i/archify-rlh inserts an entry named
            // @relay-harness/rlh-skill-filesystem), so its own name never appears
     ...
     * (e.g. `rlh plugin remove` in a terminal): the stale mount would keep
    ...
    async function restoreBackup(value) {
        ...
        try {
            ...
            const desiredBundles = [...(manifest.rlh?.profile?.bundles ?? [])];
            const dependencyNames = new Set(dependencies.map(([name]) => name));
            manifest.dependencies = {};
            if (Array.isArray(manifest.rlh?.profile?.bundles)) {
                manifest.rlh.profile.bundles = desiredBundles.filter(bundle => !dependencyNames.has(bundle));
            }
                ...
                try {
                    ...
                    if (item.exitCode === 0 && !item.timedOut && !item.cancelled
                        ...
                        if (desiredBundles.includes(name)) {
                            ...
                            current.rlh ??= {};
                            current.rlh.profile ??= {};
                            current.rlh.profile.bundles ??= [];
                            if (!current.rlh.profile.bundles.includes(name))
                                current.rlh.profile.bundles.push(name);
    ...
    const disposers = [
            ...
            handler: (request, response) => {
                ...
                try {
                    ...
                    response.writeHead(200, {
                        ...
                        'content-disposition': `attachment; filename="rlh-rlhmarket-backup-${timestamp}.json"`,
                    });
                    response.end(backup);
                }
                catch (error) {
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (body.action === 'backup') {
                        ...
                        // posts it to /rlh-market/restore, where downloadWebdav's strict
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (body.action === 'export') {
                        ...
                        // posts it to /rlh-market/restore; readGist's strict validation
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                // the rlh CLI) that state.json never sees.
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                // #125 hardening (lesson from #122: a bad order write can stop RLH
                ...
                try {
                    ...
                    // broken manifest can never stop RLH from starting (issue #125,
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (name === 'rlh-market' || name === 'rlhmarket') {
                        sendJson(response, 400, { error: 'the market cannot be disabled from its own page; use the rlh CLI' });
                        return;
                    }
                    ...
                    // which RLH's HMR applies within ~1s AND the loader re-applies on
        ...
        host.webServer.register({
            ...
            handler: (request, response) => {
                ...
                response.writeHead(200, {
                    ...
                    'content-disposition': 'attachment; filename="rlh-market-log.txt"',
                });
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[rlh-market] update failed: ${message}`);
         ...
         * never as a stray `{ name: "rlhmarket" }` on the ordinary path.
         *
         ...
         * gone from `dependencies` and `rlh.profile.bundles`.
         */
        /**
        ...
        host.webServer.register({
            kind: 'exact',
            path: '/rlh-market/self-uninstall',
            handler: async (request, response) => {
                if (request.method !== 'POST') {
                    response.writeHead(405, { allow: 'POST' });
                    response.end();
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        const installed = readInstalled(config.profile, activeProfileDir);
                        const selfName = ['rlhmarket', 'rlh-market'].find(candidate => installed[candidate] !== undefined);
        ...
        host.webServer.register({
            kind: 'exact',
            path: '/rlh-market/approve-builds',
            handler: async (request, response) => {
                if (request.method !== 'POST') {
                    response.writeHead(405, { allow: 'POST' });
                    response.end();
            ...
            handler: async (request, response) => {
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        const name = typeof body.name === 'string' ? body.name : '';
                        if (name === 'rlh-market' || name === 'rlhmarket') {
                            sendJson(response, 400, { error: 'the market cannot uninstall itself; use the rlh CLI' });
                            return;
                        }
                        if (readInstalled(config.profile, activeProfileDir)[name] === undefined) {
                            sendJson(response, 400, { error: 'plugin is not installed' });
                        ...
                        if (ok) {
                            ...
                            // wedge the whole page until a rlh restart (#37 by
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[rlh-market] uninstall failed: ${message}`);
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        if (aliasOf !== null) {
                            ...
                            try {
                                ...
                                active = (manifest.rlh?.profile?.bundles ?? []).includes(aliasOf) || liveNames().has(aliasOf);
                            }
                            catch {
                        ...
                        // plugins sharing one name (both rlh-usage-stats, four rlh-memory…).
                        ...
                        if (result.exitCode === 0 && !result.timedOut && !cancelled) {
                            ...
                            if (removedBroken.length > 0) {
                                logEvent('warn', 'install', `${target}: removed uninstallable pieces (no rlh manifest or missing build artifacts): ${removedBroken.join(', ')}`);
                            }
                        ...
                        sendJson(response, ok || cancelled ? 200 : result.busy === true ? 409 : 502, {
                            ...
                            error: conflicts.length > 0
                                ? `「${conflicts[0].name}」与已安装的「${conflicts[0].owner}」使用了相同的 loader 条目 id（${[...new Set(conflicts.map(hit => hit.id))].join(', ')}），两者无法在同一个 profile 共存——装上会导致 RLH 下次启动失败，因此已自动移除。这类插件（例如终端 TUI 插件）请装到单独的 profile。 / "${conflicts[0].name}" declares the same loader entry id(s) as the installed "${conflicts[0].owner}" (${[...new Set(conflicts.map(hit => hit.id))].join(', ')}); they cannot coexist in one profile — keeping it would stop RLH from starting, so it was removed. Install this kind of plugin (e.g. a terminal TUI bundle) into its own profile.`
                                : notAPlugin
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[rlh-market] install failed: ${message}`);
