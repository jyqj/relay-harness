...
import { conflictingEntryIds, hasRlhManifest, hasLoadableEntry, pluginSubdirs, profileDir, readInstalled, readProfileBundles } from './profile.js';
 ...
 * no rlh surface), and pnpm installs the bare fileset with exit 0. Detect
...
export async function retargetCollections(run, profile, before, target, explicitDir) {
    ...
    const junk = Object.keys(readInstalled(profile, dir)).filter((name) => {
        ...
        if (!existsSync(join(root, 'package.json')))
            return true;
        return !hasRlhManifest(root);
    });
    let allOk = true;
    for (const name of junk) {
        const root = join(dir, 'node_modules', name);
        const candidates = pluginSubdirs(root);
        logEvent('info', 'install', `${name}: collection repo (root declares no rlh manifest); plugins inside: ${candidates.join(', ') || 'none'}`);
 ...
 * piece without a rlh manifest or without its declared entry artifact
 ...
 * a web profile (both declare `id: storage`) leaves RLH unable to START —
...
export async function validateAddedPlugins(run, profile, before, explicitDir) {
    ...
    for (const n of addedNow) {
        ...
        if (!hasRlhManifest(packageDir) || !hasLoadableEntry(dir, n)) {
