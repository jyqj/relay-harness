 ...
 * server-side. Backups therefore behave like `rlh export` and carry the same
...
export const BACKUP_FORMAT = 'rlh-profile-backup';
export const MAX_BACKUP_BYTES = 2 * 1024 * 1024;
const MAX_FILES = 256;
const SKIP_NAMES = new Set(['node_modules', '.rlh-market', '.git', 'pnpm-lock.yaml']);
...
export function createProfileBackup(profile, explicitDir, opts) {
    ...
    if (opts?.includeDeps !== undefined) {
        ...
        const rlh = manifest.rlh === null || typeof manifest.rlh !== 'object' || Array.isArray(manifest.rlh)
            ? undefined
            : manifest.rlh;
        const profileBlock = rlh?.profile === null || typeof rlh?.profile !== 'object' || Array.isArray(rlh?.profile)
            ? undefined
            : rlh.profile;
        ...
        if (rlh !== undefined) {
            filteredManifest.rlh = { ...rlh, profile: { ...(profileBlock ?? {}), bundles: filteredBundles } };
        }
...
export function restoreProfileBackup(profile, value, explicitDir) {
    ...
    try {
        for (const file of backup.files) {
            ...
            const temp = `${target}.rlh-restore-${String(process.pid)}`;
...
export async function uploadWebdav(url, username, password, backup) {
    ...
    if (response.status < 200 || response.status >= 300) {
        throw new Error(response.status === 404
            ? `WebDAV upload failed: HTTP 404 — the target folder does not exist and could not be created. Some providers (e.g. Jianguoyun) refuse files at the root: use a path inside a folder, e.g. https://dav.example.com/rlh/backup.json / 目标目录不存在且无法自动创建；部分服务商（如坚果云）不允许在根目录放文件，请使用形如 https://dav.example.com/rlh/backup.json 的子目录路径`
...
export async function downloadWebdav(url, username, password) {
    ...
    if (response.status < 200 || response.status >= 300) {
        throw new Error(response.status === 404
            ? 'WebDAV download failed: HTTP 404 — no backup at that path yet. Upload one first, and check the URL points at the backup FILE (…/rlh/backup.json), not its folder / 该路径下还没有备份文件。请先执行一次上传，并确认地址指向备份文件本身（…/rlh/backup.json）而不是目录'
...
export function extractPluginSelection(backup, includeDeps) {
    ...
    const rlh = json.rlh === null || typeof json.rlh !== 'object' || Array.isArray(json.rlh)
        ? undefined
        : json.rlh;
    const profileBlock = rlh?.profile === null || typeof rlh?.profile !== 'object' || Array.isArray(rlh?.profile)
        ? undefined
...
export function mergeRestoreManifest(backupManifest, current, selection) {
    ...
    const backupBundles = Array.isArray(backupManifest.rlh?.profile?.bundles)
        ? (backupManifest.rlh.profile.bundles)
        : [];
    const currentDeps = current.dependencies === null || typeof current.dependencies !== 'object' || Array.isArray(current.dependencies)
        ? {}
        : current.dependencies;
    const currentBundles = Array.isArray(current.rlh?.profile?.bundles)
    ...
    for (const name of sourceBundles)
        if (typeof name === 'string')
            bundles.add(name);
    const currentRlh = current.rlh === null || typeof current.rlh !== 'object' || Array.isArray(current.rlh)
        ? undefined
        : current.rlh;
    const currentProfile = currentRlh?.profile === null || typeof currentRlh?.profile !== 'object' || Array.isArray(currentRlh?.profile)
        ? undefined
        : currentRlh.profile;
    const backupRlh = merged.rlh === null || typeof merged.rlh !== 'object' || Array.isArray(merged.rlh)
        ? undefined
        : merged.rlh;
    const backupProfile = backupRlh?.profile === null || typeof backupRlh?.profile !== 'object' || Array.isArray(backupRlh?.profile)
        ? undefined
        : backupRlh.profile;
    const profileMerged = { ...(backupProfile ?? {}), ...(currentProfile ?? {}), bundles: [...bundles] };
    const rlhMerged = { ...(backupRlh ?? {}), ...(currentRlh ?? {}), profile: profileMerged };
    merged.rlh = rlhMerged;
    return merged;
}
