window.__ModuleLoader__.load({ id: "rlhbot", factory: (require) => {
  const module = { exports: {} };
  const exports = module.exports;
  const react = require("react");
  const primitives = require("@relay-harness/rlh-client-ui-primitives");
  ...
  const CSS = `
.rlhbot-page, .rlhbot-form, .rlhbot-bubble, .rlhbot-roster, .rlhbot-avatar-slot {
  --rlhbot-blob-ink: var(--rlw-alias-label-primary);
  --rlhbot-blob-brown: color-mix(in srgb, var(--rlw-static-amber-600) 70%, var(--rlw-static-neutral-bluish-1000));
  --rlhbot-blob-red: var(--rlw-static-red-500);
  --rlhbot-blob-orange: var(--rlw-static-amber-500);
  --rlhbot-blob-yellow: var(--rlw-static-amber-400);
  --rlhbot-blob-green: var(--rlw-static-green-500);
  --rlhbot-blob-teal: color-mix(in srgb, var(--rlw-static-green-500) 55%, var(--rlw-static-relay-500));
  --rlhbot-blob-blue: var(--rlw-static-relay-500);
  --rlhbot-blob-purple: color-mix(in srgb, var(--rlw-static-red-500) 40%, var(--rlw-static-relay-600));
  --rlhbot-blob-pink: color-mix(in srgb, var(--rlw-static-red-400) 70%, var(--rlw-static-neutral-bluish-00));
  --rlhbot-blob-grey: var(--rlw-alias-label-tertiary);
}
.rlhbot-page { display: flex; flex-direction: column; height: 100%; min-height: 0; color: var(--rlw-alias-label-primary); }
.rlhbot-toolbar { display: flex; align-items: center; gap: 8px; padding: 0 0 8px; }
.rlhbot-toolbar .rlhbot-search { flex: 1; min-width: 0; }
.rlhbot-list { flex: 1; min-height: 0; overflow: auto; display: flex; flex-direction: column; gap: 2px; }
.rlhbot-row { display: flex; align-items: center; gap: 10px; width: 100%; border: 0; background: transparent; color: inherit; text-align: start; padding: 8px 10px; border-radius: 10px; cursor: pointer; }
.rlhbot-row:hover, .rlhbot-row[data-active="true"] { background: var(--rlw-alias-interactive-bg-hover); }
.rlhbot-row-body { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.rlhbot-row-top { display: flex; align-items: baseline; gap: 8px; }
.rlhbot-name { font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rlhbot-time { margin-left: auto; flex-shrink: 0; font-size: 12px; color: var(--rlw-alias-label-tertiary); }
.rlhbot-preview { font-size: 12px; color: var(--rlw-alias-label-secondary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rlhbot-avatar-frame { display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; overflow: visible; }
.rlhbot-avatar-frame[data-crop="circle"] { border-radius: 50%; overflow: hidden; }
.rlhbot-avatar-frame[data-crop="square"] { border-radius: 10px; overflow: hidden; }
.rlhbot-avatar-image { width: 100%; height: 100%; object-fit: cover; display: block; }
.rlhbot-avatar-frame[data-thinking="true"] .rlhbot-avatar-image { animation: rlhbot-image-think var(--rl-transition-duration-slow) var(--rl-ease-in-out) infinite alternate; }
@keyframes rlhbot-image-think { from { transform: scale(1); } to { transform: scale(1.12); } }
.rlhbot-blob { width: 100%; height: 100%; display: block; overflow: visible; }
.rlhbot-blob-eyes { transform: translate(var(--eye-x, 0px), var(--eye-y, 0px)); transition: transform var(--rl-transition-duration) var(--rl-ease-in-out); }
.rlhbot-blob-eye-white { fill: var(--rlw-static-neutral-bluish-00); stroke: var(--rlw-static-neutral-bluish-1000); stroke-width: 1.15; }
.rlhbot-avatar-frame[data-thinking="true"] .rlhbot-blob-eye {
  transform-box: fill-box;
  transform-origin: center;
  animation: rlhbot-blob-blink calc(var(--rl-transition-duration-slow) * 4) var(--rl-ease-in-out) infinite;
}
@keyframes rlhbot-blob-blink {
  0%, 70%, 100% { transform: scaleY(1); }
  76% { transform: scaleY(0.12); }
  82% { transform: scaleY(1); }
}
.rlhbot-blob-eye-pupil { fill: var(--rlw-static-neutral-bluish-1000); }
.rlhbot-picker { display: flex; flex-direction: column; gap: 12px; }
.rlhbot-picker-blob { display: flex; flex-direction: column; gap: 12px; }
.rlhbot-pills { display: flex; gap: 4px; flex-wrap: wrap; }
.rlhbot-shape-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
.rlhbot-shape-cell { aspect-ratio: 1; border: 0; border-radius: 12px; background: var(--rlw-alias-bg-module-platform); color: inherit; display: flex; align-items: center; justify-content: center; padding: 8px; cursor: pointer; }
.rlhbot-shape-cell[aria-pressed="true"] { box-shadow: inset 0 0 0 1px var(--rlw-alias-border-l2); background: var(--rlw-alias-interactive-bg-hover); }
.rlhbot-swatches { display: flex; flex-wrap: wrap; gap: 8px; }
.rlhbot-swatch { width: 28px; height: 28px; border-radius: 50%; border: 0; padding: 0; cursor: pointer; background: var(--swatch); box-shadow: inset 0 0 0 1px var(--rlw-alias-border-l2); }
.rlhbot-swatch[aria-pressed="true"] { box-shadow: inset 0 0 0 2px var(--rlw-alias-label-primary-foreground), 0 0 0 1px var(--rlw-alias-border-l2); }
.rlhbot-upload { display: flex; flex-direction: column; gap: 12px; }
.rlhbot-blob-ink { color: var(--rlhbot-blob-ink); }
.rlhbot-blob-brown { color: var(--rlhbot-blob-brown); }
.rlhbot-blob-red { color: var(--rlhbot-blob-red); }
.rlhbot-blob-orange { color: var(--rlhbot-blob-orange); }
.rlhbot-blob-yellow { color: var(--rlhbot-blob-yellow); }
.rlhbot-blob-green { color: var(--rlhbot-blob-green); }
.rlhbot-blob-teal { color: var(--rlhbot-blob-teal); }
.rlhbot-blob-blue { color: var(--rlhbot-blob-blue); }
.rlhbot-blob-purple { color: var(--rlhbot-blob-purple); }
.rlhbot-blob-pink { color: var(--rlhbot-blob-pink); }
.rlhbot-blob-grey { color: var(--rlhbot-blob-grey); }
@media (prefers-reduced-motion: reduce) {
  .rlhbot-blob-eyes { transition: none; }
  .rlhbot-avatar-frame[data-thinking="true"] .rlhbot-avatar-image,
  .rlhbot-avatar-frame[data-thinking="true"] .rlhbot-blob-eye { animation: none; }
}
.rlhbot-empty { padding: 24px 12px; color: var(--rlw-alias-label-secondary); font-size: 13px; line-height: 1.5; }
.rlhbot-roster { display: flex; flex-wrap: wrap; justify-content: center; gap: 16px; max-width: 480px; }
.rlhbot-roster-member { display: flex; flex-direction: column; align-items: center; gap: 8px; min-width: 72px; }
.rlhbot-roster-name { font-size: 13px; color: var(--rlw-alias-label-primary); text-align: center; max-width: 88px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rlhbot-error { padding: 8px 10px; color: var(--rlw-alias-state-error-primary); font-size: 12px; }
.rlhbot-rail { width: 36px; height: 36px; border: 0; border-radius: 10px; background: transparent; color: var(--rlw-alias-label-primary); display: flex; align-items: center; justify-content: center; }
.rlhbot-modal { max-height: calc(100vh - 48px); }
.rlhbot-form { display: flex; flex-direction: column; gap: 12px; min-width: min(420px, 100%); max-height: calc(100vh - 200px); overflow: auto; }
.rlhbot-field { display: flex; flex-direction: column; gap: 6px; }
.rlhbot-field label { font-size: 12px; color: var(--rlw-alias-label-secondary); }
.rlhbot-hint { font-size: 12px; color: var(--rlw-alias-label-tertiary); }
.rlhbot-members { display: flex; flex-direction: column; gap: 6px; max-height: 200px; overflow: auto; }
.rlhbot-member { display: flex; align-items: center; gap: 8px; font-size: 13px; }
.rlhbot-avatar-slot { position: relative; display: inline-flex; flex-shrink: 0; }
.rlhbot-badge {
  position: absolute;
  right: -2px;
  bottom: -2px;
  ...
  box-shadow: inset 0 0 0 1px var(--rlw-alias-border-l2);
}
.rlhbot-bubble { display: flex; flex-direction: row; align-items: flex-start; gap: 8px; padding: 0; background: transparent; max-width: 100%; }
.rlhbot-bubble-body { display: flex; flex-direction: column; gap: 4px; min-width: 0; flex: 1; }
.rlhbot-bubble-name { font-size: 12px; font-weight: 600; color: var(--rlw-alias-label-secondary); }
.rlhbot-bubble-text {
  font-size: 14px;
...
.rlhbot-bubble-text[data-pending="true"] { color: var(--rlw-alias-label-tertiary); }
.rlhbot-bubble-omit { display: none; }
.rlhbot-footer { display: flex; justify-content: flex-end; gap: 8px; }
`;

  function injectCss() {
    if (typeof document === "undefined") return;
    let el = document.querySelector('style[data-plugin-css="rlhbot"]');
    if (!el) {
      el = document.createElement("style");
      el.setAttribute("data-plugin-css", "rlhbot");
  ...
  function durationSlowMs() {
    ...
    const raw = window.getComputedStyle(window.document.documentElement)
      .getPropertyValue("--rl-transition-duration-slow")
      .trim();
  ...
  function AvatarView(props) {
    ...
    if (resolved.kind === "image") {
      return h("span", {
        className: "rlhbot-avatar-frame",
        "data-crop": resolved.crop,
        "data-thinking": thinking && live ? "true" : undefined,
        style: { width: px, height: px },
      }, h("img", { className: "rlhbot-avatar-image", src: resolved.dataUrl, alt: "" }));
    }

    return h("span", {
      className: `rlhbot-avatar-frame rlhbot-blob-${resolved.color}`,
      "data-thinking": thinking && live ? "true" : undefined,
      style: { width: px, height: px },
    }, h("svg", { className: "rlhbot-blob", viewBox: "0 0 64 64", "aria-hidden": "true" },
      h("path", { ref: pathRef, className: "rlhbot-blob-body", d: blobPath(resolved.shape, 0), fill: "currentColor" }),
      h("g", { ref: eyesRef, className: "rlhbot-blob-eyes" },
        h("g", { className: "rlhbot-blob-eye" },
          h("ellipse", { className: "rlhbot-blob-eye-white", cx: 25, cy: 30, rx: 5.2, ry: 6.4 }),
          h("circle", { className: "rlhbot-blob-eye-pupil", cx: 26.2, cy: 31, r: 2.45 }),
        ),
        h("g", { className: "rlhbot-blob-eye" },
          h("ellipse", { className: "rlhbot-blob-eye-white", cx: 39, cy: 30, rx: 5.2, ry: 6.4 }),
          h("circle", { className: "rlhbot-blob-eye-pupil", cx: 40.2, cy: 31, r: 2.45 }),
        ),
      ),
    ));
  }
  ...
  function AvatarPicker(props) {
    ...
    return h("div", { className: "rlhbot-picker" },
      h("div", { className: "rlhbot-pills" },
        h(Pill, { active: tab === "blob", onClick: () => setTab("blob") }, t("avatarBlob")),
        h(Pill, { active: tab === "upload", onClick: () => setTab("upload") }, t("avatarUpload")),
      ),
      tab === "blob"
        ? h("div", { className: "rlhbot-picker-blob" },
          h("div", { className: "rlhbot-shape-grid" },
            BLOB_SHAPES.map((shape) => h("button", {
              key: shape,
              type: "button",
              className: "rlhbot-shape-cell",
          ...
          h("div", { className: "rlhbot-swatches" },
            BLOB_COLORS.map((color) => h("button", {
              key: color,
              type: "button",
              className: "rlhbot-swatch",
              style: { "--swatch": `var(--rlhbot-blob-${color})` },
              "aria-label": color,
        ...
        : h("div", { className: "rlhbot-upload" },
          ...
          error ? h("div", { className: "rlhbot-error", role: "alert" }, error) : null,
        ),
    );
  }
  ...
  function newId() {
    return globalThis.crypto?.randomUUID?.() ?? `rlhbot-${Date.now().toString(36)}`;
  }
  ...
  function EmptyRoster(props) {
    ...
    return h("div", { className: "rlhbot-roster" },
      roster.map((item) => h("div", { key: item.id, className: "rlhbot-roster-member" },
        h(AvatarView, { avatar: item.avatar, name: item.name, seed: item.id || item.name, size: 48, live: false }),
        h("span", { className: "rlhbot-roster-name" }, item.name || item.title || item.id),
      )),
    );
  }
  ...
  function BotPage(props) {
    ...
    if (!wide) {
      return h("button", {
        type: "button",
        className: "rlhbot-rail",
        "aria-label": t("tab"),
        onClick: expandSidebar,
      }, h(IconAgentPresetOutline16, { size: 18 }));
    }

    return h("div", { className: "rlhbot-page" },
      h("div", { className: "rlhbot-toolbar" },
        h(Input, {
          className: "rlhbot-search",
          value: query,
      ...
      editor?.error ? h("div", { className: "rlhbot-error", role: "alert" }, editor.error) : null,
      visible.length === 0
        ? h("div", { className: "rlhbot-empty" }, t("empty"), h("div", { className: "rlhbot-hint" }, t("emptyHint")))
        : h("div", { className: "rlhbot-list", role: "list" },
          visible.map((item) => {
            ...
            return h("button", {
              ...
              h("span", {
                ...
                isRoom ? h("span", { className: "rlhbot-badge" }, t("roomBadge")) : null,
              ),
              h("span", { className: "rlhbot-row-body" },
                h("span", { className: "rlhbot-row-top" },
                  h("span", { className: "rlhbot-name" }, item.name),
                  h("span", { className: "rlhbot-time" }, formatTime(session?.updatedAt || item.updatedAt)),
                ),
                preview ? h("span", { className: "rlhbot-preview" }, preview) : null,
              ),
            );
          }),
        ),
  ...
  function EditorOverlay(props) {
    ...
    return h(Modal, {
      ...
      closeLabel: t("close"),
      className: "rlhbot-modal",
      footer,
    },
      isDelete
        ? h("p", null, t("confirmDelete"))
        : h("div", { className: "rlhbot-form" },
          h("div", { className: "rlhbot-field" },
          ...
          !isRoomCreate && !isRoom ? h("div", { className: "rlhbot-field" },
            h("label", null, t("title")),
            h(Input, { value: title, onChange: (event) => setTitle(event.target.value) }),
          ) : null,
          !isRoom ? h("div", { className: "rlhbot-field" },
            ...
            h("div", { className: "rlhbot-hint" }, t("personaHint")),
            h("textarea", {
              value: description,
              rows: 3,
              ...
              style: {
                ...
                padding: "8px 10px",
                borderRadius: "10px",
                border: "1px solid var(--rlw-alias-border-l2)",
                background: "var(--rlw-alias-bg-layer-1)",
          ...
          isRoom ? h("div", { className: "rlhbot-field" },
            ...
            h("div", { className: "rlhbot-hint" }, t("membersHint")),
            h("div", { className: "rlhbot-members" },
              botChoices.map((bot) => h("label", { key: bot.id, className: "rlhbot-member" },
                h("input", {
                  type: "checkbox",
          ...
          h("div", { className: "rlhbot-field" },
            ...
            workspaceLocked ? h("div", { className: "rlhbot-hint" }, t("workspaceLocked")) : null,
          ),
          !isRoom ? h("label", { className: "rlhbot-member" },
            t("notifications"),
            h(Switch, {
  ...
  function ParticipantBubble(props) {
    ...
    if (settled && !parsed.visible) {
      return h("span", { className: "rlhbot-bubble-omit", "aria-hidden": "true" });
    }
    return h("div", { className: "rlhbot-bubble" },
      h(AvatarView, { avatar: catalogAvatar, seed: botId, thinking, size: 32 }),
      h("div", { className: "rlhbot-bubble-body" },
        h("div", { className: "rlhbot-bubble-name" }, title),
        h("div", {
          className: "rlhbot-bubble-text",
  ...
  function hideRoomPresetList(api) {
    ...
    if (typeof list !== "function" || list.__rlhbotHidden) return;
      ...
      return {
        ...
        result: {
          ...
          value: {
            ...
            presets: presets.filter((row) => row.id !== "rlhbot-room"),
          },
        },
      };
    };
    wrapped.__rlhbotHidden = true;
  ...
  function apply(ctx) {
    ...
    ctx.effect(() => ctx.locale.register(NS, { zh, en }), "rlhbot: dictionaries");
    const t = ctx.locale.bind(NS);
    const catalog = ctx.settingsScope.bind({ namespace: "rlhbot" });
    ...
    const stampRoomPresets = (byId) => {
      ...
      for (const item of readItems()) {
        ...
        if (!row || row.agentPreset === "rlhbot-room") continue;
        note.call(sessionsSvc, item.sessionId, "rlhbot-room");
      }
    };
    ctx.effect(() => catalog.subscribe(() => stampRoomPresets()), "rlhbot: stamp room agentPreset");
    ctx.effect(() => {
      const list = (ctx.sessions ?? ctx.get("sessions"))?.list;
      if (!list?.subscribe) return undefined;
      return list.subscribe(() => stampRoomPresets());
    }, "rlhbot: stamp room agentPreset on list");
    stampRoomPresets();
      ...
      duplicateItem: async (itemId) => {
        ...
        try {
          const sessionId = await createContactSession(ctx, {
            ...
            agentPreset: source.kind === "room" ? "rlhbot-room" : undefined,
          });
          const now = Date.now();
          const copy = {
            ...source,
          ...
          if (workspaceChanged) {
            if (workspaceLocked || session?.blank === false) {
              ...
              sessionId = await createContactSession(ctx, {
                ...
                agentPreset: current.kind === "room" ? "rlhbot-room" : undefined,
              });
              next = { ...next, sessionId };
            }
          }
