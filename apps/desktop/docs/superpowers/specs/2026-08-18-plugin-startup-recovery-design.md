...
桌面端在首次安装、升级、或 `rlh plugin add` 之后，用户 profile 里的第三方 bundle 会把整棵 plugin tree 拖死。Harness 对组合失败 fail-loud 是对的。本设计让官方 Web UI 一定能起来，并且默认不改用户的 `package.json` / `cordis.patch.yml` 去猜该禁哪个插件。

视觉语言仍是官方 `rlh web`。启动页只改仪器画布上的状态文案和现有按钮语义，不增加市场、卸载或第二套皮肤。见 [design-language.md](../../design-language.md)。
...
2. **同一时刻只有一个 Loader 使用这份 `$RLH_HOME`。** 禁止换端口、同 home 双进程探测。
3. **每次启动都 heal 悬挂 bundle 名。** `rlh.profile.bundles` 里解析不到、又不是官方模板成员的名字从列表拿掉，`dependencies` 保留。这是数据修理，不是归因。
...
5. **就绪信号是 `rlh web:` 这一行，不是 HTTP 探活。** `web-runtime` 在 Loader 整棵树 `await()` 成功之后才打印该行。只认 HTTP 会在用户插件 apply 之前把状态标成 `ready`，随后 exit 1 被当成 runtime 崩溃并自动重启。
...
- 不在启动时跑 `rlh.compatibility` 当作恢复支柱。没有该声明的包会放行；缺 `inject` 不是 host feature。
- 不静态扫描社区插件。
- 启动页不做插件市场，不提供卸载。
- 不预装 `rlhmarket`。
...
`PROFILE_TEMPLATES.web` 是 `@relay-harness/rlh-base` 然后 `@relay-harness/rlh-web-app`。

恢复启动只解析这两层。桌面安装插件不是 bundle，活在托管 patch 里：完整启动走 profile 的 `cordis.patch.yml`；恢复启动跳过该文件，所以必须另用 launcher `--patch` 把同一段 insert 叠回去。id 仍是 `rlhd-desktop-plugin-install`。

`$RLH_HOME/cordis.patch.yml`（home 层，盖过 profile 层）在恢复启动里也不读。
...
```text
rlh web --skip-user-plugins --patch <desktop-install.yml> --host 127.0.0.1 --port 3080
```
...
|---|---|---|
| 模板 bundle | 清单里的 `rlh.profile.bundles` | 只解析 `PROFILE_TEMPLATES[name]`，不写回清单 |
...
同仓库的 vendor 切片带一条 implemented Agent Note，放在 `vendor/relay-harness/.agents/notes/implemented/`。
...
`RlhManager.waitUntilReady` 在看到 `rlh web: http://`（或 `https://`）之前不得把状态标为 `ready`。现有 HTTP 探活可以留作该行出现之后的确认，不能单独当作就绪。`attachOutput` 里任何 `127.0.0.1` URL 都算就绪的逻辑废止。
...
`buildLaunch` 的 source / 本机 `rlh` / npx 三条路径都把 `--skip-user-plugins` 和可选 `--patch` 放在 launcher 段（`web` 之后、`--host` / `--port` 之前）。app 旗标位置不变。
...
用户 bundle 名：目录或 `resolveBundleDir` 失败则从 `rlh.profile.bundles` 去掉，依赖条目不动。用户可随后 `rlh plugin install` 或在市场卸载。

`reconcileBundleLayers` 在「已经不是 dependency」之后不会删只存在于 bundles 的名字。heal 补的就是这个洞。不要在每次启动跑完整 reconcile：那会把新获得 `rlh.bundle` 的 plain 依赖激活进列表。

`stripDroppedPlugins` 继续按 `DROPPED` 从依赖和 bundles 两边删。它调用的 `stripManagedPatch` 仍剥 `# --- rlhd-gui-plugin-toggles ---`。本设计不往那一块写东西。第 3 轮热禁用必须换一块 **不会被** `stripDroppedPlugins` 剥掉的托管块，或停止无条件剥离。
...
1. 停 live child。
2. `rlh plugin --profile web add`。
3. 完整启动。
4. plugin-tree 失败 → `rlh plugin --profile web remove` 刚加的那个包 → 再完整启动。
5. 不要换端口探测，不要快照三份文件。撤销就是 `remove`。

`install_rlh_plugin` 与设置里的安装走同一条。

已经在 skip 模式时：

- **安装**：add 之后不要用「失败就 remove 新包」。完整组合事先就起不来，会误删新包。add 后保持 skip，启动页/横幅提示用「重试完整启动」检验。
- **卸载**：`rlh plugin remove` 后 **清 skip 并完整启动**。卸对了就恢复；还失败再写回 skip。
...
2. 桌面 heal、就绪改用 `rlh web:`、plugin-tree 取消自动重启、恢复重拉、`pluginRecovery` 粘性、启动页文案、市场 Tab 告示、安装/卸载规则。
...
**临时 `$RLH_HOME`。** 会话和设置不在用户目录；市场卸载会打到空 profile。
