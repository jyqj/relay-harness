# 文件栏与浏览器：搬逻辑、去标记、变成 rlhd

右边栏 Files / Browser 要对齐对照源（本地 `C:\Ai\t3code`）里已经跑通的行为。拷进来的源码去掉对照源产品标记，改成 `rlhd` 自己的名字。铬仍走 Harness `ui-primitives` + slot（贴 Pierre/lucide 会让官方 WebUI 铬失效）。函数体按原文落地，只剥 Effect/Atom/Schema 外壳。

## 决定

1. **没有「不能搬」清单。** 对照源 Files / Preview 上出现的方法、字段、菜单项、手势，全部接到 rlhd，并且接完后能用。唯一排除：原样 import 会让应用崩掉或让**已经能用的相关功能**失效。那种情况改接法，不删能力。
2. **会崩 / 会让相关功能不能用 → 改接法，不删能力。**
   ...
   | 把 `@pierre` / lucide / shadcn / Tailwind 推进 slot 树 | 官方 `rlh web` 铬失效 | 行为接到现有 `FileTree` / `Menu` / textarea |
...
3. **标记替换。** `t3code`、`T3-`、`T3 Code`、`@t3tools`、`persist:t3code-`、`application/x-t3code-*`、`t3code.` localStorage 键 → `rlhd` / `rlhd-` / `persist:rlhd-` / `application/x-rlhd-*` / `rlhd.`。注释英文，产品文案中文。生产源里不留对照源品牌。
...
| 对照源 | rlhd |
|---|---|
| `persist:t3code-preview-` + sha256(scope) | `persist:rlhd-preview-` + 同样 hash |
| `application/x-t3code-composer-mention` | `application/x-rlhd-composer-mention` |
| `t3code.fileExplorerOpen` / `t3code.renderMarkdown` | `rlhd.fileExplorerOpen` / `rlhd.renderMarkdown` |
| `tab_` 预览 id 前缀 | `rlhd-tab_` |
| `PreviewUrlNormalizationError` | 同名普通 Error（不引进 Effect Schema） |
| `FileSaveCoordinator` | 同名；`persist` 返回 `{ ok: boolean }` |
| `--t3-primary` 等客人 overlay 变量 | `--rlhd-preview-*`，值来自 `--rlw-alias-*` |
...
对照源 picker 要读页面 `__REACT_DEVTOOLS_GLOBAL_HOOK__`，所以客人 `contextIsolation=false`，但 **`sandbox=true`、`nodeIntegration=false`**。rlhd 客人 BrowserView 采用同一组。Harness **主窗口**仍 `contextIsolation: true`。preload 只合成 `ipcRenderer`，不把 Node 漏给页面。
