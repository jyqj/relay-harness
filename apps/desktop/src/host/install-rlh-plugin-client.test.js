...
} = require('./install-rlh-plugin-client');
...
test('empty spec fails before contacting the control endpoint', async () => {
  ...
  const result = await executeInstallRlhPlugin({ url: 'http://127.0.0.1:1', token: 't' }, '  ', [], request);
...
test('non-github specs fail client-side before contacting the control endpoint', async () => {
  ...
  for (const spec of invalid) {
    const result = await executeInstallRlhPlugin({ url: 'http://127.0.0.1:1', token: 't' }, spec, [], request);
...
test('invalid allowBuilds fails before contacting the control endpoint', async () => {
  let requested = false;
  const result = await executeInstallRlhPlugin(
...
test('needsAllowBuilds is a canonical result and does not mark restarting', async () => {
  ...
  assert.match(renderInstall(result), /retry install_rlh_plugin with allowBuilds: github.com\/owner\/repo/);
});
