...
import { parseRlhArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseRlhArgs(argv, '1.2.3')
...
describe('parseRlhArgs', () => {
  it('routes profile boots and the web alias, handing the rest to the app', () => {
    expect(parse(['--profile', 'tui']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: [], args: [], skipUserPlugins: false })
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['web', '--host', '127.0.0.1', '--port', '8080', '--no-open', '--future-web-flag']))
      .toEqual({ mode: 'profile', profile: 'web', patches: [], args: ['--host', '127.0.0.1', '--port', '8080', '--no-open', '--future-web-flag'], skipUserPlugins: false })
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@relay-harness/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@relay-harness/cordis'] })
