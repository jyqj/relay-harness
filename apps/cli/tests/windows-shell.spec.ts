 ...
 * the REAL shipped bundle layers (rlh-base + rlh-web-app resolved from the
...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@relay-harness/rlh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL rlh-base/rlh-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'rlh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@relay-harness/rlh-base', '@relay-harness/rlh-web-app'])
    const profile = loadProfile('rlh', 'web', anchor, home)
    ...
    for (const name of ['@relay-harness/rlh-pwsh-sandbox', '@relay-harness/rlh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'rlh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@relay-harness/rlh-base'])
    const profile = loadProfile('rlh', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
