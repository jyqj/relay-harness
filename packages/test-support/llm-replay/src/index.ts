 ...
 * @module @relay-harness/rlh-llm-replay
 */
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@relay-harness/rlh-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.RLH_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $RLH_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.RLH_SNAPSHOT_OVERRIDE
  const childEnv = process.env.RLH_SNAPSHOT_CHILD_FILES
