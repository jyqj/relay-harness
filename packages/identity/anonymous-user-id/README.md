# @relay-harness/rlh-anonymous-user-id

English | [中文](README.zh.md)

Shared anonymous identity for session telemetry, direct feedback acknowledgement, and DeepSeek provider requests. `getOrCreateAnonymousUserId()` returns a random UUID v4 scoped to one harness home, persisted as the bare line `$RLH_HOME/.anonymous-user-id` (`~/.rlh/.anonymous-user-id` when `RLH_HOME` is unset). The OpenTelemetry backend reports it as Resource `user.id`; `/feedback` includes the same value in its acknowledgement; and `rlh-llm-deepseek` sends it as `x-relay-harness-user-id`, allowing the receiving systems to correlate records without independently generated identities.
...
- **No cross-home identity** — different `$RLH_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `rlh-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
