...
describe('deriveThemeTokens', () => {
  it('maps seeds onto the alias-layer names used by ThemePresenter', () => {
    ...
    expect(tokens['--rlw-alias-bg-base']).toBe('#ffffff')
    expect(tokens['--rlw-alias-label-primary']).toBe('#0f1115')
    expect(tokens['--rlw-alias-brand-primary']).toBe('#4176e6')
    expect(tokens['--rlw-alias-state-business-primary']).toBe('#4176e6')
    expect(tokens['--rlw-alias-button-info-fill']).toBe('#4176e6')
    expect(tokens['--rlw-specific-bubble']).toMatch(/^#/)
    expect(tokens['--rlw-specific-bubble']).not.toBe('#ffffff')
    expect(tokens['--rlw-specific-sidebar-fill']).not.toBe('#ffffff')
    expect(tokens['--rlw-alias-bg-layer-1']).toMatch(/^#/)
    expect(tokens['--rlw-alias-bg-layer-2']).toMatch(/^#/)
    expect(tokens['--rlw-alias-bg-overlay']).toMatch(/^#/)
    expect(tokens['--rlw-alias-label-secondary']).toMatch(/^#/)
    expect(tokens['--rlw-alias-border-l1']).toMatch(/^rgb\(/)
    expect(tokens['--rlw-alias-border-l2']).toMatch(/^rgb\(/)
    expect(tokens['--rlw-alias-state-error-primary']).toBe('#c53b2c')
    expect(tokens['--rlw-specific-sidebar-fill']).toMatch(/^#/)
  })

  it('uses the dark status palette on a dark canvas', () => {
    const tokens = deriveThemeTokens(DARK)
    expect(tokens['--rlw-alias-state-error-primary']).toBe('#ff7b72')
    expect(tokens['--rlw-alias-state-success-primary']).toBe('#3fb950')
    expect(tokens['--rlw-alias-state-warn-primary']).toBe('#d29922')
  })
  ...
  it('accepts 8-digit hex by dropping the alpha channel', () => {
    ...
    expect(tokens['--rlw-alias-bg-layer-1']).toMatch(/^#[0-9a-f]{6}$/)
  })
  ...
  it('lets seed overrides win after derivation', () => {
    const tokens = deriveThemeTokens({
      ...LIGHT,
      overrides: {
        '--rlw-alias-brand-primary': '#ff00aa',
        '--rlw-alias-bg-base': '',
      },
    })
    expect(tokens['--rlw-alias-brand-primary']).toBe('#ff00aa')
    expect(tokens['--rlw-alias-bg-base']).toBe('#ffffff')
  })
  ...
  it('raises contrast by mixing more foreground into surfaces', () => {
    ...
    expect(soft['--rlw-alias-bg-layer-1']).not.toBe(hard['--rlw-alias-bg-layer-1'])
  })
  ...
  it('keeps paper nav-item active fills above the layer-2 contrast floor', () => {
    ...
    const layer2 = tokens['--rlw-alias-bg-layer-2']!
    const active = tokens['--rlw-specific-sidebar-nav-item-active']!
    const accent = tokens['--rlw-specific-sidebar-nav-item-active-accent']!
