/** Chrome-preset zoom table; desktop main keeps a CJS twin of this module. */

/** Discrete zoom levels mirroring Chrome's preset list. */
export const ZOOM_LEVELS = [0.25, 0.33, 0.5, 0.67, 0.75, 0.8, 0.9, 1, 1.1, 1.25, 1.5, 1.75, 2, 2.5, 3, 4, 5] as const

/** Default guest zoom factor. */
export const DEFAULT_ZOOM_FACTOR = 1

/** Equality epsilon when matching a factor to a table step. */
export const ZOOM_EPSILON = 0.001

function findZoomStep(current: number): number {
  const index = ZOOM_LEVELS.findIndex(
    level => Math.abs(level - current) < ZOOM_EPSILON || level > current,
  )
  if (index < 0) return ZOOM_LEVELS.length - 1
  const level = ZOOM_LEVELS[index]
  return level !== undefined && Math.abs(level - current) < ZOOM_EPSILON ? index : index - 1
}

/**
 * Next Chrome-preset zoom factor from `current`.
 * @param current - Live zoom factor.
 * @param direction - One table step in or out.
 * @returns The next table value, clamped at the ends.
 */
export function nextZoomLevel(current: number, direction: 'in' | 'out'): number {
  const step = findZoomStep(current)
  if (direction === 'in') {
    return ZOOM_LEVELS[Math.min(step + 1, ZOOM_LEVELS.length - 1)] ?? current
  }
  return ZOOM_LEVELS[Math.max(step - 1, 0)] ?? current
}
