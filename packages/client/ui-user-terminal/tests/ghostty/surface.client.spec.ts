...
/** Sixteen-cell row fixture shared by the link-resolution cases. */
...
describe('terminalLinkAtColumn', () => {
  it('maps terminal cells to UTF-16 offsets after a wide emoji', () => {
    const cells = [
      cell('🙂'),
      cell(''),
      ...Array.from('https://foo.test', character => cell(character)),
    ]
    ...
    expect(terminalLinkAtColumn(row, 2)).toBe('https://foo.test')
    expect(terminalLinkAtColumn(row, cells.length - 1)).toBe('https://foo.test')
  ...
  it('refuses links truncated at the viewport edges instead of mis-resolving', () => {
    ...
    const tailCut = [row('https://foo.test', false, true)]
    expect(terminalLinkAtPosition(tailCut, 0, 8)).toBeNull()
    // A partial bottom row is provably complete and still resolves.
    const complete = [row('https://foo.test', false), row('', false)]
    expect(terminalLinkAtPosition(complete, 0, 8)).toBe('https://foo.test')
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell('🙂'), wide: 1 },
        { ...cell(''), wide: 2 },
        ...Array.from('https://foo.tes', character => cell(character)),
      ],
      text: '🙂 https://foo.tes',
      isWrapContinuation: false,
      wrapsToNext: true,
    ...
    expect(terminalLinkAtPosition([unwrittenTail], 0, 8)).toBe('https://foo.test')
  })
})
