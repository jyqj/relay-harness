...
import { createScope, scopeOf } from '@relay-harness/rlh-client-runtime/client'
import type { SessionId } from '@relay-harness/rlh-client-runtime/client'
import type { ClientSessionContext, ConsumeTokenRequest, InputTriggerPick, InputTriggerSource, SubmitImageAttachment } from '@relay-harness/rlh-client-ui-input-trigger/client'
    ...
    list: {
      getSnapshot: () => ({
        byId: Object.fromEntries(
          (opts.roomSessionIds ?? []).map(id => [id, { agentPreset: 'rlhbot-room' }]),
        ),
      }),
    },
  })
