 ...
 * determinism
 * lives host-side — the pre-step boundary (`rlh-tool-skill`) recognizes a
...
import type { ConnectionHandle, SessionId, SkillEntry } from '@relay-harness/rlh-api-remotes/client'
import type { ClientContext, ISessions } from '@relay-harness/rlh-client-runtime/client'
import type { InputTriggerServiceContract, InputTriggerSource } from '@relay-harness/rlh-client-ui-input-trigger/client'
...
export function apply(ctx: ClientContext): void {
  ...
  const source: InputTriggerSource = {
    ...
    async candidates(session, { query, signal }) {
      if (sessions.list.getSnapshot().byId[session.sessionId]?.agentPreset === 'rlhbot-room') {
        return []
      }
    ...
    lexicon(session) {
      if (sessions.list.getSnapshot().byId[session.sessionId]?.agentPreset === 'rlhbot-room') return []
    ...
    onPick({ candidate }) {
      ...
      // pre-step boundary (rlh-tool-skill) recognizes the leading /name and
