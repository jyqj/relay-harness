...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const rlhHome = await mkdtemp(join(tmpdir(), 'rlh-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { rlhHome })
      ...
      await rm(rlhHome, { recursive: true, force: true })
    }
  })

  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const rlhHome = await mkdtemp(join(tmpdir(), 'rlh-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { rlhHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { rlhHome, maxImageBytes: 1 })
