# @relay-harness/rlh-session-log-export

English | [中文](README.zh.md)

Web Session-log download control over the host-streamed ZIP endpoint owned by `rlh-host-apiproxy`. The Host half registers `/export`; the browser half owns a 111×32 `Session log` action in the titlebar trailing cluster (icon-only at cozy/compact density), one download controller, and one modal shared by that button and the slash command. Interface Settings `session-log-export.titlebarAction` (default true) hides the titlebar button; `/export` and the shared Modal stay mounted. ZIP generation, raw JSONL/zstd reads, descendants, attachments, backpressure, and HTTP error semantics remain owned by the [ApiProxy download implementation](../../host/apiproxy/README.md).
...
- id: session-log-download
  name: '@relay-harness/rlh-session-log-export'
```

The Web bundle mounts the package beside `rlh-host-apiproxy`, `rlh-commands`, `rlh-client-ui-commands`, and `rlh-client-ui-conversation`. The package contributes its button and modal to the right-aligned `conversation.session.header.utilities` list, independently of the title-adjacent mode, Subagent, and Task entries in `conversation.session.header.actions`; Trajectory carries no export control.
