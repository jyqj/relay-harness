...
`ctx.longTermMemory` 与 `ctx.memoryExtractionQueue` 的 Service Definition。它们定义精确的用户／工作区／Agent Scope、追加式逻辑版本、持久 SessionEvent 证据、受治理的状态与信任、排序搜索、prepare/commit/abort 主机轮次结算，以及可跨重启恢复的自动提取 job。Provider 实现存储、检索与队列 Owner；Agent、提取与工具 Consumer 分别拥有自己的提示词和授权策略。
