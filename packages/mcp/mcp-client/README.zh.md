...
- `health` 为 `connected` 时，`mcpClientStatus` 带上该世代已注册的公开工具名，供 Settings 列出下一轮对话可调用的工具。
...
- Resources 与 Prompts 需要组合 `mcp-catalog`；独立 tools-only 部署有意不暴露非工具 catalog。
- **启动 timeout 是整次 attempt 而不是逐 page**——`startupTimeoutMs` 会限制 connect 加完整分页 discovery。SDK 单项请求保留自己的内部 deadline；更早到期的整次 attempt deadline 会关闭 generation，并阻止 stale registry swap。
