...
| `maxToolResultBytes` | both | no | Positive safe-integer UTF-8 JSON byte bound applied to one complete raw tool result before schema/rich projection (default 4194304) |
| `startupTimeoutMs` | both | no | Whole connect plus paginated discovery deadline (default 60000) |
...
- Listens for `notifications/tools/list_changed` → re-syncs; every cursor chain rejects repetition and every fetch-to-swap boundary rechecks generation ownership; a fetch-phase failure keeps the previous generation registered, while a registration conflict rolls back the attempted generation and leaves no tools from that server.
- Tool execute: `client.callTool({ name: rawName, arguments }, { signal })` with timeout + abort support—the public name is never sent to the server.
- Canonical success is `{ content: JsonValue[], structuredContent? }`; complete JSON MCP blocks survive for programmatic callers. The complete raw result must fit `maxToolResultBytes` before output-schema validation, rich decoding, or rendering. A supported advertised `outputSchema` then validates `structuredContent`; unsupported schema vocabulary falls back to unconstrained `JsonValue`.
...
- While `health` is `connected`, `mcpClientStatus` includes the public tool names registered for that generation so Settings can list what the next chat turn can call.
...
- Resources and Prompts require the composed `mcp-catalog`; a standalone tools-only deployment intentionally exposes no non-tool catalog.
- **Startup timeout is whole-attempt rather than per-page** — `startupTimeoutMs` bounds connect plus complete paginated discovery. Individual SDK requests retain their own internal deadlines; the earlier whole-attempt deadline closes the generation and prevents a stale registry swap.
...
- **The SDK parses before the result-byte admission check** — `maxToolResultBytes` prevents oversized values from entering schema projection, rich decoding, durable rendering, and Code Mode, but the MCP SDK transport has already parsed the JSON-RPC response. A pre-parse HTTP/stdio body cap requires an upstream transport seam.
