 ...
 *
 * `@relay-harness/rlh-tools`' `ToolExecution` satisfies this shape, so the policy
 * reads `exec` straight through without importing `rlh-tools` or `rlh-agent`.
 * Only the session HEADER id is read — the same identity every other subsystem
 * keys off (see `rlh-tool-bash`'s owner derivation).
 *
 * @module @relay-harness/rlh-spill-policy/types
 */
