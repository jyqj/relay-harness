/**
 * Profile machinery of `rlh-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'rlh-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { rlh: { bundle: { patch: './cordis.patch.yml' } } },
    }))
    if (spec.patch !== undefined) writeFileSync(join(dir, 'cordis.patch.yml'), spec.patch)
  }
  writeFileSync(join(appDir, 'package.json'), JSON.stringify({ name: 'rlh-app', dependencies: appDeps }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@relay-harness/rlh-base'])
    ...
    expect(readProfileManifest('t', dir).rlh?.profile?.bundles).toEqual(['@relay-harness/rlh-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', rlh: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).rlh?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each rlh.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or rlh section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @relay-harness/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@relay-harness/rlh-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).rlh?.profile?.bundles)
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    ...
    initProfile(stock, [
      '@relay-harness/rlh-base', '@relay-harness/rlh-web-app', '@relay-harness/rlh-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).rlh?.profile?.bundles)
      .toEqual(['@relay-harness/rlh-base', '@relay-harness/rlh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@relay-harness/rlh-base', '@relay-harness/rlh-web-app', '@relay-harness/rlh-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).rlh?.profile?.bundles).toEqual([
      '@relay-harness/rlh-base', '@relay-harness/rlh-web-app', '@relay-harness/rlh-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no rlh.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no rlh.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'rlh-app']) {
  ...
  it('throws when a fallback entry is a real directory', () => {
    const anchor = stageInstallation({})
    const home = tmp()
    mkdirSync(join(home, 'profiles', 'node_modules', 'rlh-app'), { recursive: true })
  ...
  it('replaces a wrong symlink', () => {
    ...
    symlinkSync(tmp(), join(fallback, 'rlh-app'), 'junction')
    healProfilesModuleFallback(anchor, home)
    expect(readlinkSync(join(fallback, 'rlh-app'))).toContain('app')
  })
