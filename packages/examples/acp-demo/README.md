...
ACP automation server app: the default agent spine, client-created agents through [`@relay-harness/rlh-acp`](../../acp/acp/README.md), JSONL persistence, and semantic checkpointing behind one JSON-RPC stdio bin. Programmatic clients create fresh sessions; this package mounts no human UI.

## Composition

| Plugin | Role |
|---|---|
| `@relay-harness/rlh-agent-spine-demo` | Providerless agent spine with no pre-created agents; `session/new` creates each agent. |
| `@relay-harness/rlh-session-persistence-jsonl` | Durable session logs used by checkpointing, observability, and snapshot replay. |
| `@relay-harness/rlh-session-checkpoint-policy` | Durability barriers before model calls and top-level tool effects, plus completed-step checkpoints. |
| `@relay-harness/rlh-session-query-sqlite` | Derived exact/FTS session-query service, opened before the ACP transport so leaf consumers are ready for the first model request. |
| `@relay-harness/rlh-acp` | Automation-only ACP transport over stdin/stdout. |
...
| `persona` | — | Deployment persona template for `rlh-system-prompt`. |
| `toolOrder` | lexicographic | Explicit model-facing tool order for `rlh-system-prompt`. |
| `tools` | `{ mode: 'native' }` | Native, Code Mode, or combined model tool transport. |
| `rlhHome` | `$RLH_HOME` or `~/.rlh` | Harness home shared by bash and local skill discovery. |
...
`rlh-acp-demo [--config path-to-cordis.yml]` (short form `-c`; default `./cordis.yml`) loads the gitignored `.env`, except in replay mode; `RLH_SNAPSHOT=replay` selects the sibling `cordis.snapshot.yml`; stdin EOF disposes the context and flushes sessions before exit. Loader's installed optional `node-addon-require-builtin` peer resolves bare plugin specifiers for the built bin under plain Node. Diagnostics use stderr because stdout is the ACP wire.

## Model Experience

Indirectly, through `rlh-agent-spine-demo` and the leaf's model-facing plugins. ACP prompt text becomes the ordinary logged user message; protocol metadata and permission choices do not enter the model request.
