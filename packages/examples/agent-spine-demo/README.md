...
```
@relay-harness/cordis-plugin-timer  timer service (writes nothing to stdout)
@relay-harness/rlh-llm              abstract LLM service + content-block vocabulary
@relay-harness/rlh-session          event-sourced session log + store
@relay-harness/rlh-session-title    log-backed title service + deterministic fallback
@relay-harness/rlh-system-prompt    prompt-section + tool-schema assembly
@relay-harness/rlh-tools            registry + guarded pre/around/post/final-result pipeline
@relay-harness/rlh-skill            skill provider registry
@relay-harness/rlh-skill-filesystem      local filesystem skill provider
@relay-harness/rlh-agent            agent registry + initiator scope + agent/* events
@relay-harness/rlh-goal             optional persisted same-session goal domain
@relay-harness/rlh-tool-goal        optional model-facing goal controls
@relay-harness/rlh-goal-round-driver     optional same-session goal-round driver
@relay-harness/rlh-llm-retry        provider-routed request retry policy
@relay-harness/rlh-jobs-local      generic background-job registry
@relay-harness/rlh-invariants       configurable invariant registry service
@relay-harness/rlh-session/invariant
@relay-harness/rlh-agent/invariant
@relay-harness/rlh-scope/invariant
@relay-harness/rlh-agent-loop/invariant
                                  package-owned relational checks
@relay-harness/rlh-tool-bash        the model-facing bash schema (unless toolBash=false)
@relay-harness/rlh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@relay-harness/rlh-tool-skill       session-prefix skill catalog + model-facing loader schema
@relay-harness/rlh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@relay-harness/rlh-agent-loop       THE concrete loop (gets the forwarded `agents`)
                                  (rlh-system-prompt gets the forwarded `persona`)
```
...
```ts
import type { Config } from '@relay-harness/rlh-agent-spine-demo'
// { agents?, maxParallelToolCalls?, includeHarnessIdentity?, includeRuntimeContext?, persona?, toolOrder?, tools?, rlhHome?, sessionTitle?, skills?, workspaceContext, toolBash?, jobs?, toolJobs?, goals?, invariants? }
// workspaceContext requires { maxBytes } or false; the other owner schemas supply defaults.
```

The bundle forwards each field to the child that owns it. App packages supply any pre-created agents: headless and JSON-RPC compositions create `main`, while the ACP app creates agents on demand at `session/new`. `includeRuntimeContext: false` is forwarded to `rlh-system-prompt` and suppresses all dynamic context snapshots for fresh sessions without disabling their policy services. Prompt, tool, title, skill, agent-instructions, invariant, goal, and task settings retain the schemas and defaults documented by their owning packages; `jobs.maxConcurrentJobsPerOwner` configures the local provider independently of the model-facing `toolJobs` controls. `pickSpineConfig()` copies only fields owned by this bundle, and conflicting `rlhHome` values fail during composition.

For example, `{ invariants: { enabled: true, package_allowlist: ['^@relay-harness/rlh-'], package_blocklist: ['agent-loop$'] } }` keeps the package-owned companions mounted but suppresses the blocked owner. Blocklist matches override allowlist matches; see [`rlh-invariants`](../../runtime-diagnostics/invariants/README.md) for regex and lifecycle rules.
...
Indirectly, through `rlh-system-prompt`, `rlh-tool-skill`, `rlh-tool-bash`, `rlh-tools`, and `rlh-llm-retry`, plus `rlh-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
