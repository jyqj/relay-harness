...
import { LAUNCHER_FAILURE_EXIT } from '@relay-harness/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@relay-harness/rlh-sandbox'
import type { SandboxPolicy } from '@relay-harness/rlh-sandbox'
import {
  LocalSandboxProvider,
} from '@relay-harness/rlh-sandbox-local'
import type { Config } from '@relay-harness/rlh-sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'rlh-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'rlh-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'rlh-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @relay-harness/rlh-sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'rlh-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'rlh-fake-acl-entry-'))
