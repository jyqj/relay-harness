/**
 * Integration: the real fetch backend (`rlh-web-fetch-http`) + a real search provider
 * (`rlh-web-search-exa`) + the real seam (`rlh-web`) + the model tool (`rlh-tool-web`) + the
 * tool-call timeout policy (`rlh-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@relay-harness/rlh-tools'
...
import * as TimeoutPolicy from '@relay-harness/rlh-tool-call-timeout-policy'
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by rlh-tool-call-timeout-policy,
