...
import type { WebSearchProvider, WebSearchResult } from '@relay-harness/rlh-web'
...
} from '@relay-harness/rlh-tool-web'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@relay-harness/rlh-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by rlh-tool-call-timeout-policy over exec.signal.
