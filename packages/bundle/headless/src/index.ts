/**
 * @relay-harness/rlh-headless — one-shot direct Agent driver. The bundle patch
 * rides over rlh-base without Host, HTTP, or browser plugins; this runner
 ...
 *
 * @module @relay-harness/rlh-headless
 */
...
import type { ModelSelectionRef } from '@relay-harness/rlh-agent'
...
import type {} from '@relay-harness/rlh-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`rlh: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`rlh: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
