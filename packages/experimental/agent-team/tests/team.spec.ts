...
import { SessionId, type Session } from '@relay-harness/rlh-session'
import JsonlSessionPersistence from '@relay-harness/rlh-session-persistence-jsonl'
import SubagentService from '@relay-harness/rlh-subagent'
...
async function setup(
  ...
  const storageRoot = mkdtempSync(join(tmpdir(), 'rlh-team-'))
...
describe('Team identity and provisioning', () => {
  ...
  it('supports direct-constructor defaults and recovers roots that already exist', async () => {
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const storageRoot = mkdtempSync(join(tmpdir(), 'rlh-team-direct-'))
    roots.push(storageRoot)
    await ctx.plugin(JsonlSessionPersistence, { root: storageRoot })
    await ctx.plugin(AgentLoop, { agents: [] })
    await ctx.plugin(SubagentService)
...
describe('Team mailbox and waiting', () => {
  ...
  it('waits for one change, supports cancellation, times out, and releases waiters on HMR disposal', async () => {
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const storageRoot = mkdtempSync(join(tmpdir(), 'rlh-team-wait-'))
    roots.push(storageRoot)
    await ctx.plugin(JsonlSessionPersistence, { root: storageRoot })
    await ctx.plugin(AgentLoop, { agents: [] })
    await ctx.plugin(SubagentService)
