 ...
 * @module @relay-harness/rlh-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSessionId } from '@relay-harness/rlh-terminal'
import { deadline, timeoutOf } from '@relay-harness/rlh-timeout'
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__RLH_PERSISTENT_BASH_START_${nonce}__`,
    end: `__RLH_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __rlh_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__rlh_persistent_bash_status"`
}
