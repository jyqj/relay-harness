...
import type { ShellRunResult, CollectedOutput } from '@relay-harness/rlh-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@relay-harness/rlh-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@relay-harness/rlh-sandbox'
...
import type { Config } from '@relay-harness/rlh-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'rlh-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'RLH_WRAP=1', 'bash', '-c', 'printf "%s" "$RLH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$RLH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'rlh-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.RLH_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'rlh-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'rlh-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
