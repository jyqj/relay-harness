 ...
 *
 * @module @relay-harness/rlh-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors rlh-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@relay-harness/cordis'
import z from '@relay-harness/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@relay-harness/rlh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@relay-harness/rlh-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@relay-harness/rlh-subprocess'
import { installSettingsSection } from '@relay-harness/rlh-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@relay-harness/rlh-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of rlh-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `rlh-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.rlhEnv },
    }
  }
   ...
   * `rlh-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@relay-harness/rlh-pwsh-sandbox`.
