 ...
 * @module @relay-harness/rlh-terminal-bash
 */

import { Context } from '@relay-harness/cordis'
import type { Agent } from '@relay-harness/rlh-agent'
import type { Session, SessionEvent } from '@relay-harness/rlh-session'
import { TerminalBackendCleanupError } from '@relay-harness/rlh-terminal'
import type { TerminalBackend, TerminalBackendSpawnSpec } from '@relay-harness/rlh-terminal'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@relay-harness/rlh-subprocess'
import type { SandboxExecutionPolicy } from '@relay-harness/rlh-sandbox'
import { effectiveSandboxMode } from '@relay-harness/rlh-sandbox-policy'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    RLH_SHELL: '1',
    RLH_SESSION_ID: spec.owner.id,
    RLH_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {
