...
import type { TerminalSendOperation } from '@relay-harness/rlh-terminal'
...
import { resolvePwshPath } from '@relay-harness/rlh-pwsh-local/src/resolve.ts'
...
async function harness(
  ...
  const root = mkdtempSync(join(tmpdir(), 'rlh-pty-local-'))
  roots.push(root)
...
describe.skipIf(process.platform === 'win32')('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.RLH_TEST_SECRET
    process.env.RLH_TEST_SECRET = 'must-not-leak'
    try {
      const { ctx, root, agent } = await harness('danger-full-access')
      const created = await ctx.terminals.spawn(agent, { type: 'shell', name: 'main', cwd: root })
      expect(created.motd).toContain('rlh> ')
      ...
      if (previous === undefined) delete process.env.RLH_TEST_SECRET
      else process.env.RLH_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('restores the controlled prompt after an in-shell PS1 override', async () => {
    ...
    expect(result.viewport).toContain('healed=[rlh> ]')
  ...
  it('quiesces a disowned same-session descendant after the shell exits naturally', async () => {
    ...
    try {
      const background = ctx.terminals.startSend(agent, created.sessionId, {
        text: `sh -c 'trap "" TERM; printf "%s" "$$" > "$1"; sleep 60' rlh "${pidFile}" & disown`,
        submit: true,
      })
  ...
  it('bootstraps a persistent pwsh, persists state, and scrubs secrets', async () => {
    ...
    try {
      ...
      const second = ctx.terminals.startSend(agent, created.sessionId, {
        text: 'Write-Output "keep=$env:KEEP secret=$env:RLH_TEST_SECRET"',
        submit: true,
      })
