...
import { resolveExampleLaunch, runLoaderSmoke } from '@relay-harness/rlh-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-rlh-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'rlh-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'rlh-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        RLH_TEST_CHILD_COMMAND: childLaunch.command,
        RLH_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        RLH_TEST_CHILD_ENV: JSON.stringify({
