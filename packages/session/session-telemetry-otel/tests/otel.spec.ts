...
import { getOrCreateAnonymousUserId } from '@relay-harness/rlh-anonymous-user-id'
import Loader from '@relay-harness/cordis-plugin-loader'
import { recordFeedback } from '@relay-harness/rlh-command-feedback'
...
// pin RLH_HOME to a temp dir so the suite never touches the ambient ~/.rlh.
let tempHome: string
let previousRlhHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'rlh-otel-home-'))
  previousRlhHome = process.env.RLH_HOME
  process.env.RLH_HOME = tempHome
})
afterAll(() => {
  if (previousRlhHome === undefined) delete process.env.RLH_HOME
  else process.env.RLH_HOME = previousRlhHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'relay-harness' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@relay-harness/rlh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@relay-harness/rlh-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@relay-harness/rlh-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('rlh-session-telemetry-otel real-load-path guard', () => {
