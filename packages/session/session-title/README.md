...
Forks inherit title events in their seed unchanged. The first-prompt cadence does not automatically retitle a child that already has an eligible inherited human message. A child whose seed has none, and whose latest title is not user-pinned, schedules first-prompt from its first new eligible message. The all-messages cadence may append a new revision after the child receives a later human prompt.
