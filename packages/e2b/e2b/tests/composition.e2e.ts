...
import { Inbox } from '@relay-harness/rlh-agent'
import type { Agent } from '@relay-harness/rlh-agent'
import { runLoaderSmoke } from '@relay-harness/rlh-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@relay-harness/rlh-e2b'
import TerminalSessionService, { TerminalSessionId } from '@relay-harness/rlh-terminal'
import { BashTerminalBackend } from '@relay-harness/rlh-terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', RLH_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/rlh-e2b-bootstrap-profile-leak'
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'rlh_leak=0',
          'for rlh_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$rlh_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$rlh_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then rlh_leak=1; fi',
          'done',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> RLH=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$RLH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> RLH=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'rlh-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.rlh-e2b/terminals/')
