...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$rlh_e2b_env_bin" -i "$rlh_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$rlh_e2b_tee" --output-error=warn-nopipe >("$rlh_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$rlh_e2b_tee" --output-error=warn-nopipe >("$rlh_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'rlh_e2b_rm=$7',
    'shift 7',
    'rlh_e2b_pgid="$("$rlh_e2b_ps" -o pgid= -p "$$" | "$rlh_e2b_tr" -d " ")"',
    `printf '%s\\n' "$rlh_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t rlh_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$rlh_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$rlh_e2b_env_bin" -i -- "\${rlh_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'rlh_e2b_status=$?',
    `printf '%s\\n' "$rlh_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$rlh_e2b_status"',
  ].join('\n')
  ...
  const bootstrap = [
    `mapfile -d '' -t rlh_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'rlh_e2b_env_bin="$(command -v env)"',
    'rlh_e2b_setsid="$(command -v setsid)"',
    'rlh_e2b_bash="$(command -v bash)"',
    'rlh_e2b_node="$(command -v node)"',
    'rlh_e2b_ps="$(command -v ps)"',
    'rlh_e2b_tr="$(command -v tr)"',
    'rlh_e2b_tee="$(command -v tee)"',
    'rlh_e2b_head="$(command -v head)"',
    'rlh_e2b_rm="$(command -v rm)"',
    'for rlh_e2b_tool in "$rlh_e2b_env_bin" "$rlh_e2b_setsid" "$rlh_e2b_bash" "$rlh_e2b_node" "$rlh_e2b_ps" "$rlh_e2b_tr" "$rlh_e2b_tee" "$rlh_e2b_head" "$rlh_e2b_rm"; do',
    '  [[ "$rlh_e2b_tool" == /* && -x "$rlh_e2b_tool" ]] || exit 125',
    'done',
    `exec "$rlh_e2b_env_bin" -i -- "\${rlh_e2b_env[@]}" "$rlh_e2b_setsid" --wait -- "$rlh_e2b_bash" -c ${quoteE2BShellArg(inner)} rlh-e2b "$rlh_e2b_env_bin" "$rlh_e2b_node" "$rlh_e2b_ps" "$rlh_e2b_tr" "$rlh_e2b_tee" "$rlh_e2b_head" "$rlh_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
