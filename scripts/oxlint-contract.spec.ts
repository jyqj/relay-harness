...
`
  ...
  it('preserves successful fix output channels', () => {
    ...
    const runner: OxlintProcessRunner = (_invocation, mode) => {
      ...
      return {
        signal: null,
        status: 0,
        stdout: 'successful fix output\n',
        stderr: 'successful fix warning\n',
      }
    }
    ...
    expect(modes).toEqual(['capture'])
    expect(stdout).toEqual(['successful fix output\n'])
    expect(stderr).toEqual(['successful fix warning\n'])
  })
