 ...
 * owns: `rlh` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@relay-harness/rlh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@relay-harness/')) throw new Error(`${normalized} must name an @relay-harness package`)
...
class RlhFamily extends ReleaseFamily {
  readonly id = 'rlh'
  // apps/desktop carries its own installer version line (private, unscoped) and
  // releases through electron-builder, not this family.
  readonly patterns = ['packages/!(experimental)/*/package.json', 'apps/!(desktop)/package.json'] as const
  readonly tagPrefix = 'rlh-v'
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`rlh release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@relay-harness/rlh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@relay-harness/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new RlhFamily(), new VendorFamily()]
}
