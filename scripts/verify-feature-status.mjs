...
async function verifyFeatures() {
  ...
  if (model.schemaVersion !== 1 || model.runtimeRoot !== '.') fail('unsupported manifest header')
  if (model.repositoryGovernance?.sourceLayout !== 'root-monorepo'
...
async function verifyWorkflowDiscovery() {
  if (await exists(resolve(ROOT, 'relay-harness'))) fail('physical relay-harness/ survived the flatten')
  const { stdout: nestedTracked } = await execFileAsync('git', ['ls-files', '--', 'relay-harness'], { cwd: ROOT })
  if (nestedTracked.trim() !== '') fail('tracked relay-harness/ paths survived the flatten')
  ...
  for (const name of names) {
    ...
    if (/working-directory:\s*relay-harness(?:\/|\s|$)/.test(text)) fail(`${name}: nested working-directory survived`)
    if (/(?<![@\w/-])relay-harness\//.test(text)) fail(`${name}: nested operational path survived`)
    if (/node \.\.\/(?:scripts|\.github)\//.test(text)) fail(`${name}: root command still escapes a nested cwd`)
  }
  const dependabot = await readFile(resolve(ROOT, '.github/dependabot.yml'), 'utf8')
  if (/directory:\s*["']\/relay-harness(?:\/|["'])/.test(dependabot)) {
    fail('Dependabot still targets the nested runtime')
  }
  if (!dependabot.includes('directory: "/"') || !dependabot.includes('directory: "/python/sdk"')) {
    fail('Dependabot directories do not target the flattened root')
  }
}
