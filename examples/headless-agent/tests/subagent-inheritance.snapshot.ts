...
import { normalizeSessionLog, scrubRequestHeaders, type NormalizeContext } from '@relay-harness/rlh-acp-snapshot'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@relay-harness/rlh-session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'rlh-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        RLH_SNAPSHOT_FILE: replayOverride,
        RLH_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@relay-harness/rlh-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the RLH file sandbox cannot modify files in the standing mode.')
